...
import { n as loadDeviceIdentityIfPresent } from "./device-identity-cacJqJr9.js";
...
import { c as listApprovedPairedDeviceRoles, l as listDevicePairing, v as summarizeDeviceTokens } from "./device-pairing-DUA4LHep.js";
...
function formatPairedRecordIssue(issue) {
...
function formatLocalDeviceAuthIssue(issue) {
	return `- ${issue.message}`;
}
function formatLegacyPairingStoreIssue(filePath) {
...
async function collectLegacyPairingStoreIssues(cfg) {
	if (cfg.gateway?.mode === "remote") return [];
	const { listLegacyDevicePairingStoreFiles } = await import("./device-pairing-migration-CGlApJJS.js");
...
function legacyPairingStoreIssueToHealthFinding(message) {
	return {
		checkId: DEVICE_PAIRING_CHECK_ID,
		severity: "warning",
		...
		fixHint: "Restart the gateway so it imports the legacy store; if the file persists, fix or move it aside first."
	};
}
async function collectDevicePairingHealthFindings(params) {
	const legacyStoreFindings = (await collectLegacyPairingStoreIssues(params.cfg)).map(legacyPairingStoreIssueToHealthFinding);
...
async function noteDevicePairingHealth(params) {
	...
	const lines = [...legacyStoreLines, ...snapshot ? [
