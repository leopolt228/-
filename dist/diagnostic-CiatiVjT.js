...
import { a as emitInternalDiagnosticEvent, f as isDiagnosticsEnabled, t as areDiagnosticsEnabledForProcess, u as getInternalDiagnosticEventSequence } from "./diagnostic-events-Dt41CZkD.js";
...
import { a as getDiagnosticSessionActivitySnapshot, d as startDiagnosticRunActivityTracking, f as stopDiagnosticRunActivityTracking, i as getDiagnosticEmbeddedRunActivitySequence, l as resetDiagnosticRunActivityForTest, r as clearDiagnosticEmbeddedRunActivityForSession, t as BLOCKED_TOOL_CALL_ABORT_FLOOR_MS } from "./diagnostic-run-activity-CneCqy92.js";
import { a as markDiagnosticActivity, n as getLastDiagnosticActivityAt, o as resetDiagnosticActivityForTest, t as diagnosticLogger } from "./diagnostic-runtime-BMkiuyH7.js";
import { i as resetDiagnosticStabilityRecorderForTest, o as startDiagnosticStabilityRecorder, s as stopDiagnosticStabilityRecorder } from "./diagnostic-stability-BMZtzKD2.js";
import { c as writeDiagnosticMemoryPressureBundleSync, o as resetDiagnosticStabilityBundleForTest, r as installDiagnosticStabilityFatalHook, s as uninstallDiagnosticStabilityFatalHook } from "./diagnostic-stability-bundle-0tcSxw3r.js";
import { n as getRecentDiagnosticPhases, r as resetDiagnosticPhasesForTest, t as getCurrentDiagnosticPhase } from "./diagnostic-phase-bPtfbE0s.js";
import { n as loadCronJobsStoreSync, s as resolveCronJobsStorePath } from "./store-CFkN1_TJ.js";
...
function emitDiagnosticMemorySample(options) {
	...
	if (pressure && shouldEmitPressure(pressure, now, thresholds.pressureRepeatMs)) {
		...
		} else if (pressure.level === "critical") log.warn("critical memory pressure snapshot disabled");
	}
	return memory;
}
...
function readLastAssistantFromSessionFile(filePath) {
	...
	for (let index = lines.length - 1; index >= 0; index -= 1) try {
		const parsed = JSON.parse(expectDefined(lines[index], "lines entry at index"));
...
function startDiagnosticHeartbeat(config, opts) {
	...
	heartbeatInterval = setInterval(() => {
		...
		if (shouldDeferRecovery) diagnosticLogger.warn(`liveness heartbeat delayed ${Math.round(heartbeatElapsedMs)}ms; deferring recovery decisions`);
