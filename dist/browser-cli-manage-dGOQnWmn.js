...
import { n as info, t as danger } from "./globals-DBBT7Ru5.js";
import { n as runCommandWithRuntime } from "./cli-utils-B33Avitx.js";
import { n as redactCdpUrl } from "./browser-config-Y5s979Hx.js";
import "./core-api-Cu0Rm4aq.js";
import { n as formatBrowserGraphicsSummary } from "./routes-CL1VzTjl.js";
import "./core-api-Bt-Knpr5.js";
import { n as callBrowserRequest, s as parseBrowserPositiveIntegerValue, t as BROWSER_TAB_REFERENCE_HELP } from "./browser-cli-shared-CVHbBbuF.js";
...
function formatDoctorLine(check) {
	return `${check.warning ? "WARN" : check.ok ? "OK" : "FAIL"} ${check.name}${check.detail ? `: ${check.detail}` : ""}`;
}
...
function registerBrowserManageCommands(browser, parentOpts) {
	...
	tab.command("label").description("Assign a friendly label to a tab").argument("<targetId>", BROWSER_TAB_REFERENCE_HELP).argument("<label>", "Friendly label").action(async (targetId, label, _opts, cmd) => {
		...
		await runBrowserCommand(async () => {
			...
			defaultRuntime.log(`labeled tab ${tabValue?.tabId ?? targetId} as ${tabValue?.label ?? label}`);
		});
	});
	...
	browser.command("open").description("Open a URL in a new tab").argument("<url>", "URL to open").option("--label <label>", "Assign a friendly tab label").action(async (url, opts, cmd) => {
		...
		const profile = parent?.browserProfile;
		await runBrowserCommand(async () => {
			const tabLocal = await callBrowserRequest(parent, {
				method: "POST",
				path: "/tabs/open",
				query: resolveProfileQuery(profile),
				body: {
	...
	browser.command("system-profiles").description("List Chrome-family profiles available for cookie import").option("--browser <browser>", "System browser (chrome|brave|edge|chromium); omit to list all").action(async (opts, cmd) => {
		const parent = parentOpts(cmd);
		await runBrowserCommand(async () => {
			const systemProfiles = (await callBrowserRequest(parent, {
				method: "GET",
				path: "/system-profiles",
				query: opts.browser ? { browser: opts.browser } : void 0
			}, { timeoutMs: BROWSER_MANAGE_REQUEST_TIMEOUT_MS })).systemProfiles ?? [];
			if (printJsonResult(parent, { systemProfiles })) return;
	...
	browser.command("import-profile").description("Import cookies from a macOS Chrome-family profile").option("--browser <browser>", "System browser (chrome|brave|edge|chromium)", "chrome").option("--system <profile>", "System profile directory", "Default").option("--into <profile>", "Managed target profile", "imported").option("--domains <domains>", "Comma-separated domain filter").action(async (opts, cmd) => {
		...
		await runBrowserCommand(async () => {
			...
			if (printJsonResult(parent, result)) return;
			defaultRuntime.log(info(`Imported cookies into "${result.into}": ${result.cookies.imported}/${result.cookies.total} imported, ${result.cookies.failed} failed, ${result.cookies.skipped} skipped; ${result.domains.length} domains`));
		});
	});
