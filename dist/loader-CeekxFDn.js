...
import { r as shouldIncludeHook } from "./config-DHK8km67.js";
import { n as isKnownInternalHookEventKey, t as loadWorkspaceHookEntries } from "./workspace-C_mCNxFl.js";
import { n as resolveConfiguredInternalHookNames, r as getLegacyInternalHookHandlers, t as hasConfiguredInternalHooks } from "./configured-pV8SaeM2.js";
...
async function loadInternalHooks(cfg, workspaceDir, opts) {
	...
	for (const handlerConfig of handlers) try {
		...
		if (!isKnownInternalHookEventKey(handlerConfig.event)) log.warn(`Legacy hook handler ${safeLogValue(rawModule)} subscribes to event ${safeLogValue(handlerConfig.event)} not emitted by OpenClaw core — likely a typo; unless a plugin emits it, the hook never fires. Known events: https://docs.openclaw.ai/automation/hooks`);
