...
import { C as resolveExpiresAtMsFromDurationMs, _ as parseStrictFiniteNumber, c as asFiniteNumberInRange, j as resolveTimerTimeoutMs, m as isFutureDateTimestampMs, o as asDateTimestampMs, y as parseStrictNonNegativeInteger } from "./number-coercion-Crk_c9KW.js";
...
import { i as loadStoredZaloCredentials, n as clearStoredZaloCredentials, s as refreshStoredZaloCredentials, u as saveStoredZaloCredentials } from "./session-state-BvccD4Ui.js";
...
const loadZcaJsRuntime = createLazyRuntimeModule(() => import("./dist-7pXzEDos.js").then((mod) => mod));
...
function writeCredentials(profile, credentials, allowRevokedReplace) {
	...
	if (!(allowRevokedReplace ? (saveStoredZaloCredentials(profile, stored), true) : refreshStoredZaloCredentials(profile, stored))) return false;
...
function writeApiCredentials(profile, api, fallback, allowRevokedReplace = false) {
...
async function ensureApi(profileInput, timeoutMs = API_LOGIN_TIMEOUT_MS, credentialPersistence = "persist") {
	...
	const initPromise = (async () => {
		...
		if (credentialPersistence === "persist") writeApiCredentials(profile, api, stored);
		return api;
	})();
...
async function withZaloApi(profileInput, operation, options = {}) {
	...
	if (credentialPersistence === "persist" && (options.shouldPersist?.(result) ?? true)) persistApiCredentialsIfChanged(profile, api);
	return result;
}
...
function normalizeZaloInboundMessage(message, ownUserId) {
...
async function checkZaloAuthenticated(profileInput, options) {
	const profile = normalizeProfile(profileInput);
	if (!zalouserSessionExists(profile)) return false;
	try {
		await withZaloApi(profile, async (api) => await withTimeout(api.fetchAccountInfo(), 12e3, { message: "Timed out checking Zalo session" }), {
			timeoutMs: 12e3,
...
async function listZaloFriends(profileInput, options) {
	return await withZaloApi(normalizeProfile(profileInput), async (api) => {
...
async function listZaloGroups(profileInput, options) {
	return await withZaloApi(normalizeProfile(profileInput), async (api) => {
...
async function resolveZaloOwnUserId(profileInput) {
...
async function startZaloQrLogin(params) {
	...
	if (existing && params.beforeCredentialPersistence && existing.beforeCredentialPersistence !== params.beforeCredentialPersistence) {
	...
	if (!activeQrLogins.has(profile)) {
		const login = {
			...
			profile,
			startedAt: Date.now(),
			...params.beforeCredentialPersistence ? { beforeCredentialPersistence: params.beforeCredentialPersistence } : {},
			connected: false,
		...
		login.waitPromise = (async () => {
			...
			try {
				...
				if (!owned || owned.id !== login.id) return;
				writeApiCredentials(profile, api, capturedCredentials ?? void 0, true);
				invalidateApi(profile);
				apiByProfile.set(profile, api);
...
async function startZaloListener(params) {
	...
	const onMessage = (incoming) => {
		...
		Promise.resolve(params.onMessage(incoming)).catch((error) => {
...
async function resolveZaloGroupsByEntries(params) {
	const groups = await listZaloGroups(params.profile, { credentialPersistence: params.credentialPersistence ?? "persist" });
...
async function resolveZaloAllowFromEntries(params) {
	const friends = await listZaloFriends(params.profile, { credentialPersistence: params.credentialPersistence ?? "persist" });
	const byName = /* @__PURE__ */ new Map();
...
//#endregion
export { createZalouserSendReceipt as C, waitForZaloQrLogin as S, ThreadType as T, sendZaloSeenEvent as _, listZaloGroupMembers as a, startZaloListener as b, logoutZaloProfile as c, resolveZaloGroupContext as d, resolveZaloGroupsByEntries as f, sendZaloReaction as g, sendZaloLink as h, listZaloFriendsMatching as i, normalizeZaloInboundMessage as l, sendZaloDeliveredEvent as m, getZaloUserInfo as n, listZaloGroups as o, resolveZaloOwnUserId as p, listZaloFriends as r, listZaloGroupsMatching as s, checkZaloAuthenticated as t, resolveZaloAllowFromEntries as u, sendZaloTextMessage as v, TextStyle as w, startZaloQrLogin as x, sendZaloTypingEvent as y };
