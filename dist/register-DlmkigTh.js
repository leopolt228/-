...
import { o as isRecord$2 } from "./record-coerce-DHZ4bFlT.js";
import { _ as uniqueStrings$2 } from "./string-normalization-CRyoFBPt.js";
...
//#endregion
//#region extensions/policy/src/doctor/fix-metadata.ts
const m = (checkId, fixClass, summary, options = {}) => ({
	checkId,
	fixClass,
	summary,
	...options
...
const POLICY_FIX_METADATA = [
	...
	m(CHECK_IDS.policyHashMismatch, "manual", "Restore the approved artifact or update the expected hash after review.", { configTargets: ["plugins.entries.policy.config.expectedHash"] }),
	m(CHECK_IDS.policyAttestationMismatch, "manual", "Review the current attestation and update accepted hashes after approval.", { configTargets: ["plugins.entries.policy.config.expectedAttestationHash"] }),
	m(CHECK_IDS.policyDeniedChannelProvider, "automatic", "Disable product-managed channels matching the denied provider.", {
	...
	m(CHECK_IDS.policyUnapprovedMcpServer, "reviewRequired", "Remove the unapproved MCP server or select an approved replacement.", {
		policyPath: [
			"mcp",
			"servers",
			"allow"
	...
	m(CHECK_IDS.policyDeniedModelProvider, "reviewRequired", "Remove the model provider or switch references to an approved provider.", {
		policyPath: [
			"models",
			"providers",
			"deny"
	...
	m(CHECK_IDS.policyUnapprovedModelProvider, "reviewRequired", "Select an approved model provider.", {
		policyPath: [
			"models",
			"providers",
			"allow"
	...
	m(CHECK_IDS.policyPrivateNetworkAccess, "reviewRequired", "Disable the concrete private-network access opt-in.", {
		policyPath: [
			"network",
			"privateNetwork",
			"allow"
	...
	m(CHECK_IDS.policyIngressDmPolicyUnapproved, "reviewRequired", "Set channel DM policy to an allowed value.", {
		policyPath: [
			"ingress",
			"channels",
			"allowDmPolicies"
	...
	m(CHECK_IDS.policyIngressDmScopeUnapproved, "reviewRequired", "Move session DM scope to the required or stricter ordered value.", {
		policyPath: [
			"ingress",
			"session",
			"requireDmScope"
	...
	m(CHECK_IDS.policyIngressOpenGroupsDenied, "automatic", "Disable product-managed open group ingress.", {
		policyPath: [
			"ingress",
			"channels",
			"denyOpenGroups"
	...
	m(CHECK_IDS.policyIngressGroupMentionRequired, "automatic", "Require mention in product-managed group channels.", {
		policyPath: [
			"ingress",
			"channels",
	...
	m(CHECK_IDS.policyGatewayNonLoopbackBind, "reviewRequired", "Set gateway bind address to loopback when remote exposure is not intended.", {
		policyPath: [
			"gateway",
			"exposure",
	...
	m(CHECK_IDS.policyGatewayAuthDisabled, "manual", "Configure token, password, or trusted-proxy auth.", {
		policyPath: [
			"gateway",
			"auth",
			"requireAuth"
	...
	m(CHECK_IDS.policyGatewayRateLimitMissing, "reviewRequired", "Add explicit gateway auth rate limits from product defaults.", {
		policyPath: [
			"gateway",
			"auth",
	...
	m(CHECK_IDS.policyGatewayControlUiInsecure, "automatic", "Disable the insecure Control UI toggle.", {
		policyPath: [
			"gateway",
			"controlUi",
			"allowInsecure"
	...
	m(CHECK_IDS.policyGatewayTailscaleFunnel, "reviewRequired", "Disable Tailscale funnel or serve exposure.", {
		policyPath: [
			"gateway",
			"exposure",
	...
	m(CHECK_IDS.policyGatewayRemoteEnabled, "automatic", "Disable product-managed remote gateway mode.", {
		policyPath: [
			"gateway",
			"remote",
			"allow"
	...
	m(CHECK_IDS.policyGatewayHttpEndpointEnabled, "automatic", "Disable denied Gateway HTTP endpoints.", {
		policyPath: [
			"gateway",
			"http",
			"denyEndpoints"
	...
	m(CHECK_IDS.policyGatewayHttpUrlFetchUnrestricted, "manual", "Add URL allowlists for each URL-fetch input.", {
		policyPath: [
			"gateway",
			"http",
	...
	m(CHECK_IDS.policyGatewayNodeCommandDenied, "reviewRequired", "Add the command to gateway node denyCommands or update policy after review.", {
		policyPath: [
			"gateway",
			"nodes",
			"denyCommands"
	...
	m(CHECK_IDS.policyAgentsWorkspaceAccessDenied, "reviewRequired", "Set agent workspace access to an allowed mode.", {
		policyPath: [
			"agents",
			"workspace",
			"allowedAccess"
	...
	m(CHECK_IDS.policyAgentsToolNotDenied, "automatic", "Merge required built-in workspace tool denies.", {
		policyPath: [
			"agents",
			"workspace",
			"denyTools"
	...
	m(CHECK_IDS.policyToolsProfileUnapproved, "reviewRequired", "Set the tool profile to an allowed profile.", {
		policyPath: [
			"tools",
			"profiles",
			"allow"
	...
	m(CHECK_IDS.policyToolsFsWorkspaceOnlyRequired, "reviewRequired", "Set workspace-only filesystem posture when required assets remain readable.", {
		policyPath: [
			"tools",
			"fs",
	...
	m(CHECK_IDS.policyToolsExecSecurityUnapproved, "reviewRequired", "Set exec security to an allowed value.", {
		policyPath: [
			"tools",
			"exec",
			"allowSecurity"
	...
	m(CHECK_IDS.policyToolsExecAskUnapproved, "reviewRequired", "Set exec ask mode to an allowed value.", {
		policyPath: [
			"tools",
			"exec",
			"requireAsk"
	...
	m(CHECK_IDS.policyToolsExecHostUnapproved, "reviewRequired", "Move exec host to an allowed host mode.", {
		policyPath: [
			"tools",
			"exec",
			"allowHosts"
	...
	m(CHECK_IDS.policyToolsAlsoAllowMissing, "reviewRequired", "Add expected alsoAllow entries only when policy intentionally grants them.", {
		policyPath: [
			"tools",
			"alsoAllow",
			"expected"
	...
	m(CHECK_IDS.policyToolsAlsoAllowUnexpected, "reviewRequired", "Remove unexpected alsoAllow entries.", {
		policyPath: [
			"tools",
			"alsoAllow",
			"expected"
	...
	m(CHECK_IDS.policyToolsRequiredDenyMissing, "automatic", "Merge required built-in deny tool classes.", {
		policyPath: ["tools", "denyTools"],
		configTargets: ["tools.deny", "agents.list[].tools.deny"]
	}),
	m(CHECK_IDS.policySandboxModeUnapproved, "reviewRequired", "Set sandbox mode to an allowed value.", {
		policyPath: ["sandbox", "requireMode"],
		configTargets: ["sandbox.mode"]
	}),
	m(CHECK_IDS.policySandboxBackendUnapproved, "reviewRequired", "Choose an approved sandbox backend that is installed.", {
		policyPath: ["sandbox", "allowBackends"],
		configTargets: ["sandbox.backend"]
	}),
	m(CHECK_IDS.policySandboxContainerPostureUnobservable, "unsupported", "Add observable container posture evidence before patching."),
	m(CHECK_IDS.policySandboxContainerHostNetworkDenied, "reviewRequired", "Disable container host networking.", {
		policyPath: [
			"sandbox",
			"containers",
			"denyHostNetwork"
	...
	m(CHECK_IDS.policySandboxContainerNamespaceJoinDenied, "reviewRequired", "Disable joining container namespaces.", {
		policyPath: [
			"sandbox",
			"containers",
	...
	m(CHECK_IDS.policySandboxContainerMountModeRequired, "reviewRequired", "Change required mounts to read-only.", {
		policyPath: [
			"sandbox",
			"containers",
	...
	m(CHECK_IDS.policySandboxContainerRuntimeSocketMount, "reviewRequired", "Remove container runtime socket binds.", {
		policyPath: [
			"sandbox",
			"containers",
	...
	m(CHECK_IDS.policySandboxContainerUnconfinedProfile, "reviewRequired", "Remove unconfined container profiles.", {
		policyPath: [
			"sandbox",
			"containers",
	...
	m(CHECK_IDS.policySandboxBrowserCdpSourceRangeMissing, "manual", "Add an explicit browser CDP source range.", {
		policyPath: [
			"sandbox",
			"browser",
	...
	m(CHECK_IDS.policyDataHandlingRedactionDisabled, "automatic", "Set sensitive logging to a redacting mode.", {
		policyPath: [
			"dataHandling",
			"sensitiveLogging",
			"requireRedaction"
	...
	m(CHECK_IDS.policyDataHandlingTelemetryContentCapture, "automatic", "Disable telemetry content capture.", {
		policyPath: [
			"dataHandling",
			"telemetry",
			"denyContentCapture"
	...
	m(CHECK_IDS.policyDataHandlingSessionRetentionNotEnforced, "reviewRequired", "Set session maintenance to enforced mode.", {
		policyPath: [
			"dataHandling",
			"retention",
	...
	m(CHECK_IDS.policyDataHandlingSessionTranscriptMemory, "reviewRequired", "Disable transcript indexing for the affected agent scope.", {
		policyPath: [
			"dataHandling",
			"memory",
	...
	m(CHECK_IDS.policySecretsUnmanagedProvider, "manual", "Migrate the secret to a managed provider.", {
		policyPath: ["secrets", "requireManagedProviders"],
		configTargets: ["secrets"]
	}),
	m(CHECK_IDS.policySecretsDeniedProviderSource, "reviewRequired", "Move the secret out of the denied source.", {
		policyPath: ["secrets", "denySources"],
		configTargets: ["secrets"]
	}),
	m(CHECK_IDS.policySecretsInsecureProvider, "reviewRequired", "Remove insecure provider overrides.", {
		policyPath: ["secrets", "allowInsecureProviders"],
		configTargets: ["secrets"]
	}),
	m(CHECK_IDS.policyAuthProfileInvalidMetadata, "manual", "Add required provider and mode metadata to auth profiles.", {
		policyPath: [
			"auth",
			"profiles",
			"requireMetadata"
	...
	m(CHECK_IDS.policyAuthProfileUnapprovedMode, "manual", "Change auth mode and credentials through the auth owner flow.", {
		policyPath: [
			"auth",
			"profiles",
			"allowModes"
	...
	m(CHECK_IDS.policyExecApprovalsMissing, "manual", "Restore an attributable exec-approvals evidence file.", {
	...
	m(CHECK_IDS.policyExecApprovalsDefaultSecurityUnapproved, "manual", "Update reviewed default approval evidence or policy.", {
		policyPath: [
			"execApprovals",
			"defaults",
			"allowSecurity"
	...
	m(CHECK_IDS.policyExecApprovalsAgentSecurityUnapproved, "manual", "Update reviewed agent approval evidence or policy.", {
		policyPath: [
			"execApprovals",
			"agents",
			"allowSecurity"
	...
	m(CHECK_IDS.policyExecApprovalsAutoAllowSkillsEnabled, "reviewRequired", "Disable auto-allow skills in the approval owner surface.", {
		policyPath: [
			"execApprovals",
			"agents",
	...
	m(CHECK_IDS.policyExecApprovalsAllowlistMissing, "manual", "Add expected approval patterns through approval review.", {
		policyPath: [
			"execApprovals",
			"agents",
			"allowlist",
	...
	m(CHECK_IDS.policyExecApprovalsAllowlistUnexpected, "manual", "Remove unexpected approval patterns through approval review.", {
		policyPath: [
			"execApprovals",
			"agents",
			"allowlist",
	...
	m(CHECK_IDS.policyMissingToolRisk, "manual", "Add tool risk metadata in the owning tool declaration.", {
	...
	m(CHECK_IDS.policyMissingToolSensitivity, "manual", "Add tool sensitivity metadata in the owning tool declaration.", {
		policyPath: ["tools", "requireMetadata"],
		configTargets: ["tools"]
	}),
	m(CHECK_IDS.policyMissingToolOwner, "manual", "Add owner metadata in the owning tool declaration.", {
		policyPath: ["tools", "requireMetadata"],
		configTargets: ["tools"]
	}),
...
const POLICY_FIX_METADATA_BY_CHECK_ID = new Map(POLICY_FIX_METADATA.map((rule) => [rule.checkId, rule]));
//#endregion
//#region extensions/policy/src/doctor/automatic-repairs.ts
const AUTOMATIC_REPAIR_CHECK_IDS = /* @__PURE__ */ new Set([
	CHECK_IDS.policyAgentsToolNotDenied,
	CHECK_IDS.policyToolsElevatedEnabled,
	CHECK_IDS.policyToolsRequiredDenyMissing,
	CHECK_IDS.policyGatewayControlUiInsecure,
...
function repairPolicyAutomaticNarrower(ctx, findings, checkId) {
	if (!workspaceRepairsEnabled$1(ctx)) return Promise.resolve(workspaceRepairsDisabledResult$1());
	if (!AUTOMATIC_REPAIR_CHECK_IDS.has(checkId)) return Promise.resolve({
		status: "skipped",
		reason: "policy finding is not an automatic narrowing repair",
		changes: []
	});
	if (findings.length === 0 || findings.some((finding) => finding.checkId !== checkId || POLICY_FIX_METADATA_BY_CHECK_ID.get(finding.checkId)?.fixClass !== "automatic")) return Promise.resolve({
		status: "skipped",
		reason: "policy finding is not classified as automatic",
		changes: []
	});
	const patch = applyAutomaticPatch(ctx.cfg, findings, checkId);
	if (patch.changes.length === 0) return Promise.resolve({
		status: "skipped",
		reason: "policy automatic repair had no config changes to apply",
		changes: [],
		...patch.warnings !== void 0 ? { warnings: patch.warnings } : {}
	});
...
function applyAutomaticPatch(cfg, findings, checkId) {
	switch (checkId) {
		case CHECK_IDS.policyAgentsToolNotDenied: return mergeRequiredDenyTools(cfg, findings);
		case CHECK_IDS.policyToolsElevatedEnabled:
			if (hasScopedPolicyRequirement(findings)) return skippedUnsafeScopedRepair(cfg, "Skipped scoped tools repair. Scoped elevated-tools policy findings are detect-only because automatic repair cannot safely choose between shared and agent-local config targets.");
			return disableElevatedTools(cfg, findings);
		case CHECK_IDS.policyToolsRequiredDenyMissing: return mergeRequiredDenyTools(cfg, findings);
		case CHECK_IDS.policyGatewayControlUiInsecure: return disableInsecureControlUi(cfg, findings);
		case CHECK_IDS.policyGatewayHttpEndpointEnabled: return setFindingConfigValues(cfg, findings, "enabled", false);
		case CHECK_IDS.policyGatewayRemoteEnabled: return disableRemoteGatewayMode(cfg, findings);
		case CHECK_IDS.policyIngressOpenGroupsDenied: return setFindingConfigValues(cfg, findings, "groupPolicy", "allowlist");
		case CHECK_IDS.policyIngressGroupMentionRequired: return setFindingConfigValues(cfg, findings, "requireMention", true);
		case CHECK_IDS.policyDataHandlingRedactionDisabled:
			if (hasScopedPolicyRequirement(findings)) return skippedUnsafeScopedRepair(cfg, "Skipped scoped data-handling repair. The finding reports shared logging config, so changing it would affect more than the scoped policy target.");
			return enableSensitiveLoggingRedaction(cfg);
		case CHECK_IDS.policyDataHandlingTelemetryContentCapture:
			if (hasScopedPolicyRequirement(findings)) return skippedUnsafeScopedRepair(cfg, "Skipped scoped data-handling repair. The finding reports shared telemetry config, so changing it would affect more than the scoped policy target.");
...
function mergeRequiredDenyTools(cfg, findings) {
	const next = cloneConfig(cfg);
	const changes = [];
	const warnings = [];
	for (const finding of findings) {
		const tool = missingRequiredTool(finding);
		if (tool === void 0 || finding.ocPath === void 0) continue;
		if (hasScopedPolicyRequirement([finding]) && finding.ocPath === "oc://openclaw.config/tools/deny") {
			warnings.push(`Skipped scoped deny repair for ${tool}. The finding reports inherited root tools.deny, so changing it would affect more than the scoped policy target.`);
			continue;
		}
		if (mergeStringArrayAtOcPath(next, finding.ocPath, tool)) changes.push(`Added ${tool} to ${configPathLabel(finding.ocPath)} for policy conformance.`);
	}
	return changes.length > 0 ? {
		config: next,
		changes: uniqueStrings$1(changes),
		warnings
	} : {
		config: cfg,
		changes,
		warnings: uniqueStrings$1(warnings)
	};
}
function disableElevatedTools(cfg, findings) {
	if (!findings.some((finding) => finding.ocPath === "oc://openclaw.config/tools/elevated/enabled")) return {
		config: cfg,
		changes: []
	};
	const next = cloneConfig(cfg);
	const elevated = ensureRecord(ensureRecord(next, "tools"), "elevated");
	if (elevated.enabled === false) return {
		config: cfg,
		changes: []
	};
...
function disableInsecureControlUi(cfg, findings) {
	const next = cloneConfig(cfg);
	const controlUi = ensureRecord(ensureRecord(next, "gateway"), "controlUi");
	const changes = [];
	const fields = [
		["allowInsecureAuth", "oc://openclaw.config/gateway/controlUi/allowInsecureAuth"],
		["dangerouslyDisableDeviceAuth", "oc://openclaw.config/gateway/controlUi/dangerouslyDisableDeviceAuth"],
		["dangerouslyAllowHostHeaderOriginFallback", "oc://openclaw.config/gateway/controlUi/dangerouslyAllowHostHeaderOriginFallback"]
	];
	const findingPaths = new Set(findings.map((finding) => finding.ocPath));
	for (const [field, ocPath] of fields) if (findingPaths.has(ocPath) && controlUi[field] !== false) {
...
function disableRemoteGatewayMode(cfg, findings) {
	if (!findings.some((finding) => finding.ocPath === "oc://openclaw.config/gateway/mode")) return {
		config: cfg,
		changes: []
	};
	const next = cloneConfig(cfg);
...
function enableSensitiveLoggingRedaction(cfg) {
	const next = cloneConfig(cfg);
	const logging = ensureRecord(next, "logging");
	if (logging.redactSensitive !== "off") return {
		config: cfg,
		changes: []
	};
...
function disableTelemetryContentCapture(cfg) {
	const next = cloneConfig(cfg);
	const otel = ensureRecord(ensureRecord(next, "diagnostics"), "otel");
	if (otel.captureContent === false) return {
		config: cfg,
		changes: []
	};
...
function setFindingConfigValues(cfg, findings, fieldName, value) {
	const next = cloneConfig(cfg);
	const changes = [];
	const warnings = [];
	for (const finding of findings) {
		if (isScopedInheritedChannelDefaultFinding(finding)) {
			warnings.push(`Skipped scoped channel ingress repair for ${configPathLabel(finding.ocPath ?? "")}. The finding reports inherited channels.defaults config, so changing it would affect more than the scoped channel target.`);
			continue;
		}
		if (finding.ocPath === void 0 || configPathSegments(finding.ocPath).at(-1) !== fieldName || !setValueAtOcPath(next, finding.ocPath, value)) continue;
...
function mergeStringArrayAtOcPath(cfg, ocPath, entry) {
	const segments = configPathSegments(ocPath);
	if (segments.length === 0 || segments.at(-1) !== "deny") return false;
	let current = cfg;
	for (let index = 0; index < segments.length - 1; index += 1) {
		...
		if (segment.startsWith("#")) {
			...
			if (!Array.isArray(current) || !Number.isInteger(arrayIndex) || arrayIndex < 0) return false;
			current = current[arrayIndex];
			continue;
		}
		if (!isRecord$1(current)) return false;
		const nextSegment = segments[index + 1];
		if (current[segment] === void 0) current[segment] = nextSegment?.startsWith("#") ? [] : {};
		current = current[segment];
	}
	if (!isRecord$1(current)) return false;
	const existing = current.deny;
	if (existing !== void 0 && !Array.isArray(existing)) return false;
	const deny = existing ?? [];
	if (deny.some((value) => typeof value === "string" && value === entry)) return false;
	current.deny = [...deny, entry];
	return true;
}
function configPathSegments(ocPath) {
	if (!ocPath.startsWith("oc://openclaw.config/")) return [];
	return splitConfigPath(ocPath.slice(21));
}
...
function configPathLabel(ocPath) {
	let label = "";
	for (const segment of configPathSegments(ocPath)) if (segment.startsWith("#")) label += `[${segment.slice(1)}]`;
	else label += label === "" ? segment : `.${segment}`;
	return label;
}
function missingRequiredTool(finding) {
	return finding.message.match(/required tool '([^']+)'/)?.[1]?.trim();
}
function setValueAtOcPath(cfg, ocPath, value) {
	const segments = configPathSegments(ocPath);
	if (segments.length === 0) return false;
	let current = cfg;
	for (let index = 0; index < segments.length - 1; index += 1) {
		const segment = segments[index];
		if (segment === void 0 || segment.startsWith("#")) return false;
		if (!isRecord$1(current)) return false;
		const existing = current[segment];
		if (existing !== void 0 && !isRecord$1(existing)) return false;
		if (existing === void 0) current[segment] = {};
		current = current[segment];
	}
	if (!isRecord$1(current)) return false;
	const last = segments.at(-1);
	if (last === void 0 || last.startsWith("#") || current[last] === value) return false;
	current[last] = value;
	return true;
}
function workspaceRepairsEnabled$1(ctx) {
	const plugins = isRecord$1(ctx.cfg.plugins) ? ctx.cfg.plugins : {};
	const entries = isRecord$1(plugins.entries) ? plugins.entries : {};
	const policy = isRecord$1(entries.policy) ? entries.policy : {};
	return (isRecord$1(policy.config) ? policy.config : {}).workspaceRepairs === true;
}
function workspaceRepairsDisabledResult$1() {
	return {
		status: "skipped",
		reason: "workspace repairs are disabled",
		changes: [],
		warnings: ["Skipped policy config repair. Enable plugins.entries.policy.config.workspaceRepairs to let doctor --fix edit workspace policy config."]
	};
}
function hasScopedPolicyRequirement(findings) {
	return findings.some((finding) => finding.requirement?.includes("/scopes/") === true);
}
function skippedUnsafeScopedRepair(cfg, warning) {
	return {
		config: cfg,
		changes: [],
		warnings: [warning]
...
function isScopedInheritedChannelDefaultFinding(finding) {
	return hasScopedPolicyRequirement([finding]) && finding.ocPath?.startsWith("oc://openclaw.config/channels/defaults/") === true;
}
...
function createPolicyIngressChecks(deps) {
	...
	return [
		...
		},
		{
			...
			async repair(ctx, findings) {
				return repairPolicyAutomaticNarrower(ctx, findings, CHECK_IDS.policyIngressGroupMentionRequired);
			}
		}
	];
}
	...
	return [
		{
			...
			repair(ctx, findings) {
				return repairPolicyAutomaticNarrower(ctx, findings, CHECK_IDS.policyDataHandlingRedactionDisabled);
			}
		},
		{
			...
			repair(ctx, findings) {
				return repairPolicyAutomaticNarrower(ctx, findings, CHECK_IDS.policyDataHandlingTelemetryContentCapture);
			}
		},
		{
...
//#endregion
//#region extensions/policy/src/doctor/review-required-repairs.ts
const REVIEW_REQUIRED_REPAIR_CHECK_IDS = /* @__PURE__ */ new Set([CHECK_IDS.policyGatewayNonLoopbackBind, CHECK_IDS.policyGatewayNodeCommandDenied]);
function previewPolicyReviewRequiredRepair(_ctx, findings, checkId) {
	const metadata = POLICY_FIX_METADATA_BY_CHECK_ID.get(checkId);
	if (!REVIEW_REQUIRED_REPAIR_CHECK_IDS.has(checkId) || metadata?.fixClass !== "reviewRequired") return Promise.resolve({
		status: "skipped",
		reason: "policy finding does not have a review-required repair preview",
		changes: []
	});
	if (findings.length === 0 || findings.some((finding) => finding.checkId !== checkId || POLICY_FIX_METADATA_BY_CHECK_ID.get(finding.checkId)?.fixClass !== "reviewRequired")) return Promise.resolve({
		status: "skipped",
	...
	if (previews.length === 0) return Promise.resolve({
		status: "skipped",
...
function previewForFinding(finding, checkId) {
	switch (checkId) {
		case CHECK_IDS.policyGatewayNonLoopbackBind: return previewGatewayLoopbackBind(finding);
		case CHECK_IDS.policyGatewayNodeCommandDenied: return previewGatewayNodeDenyCommand(finding);
		default: return [];
	}
}
function previewGatewayLoopbackBind(finding) {
	if (finding.ocPath !== "oc://openclaw.config/gateway/bind" && finding.ocPath !== "oc://openclaw.config/gateway/customBindHost") return [];
	return [{
...
function previewGatewayNodeDenyCommand(finding) {
	const command = finding.message.match(/Gateway node command '([^']+)'/)?.[1]?.trim();
	if (command === void 0 || command === "" || finding.ocPath !== "oc://openclaw.config/gateway/nodes/denyCommands") return [];
	return [{
	...
	return [
		{
			...
			repair(ctx, findings) {
				return previewPolicyReviewRequiredRepair(ctx, findings, CHECK_IDS.policyGatewayNonLoopbackBind);
			}
		},
		{
		...
		},
		{
			...
			repair(ctx, findings) {
				return repairPolicyAutomaticNarrower(ctx, findings, CHECK_IDS.policyGatewayHttpEndpointEnabled);
			}
		},
		{
		...
		},
		{
			...
			repair(ctx, findings) {
				return previewPolicyReviewRequiredRepair(ctx, findings, CHECK_IDS.policyGatewayNodeCommandDenied);
			}
		}
	];
}
...
function gatewayNodeCommandFindings(policy, policyDocName, evidence) {
	if (!hasValidOptionalStringList(policy, [
		"gateway",
		"nodes",
		"denyCommands"
	])) return [];
	const policyDenied = readStringList(policy, [
		"gateway",
		"nodes",
		"denyCommands"
	], { lowercase: false });
	if (policyDenied.length === 0) return [];
	const configDenied = new Set((evidence.gatewayExposure ?? []).filter((entry) => entry.kind === "nodeDenyCommand" && entry.command !== void 0).map((entry) => entry.command));
	return policyDenied.filter((command) => !configDenied.has(command)).map((command) => {
		return {
...
function hasValidOptionalStringList(policy, path) {
	let current = policy;
	for (const part of path) {
		if (!isRecord(current)) return true;
		current = current[part];
	}
	return current === void 0 || Array.isArray(current) && current.every((entry) => typeof entry === "string" && entry.trim() !== "");
}
...
//#endregion
//#region extensions/policy/src/policy-state-helpers.ts
...
//#endregion
//#region extensions/policy/src/policy-state-types.ts
...
//#endregion
//#region extensions/policy/src/policy-state-core.ts
function scanPolicyChannels(cfg) {
	return Object.entries(configuredChannels(cfg)).filter(([id]) => !RESERVED_CHANNEL_CONFIG_KEYS.has(id)).toSorted(([a], [b]) => a.localeCompare(b)).map(([id, value]) => {
		...
		if (isRecord$2(value) && typeof value.enabled === "boolean") entry.enabled = value.enabled;
		return entry;
	});
}
...
function configuredChannels(cfg) {
	return isRecord$2(cfg.channels) ? cfg.channels : {};
}
function configuredMcpServers(cfg) {
	return isRecord$2(cfg.mcp) && isRecord$2(cfg.mcp.servers) ? cfg.mcp.servers : {};
}
function mcpServerTransport(value) {
	if (!isRecord$2(value)) return "unknown";
...
function configuredModelProviders(cfg) {
	return isRecord$2(cfg.models) && isRecord$2(cfg.models.providers) ? cfg.models.providers : {};
}
...
function collectModelRefsFromRecord(refs, value, source) {
	for (const [key, child] of Object.entries(value)) {
		...
		if (Array.isArray(child)) {
			for (const [index, item] of child.entries()) if (isRecord$2(item)) collectModelRefsFromRecord(refs, item, `${childPath}/#${index}`);
			continue;
		}
		if (isRecord$2(child)) collectModelRefsFromRecord(refs, child, childPath);
	}
}
function collectModelRefsFromAgentAllowlist(refs, agents) {
	const defaults = agents.defaults;
	if (isRecord$2(defaults) && isRecord$2(defaults.models)) collectModelRefsFromModelMap(refs, defaults.models, "oc://openclaw.config/agents/defaults/models");
	const list = agents.list;
	if (!Array.isArray(list)) return;
	for (const [index, agent] of list.entries()) {
		if (!isRecord$2(agent) || !isRecord$2(agent.models)) continue;
...
//#endregion
//#region extensions/policy/src/policy-state-data.ts
...
function scanPolicyAuthProfiles(cfg) {
	const auth = isRecord$2(cfg.auth) ? cfg.auth : {};
	const profiles = isRecord$2(auth.profiles) ? auth.profiles : {};
...
function scanPolicyDataHandling(cfg) {
	const entries = [];
	const logging = isRecord$2(cfg.logging) ? cfg.logging : {};
	entries.push({
	...
	const diagnostics = isRecord$2(cfg.diagnostics) ? cfg.diagnostics : {};
	const otel = isRecord$2(diagnostics.otel) ? diagnostics.otel : {};
	...
	const session = isRecord$2(cfg.session) ? cfg.session : {};
	const maintenance = isRecord$2(session.maintenance) ? session.maintenance : {};
...
function pushMemorySessionTranscriptIndexing(entries, cfg) {
	const memory = isRecord$2(cfg.memory) ? cfg.memory : {};
	const qmd = isRecord$2(memory.qmd) ? memory.qmd : {};
	const qmdSessions = isRecord$2(qmd.sessions) ? qmd.sessions : {};
	...
	const agents = isRecord$2(cfg.agents) ? cfg.agents : {};
	const defaults = isRecord$2(agents.defaults) ? agents.defaults : {};
	const defaultsMemorySearch = isRecord$2(defaults.memorySearch) ? defaults.memorySearch : {};
...
function memorySearchSessionTranscriptIndexing(memorySearch, inheritedMemorySearch) {
	if (!isRecord$2(memorySearch)) return;
	const experimental = isRecord$2(memorySearch.experimental) ? memorySearch.experimental : {};
	const inherited = isRecord$2(inheritedMemorySearch) ? inheritedMemorySearch : {};
	const inheritedExperimental = isRecord$2(inherited.experimental) ? inherited.experimental : {};
...
function memorySearchSessionTranscriptIndexingHasLocalConfig(memorySearch) {
	if (!isRecord$2(memorySearch)) return false;
	const experimental = isRecord$2(memorySearch.experimental) ? memorySearch.experimental : {};
...
function memorySearchSourcesIncludeSessions(memorySearch) {
	if (!isRecord$2(memorySearch) || memorySearch.sources === void 0) return;
...
function scanPolicySecretProviders(cfg) {
	const secrets = isRecord$2(cfg.secrets) ? cfg.secrets : {};
	const providers = isRecord$2(secrets.providers) ? secrets.providers : {};
	return Object.entries(providers).map(([id, value]) => {
		...
		if (isRecord$2(value) && typeof value.source === "string") entry.providerSource = value.source;
...
function scanPolicySecretInputs(cfg) {
	const entries = [];
	collectSecretInputs(entries, cfg, [], secretRefDefaults((isRecord$2(cfg.secrets) ? cfg.secrets : {}).defaults));
	return entries;
}
...
//#endregion
//#region extensions/policy/src/policy-state-exec-approvals.ts
...
function scanPolicyExecApprovals(raw) {
	...
	if (!isRecord$2(parsed) || parsed.version !== 1) return [];
	const evidence = [];
	const defaults = isRecord$2(parsed.defaults) ? parsed.defaults : {};
	...
	for (const agent of normalizedExecApprovalAgents(parsed.agents)) {
		...
		for (const [index, entry] of agent.allowlistEntries.entries()) {
			const allowlistSource = `oc://exec-approvals.json/agents/${ocPathSegment(entry.sourceAgentId)}/allowlist/#${entry.index}`;
			evidence.push({
...
function normalizedExecApprovalAgents(rawAgents) {
	if (!isRecord$2(rawAgents)) return [];
	const agents = Object.entries(rawAgents).filter((entry) => isRecord$2(entry[1]));
...
//#endregion
//#region extensions/policy/src/policy-state-gateway.ts
function scanPolicyGatewayExposure(cfg) {
	const gateway = isRecord$2(cfg.gateway) ? cfg.gateway : {};
	const entries = [];
	...
	const tailscale = isRecord$2(gateway.tailscale) ? gateway.tailscale : {};
	...
	const auth = isRecord$2(gateway.auth) ? gateway.auth : {};
	entries.push({
	...
	entries.push({
		...
		value: isRecord$2(auth.rateLimit),
		explicit: isRecord$2(auth.rateLimit)
	});
	const controlUi = isRecord$2(gateway.controlUi) ? gateway.controlUi : {};
	...
	const remote = isRecord$2(gateway.remote) ? gateway.remote : {};
	if (gateway.mode === "remote") {
		entries.push({
	...
	const http = isRecord$2(gateway.http) ? gateway.http : {};
	const endpoints = isRecord$2(http.endpoints) ? http.endpoints : {};
	...
	pushGatewayNodeCommandEvidence(entries, isRecord$2(gateway.nodes) ? gateway.nodes : {});
	return entries.toSorted((a, b) => a.source.localeCompare(b.source));
}
...
function pushGatewayHttpUrlFetchEvidence(entries, endpointSource, endpoint, path, value) {
	const allowUrl = isRecord$2(value) ? value.allowUrl : void 0;
	if (allowUrl === false || allowUrl !== true && endpoint !== "responses") return;
	const allowlist = isRecord$2(value) ? value.urlAllowlist : void 0;
...
function pushGatewayNodeCommandEvidence(entries, nodes) {
	const deniedCommands = new Set(Array.isArray(nodes.denyCommands) ? nodes.denyCommands.filter((command) => typeof command === "string").map((command) => command.trim()) : []);
	if (Array.isArray(nodes.denyCommands)) nodes.denyCommands.forEach((command, index) => {
	...
	if (!Array.isArray(nodes.allowCommands)) return;
	nodes.allowCommands.forEach((command, index) => {
		if (typeof command !== "string") return;
		const normalized = command.trim();
		if (normalized === "" || deniedCommands.has(normalized)) return;
		entries.push({
...
//#endregion
//#region extensions/policy/src/policy-state-tool-posture.ts
function scanPolicyToolPosture(cfg) {
	const globalTools = isRecord$2(cfg.tools) ? cfg.tools : {};
	const agents = isRecord$2(cfg.agents) ? cfg.agents : {};
	const defaults = isRecord$2(agents.defaults) ? agents.defaults : {};
	const defaultSandbox = isRecord$2(defaults.sandbox) ? defaults.sandbox : {};
	const entries = [];
...
function pushToolFsPosture(entries, params) {
	const localFs = isRecord$2(params.tools.fs) ? params.tools.fs : {};
	const inheritedFs = isRecord$2(params.inheritedTools.fs) ? params.inheritedTools.fs : {};
...
function pushToolExecPosture(entries, params) {
	const localExec = isRecord$2(params.tools.exec) ? params.tools.exec : {};
	const inheritedExec = isRecord$2(params.inheritedTools.exec) ? params.inheritedTools.exec : {};
...
function pushToolElevatedPosture(entries, params) {
	const localElevated = isRecord$2(params.tools.elevated) ? params.tools.elevated : {};
	const inheritedElevated = isRecord$2(params.inheritedTools.elevated) ? params.inheritedTools.elevated : {};
	...
	const localAllowFrom = isRecord$2(localElevated.allowFrom) ? localElevated.allowFrom : {};
	const inheritedAllowFrom = isRecord$2(inheritedElevated.allowFrom) ? inheritedElevated.allowFrom : {};
	const providers = [.../* @__PURE__ */ new Set([...Object.keys(inheritedAllowFrom), ...Object.keys(localAllowFrom)])].toSorted((a, b) => a.localeCompare(b));
...
//#endregion
//#region extensions/policy/src/policy-state-ingress.ts
const ALLOWLIST_DEFAULT_INGRESS_GROUP_POLICY_CHANNELS = /* @__PURE__ */ new Set([
	"googlechat",
	"irc",
	"line",
	"mattermost",
...
const OPEN_GROUPS_DEFAULT_TO_NO_MENTION_CHANNELS = /* @__PURE__ */ new Set(["feishu", "qa-channel"]);
function scanPolicyIngress(cfg) {
	const channels = configuredChannels(cfg);
	const inheritedChannelDefaults = pickSupportedIngressDefaults(isRecord$2(channels.defaults) ? channels.defaults : {});
	const channelDefaultsSource = "oc://openclaw.config/channels/defaults";
	const entries = [];
	const dmScope = normalizeOptionalString((isRecord$2(cfg.session) ? cfg.session : {}).dmScope)?.toLowerCase();
	entries.push({
		id: "session-dm-scope",
		kind: "sessionDmScope",
	...
	for (const [channel, value] of Object.entries(channels)) {
		if (RESERVED_CHANNEL_CONFIG_KEYS.has(channel) || !isRecord$2(value) || value.enabled === false) continue;
		const channelSource = `oc://openclaw.config/channels/${ocPathSegment(channel)}`;
		const accounts = isRecord$2(value.accounts) ? value.accounts : {};
		const configuredAccounts = Object.entries(accounts).filter((entry) => isRecord$2(entry[1]));
...
function channelImplicitGroupPolicy(params) {
	for (const [config, sourceBase] of [
		...
		for (const key of ["groups"]) {
			const container = isRecord$2(config[key]) ? config[key] : void 0;
...
function pushChannelRequireMentionIngress(entries, params) {
	...
	for (const { containerKey, container, sourceBase } of containers) for (const [groupId, groupConfig] of Object.entries(container)) {
		if (!isRecord$2(groupConfig)) continue;
...
function channelWildcardRequireMention(params) {
	for (const [config, sourceBase] of [
		...
		for (const key of [
			...
			const container = isRecord$2(config[key]) ? config[key] : void 0;
			const wildcard = isRecord$2(container?.["*"]) ? container["*"] : void 0;
...
function nestedIngressContainers(params) {
	...
	for (const key of [
		...
		const local = isRecord$2(params.config[key]) ? params.config[key] : void 0;
		const inherited = isRecord$2(params.inheritedConfig[key]) ? params.inheritedConfig[key] : void 0;
		if (local !== void 0) {
...
function pushNestedRequireMentionIngress(entries, params, containerKey, groupId, config, parentSourceBase) {
	...
	for (const nestedKey of ["channels", "topics"]) {
		...
		for (const [nestedId, nestedConfig] of Object.entries(nested)) if (isRecord$2(nestedConfig)) pushNestedRequireMentionIngress(entries, params, `${containerKey}/${ocPathSegment(groupId)}/${nestedKey}`, nestedId, nestedConfig, parentSourceBase);
	}
}
function channelDmPolicy(config) {
	const dm = isRecord$2(config.dm) ? config.dm : {};
...
//#endregion
//#region extensions/policy/src/policy-state-sandbox.ts
...
function scanPolicySandboxPosture(cfg) {
	...
	(Array.isArray(agents.list) ? agents.list : []).forEach((agent, index) => {
		if (!isRecord$2(agent)) return;
		const agentId = typeof agent.id === "string" && agent.id.trim() !== "" ? agent.id.trim() : void 0;
		const sandbox = isRecord$2(agent.sandbox) ? agent.sandbox : {};
		pushSandboxPostureEvidence(entries, {
			id: agentId ?? `agent-${index}`,
			scope: "agent",
			agentId,
...
function pushSandboxDockerPosture(entries, params) {
	const localDocker = !params.sharedSandboxScope && isRecord$2(params.sandbox.docker) ? params.sandbox.docker : {};
	const inheritedDocker = isRecord$2(params.inheritedSandbox.docker) ? params.inheritedSandbox.docker : {};
...
function pushSandboxBrowserPosture(entries, params) {
	const localBrowser = !params.sharedSandboxScope && isRecord$2(params.sandbox.browser) ? params.sandbox.browser : {};
	const inheritedBrowser = isRecord$2(params.inheritedSandbox.browser) ? params.inheritedSandbox.browser : {};
	...
	if (inheritedBrowser.binds !== void 0 || localBrowser.binds !== void 0) pushSandboxBindPosture(entries, params, {
		...
		const localDocker = !params.sharedSandboxScope && isRecord$2(params.sandbox.docker) ? params.sandbox.docker : {};
		pushSandboxBindPosture(entries, params, {
			inheritedBinds: readStringArray((isRecord$2(params.inheritedSandbox.docker) ? params.inheritedSandbox.docker : {}).binds),
			localBinds: readStringArray(localDocker.binds),
			sourceSuffix: "docker/binds",
			surface: "browser"
		});
...
//#endregion
//#region extensions/policy/src/policy-state-tools.ts
...
//#endregion
//#region extensions/policy/src/policy-state-workspace.ts
function scanPolicyAgentWorkspace(cfg) {
	const agents = isRecord$2(cfg.agents) ? cfg.agents : {};
	const defaults = isRecord$2(agents.defaults) ? agents.defaults : {};
	const defaultSandbox = isRecord$2(defaults.sandbox) ? defaults.sandbox : {};
	const defaultTools = isRecord$2(cfg.tools) ? cfg.tools : {};
	const entries = [];
	...
	(Array.isArray(agents.list) ? agents.list : []).forEach((agent, index) => {
		...
		const agentId = typeof agent.id === "string" && agent.id.trim() !== "" ? agent.id.trim() : void 0;
		const sandbox = isRecord$2(agent.sandbox) ? agent.sandbox : {};
		const tools = isRecord$2(agent.tools) ? agent.tools : {};
		pushAgentWorkspaceEvidence(entries, {
			id: agentId ?? `agent-${index}`,
			scope: "agent",
			agentId,
...
function configuredSandboxToolDenyEntries(tools) {
	const sandbox = isRecord$2(tools.sandbox) ? tools.sandbox : {};
	const sandboxTools = isRecord$2(sandbox.tools) ? sandbox.tools : {};
	return Array.isArray(sandboxTools.deny) ? readStringArray(sandboxTools.deny) : void 0;
}
//#endregion
//#region extensions/policy/src/policy-state-attestation.ts
...
function stableJson(value) {
	...
	if (isRecord$2(value)) return `{${Object.entries(value).toSorted(([a], [b]) => a.localeCompare(b)).map(([key, child]) => `${JSON.stringify(key)}:${stableJson(child)}`).join(",")}}`;
	return JSON.stringify(value);
}
//#endregion
...
//#endregion
//#region extensions/policy/src/doctor/policy-constants.ts
...
//#endregion
//#region extensions/policy/src/doctor/policy-runtime.ts
...
function parseExecApprovalsFile(raw) {
	try {
		...
		if (!isRecord$2(value) || value.version !== 1) return {
			ok: false,
...
function readChannelDenyRules(policy, policyDocName) {
	if (!isRecord$2(policy) || !isRecord$2(policy.channels) || !Array.isArray(policy.channels.denyRules)) return [];
	return policy.channels.denyRules.map((rule, index) => ({
		rule,
		index
	})).filter((entry) => isChannelDenyRule$1(entry.rule)).map(({ rule, index }) => {
		const next = {
			when: rule.when,
...
function isChannelDenyRule$1(value) {
	return isRecord$2(value) && (value.id === void 0 || typeof value.id === "string") && (value.reason === void 0 || typeof value.reason === "string") && isRecord$2(value.when) && typeof value.when.provider === "string";
}
...
function disableChannels(cfg, channelIds) {
	if (!isRecord$2(cfg.channels)) return {
		config: cfg,
		changed: []
	};
	...
	for (const id of channelIds) {
		...
		if (!isRecord$2(current) || current.enabled === false) continue;
		channels[id] = {
			...current,
			enabled: false
		};
...
function policySettings(ctx) {
	...
	if (!isRecord$2(pluginConfig)) return {};
	return pluginConfig;
}
function policyChecksEnabled(ctx, settings) {
	const entry = ctx.cfg.plugins?.entries?.["policy"];
	if (!isRecord$2(entry) || entry.enabled === false) return false;
...
//#endregion
//#region extensions/policy/src/doctor/shape-helpers.ts
...
function policyStringArrayShapeFinding(value, params) {
	...
	if (!isRecord$2(value)) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/${params.target}`, `${params.policyPath} ${params.property} must be an object.`, `Fix ${params.policyPath} so ${params.property} is an object.`);
	const unsupportedKey = unsupportedPolicyKey(value, ["allow", "deny"]);
	if (unsupportedKey !== void 0) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/${params.target}/${ocPathSegment$1(unsupportedKey)}`, `${params.policyPath} ${params.property}.${unsupportedKey} is not supported in policy.`, `Remove ${params.property}.${unsupportedKey} or use ${params.property}.allow or ${params.property}.deny.`);
...
//#endregion
//#region extensions/policy/src/doctor/access-findings.ts
function authProfileMetadataRequirementFindings(policy, policyPath, policyDocName) {
	if (!isRecord$2(policy) || !isRecord$2(policy.auth) || !isRecord$2(policy.auth.profiles) || policy.auth.profiles.requireMetadata === void 0) return [];
...
function invalidChannelDenyRuleFindings(policy, policyPath, policyDocName) {
	if (!isRecord$2(policy) || !isRecord$2(policy.channels) || policy.channels.denyRules === void 0) return [];
	...
	for (const [index, rule] of policy.channels.denyRules.entries()) {
		...
		if (unsupportedRuleKey !== void 0) return [{
			...
			source: "policy",
			path: policyPath,
			target: `oc://${policyDocName}/channels/denyRules/#${index}/${ocPathSegment$1(unsupportedRuleKey)}`,
			fixHint: `Remove channels.denyRules[${index}].${unsupportedRuleKey} or use id, when.provider, and reason.`
		}];
		if (isRecord$2(rule.when)) {
			...
			if (unsupportedWhenKey !== void 0) return [{
				...
				source: "policy",
				path: policyPath,
				target: `oc://${policyDocName}/channels/denyRules/#${index}/when/${ocPathSegment$1(unsupportedWhenKey)}`,
				fixHint: `Remove channels.denyRules[${index}].when.${unsupportedWhenKey} or use when.provider.`
			}];
		}
	}
	const invalid = policy.channels.denyRules.findIndex((rule) => !isChannelDenyRule$1(rule));
	if (invalid < 0) return [];
	return [{
		checkId: CHECK_IDS.policyInvalidFile,
		severity: "error",
...
//#endregion
//#region extensions/policy/src/doctor/agent-tool-shapes.ts
function agentWorkspacePolicyShapeFinding(value, params) {
	if (value === void 0) return;
	if (!isRecord$2(value)) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/${params.targetPrefix}`, `${params.policyPath} ${params.propertyPrefix} must be an object.`, `Fix ${params.policyPath} so ${params.propertyPrefix} is an object.`);
	const unsupportedWorkspaceKey = unsupportedPolicyKey(value, ["allowedAccess", "denyTools"]);
	if (unsupportedWorkspaceKey !== void 0) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/${params.targetPrefix}/${ocPathSegment$1(unsupportedWorkspaceKey)}`, `${params.policyPath} ${params.propertyPrefix}.${unsupportedWorkspaceKey} is not supported in agent workspace policy.`, `Remove ${params.propertyPrefix}.${unsupportedWorkspaceKey} or use a supported agent workspace policy rule.`);
...
function toolPosturePolicyShapeFinding(tools, params) {
	...
	if (unsupportedTopLevel !== void 0) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/${targetPrefix}/${ocPathSegment$1(unsupportedTopLevel)}`, `${params.policyPath} ${propertyPrefix}.${unsupportedTopLevel} is not supported in tools policy.`, `Remove ${propertyPrefix}.${unsupportedTopLevel} or use a supported tools policy rule.`);
	for (const section of [
		...
		"elevated",
		"alsoAllow"
	]) if (tools[section] !== void 0 && !isRecord$2(tools[section])) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/${targetPrefix}/${section}`, `${params.policyPath} ${propertyPrefix}.${section} must be an object.`, `Fix ${params.policyPath} so ${propertyPrefix}.${section} is an object.`);
	const profiles = isRecord$2(tools.profiles) ? tools.profiles : {};
	const unsupportedProfileKey = unsupportedPolicyKey(profiles, ["allow"]);
	if (unsupportedProfileKey !== void 0) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/${targetPrefix}/profiles/${ocPathSegment$1(unsupportedProfileKey)}`, `${params.policyPath} ${propertyPrefix}.profiles.${unsupportedProfileKey} is not supported in tools policy.`, `Remove ${propertyPrefix}.profiles.${unsupportedProfileKey} or use ${propertyPrefix}.profiles.allow.`);
	...
	const alsoAllow = isRecord$2(tools.alsoAllow) ? tools.alsoAllow : {};
	const unsupportedAlsoAllowKey = unsupportedPolicyKey(alsoAllow, ["expected"]);
	if (unsupportedAlsoAllowKey !== void 0) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/${targetPrefix}/alsoAllow/${ocPathSegment$1(unsupportedAlsoAllowKey)}`, `${params.policyPath} ${propertyPrefix}.alsoAllow.${unsupportedAlsoAllowKey} is not supported in tools policy.`, `Remove ${propertyPrefix}.alsoAllow.${unsupportedAlsoAllowKey} or use ${propertyPrefix}.alsoAllow.expected.`);
...
//#endregion
//#region extensions/policy/src/doctor/exec-approval-rules.ts
function execApprovalAllowlistExpectedShapeFinding(value, params) {
	...
	const invalidIndex = value.findIndex((entry) => execApprovalAllowlistRequirement$1(entry) === void 0);
	if (invalidIndex < 0) return;
...
function readExecApprovalAllowlistRequirements(policy, path) {
	...
	const entries = current.map(execApprovalAllowlistRequirement$1);
...
function execApprovalAllowlistMissingTarget(agentId) {
	return agentId === void 0 ? "oc://exec-approvals.json" : `oc://exec-approvals.json/agents/${ocPathSegment$1(agentId)}/allowlist`;
}
...
//#endregion
//#region extensions/policy/src/doctor/access-shapes.ts
function ingressPolicyShapeFinding(value, params) {
	...
	if (value === void 0) return;
	if (!isRecord$2(value)) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/${targetPrefix}`, `${params.policyPath} ${propertyPrefix} must be an object.`, `Fix ${params.policyPath} so ${propertyPrefix} is an object.`);
	...
	if (unsupportedIngressKey !== void 0) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/${targetPrefix}/${ocPathSegment$1(unsupportedIngressKey)}`, `${params.policyPath} ${propertyPrefix}.${unsupportedIngressKey} is not supported in ingress policy.`, `Remove ${propertyPrefix}.${unsupportedIngressKey} or use ingress.session or ingress.channels.`);
	for (const section of ["session", "channels"]) if (value[section] !== void 0 && !isRecord$2(value[section])) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/${targetPrefix}/${section}`, `${params.policyPath} ${propertyPrefix}.${section} must be an object.`, `Fix ${params.policyPath} so ${propertyPrefix}.${section} is an object.`);
	const session = isRecord$2(value.session) ? value.session : {};
	const unsupportedSessionKey = unsupportedPolicyKey(session, ["requireDmScope"]);
	if (unsupportedSessionKey !== void 0) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/${targetPrefix}/session/${ocPathSegment$1(unsupportedSessionKey)}`, `${params.policyPath} ${propertyPrefix}.session.${unsupportedSessionKey} is not supported in ingress policy.`, `Remove ${propertyPrefix}.session.${unsupportedSessionKey} or use ${propertyPrefix}.session.requireDmScope.`);
	...
	const channels = isRecord$2(value.channels) ? value.channels : {};
	const unsupportedChannelsKey = unsupportedPolicyKey(channels, [
		"allowDmPolicies",
		"denyOpenGroups",
		"requireMentionInGroups"
	]);
	if (unsupportedChannelsKey !== void 0) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/${targetPrefix}/channels/${ocPathSegment$1(unsupportedChannelsKey)}`, `${params.policyPath} ${propertyPrefix}.channels.${unsupportedChannelsKey} is not supported in ingress policy.`, `Remove ${propertyPrefix}.channels.${unsupportedChannelsKey} or use a supported ingress channel policy rule.`);
...
function execApprovalsPolicyShapeFinding(value, params) {
	...
	if (unsupportedTopLevel !== void 0) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/${targetPrefix}/${ocPathSegment$1(unsupportedTopLevel)}`, `${params.policyPath} ${propertyPrefix}.${unsupportedTopLevel} is not supported in exec approvals policy.`, `Remove ${propertyPrefix}.${unsupportedTopLevel} or use a supported execApprovals rule.`);
	...
	for (const section of allowDefaults ? ["defaults", "agents"] : ["agents"]) if (value[section] !== void 0 && !isRecord$2(value[section])) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/${targetPrefix}/${section}`, `${params.policyPath} ${propertyPrefix}.${section} must be an object.`, `Fix ${params.policyPath} so ${propertyPrefix}.${section} is an object.`);
	const defaults = allowDefaults && isRecord$2(value.defaults) ? value.defaults : {};
	const unsupportedDefaultsKey = unsupportedPolicyKey(defaults, ["allowSecurity"]);
	if (unsupportedDefaultsKey !== void 0) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/${targetPrefix}/defaults/${ocPathSegment$1(unsupportedDefaultsKey)}`, `${params.policyPath} ${propertyPrefix}.defaults.${unsupportedDefaultsKey} is not supported in exec approvals policy.`, `Use execApprovals.defaults.allowSecurity or remove the unsupported rule.`);
	...
	const agents = isRecord$2(value.agents) ? value.agents : {};
	const unsupportedAgentsKey = unsupportedPolicyKey(agents, [
		"allowAutoAllowSkills",
		"allowSecurity",
		"allowlist"
	]);
	if (unsupportedAgentsKey !== void 0) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/${targetPrefix}/agents/${ocPathSegment$1(unsupportedAgentsKey)}`, `${params.policyPath} ${propertyPrefix}.agents.${unsupportedAgentsKey} is not supported in exec approvals policy.`, `Use execApprovals.agents.allowSecurity, execApprovals.agents.allowAutoAllowSkills, or execApprovals.agents.allowlist.expected.`);
	...
	if (agents.allowlist !== void 0 && !isRecord$2(agents.allowlist)) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/${targetPrefix}/agents/allowlist`, `${params.policyPath} ${propertyPrefix}.agents.allowlist must be an object.`, `Fix ${params.policyPath} so ${propertyPrefix}.agents.allowlist is an object.`);
	const allowlist = isRecord$2(agents.allowlist) ? agents.allowlist : {};
	const unsupportedAllowlistKey = unsupportedPolicyKey(allowlist, ["expected"]);
	if (unsupportedAllowlistKey !== void 0) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/${targetPrefix}/agents/allowlist/${ocPathSegment$1(unsupportedAllowlistKey)}`, `${params.policyPath} ${propertyPrefix}.agents.allowlist.${unsupportedAllowlistKey} is not supported in exec approvals policy.`, `Use execApprovals.agents.allowlist.expected or remove the unsupported rule.`);
...
function agentsPolicyShapeFinding(value, params) {
	if (value === void 0) return;
	if (!isRecord$2(value)) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/agents`, `${params.policyPath} agents must be an object.`, `Fix ${params.policyPath} so agents is an object.`);
	const unsupportedAgentsKey = unsupportedPolicyKey(value, ["workspace"]);
	if (unsupportedAgentsKey !== void 0) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/agents/${ocPathSegment$1(unsupportedAgentsKey)}`, `${params.policyPath} agents.${unsupportedAgentsKey} is not supported in agents policy.`, `Remove agents.${unsupportedAgentsKey} or use agents.workspace.`);
...
function scopedDataHandlingPolicyShapeFinding(dataHandling, params) {
	...
	if (unsupportedKey !== void 0) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/${params.targetPrefix}/dataHandling/${ocPathSegment$1(unsupportedKey)}`, `${params.policyPath} scopes.${params.scopeName}.dataHandling.${unsupportedKey} is not a supported scoped policy section.`, `Move global data-handling rules to top-level dataHandling, or use dataHandling.memory with agentIds.`);
	if (dataHandling.memory !== void 0 && !isRecord$2(dataHandling.memory)) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/${params.targetPrefix}/dataHandling/memory`, `${params.policyPath} scopes.${params.scopeName}.dataHandling.memory must be an object.`, `Fix ${params.policyPath} so the scoped dataHandling.memory policy section is an object.`);
	if (!isRecord$2(dataHandling.memory)) return;
	const unsupportedMemoryKey = Object.keys(dataHandling.memory).find((key) => key !== "denySessionTranscriptIndexing");
	if (unsupportedMemoryKey !== void 0) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/${params.targetPrefix}/dataHandling/memory/${ocPathSegment$1(unsupportedMemoryKey)}`, `${params.policyPath} scopes.${params.scopeName}.dataHandling.memory.${unsupportedMemoryKey} is not a supported scoped policy rule.`, `Use dataHandling.memory.denySessionTranscriptIndexing or remove the unsupported rule.`);
...
function scopedToolsPolicyShapeFinding(value, params) {
	const allowedTopLevel = /* @__PURE__ */ new Set([
		"profiles",
		"fs",
		"exec",
		"elevated",
	...
	if (unsupportedTopLevel !== void 0) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/${params.targetPrefix}/${ocPathSegment$1(unsupportedTopLevel)}`, `${params.policyPath} ${params.propertyPrefix}.${unsupportedTopLevel} is not supported in agent-scoped tools policy.`, `Move ${params.propertyPrefix}.${unsupportedTopLevel} to top-level tools or use a supported scoped tools posture rule.`);
	for (const [section, allowedKeys] of [
		...
		if (!isRecord$2(sectionValue)) continue;
		const allowed = new Set(allowedKeys);
		const unsupportedKey = Object.keys(sectionValue).find((key) => !allowed.has(key));
		if (unsupportedKey !== void 0) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/${params.targetPrefix}/${section}/${ocPathSegment$1(unsupportedKey)}`, `${params.policyPath} ${params.propertyPrefix}.${section}.${unsupportedKey} is not supported in agent-scoped tools policy.`, `Move ${params.propertyPrefix}.${section}.${unsupportedKey} to top-level tools or use a supported scoped tools posture rule.`);
	}
...
//#endregion
//#region extensions/policy/src/doctor/policy-scope.ts
...
function policyHasExecApprovalsRules(policy) {
	if (!isRecord$2(policy)) return false;
...
function execApprovalsPolicyHasRules(value) {
	return isRecord$2(value) && (value.requireFile !== void 0 || isRecord$2(value.defaults) || isRecord$2(value.agents));
}
function policyHasSecretRules(policy) {
	if (!isRecord$2(policy) || !isRecord$2(policy.secrets)) return false;
...
function policyHasAuthProfileRules(policy) {
	return isRecord$2(policy) && isRecord$2(policy.auth) && isRecord$2(policy.auth.profiles) && (policy.auth.profiles.requireMetadata !== void 0 || policy.auth.profiles.allowModes !== void 0);
}
...
function ingressPolicyHasRules(value) {
	if (!isRecord$2(value)) return false;
	const ingress = value;
	return isRecord$2(ingress.session) && ingress.session.requireDmScope !== void 0 || isRecord$2(ingress.channels) && (ingress.channels.allowDmPolicies !== void 0 || ingress.channels.denyOpenGroups !== void 0 || ingress.channels.requireMentionInGroups !== void 0);
}
function policyHasGatewayRules(policy) {
	if (!isRecord$2(policy) || !isRecord$2(policy.gateway)) return false;
	const gateway = policy.gateway;
	return isRecord$2(gateway.exposure) && (gateway.exposure.allowNonLoopbackBind !== void 0 || gateway.exposure.allowTailscaleFunnel !== void 0) || isRecord$2(gateway.auth) && (gateway.auth.requireAuth !== void 0 || gateway.auth.requireExplicitRateLimit !== void 0) || isRecord$2(gateway.controlUi) && gateway.controlUi.allowInsecure !== void 0 || isRecord$2(gateway.remote) && gateway.remote.allow !== void 0 || isRecord$2(gateway.http) && (gateway.http.denyEndpoints !== void 0 || gateway.http.requireUrlAllowlists !== void 0) || isRecord$2(gateway.nodes) && gateway.nodes.denyCommands !== void 0;
}
function policyHasAgentWorkspaceRules(policy) {
	if (!isRecord$2(policy)) return false;
	if (isRecord$2(policy.agents) && workspacePolicyHasRules(policy.agents.workspace)) return true;
	return agentScopedPolicyOverlays(policy).some(([, overlay]) => {
		return workspacePolicyHasRules((isRecord$2(overlay.agents) ? overlay.agents : {}).workspace);
	});
}
...
function sandboxPosturePolicyHasRules(value) {
	if (!isRecord$2(value)) return false;
	const sandbox = value;
	const containers = isRecord$2(sandbox.containers) ? sandbox.containers : void 0;
	const browser = isRecord$2(sandbox.browser) ? sandbox.browser : void 0;
...
function dataHandlingPolicyHasRules(value) {
	...
	return isRecord$2(dataHandling.sensitiveLogging) && dataHandling.sensitiveLogging.requireRedaction !== void 0 || isRecord$2(dataHandling.telemetry) && dataHandling.telemetry.denyContentCapture !== void 0 || isRecord$2(dataHandling.retention) && dataHandling.retention.requireSessionMaintenance !== void 0 || isRecord$2(dataHandling.memory) && dataHandling.memory.denySessionTranscriptIndexing !== void 0;
}
...
function workspacePolicyHasRules(value) {
	return isRecord$2(value) && (value.allowedAccess !== void 0 || value.denyTools !== void 0);
}
function toolPosturePolicyHasRules(value) {
	if (!isRecord$2(value)) return false;
	const tools = value;
	return isRecord$2(tools.profiles) && tools.profiles.allow !== void 0 || isRecord$2(tools.fs) && tools.fs.requireWorkspaceOnly !== void 0 || isRecord$2(tools.exec) && (tools.exec.allowSecurity !== void 0 || tools.exec.requireAsk !== void 0 || tools.exec.allowHosts !== void 0) || isRecord$2(tools.elevated) && tools.elevated.allow !== void 0 || isRecord$2(tools.alsoAllow) && tools.alsoAllow.expected !== void 0 || tools.denyTools !== void 0;
}
function agentScopedPolicyOverlays(policy) {
	if (!isRecord$2(policy) || !isRecord$2(policy.scopes)) return [];
	return Object.entries(policy.scopes).filter((entry) => isRecord$2(entry[1]));
}
...
function duplicateScopedFieldFinding(scopes, params) {
	...
	for (const [scopeName, overlay] of Object.entries(scopes)) {
		if (!isRecord$2(overlay)) continue;
...
function scopedPolicyFields(scopeName, overlay, selector) {
	const prefix = `scopes/${ocPathSegment$1(scopeName)}`;
	return POLICY_RULES.filter((rule) => rule.scopeSelectors?.includes(selector) === true).map((rule) => ({
		...
		targetPath: `${prefix}/${rule.policyPath.map(ocPathSegment$1).join("/")}`,
		metadata: rule,
		value
	}));
}
function scopedPolicyValue(overlay, path) {
	...
	if (!root) return;
	return getPolicyPath(root === "agents" ? overlay.agents : overlay[root], remainingPath);
}
...
//#endregion
//#region extensions/policy/src/doctor/sandbox-gateway-shapes.ts
function sandboxPolicyShapeFinding(value, params) {
	...
	if (unsupportedTopLevel !== void 0) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/${targetPrefix}/${ocPathSegment$1(unsupportedTopLevel)}`, `${params.policyPath} ${propertyPrefix}.${unsupportedTopLevel} is not supported in sandbox policy.`, `Remove ${propertyPrefix}.${unsupportedTopLevel} or use a supported sandbox posture rule.`);
	...
	for (const section of ["containers", "browser"]) if (value[section] !== void 0 && !isRecord$2(value[section])) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/${targetPrefix}/${section}`, `${params.policyPath} ${propertyPrefix}.${section} must be an object.`, `Fix ${params.policyPath} so ${propertyPrefix}.${section} is an object.`);
	const containers = isRecord$2(value.containers) ? value.containers : {};
	const unsupportedContainerKey = unsupportedPolicyKey(containers, SANDBOX_CONTAINER_POLICY_RULES.map((rule) => rule.key));
	if (unsupportedContainerKey !== void 0) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/${targetPrefix}/containers/${ocPathSegment$1(unsupportedContainerKey)}`, `${params.policyPath} ${propertyPrefix}.containers.${unsupportedContainerKey} is not supported in sandbox policy.`, `Remove ${propertyPrefix}.containers.${unsupportedContainerKey} or use a supported sandbox container posture rule.`);
	...
	const browser = isRecord$2(value.browser) ? value.browser : {};
	const unsupportedBrowserKey = unsupportedPolicyKey(browser, ["requireCdpSourceRange"]);
	if (unsupportedBrowserKey !== void 0) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/${targetPrefix}/browser/${ocPathSegment$1(unsupportedBrowserKey)}`, `${params.policyPath} ${propertyPrefix}.browser.${unsupportedBrowserKey} is not supported in sandbox policy.`, `Remove ${propertyPrefix}.browser.${unsupportedBrowserKey} or use a supported sandbox browser posture rule.`);
...
function gatewayPolicyShapeFinding(value, params) {
	if (value === void 0) return;
	if (!isRecord$2(value)) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/gateway`, `${params.policyPath} gateway must be an object.`, `Fix ${params.policyPath} so gateway is an object.`);
	for (const section of [
		"exposure",
		"auth",
		"controlUi",
		...
		"http",
		"nodes"
	]) if (value[section] !== void 0 && !isRecord$2(value[section])) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/gateway/${section}`, `${params.policyPath} gateway.${section} must be an object.`, `Fix ${params.policyPath} so gateway.${section} is an object.`);
	const unsupportedGatewayKey = unsupportedPolicyKey(value, SUPPORTED_GATEWAY_POLICY_SECTIONS);
	if (unsupportedGatewayKey !== void 0) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/gateway/${ocPathSegment$1(unsupportedGatewayKey)}`, `${params.policyPath} gateway.${unsupportedGatewayKey} is not supported in Gateway policy.`, `Remove gateway.${unsupportedGatewayKey} or use a supported Gateway policy section.`);
	const exposure = isRecord$2(value.exposure) ? value.exposure : {};
	const auth = isRecord$2(value.auth) ? value.auth : {};
	const controlUi = isRecord$2(value.controlUi) ? value.controlUi : {};
	const remote = isRecord$2(value.remote) ? value.remote : {};
	const http = isRecord$2(value.http) ? value.http : {};
	const nodes = isRecord$2(value.nodes) ? value.nodes : {};
	for (const [section, sectionValue, allowedKeys] of [
		...
		if (unsupportedKey !== void 0) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/gateway/${section}/${ocPathSegment$1(unsupportedKey)}`, `${params.policyPath} gateway.${section}.${unsupportedKey} is not supported in Gateway policy.`, `Remove gateway.${section}.${unsupportedKey} or use a supported Gateway policy rule.`);
	}
	const booleanRules = [
		[
	...
	if (denyCommands !== void 0 && !Array.isArray(denyCommands)) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/gateway/nodes/denyCommands`, `${params.policyPath} gateway.nodes.denyCommands must be an array.`, "Use an array of node command ids such as [\"system.run\"] or remove gateway.nodes.denyCommands.");
	if (Array.isArray(denyCommands)) {
		const invalidIndex = denyCommands.findIndex((entry) => typeof entry !== "string" || entry.trim() === "");
		if (invalidIndex >= 0) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/gateway/nodes/denyCommands/#${invalidIndex}`, `${params.policyPath} gateway.nodes.denyCommands[${invalidIndex}] must be a non-empty node command id.`, "Use non-empty node command ids.");
	}
}
//#endregion
//#region extensions/policy/src/doctor/scoped-policy-shape.ts
function scopedPolicyShapeFinding(value, params) {
	if (value === void 0) return;
	if (!isRecord$2(value)) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/scopes`, `${params.policyPath} scopes must be an object.`, `Fix ${params.policyPath} so scopes maps scope names to policy overlays with selectors such as agentIds.`);
	for (const [scopeName, overlay] of Object.entries(value)) {
		const targetPrefix = `scopes/${ocPathSegment$1(scopeName)}`;
		if (!isRecord$2(overlay)) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/${targetPrefix}`, `${params.policyPath} scopes.${scopeName} must be an object.`, `Fix ${params.policyPath} so the named policy scope is an object.`);
		...
		if (unsupportedKey !== void 0) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/${targetPrefix}/${ocPathSegment$1(unsupportedKey)}`, `${params.policyPath} scopes.${scopeName}.${unsupportedKey} is not a supported scoped policy section.`, `Use agentIds with agents.workspace, dataHandling.memory, execApprovals, tools, or sandbox, and channelIds with ingress.channels.`);
		if (overlay.dataHandling !== void 0 && !isRecord$2(overlay.dataHandling)) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/${targetPrefix}/dataHandling`, `${params.policyPath} scopes.${scopeName}.dataHandling must be an object.`, `Fix ${params.policyPath} so the scoped dataHandling policy section is an object.`);
		if (isRecord$2(overlay.dataHandling)) {
		...
		if (overlay.agents !== void 0 && !isRecord$2(overlay.agents)) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/${targetPrefix}/agents`, `${params.policyPath} scopes.${scopeName}.agents must be an object.`, `Fix ${params.policyPath} so the scoped agents policy section is an object.`);
		const scopedAgents = isRecord$2(overlay.agents) ? overlay.agents : {};
		const unsupportedAgentKey = Object.keys(scopedAgents).find((key) => key !== "workspace");
		if (unsupportedAgentKey !== void 0) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/${targetPrefix}/agents/${ocPathSegment$1(unsupportedAgentKey)}`, `${params.policyPath} scopes.${scopeName}.agents.${unsupportedAgentKey} is not supported by the agentIds selector.`, `Move the rule under agents.workspace or a supported scoped top-level section.`);
		...
		if (overlay.tools !== void 0 && !isRecord$2(overlay.tools)) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/${targetPrefix}/tools`, `${params.policyPath} scopes.${scopeName}.tools must be an object.`, `Fix ${params.policyPath} so the scoped tools policy overlay is an object.`);
		if (isRecord$2(overlay.tools)) {
...
function hasValidScopedPolicy(policy, policyPath, policyDocName) {
	return isRecord$2(policy) && scopedPolicyShapeFinding(policy.scopes, {
		policyDocName,
		policyPath,
		policy
	}) === void 0;
...
//#endregion
//#region extensions/policy/src/doctor/agent-workspace-findings.ts
function agentWorkspaceFindings(policy, policyPath, policyDocName, evidence) {
	if (agentsPolicyShapeFinding(isRecord$2(policy) ? policy.agents : void 0, {
		policyDocName,
		policyPath
	}) !== void 0) return [];
	return [
...
function agentScopedWorkspaceFindings(policy, policyPath, policyDocName, evidence) {
	...
	for (const target of agentScopedPolicyTargets(policy)) {
		const scopedAgents = isRecord$2(target.overlay.agents) ? target.overlay.agents : {};
		const workspace = isRecord$2(scopedAgents.workspace) ? scopedAgents.workspace : {};
		const requirementBase = `scopes/${ocPathSegment$1(target.scopeName)}/agents/workspace`;
...
//#endregion
//#region extensions/policy/src/doctor/data-auth-shapes.ts
function dataHandlingPolicyShapeFindings(policy, policyPath, policyDocName) {
	if (!isRecord$2(policy)) return [];
	if (!isRecord$2(policy.dataHandling)) return [];
	return [
...
function dataHandlingBooleanShapeFinding(dataHandling, params) {
	...
	if (isRecord$2(dataHandling) && typeof params.path[0] === "string") {
		const section = dataHandling[params.path[0]];
		if (isRecord$2(section) && typeof params.path[1] === "string") {
			const sectionPath = params.path.slice(0, -1).join(".");
			const unsupportedKey = unsupportedPolicyKey(section, [params.path[1]]);
			if (unsupportedKey !== void 0) return policyShapeFinding(params.policyPath, `oc://${params.policyDocName}/${params.targetPath.split("/").slice(0, -1).join("/")}/${ocPathSegment$1(unsupportedKey)}`, `${params.policyPath} dataHandling.${sectionPath}.${unsupportedKey} is not supported in data-handling policy.`, `Remove dataHandling.${sectionPath}.${unsupportedKey} or use ${params.propertyPath}.`);
		}
	}
...
function secretPolicyShapeFindings(policy, policyPath, policyDocName) {
	if (!isRecord$2(policy) || !isRecord$2(policy.secrets)) return [];
	const findings = [];
...
function authProfileAllowModesShapeFindings(policy, policyPath, policyDocName) {
	if (!isRecord$2(policy) || !isRecord$2(policy.auth) || !isRecord$2(policy.auth.profiles) || policy.auth.profiles.allowModes === void 0) return [];
...
//#endregion
//#region extensions/policy/src/doctor/data-auth-findings.ts
...
function dataHandlingFindings(policy, policyPath, policyDocName, evidence) {
	...
	for (const target of agentScopedPolicyTargets(policy)) {
		...
		findings.push(...dataHandlingFindingsForRule(target.overlay, policyDocName, `scopes/${ocPathSegment$1(target.scopeName)}/dataHandling`, evidence, (entry) => entry.kind !== "memorySessionTranscriptIndexing" || scopedDataHandlingAgentMatches(entry, target.agentId, evidence.dataHandling ?? [])));
	}
	return findings;
}
...
function dataHandlingFindingsForRule(policy, policyDocName, requirementBase, evidence, evidenceFilter) {
	const dataHandling = isRecord$2(policy) ? policy.dataHandling : void 0;
	if (!isRecord$2(dataHandling)) return [];
	const findings = [];
...
//#endregion
//#region extensions/policy/src/doctor/exec-approval-findings.ts
function execApprovalsFindings(policy, policyPath, policyDocName, evidence, file, displayName) {
	...
	if (isRecord$2(policy.execApprovals)) {
	...
	for (const target of agentScopedPolicyTargets(policy)) {
		if (!isRecord$2(target.overlay.execApprovals)) continue;
		const requirementBase = `scopes/${ocPathSegment$1(target.scopeName)}/execApprovals`;
...
function execApprovalsPolicyNeedsArtifactEvidence(execApprovalsPolicy) {
	return isRecord$2(execApprovalsPolicy.defaults) || isRecord$2(execApprovalsPolicy.agents);
}
function execApprovalsRuleFindings(execApprovalsPolicy, params) {
	...
	if (expected !== void 0) {
		...
		const actual = actualEntries.map((entry) => execApprovalAllowlistRequirementKey$1(entry.pattern, entry.argPattern)).toSorted();
		...
		for (const key of actualSet) {
			...
			const entry = actualEntries.find((candidate) => candidate.pattern !== void 0 && execApprovalAllowlistRequirementKey$1(candidate.pattern, candidate.argPattern) === key);
			findings.push(execApprovalFinding(entry, {
...
//#endregion
//#region extensions/policy/src/doctor/ingress-findings.ts
function ingressFindings(policy, policyPath, policyDocName, evidence) {
	...
	if (ingressPolicyShapeFinding(ingressPolicy, {
		policyDocName,
		policyPath
	}) === void 0 && isRecord$2(ingressPolicy)) findings.push(...ingressFindingsForRule(ingressPolicy, policyDocName, "ingress", evidence, () => true));
	if (hasValidScopedPolicy(policy, policyPath, policyDocName)) for (const target of channelScopedPolicyTargets(policy)) {
		if (ingressPolicyShapeFinding(target.overlay.ingress, {
			policyDocName,
			policyPath,
			targetPrefix: `scopes/${ocPathSegment$1(target.scopeName)}/ingress`,
			propertyPrefix: `scopes.${target.scopeName}.ingress`,
			allowSession: false
		}) !== void 0 || !isRecord$2(target.overlay.ingress)) continue;
		findings.push(...ingressFindingsForRule(target.overlay.ingress, policyDocName, `scopes/${ocPathSegment$1(target.scopeName)}/ingress`, evidence, (entry) => scopedIngressChannelMatches(entry, target.channelId)));
	}
	return findings;
}
function ingressFindingsForRule(ingressPolicy, policyDocName, requirementBase, evidence, evidenceFilter) {
	if (!isRecord$2(ingressPolicy)) return [];
	return [
...
//#endregion
//#region extensions/policy/src/doctor/policy-shape.ts
function policyContainerShapeFindings(policy, policyPath, policyDocName) {
	if (!isRecord$2(policy)) return [policyShapeFinding(policyPath, `oc://${policyDocName}`, `${policyPath} must contain a policy object.`, `Fix ${policyPath} so the top-level policy is an object.`)];
	const unsupportedTopLevel = unsupportedPolicyKey(policy, SUPPORTED_POLICY_SECTIONS);
	if (unsupportedTopLevel !== void 0) return [policyShapeFinding(policyPath, `oc://${policyDocName}/${ocPathSegment$1(unsupportedTopLevel)}`, `${policyPath} ${unsupportedTopLevel} is not a supported policy section.`, `Remove ${unsupportedTopLevel} or use a supported policy section.`)];
	if (policy.tools !== void 0 && !isRecord$2(policy.tools)) return [policyShapeFinding(policyPath, `oc://${policyDocName}/tools`, `${policyPath} tools must be an object.`, `Fix ${policyPath} so tools is an object.`)];
	if (isRecord$2(policy.tools)) {
	...
	if (policy.channels !== void 0 && !isRecord$2(policy.channels)) return [policyShapeFinding(policyPath, `oc://${policyDocName}/channels`, `${policyPath} channels must be an object.`, `Fix ${policyPath} so channels is an object.`)];
	if (isRecord$2(policy.channels)) {
		const unsupportedChannelKey = unsupportedPolicyKey(policy.channels, ["denyRules"]);
		if (unsupportedChannelKey !== void 0) return [policyShapeFinding(policyPath, `oc://${policyDocName}/channels/${ocPathSegment$1(unsupportedChannelKey)}`, `${policyPath} channels.${unsupportedChannelKey} is not supported in channel policy.`, `Remove channels.${unsupportedChannelKey} or use channels.denyRules.`)];
	}
	if (policy.mcp !== void 0 && !isRecord$2(policy.mcp)) return [policyShapeFinding(policyPath, `oc://${policyDocName}/mcp`, `${policyPath} mcp must be an object.`, `Fix ${policyPath} so mcp is an object.`)];
	if (isRecord$2(policy.mcp)) {
		const unsupportedMcpKey = unsupportedPolicyKey(policy.mcp, ["servers"]);
		if (unsupportedMcpKey !== void 0) return [policyShapeFinding(policyPath, `oc://${policyDocName}/mcp/${ocPathSegment$1(unsupportedMcpKey)}`, `${policyPath} mcp.${unsupportedMcpKey} is not supported in MCP policy.`, `Remove mcp.${unsupportedMcpKey} or use mcp.servers.`)];
	}
	if (policy.dataHandling !== void 0 && !isRecord$2(policy.dataHandling)) return [policyShapeFinding(policyPath, `oc://${policyDocName}/dataHandling`, `${policyPath} dataHandling must be an object.`, `Fix ${policyPath} so dataHandling is an object.`)];
	if (isRecord$2(policy.mcp)) {
	...
	if (policy.models !== void 0 && !isRecord$2(policy.models)) return [policyShapeFinding(policyPath, `oc://${policyDocName}/models`, `${policyPath} models must be an object.`, `Fix ${policyPath} so models is an object.`)];
	if (isRecord$2(policy.models)) {
		const unsupportedModelsKey = unsupportedPolicyKey(policy.models, ["providers"]);
		if (unsupportedModelsKey !== void 0) return [policyShapeFinding(policyPath, `oc://${policyDocName}/models/${ocPathSegment$1(unsupportedModelsKey)}`, `${policyPath} models.${unsupportedModelsKey} is not supported in model policy.`, `Remove models.${unsupportedModelsKey} or use models.providers.`)];
	}
	if (isRecord$2(policy.models)) {
	...
	if (policy.network !== void 0 && !isRecord$2(policy.network)) return [policyShapeFinding(policyPath, `oc://${policyDocName}/network`, `${policyPath} network must be an object.`, `Fix ${policyPath} so network is an object.`)];
	if (isRecord$2(policy.network)) {
		const unsupportedNetworkKey = unsupportedPolicyKey(policy.network, ["privateNetwork"]);
		if (unsupportedNetworkKey !== void 0) return [policyShapeFinding(policyPath, `oc://${policyDocName}/network/${ocPathSegment$1(unsupportedNetworkKey)}`, `${policyPath} network.${unsupportedNetworkKey} is not supported in network policy.`, `Remove network.${unsupportedNetworkKey} or use network.privateNetwork.`)];
		if (policy.network.privateNetwork !== void 0 && !isRecord$2(policy.network.privateNetwork)) return [policyShapeFinding(policyPath, `oc://${policyDocName}/network/privateNetwork`, `${policyPath} network.privateNetwork must be an object.`, `Fix ${policyPath} so network.privateNetwork is an object.`)];
		if (isRecord$2(policy.network.privateNetwork)) {
			const unsupportedPrivateNetworkKey = unsupportedPolicyKey(policy.network.privateNetwork, ["allow"]);
			if (unsupportedPrivateNetworkKey !== void 0) return [policyShapeFinding(policyPath, `oc://${policyDocName}/network/privateNetwork/${ocPathSegment$1(unsupportedPrivateNetworkKey)}`, `${policyPath} network.privateNetwork.${unsupportedPrivateNetworkKey} is not supported in network policy.`, `Remove network.privateNetwork.${unsupportedPrivateNetworkKey} or use network.privateNetwork.allow.`)];
		}
		if (isRecord$2(policy.network.privateNetwork) && policy.network.privateNetwork.allow !== void 0 && typeof policy.network.privateNetwork.allow !== "boolean") return [policyShapeFinding(policyPath, `oc://${policyDocName}/network/privateNetwork/allow`, `${policyPath} network.privateNetwork.allow must be a boolean.`, `Fix ${policyPath} so network.privateNetwork.allow is true or false.`)];
	}
	if (policy.secrets !== void 0 && !isRecord$2(policy.secrets)) return [policyShapeFinding(policyPath, `oc://${policyDocName}/secrets`, `${policyPath} secrets must be an object.`, `Fix ${policyPath} so secrets is an object.`)];
	if (isRecord$2(policy.secrets)) {
		const unsupportedSecretsKey = unsupportedPolicyKey(policy.secrets, [
			"allowInsecureProviders",
			"denySources",
			"requireManagedProviders"
		]);
		if (unsupportedSecretsKey !== void 0) return [policyShapeFinding(policyPath, `oc://${policyDocName}/secrets/${ocPathSegment$1(unsupportedSecretsKey)}`, `${policyPath} secrets.${unsupportedSecretsKey} is not supported in secrets policy.`, `Remove secrets.${unsupportedSecretsKey} or use a supported secrets policy rule.`)];
	}
	if (policy.auth !== void 0 && !isRecord$2(policy.auth)) return [policyShapeFinding(policyPath, `oc://${policyDocName}/auth`, `${policyPath} auth must be an object.`, `Fix ${policyPath} so auth is an object.`)];
	if (isRecord$2(policy.auth)) {
		const unsupportedAuthKey = unsupportedPolicyKey(policy.auth, ["profiles"]);
		if (unsupportedAuthKey !== void 0) return [policyShapeFinding(policyPath, `oc://${policyDocName}/auth/${ocPathSegment$1(unsupportedAuthKey)}`, `${policyPath} auth.${unsupportedAuthKey} is not supported in auth policy.`, `Remove auth.${unsupportedAuthKey} or use auth.profiles.`)];
	}
	if (isRecord$2(policy.auth) && policy.auth.profiles !== void 0 && !isRecord$2(policy.auth.profiles)) return [policyShapeFinding(policyPath, `oc://${policyDocName}/auth/profiles`, `${policyPath} auth.profiles must be an object.`, `Fix ${policyPath} so auth.profiles is an object.`)];
	if (isRecord$2(policy.auth) && isRecord$2(policy.auth.profiles)) {
		const unsupportedProfilesKey = unsupportedPolicyKey(policy.auth.profiles, ["allowModes", "requireMetadata"]);
		if (unsupportedProfilesKey !== void 0) return [policyShapeFinding(policyPath, `oc://${policyDocName}/auth/profiles/${ocPathSegment$1(unsupportedProfilesKey)}`, `${policyPath} auth.profiles.${unsupportedProfilesKey} is not supported in auth profile policy.`, `Remove auth.profiles.${unsupportedProfilesKey} or use a supported auth profile policy rule.`)];
	}
...
//#endregion
//#region extensions/policy/src/doctor/sandbox-findings.ts
function sandboxPostureFindings(policy, policyPath, policyDocName, evidence) {
	if (!isRecord$2(policy)) return [];
	const findings = [];
	const sandboxPolicy = policy.sandbox;
	if (isRecord$2(sandboxPolicy) && sandboxPolicyShapeFinding(sandboxPolicy, {
		policyDocName,
		policyPath
	...
	for (const target of agentScopedPolicyTargets(policy)) {
		...
		if (sandboxPolicyShapeFinding(scopedSandboxPolicy, {
			policyDocName,
			policyPath,
			targetPrefix: `scopes/${ocPathSegment$1(target.scopeName)}/sandbox`,
			propertyPrefix: `scopes.${target.scopeName}.sandbox`
		}) !== void 0 || !isRecord$2(scopedSandboxPolicy)) continue;
		findings.push(...sandboxPostureFindingsForRule(scopedSandboxPolicy, policyDocName, `scopes/${ocPathSegment$1(target.scopeName)}/sandbox`, evidence, (entry) => scopedSandboxAgentMatches(entry, target.agentId, evidence.sandboxPosture ?? [])));
	}
	return findings;
}
function sandboxPostureFindingsForRule(sandboxPolicy, policyDocName, requirementBase, evidence, evidenceFilter) {
	if (!isRecord$2(sandboxPolicy)) return [];
	return [
...
const CONTAINER_RUNTIME_SOCKET_BASENAMES = /* @__PURE__ */ new Set([
	"containerd.sock",
	"docker.sock",
	"podman.sock"
]);
const CONTAINER_RUNTIME_SOCKET_PATHS = /* @__PURE__ */ new Set([
...
//#endregion
//#region extensions/policy/src/doctor/tool-findings.ts
function toolPostureFindings(policy, policyPath, policyDocName, evidence) {
	const findings = [];
	if (isRecord$2(policy) && isRecord$2(policy.tools) && toolPosturePolicyShapeFinding(policy.tools, {
		policyDocName,
		policyPath
	}) === void 0) findings.push(...toolPostureFindingsForRule(policy.tools, policyDocName, "tools", evidence, () => true));
	if (!hasValidScopedPolicy(policy, policyPath, policyDocName)) return findings;
	for (const target of agentScopedPolicyTargets(policy)) {
		if (!isRecord$2(target.overlay.tools)) continue;
		const requirementBase = `scopes/${ocPathSegment$1(target.scopeName)}/tools`;
...
function toolAlsoAllowExpectedFindings(toolsPolicy, policyDocName, requirementBase, evidence, evidenceFilter) {
	if ((isRecord$2(toolsPolicy.alsoAllow) ? toolsPolicy.alsoAllow : {}).expected === void 0) return [];
...
function toolRequiredDenyFindings(toolsPolicy, policyDocName, requirementBase, evidence, evidenceFilter) {
	...
	const requiredTools = uniqueStrings$2(required.flatMap(expandPolicyToolRequirement$1));
	const findings = [];
	for (const entry of toolPostureEntries(evidence, "deny").filter(evidenceFilter)) for (const tool of requiredTools) {
		if (toolListCoversTool$1(entry.entries ?? [], tool)) continue;
		findings.push(toolPostureFinding(entry, {
...
//#endregion
//#region extensions/policy/src/doctor/evaluation.ts
...
function toolMetadataRequirementFindings(policy, policyPath, policyDocName) {
	if (!isRecord$2(policy) || !isRecord$2(policy.tools) || policy.tools.requireMetadata === void 0) return [];
...
let policyDoctorChecks;
const registeredPolicyDoctorRegistrars = /* @__PURE__ */ new WeakSet();
function registerPolicyDoctorChecks(host) {
	if (host !== void 0 && registeredPolicyDoctorRegistrars.has(host.registerHealthCheck)) return;
	...
	for (const check of policyDoctorChecks) {
		if (host === void 0 && getHealthCheck(check.id) === check) continue;
		registerHealthCheck$1(check);
	}
	registeredPolicyDoctorRegistrars.add(registerHealthCheck$1);
}
//#endregion
export { POLICY_RULE_METADATA as a, POLICY_CHECK_IDS as c, isPolicyValueAtLeastAsStrict as i, evaluatePolicy as n, createPolicyAttestation as o, policyContainerShapeFindings as r, POLICY_FIX_METADATA_BY_CHECK_ID as s, registerPolicyDoctorChecks as t };
