...
import { i as parseDotPath } from "./shared-hYiou55H.js";
import { a as resolvePluginMetadataSnapshot } from "./plugin-metadata-snapshot-xuKL7EBL.js";
import { n as loadChannelSecretContractApiForRecord, t as loadChannelSecretContractApi } from "./channel-contract-api-BUgyVo9B.js";
...
function getCompiledCoreAuthProfileTargetState() {
	...
	const entries = getCoreSecretTargetRegistry().filter((entry) => entry.configFile === "auth-profiles.json").map(compileTargetRegistryEntry);
...
function getConfiguredChannelOpenClawTargets(config) {
	return Object.keys(config.channels ?? {}).flatMap((channelId) => getCompiledChannelOpenClawTargets(channelId) ?? []);
}
...
function resolveSecretPlanTargetByPath(params) {
	if (params.configFile === "openclaw.json") return resolveConfigSecretTargetByPath(params.pathSegments);
...
function discoverConfigSecretTargetsByIds(config, targetIds) {
	...
	const configuredEntries = allowedTargetIds !== null && Array.from(allowedTargetIds).every((targetId) => coreState.knownTargetIds.has(targetId)) ? coreState.openClawCompiledSecretTargets : allowedTargetIds !== null && !configHasPluginEntries(config) ? [...coreState.openClawCompiledSecretTargets, ...getConfiguredChannelOpenClawTargets(config)] : null;
...
function listAuthProfileSecretTargetEntries() {
	return getCoreSecretTargetRegistry().filter((entry) => entry.configFile === "auth-profiles.json" && entry.includeInAudit);
}
//#endregion
export { isKnownSecretTargetId as a, resolveConfigSecretTargetByPath as c, isKnownCoreSecretTargetId as i, resolvePlanTargetAgainstRegistry as l, discoverConfigSecretTargets as n, listAuthProfileSecretTargetEntries as o, discoverConfigSecretTargetsByIds as r, listSecretTargetRegistryEntries as s, discoverAuthProfileSecretTargets as t, resolveSecretPlanTargetByPath as u };
