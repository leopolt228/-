...
function loadApprovalsFromDatabase() {
	try {
		...
		const approvalsDb = getNodeSqliteKysely(database.db);
		return { approvals: executeSqliteQuerySync(database.db, approvalsDb.selectFrom("plugin_binding_approvals").select([
			"plugin_root",
			"plugin_id",
			"plugin_name",
			"channel",
...
async function persistApprovalEntry(entry) {
	...
	const writeApprovals = state.approvalsSaveChain.catch(() => void 0).then(() => {
		runOpenClawStateWriteTransaction(({ db }) => {
			executeSqliteQuerySync(db, getNodeSqliteKysely(db).insertInto("plugin_binding_approvals").values(row).onConflict((conflict) => conflict.columns([
				"plugin_root",
				"channel",
				"account_id"
			]).doUpdateSet({
	...
	state.approvalsSaveChain = writeApprovals.catch(() => void 0);
...
async function bindConversationNow(params) {
	...
	const binding = toPluginConversationBinding(await createConversationBindingRecord({
		targetSessionKey: normalizeOptionalString(params.targetSessionKey) ?? buildPluginBindingSessionKey({
...
//#endregion
export { createConversationBindingRecord as _, buildPluginBindingIdentity as a, unbindConversationBindingRecord as b, detachPluginConversationBinding as c, isPluginOwnedSessionBindingRecord as d, markPluginBindingFallbackNoticeShown as f, toPluginConversationBinding as g, resolvePluginConversationBindingApproval as h, buildPluginBindingErrorText as i, getCurrentPluginConversationBinding as l, requestPluginConversationBinding as m, buildPluginBindingApprovalCustomId as n, buildPluginBindingResolvedText as o, parsePluginBindingApprovalCustomId as p, buildPluginBindingDeclinedText as r, buildPluginBindingUnavailableText as s, bindConversationNow as t, hasShownPluginBindingFallbackNotice as u, resolveConversationBindingRecord as v, touchConversationBindingRecord as y };
