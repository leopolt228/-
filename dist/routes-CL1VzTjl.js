...
import { _ as parseStrictFiniteNumber, b as parseStrictPositiveInteger, d as clampPositiveTimerTimeoutMs, v as parseStrictInteger, y as parseStrictNonNegativeInteger } from "./number-coercion-Crk_c9KW.js";
import { _ as uniqueStrings } from "./string-normalization-CRyoFBPt.js";
import { c as resolveUserPath } from "./home-dir-DxrrpDft.js";
import { r as formatErrorMessage } from "./errors-DdbcjW1Y.js";
import { d as pathScope } from "./fs-safe-Dy0g6QwA.js";
...
import { i as ensureMediaDir, l as saveMediaBuffer } from "./store-NmJjqmad.js";
import "./string-coerce-runtime-DBMkn-gE.js";
import "./text-utility-runtime-Bs8FhB83.js";
import "./number-runtime-C6TGSEc_.js";
import { t as expectDefined } from "./expect-runtime--WgnKYXT.js";
import { n as redactCdpUrl, t as parseBrowserHttpUrl } from "./browser-config-Y5s979Hx.js";
import { l as DEFAULT_BROWSER_SCREENSHOT_TIMEOUT_MS, n as DEFAULT_AI_SNAPSHOT_MAX_CHARS, t as DEFAULT_AI_SNAPSHOT_EFFICIENT_MAX_CHARS } from "./constants-C2_ZjRRD.js";
import { a as normalizeActBoundedNonNegativeMs, n as ACT_MAX_VIEWPORT_DIMENSION, r as ACT_MAX_WAIT_TIME_MS, t as ACT_MAX_CLICK_DELAY_MS } from "./act-policy-D1rdxM-I.js";
import { a as resolveSuggestedImportTarget, i as recordSystemProfileImport, n as dismissSystemProfileImportPrompt, r as readSystemProfileImportState } from "./system-profile-import-state-DFy4mHBX.js";
import { a as resolveProfile, i as resolveManagedBrowserHeadlessMode, l as resolveExistingUploadPaths, n as getOwnBrowserProfile, o as DEFAULT_DOWNLOAD_DIR, r as resolveBrowserConfig, s as DEFAULT_TRACE_DIR } from "./config-BP-Yt4hA.js";
import { A as BrowserValidationError, C as BrowserError, M as toBrowserErrorResponse, O as BrowserTabNotFoundError, S as BrowserConflictError, T as BrowserProfileUnavailableError, _ as normalizeBrowserTimerDelayMs, f as redactCdpErrorText, g as withCdpSocket, k as BrowserTargetAmbiguousError, n as assertCdpEndpointAllowed, w as BrowserProfileNotFoundError } from "./tmp-openclaw-dir-yVXRKZ8m.js";
import "./config-Dal53Qjv.js";
import "./errors-l5qkvvL8.js";
import { a as isValidProfileName, i as setDefaultBrowserProfile, n as deleteBrowserProfileConfig, t as createBrowserProfileConfig } from "./config-mutations-D1PPxUf1.js";
import { r as resolveBrowserExecutableForPlatform } from "./chrome.executables-GOH5mZp7.js";
import { A as assertBrowserNavigationResultAllowed, E as STRUCTURAL_ROLES, F as resolveBrowserNavigationProxyMode, N as withBrowserNavigationPolicy, O as assertBrowserNavigationAllowed, S as finalizeRoleSnapshot, T as INTERACTIVE_ROLES, l as ensureOutputDirectory, m as captureScreenshot, n as getChromeWebSocketUrl, s as resolveOpenClawUserDataDir, v as snapshotAria, w as CONTENT_ROLES, y as snapshotRoleViaCdp } from "./chrome-BXIrXTbw.js";
import { a as getOrCreateProfileRuntime, g as getPwAiModule$1, l as isProfileRestartRequiredError, o as getProfileLifecycle, r as beginProfileTransition } from "./server-context.lifecycle-Dq-pSnXx.js";
import { i as shouldUsePlaywrightForScreenshot, n as resolveDefaultSnapshotFormat, r as shouldUsePlaywrightForAriaSnapshot, t as getBrowserProfileCapabilities } from "./profile-capabilities-DqO0AgW7.js";
import { n as resolveCdpControlPolicy, t as movePathToTrash } from "./trash-CPISlM1A.js";
import { n as normalizeString } from "./record-shared-CyHusTXE.js";
import { D as takeChromeMcpSnapshot, E as takeChromeMcpScreenshot, O as uploadChromeMcpFile, S as resizeChromeMcpPage, _ as navigateChromeMcpPage, b as pressChromeMcpKey, c as dragChromeMcpElement, d as fillChromeMcpElement, f as fillChromeMcpForm, h as hoverChromeMcpElement, k as withChromeMcpDocument, m as getChromeMcpPid, n as clickChromeMcpCoords, r as clickChromeMcpElement, t as ChromeMcpDocumentUnavailableError, u as evaluateChromeMcpScript } from "./chrome-mcp-CLMYMkT-.js";
import { i as resolveTargetIdFromTabs, r as withProfileContextOperation } from "./server-context-Cb2rq3u2.js";
import { n as listSystemProfiles, t as importSystemProfileCookies } from "./system-profiles-CHPEDDz6.js";
import "./sdk-setup-tools-DMZl9CMQ.js";
import { a as scaleAnnotations, l as matchBrowserUrlPattern, s as normalizeBrowserFormField, t as appendSnapshotUrls, u as normalizeBrowserEvaluateFunctionSource } from "./snapshot-urls-CsnEtSO0.js";
...
function buildExistingSessionWaitPredicate(params) {
	...
	return checks.length === 1 ? expectDefined(checks.at(0), "single existing-session condition") : checks.map((check) => `(${check})`).join(" && ");
}
async function waitForExistingSessionCondition(params) {
	...
	while (Date.now() < deadline) {
		try {
			if (await withChromeMcpDocument(params, async (document) => {
		...
		await setTimeout$1(250, void 0, { signal: params.signal });
	}
		...
		await withRouteTabContext({
			...
			run: async ({ profileCtx, cdpUrl, tab, signal, resolveTabUrl }) => {
				...
				const existingSessionCallOptions = {
					timeoutMs: ("timeoutMs" in action && typeof action.timeoutMs === "number" ? action.timeoutMs : void 0) ?? ctx.state().resolved.actionTimeoutMs,
					signal
				};
				...
				if (isExistingSession) {
					...
					const initialTabTargetIds = hasNavigationResultPolicy ? new Set((await profileCtx.listTabs(existingSessionCallOptions)).map((currentTab) => currentTab.targetId)) : /* @__PURE__ */ new Set();
					...
					switch (action.kind) {
						...
						case "wait":
							await runExistingSessionActionWithNavigationGuard({
								execute: () => waitForExistingSessionCondition({
									...existingSessionTarget,
				...
				switch (action.kind) {
					...
					case "click":
					case "clickCoords": return await jsonOk(downloads ? { downloads } : void 0, { resolveCurrentTarget: true });
					case "resize": return await jsonOk(downloads ? { downloads } : void 0);
					default: return await jsonOk(downloads ? { downloads } : void 0, { resolveCurrentTarget: true });
				}
			}
		});
	});
...
async function collectChromeMcpSnapshotUrls(params) {
	...
	return Array.isArray(result) ? result.filter((entry) => entry && typeof entry === "object" && typeof entry.text === "string" && typeof entry.url === "string").map((entry) => {
		...
		await withRouteTabContext({
			...
			run: async ({ profileCtx, tab, cdpUrl, signal }) => {
				if (getBrowserProfileCapabilities(profileCtx.profile).usesChromeMcp) {
					...
					if (labels) {
						const built = buildAiSnapshotFromChromeMcpSnapshot({ root: await takeChromeMcpSnapshot(operation) });
...
function normalizeChromeGraphicsInfo(value, observedAt = Date.now()) {
	const gpu = asRecord(asRecord(value)?.gpu);
	if (!gpu) return {
		status: "unavailable",
		observedAt,
...
async function inspectChromeGraphicsDiagnostics(cdpUrl, options = {}) {
	...
	try {
		...
		return normalizeChromeGraphicsInfo(await withCdpSocket(wsUrl, async (send) => await send("SystemInfo.getInfo"), {
...
function buildBrowserDoctorReport(params) {
	...
	else if (transport === "extension") checks.push({
		id: "extension-relay",
		label: "Chrome extension relay",
		status: status.running ? "pass" : "fail",
		summary: status.running ? "OpenClaw Chrome extension is connected" : "OpenClaw Chrome extension is not connected",
		...status.running ? {} : { fixHint: "Install the OpenClaw Chrome extension (openclaw browser extension path), run openclaw browser extension pair, and paste the pairing string into the extension popup." }
	});
	else {
		checks.push({
...
function createBrowserProfilesService(ctx) {
	...
	const createProfile = async (params) => {
		...
		if (getOwnBrowserProfile(getRuntimeConfig().browser?.profiles ?? {}, name)) throw new BrowserConflictError(`profile "${name}" already exists`);
...
function registerBrowserBasicRoutes(app, ctx) {
	...
	app.get("/doctor", async (req, res) => {
		...
		try {
			const report = await runProfileRouteOperation({
				...
				signal: req.signal,
				run: async (signal) => {
					const doctorReport = buildBrowserDoctorReport({ status: await buildBrowserStatus(ctx, profileCtx, signal) });
	...
	app.post("/profiles/import", async (req, res) => {
		...
		try {
			const result = await createBrowserProfilesService(ctx).importSystemProfile({
		...
		try {
			const granted = await runProfileRouteOperation({
				...
				run: async (signal) => {
					...
					signal.throwIfAborted();
					if (!wsUrl) throw new BrowserProfileUnavailableError("browser CDP WebSocket unavailable");
...
function registerBrowserTabRoutes(app, ctx) {
	...
	app.post("/tabs/open", async (req, res) => {
		...
		const result = await runTabsProfileRoute({
			...
			run: async (profileCtx, signal) => {
				...
				await profileCtx.ensureBrowserAvailable({ signal });
				return {
					...await profileCtx.openTab(url, { label }),
	...
	app.delete("/tabs/:targetId", async (req, res) => {
		...
		await runTabTargetMutation({
			...
			targetId,
			mutate: async (profileCtx, id) => {
				await profileCtx.closeTab(id, targetIdMode === "raw" ? { exactTargetId: true } : void 0);
			}
		});
	});
		...
		const result = await runTabsProfileRoute({
			...
			run: async (profileCtx, signal) => {
				...
				if (action === "new") {
					...
					return {
						ok: true,
						tab: await profileCtx.openTab("about:blank", { label })
					};
				}
				if (action === "label") {
...
//#endregion
export { DEFAULT_BROWSER_SCREENSHOT_MAX_SIDE as a, DEFAULT_BROWSER_SCREENSHOT_MAX_BYTES as i, formatBrowserGraphicsSummary as n, normalizeBrowserScreenshot as o, parseSystemProfileDomains as r, registerBrowserRoutes as t };
