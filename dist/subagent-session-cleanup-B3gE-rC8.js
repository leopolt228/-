...
import { c as readSessionEntry } from "./store-DDuGv_UJ.js";
import { t as formatDurationCompact } from "./format-duration-DKk9BtRb.js";
import { r as buildAgentRunTerminalOutcomeFromWaitResult } from "./agent-run-terminal-outcome-C9geO1r1.js";
import "./runs-DDczt14d.js";
...
import { n as sanitizeTextContent, t as extractAssistantText } from "./chat-history-text-BxdZZn3v.js";
...
function selectChildCompletionResultText(child) {
	...
	const fallback = child.completion?.fallbackResultText ?? child.delivery?.payload?.fallbackFrozenResultText ?? child.frozenResultText;
	if (child.outcome?.status === "ok") return selectDeliverableSessionsReply(primary, fallback);
...
function buildChildCompletionFindings(children) {
	...
	for (const [index, child] of sorted.entries()) {
		...
		if (child.outcome?.status === "ok" && !resultText && hasCapturedChildCompletionReply(child)) continue;
