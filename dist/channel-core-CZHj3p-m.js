import { r as createChannelPluginBase$1 } from "./core-Bo6nGN10.js";
//#region src/plugin-sdk/channel-config-ui-hints.ts
function createChannelDmPolicyUiHints(params) {
	...
	const legacyHint = { "dm.policy": {
		label: hint.label,
		help: `${hint.help.slice(0, -1)} (legacy: channels.${params.channelKey}.dm.allowFrom).`
	} };
	if (!params.includeLegacyNestedPolicy) return { dmPolicy: hint };
...
function createChannelConfigWritesUiHint(channelLabel) {
	return { configWrites: {
...
function createChannelNativeCommandUiHints(channelLabel) {
	return {
		"commands.native": {
...
function createChannelImplicitMentionsUiHints(channelLabel) {
	return {
		implicitMentions: {
...
function createChannelProgressUiHints(params) {
	...
	const commentaryHint = { "streaming.progress.commentary": {
	...
	return {
		...
		...params.includeCommentary && params.commentaryOrder !== "after-command" ? commentaryHint : {},
...
function createChannelRetryUiHints(channelLabel) {
	return {
		"retry.attempts": {
...
function createChannelConfigUiHints(params) {
	return {
		...params.dmPolicy ? createChannelDmPolicyUiHints({
			channelLabel: params.channelLabel,
			...params.dmPolicy
		}) : {},
		...params.configWrites ? createChannelConfigWritesUiHint(params.channelLabel) : {},
		...params.mentionPatterns ? createChannelMentionPatternUiHints({
			channelLabel: params.channelLabel,
		...
		...params.progress ? createChannelProgressUiHints({
			channelLabel: params.channelLabel,
			...params.progress
		}) : {},
