...
import { i as GENERATED_BUNDLED_CHANNEL_CONFIG_METADATA, t as CHANNEL_IDS } from "./ids-retRJEzF.js";
import { R as ZodNever, W as ZodObject, Z as ZodRecord, dt as ZodUnion, k as ZodIntersection, n as ZodArray, v as ZodDiscriminatedUnion } from "./schemas-CBJjibl3.js";
...
import { r as isSensitiveUrlConfigPath, t as SENSITIVE_URL_HINT_TAG } from "./redact-sensitive-url-CkPr90q0.js";
...
const AGENT_FIELD_HELP = {
	...
	"agents.list.*.utilityModel": "Optional per-agent utility model override for short internal tasks. Overrides agents.defaults.utilityModel.",
	"agents.list.*.models": "Per-agent model catalog overrides keyed by full provider/model IDs.",
	"agents.list.*.modelPolicy": "Per-agent model override policy. An explicit allow list replaces the default policy for this agent.",
	"agents.list.*.modelPolicy.allow": "Allowed model override refs for this agent. Accepts aliases, full \"provider/model\" refs, and trailing prefix wildcards such as \"provider/*\" or \"provider/namespace/*\"; empty permits any model.",
	...
	"agents.defaults.compaction.truncateAfterCompaction": "When enabled, rotates the active session transcript after compaction so future turns load only the summary and unsummarized tail while the previous full transcript remains archived. Prevents unbounded active transcript growth in long-running sessions. Default: false.",
...
const AUTOMATION_FIELD_HELP = {
	...
	"session.maintenance.maxDiskBytes": "Per-agent sessions-directory disk budget (for example `500mb`). Defaults to `10gb`; when exceeded, warn mode reports pressure and enforce mode performs oldest-first cleanup (archived transcripts before live sessions). Set `false` to disable.",
...
const CORE_FIELD_HELP = {
	...
	"meta.migrations.modelPolicyAllowlist": "Records that doctor preserved or evaluated legacy default and per-agent model-map override restrictions.",
	marketplaces: "Marketplace feed and local package source profile settings. Feeds provide package selection and governance metadata, while sources define the local source names that install candidates may reference.",
	"marketplaces.feeds": "Named marketplace feed profiles. The default public profile can be used as shipped, and deployments can add or override profiles to point OpenClaw at their effective feed endpoint.",
	"marketplaces.feeds.*.url": "HTTPS URL for the marketplace feed profile. Remote feed documents cannot introduce new registry domains or credentials; they only reference locally configured sources by name.",
	"marketplaces.feeds.*.verification": "Feed authenticity policy. Use unsigned only as an explicit local opt-in for self-hosted feeds. Use signed to name locally trusted publisher public keys before refresh enforcement consumes verified envelopes.",
	"marketplaces.feeds.*.verification.mode": "Feed verification mode: \"unsigned\" for explicit unsigned opt-in, or \"signed\" for local direct-key verification once refresh enforcement is wired.",
	"marketplaces.feeds.*.verification.keys": "Trusted publisher public keys for signed feed envelopes. These non-secret local trust anchors belong in source-profile config; remote feed documents cannot introduce or rotate them by themselves.",
	"marketplaces.feeds.*.verification.keys[].keyId": "Stable publisher key identifier expected in a signed feed envelope signature.",
	"marketplaces.feeds.*.verification.keys[].publicKey": "Ed25519 publisher public key for feed envelope verification, encoded as PEM or raw base64url. Feed signing private keys are publisher-side secrets managed outside this config; do not reuse platform signing identities or certificates here.",
	"marketplaces.feeds.*.verification.threshold": "Optional number of trusted signatures required for a signed feed. Defaults to one in the later refresh enforcement slice.",
	"marketplaces.sources": "Named package source profiles that feed entries can reference using sourceRef. Keep credentials and registry endpoints local so remote feeds cannot bootstrap trust roots.",
	"marketplaces.sources.*.type": "Package source profile type: npm, clawhub, or git. This slice validates sourceRef names only; registry and host endpoints are added when installer resolution can enforce them.",
	...
	"wizard.localModelLeanAutoModel": "System-managed model reference indicating that inference onboarding enabled the global lean-model surface. A later verified route uses this marker to remove only the onboarding-owned setting; a model changed outside onboarding hands ownership back to the user.",
	"wizard.securityAcknowledgedAt": "ISO timestamp for when the setup security acknowledgement was accepted on this config. Setup uses this to avoid repeating the acknowledgement on later wizard runs.",
	audit: "Bounded metadata-only audit history for operator review. Run and tool records are enabled by default; message lifecycle metadata is a separate privacy-sensitive opt-in. The background writer is best-effort rather than a lossless compliance archive.",
	"audit.enabled": "Records new run, tool, and enabled message audit events. Default: true. Disabling event inserts does not immediately delete existing records; retained rows remain queryable until they expire.",
	"audit.messages": "Controls content-free message lifecycle records: \"off\" (default), \"direct\" for known direct conversations only, or \"all\" for direct, group, channel, and unknown conversation kinds. Both audit.enabled and audit.messages are startup-scoped; restart the Gateway after changing either setting.",
	...
	"cloudWorkers.profiles": "Named cloud worker profiles. Each profile selects a worker provider registered by a plugin and carries provider-owned settings plus optional stored lifetime policy.",
	"cloudWorkers.profiles.*": "One cloud worker profile selected by name when creating an environment. Keep provider credentials in supported references rather than embedding secret material in this block.",
	"cloudWorkers.profiles.*.provider": "Worker provider id registered by a plugin. The configured plugin must expose this id before the gateway can provision environments from the profile.",
	"cloudWorkers.profiles.*.install": "Worker installation method: \"bundle\" (default) transfers the gateway's content-hashed installed build and supports released, development, and unreleased versions; \"npm\" installs the exact gateway version and is available only when that version is released.",
	"cloudWorkers.profiles.*.settings": "Provider-owned settings validated by the selected plugin. Use SecretRef objects for secret-bearing values; opaque settings do not gain automatic secret resolution.",
	"cloudWorkers.profiles.*.lifetime": "Stored environment lifetime policy. This first cloud-worker slice records these values as data; automatic idle and maximum-lifetime enforcement lands in later lifecycle work.",
	"cloudWorkers.profiles.*.lifetime.idleTimeoutMinutes": "Positive inactivity interval in minutes after which later lifecycle policy may reclaim an idle environment. Omit to leave idle cleanup unspecified.",
	"cloudWorkers.profiles.*.lifetime.maxLifetimeMinutes": "Positive maximum environment lifetime in minutes for later lifecycle enforcement. Omit to leave the maximum lifetime unspecified.",
...
const NODE_CAPABILITY_FIELD_HELP = {
	...
	"gateway.nodes.skills.enabled": "Accept skills published by paired nodes while they are connected (default: true). Set false to ignore node-published skills."
};
...
const RUNTIME_FIELD_HELP = {
	...
	"tools.experimental": "Experimental built-in tool flags. Use each tool's switch to opt in or out of its documented default.",
	"tools.experimental.planTool": "Structured `update_plan` checklist tool for non-trivial multi-step work. Enabled by default for embedded models; set false to opt out.",
	...
	"gateway.controlUi.chatMessageMaxWidth": "Optional CSS max-width for the centered Control UI chat transcript, for example \"960px\", \"82%\", or \"min(1280px, 82%)\". Values are validated against a constrained width grammar before reaching the browser.",
	...
	"agents.list[].tools.swarm": "Per-agent swarm override. Values merge over the top-level tools.swarm configuration.",
...
const FIELD_LABELS = {
	...
	"meta.migrations": "Config Migrations",
	...
	"agents.list.*.modelPolicy": "Agent Model Policy",
	"agents.list.*.modelPolicy.allow": "Allowed Agent Models",
...
const LEGACY_HIDDEN_PUBLIC_PATHS = ["hooks.internal.handlers"];
...
//#endregion
export { lookupConfigSchema as n, buildConfigSchema as t };
