...
import { t as AGENT_MODEL_CONFIG_KEYS } from "./configured-model-refs-wBBEGQ5a.js";
...
import { D as isValidAgentHarnessSessionStoreEntry, a as updateSessionStore, o as loadSessionStore } from "./store-DDuGv_UJ.js";
import { n as detectWindowsSpawnCommandInlineArgs } from "./windows-spawn-C5RDaB22.js";
import { S as toOpenAIModelId, _ as readModelConfigPrimaryRef, a as isBlockedLegacyCodexModelRef, b as resolveRuntimeModelRef, c as isOpenAICodexModelRef, d as modelRefUsesCodexRuntime, f as normalizeDefaultProviderModelRef, g as readLegacyDefaultsRuntime, h as readAgentPrimaryModelRef, i as isBlockedLegacyCodexModelPair, l as isProviderlessModelRef, m as parseModelRef, n as asAgentRuntimePolicyConfig, o as isLegacyCodexProviderId, p as normalizeRuntimeString, r as canonicalOpenAIModelUsesCodexRuntime, s as isOpenAICodexAuthProfileRef, t as agentUsesCodexRuntimeForCompaction, v as resolveImplicitDefaultAgentModelRef, x as toCanonicalOpenAIModelRef, y as resolveRuntime } from "./codex-route-model-ref-DLaEiamX.js";
import { n as collectBlockedLegacyOpenAICodexProviderPlan } from "./legacy-config-migrations.runtime.models-Cm95tnMp.js";
...
function collectUnsupportedCodexCompactionOverridesForAgent(params) {
	...
	if (normalizeOptionalLowercaseString(compaction?.provider ?? inheritedCompaction?.provider) === "lossless-claw") return [];
...
function collectLegacyLosslessCompactionConfigs(params) {
	...
	for (const [index, agent] of agents.entries()) {
		...
		const id = readAgentPathId$2(agentRecord, index);
...
function readAgentPathId$2(agent, index) {
...
function rewriteAgentCompactionRefs(params) {
	...
	if (normalizeOptionalLowercaseString(compaction?.provider ?? inheritedCompaction?.provider) === "lossless-claw") rewriteLosslessCompactionModel(params, compaction, inheritedCompaction);
	else {
...
const AGENT_MEDIA_MODEL_CONFIG_KEYS$1 = ["imageGenerationModel", "videoGenerationModel"];
...
function rewriteAgentModelRefs(params) {
	...
	for (const key of AGENT_MEDIA_MODEL_CONFIG_KEYS$1) rewriteModelConfigSlot({
		hits: params.hits,
		container: params.agent,
		key,
		path: `${params.path}.${key}`,
...
function rewriteConfigModelRefsWithCompactionPolicy(params) {
	...
	for (const [index, agent] of agents.entries()) {
		const agentRecord = asOptionalRecord(agent);
		if (!agentRecord) continue;
		const id = readAgentPathId$1(agentRecord, index);
		rewriteAgentModelRefs({
			cfg: nextConfig,
			preRepairCfg: params.cfg,
			hits,
...
function rewriteNonAgentModelRefs(params) {
	...
	rewriteStringModelSlotIfCanonicalCodexRuntime({
		cfg: params.cfg,
		hits: params.hits,
		container: asOptionalRecord(params.cfg.messages?.tts),
		key: "summaryModel",
...
function readAgentPathId$1(agent, index) {
	return typeof agent.id === "string" && agent.id.trim() ? agent.id.trim() : String(index);
}
//#endregion
...
function collectConfigModelRefs(cfg, blockedModelIdentities) {
	...
	for (const [index, agent] of agents.entries()) {
		...
		collectAgentModelRefs({
			hits,
			agent: agentRecord,
			path: `agents.list.${readAgentPathId(agentRecord, index)}`,
...
function readAgentPathId(agent, index) {
	return typeof agent.id === "string" && agent.id.trim() ? agent.id.trim() : String(index);
}
//#endregion
...
function scanCodexSessionStoreRoutes(store, blockedModelIdentities) {
	return Object.entries(store).flatMap(([sessionKey, entry]) => {
		if (!entry || isValidAgentHarnessSessionStoreEntry(sessionKey, entry)) return [];
		...
		const fallbackNoticeEndpoints = [entry.fallbackNoticeSelectedModel, entry.fallbackNoticeActiveModel];
...
async function maybeRepairCodexSessionRoutes(params) {
	...
	if (!params.shouldRepair) {
		const stale = targets.flatMap((target) => {
			return scanCodexSessionStoreRoutes(loadSessionStore(target.storePath, {
				skipCache: true,
				clone: false
			}), params.blockedModelIdentities).map((sessionKey) => `${target.agentId}:${sessionKey}`);
		});
		return {
	...
	for (const target of targets) {
		if (scanCodexSessionStoreRoutes(loadSessionStore(target.storePath, {
			skipCache: true,
			clone: false
		}), params.blockedModelIdentities).length === 0) continue;
		const result = await updateSessionStore(target.storePath, (store) => repairCodexSessionStoreRoutes({
			store,
			blockedModelIdentities: params.blockedModelIdentities
...
function collectCodexComputerUseWarnings(cfg) {
	const computerUse = asOptionalRecord(asOptionalRecord(asOptionalRecord(asOptionalRecord(asOptionalRecord(cfg.plugins)?.entries)?.codex)?.config)?.computerUse);
...
function maybeRepairCodexRoutes(params) {
	...
	if (hits.length === 0 && disabledCodexPluginHits.length === 0 && unsupportedCompactionOverrides.length === 0 && legacyLosslessCompactionConfigs.length === 0 && !blockedProviderPlan.warning) return {
		cfg: params.cfg,
		warnings: [],
		changes: []
	};
...
//#endregion
export { collectDisabledCodexPluginRouteIssues as i, maybeRepairCodexRoutes as n, maybeRepairCodexSessionRoutes as r, collectCodexRouteWarnings as t };
