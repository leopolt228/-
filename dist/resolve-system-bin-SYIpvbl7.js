import { i as getWindowsProgramFilesRoots, n as getWindowsInstallRoots } from "./windows-install-roots-BTRBDwn4.js";
...
const isExecutableFn = defaultIsExecutable;
/**
...
function resolveSystemBin(name, opts) {
	...
	for (const dir of dirs) if (isWin && !hasExt) for (const ext of WIN_PATHEXT) {
		...
		if (isExecutableFn(candidate)) {
			if (!hasExtra) cacheResolvedSystemBin(cache, name, candidate);
			return candidate;
		}
	}
	else {
