...
function stripActiveMemoryPromptPrefixBlocks(lines) {
	...
	for (let index = 0; index < lines.length; index += 1) {
		...
		if (line.trim() === UNTRUSTED_CONTEXT_HEADER && lines[index + 1]?.trim() === ACTIVE_MEMORY_OPEN_TAG) {
			let closeIndex = -1;
...
function stripLeadingInboundMetadata(text) {
	...
	if (!isInboundMetaSentinelLine(firstContentLine) && !isChatWindowContextHeaderLine(firstContentLine)) return stripTrailingUntrustedContextSuffix(strippedDeliveryHint ? lines.slice(index) : lines).join("\n");
