...
import { c as isUnscopedSessionKeySentinel, d as resolveAgentIdFromSessionKey, h as scopeLegacySessionKeyToAgent, s as classifySessionKeyShape } from "./session-key-Drrs61Fd.js";
import { c as resolveDefaultAgentId, n as listAgentIds } from "./agent-scope-config-S7z_Yn4H.js";
import { n as isGatewaySecretRefUnavailableError } from "./credentials-avJwgw8n.js";
import { i as GATEWAY_CLIENT_NAMES, r as GATEWAY_CLIENT_MODES } from "./client-info-D4mGPeue.js";
import { _ as randomIdempotencyKey, b as readGatewayDispatchConfig, c as callGateway, g as isGatewayTransportError, h as isGatewayExplicitAuthRequiredError, m as isGatewayCredentialsRequiredError, x as readGatewayDispatchConfigWithShellEnvFallback } from "./call-ChM1o8yU.js";
...
const defaultAgentSessionModuleLoader = () => import("./session-DJl5LWi1.js");
let agentSessionModuleLoader = defaultAgentSessionModuleLoader;
const embeddedAgentCommandLoader = createLazyPromiseLoader(() => import("./agent-B2q7Kd3I.js").then((module) => module.agentCommand), { cacheRejections: true });
const agentSessionModuleCache = createLazyPromiseLoader(() => agentSessionModuleLoader(), { cacheRejections: true });
const runtimeConfigModuleLoader = createLazyPromiseLoader(() => import("./io-BBSFa0i7.js"), { cacheRejections: true });
...
function resolveGatewayAgentEmbeddedFallbackReason(err) {
	if (isGatewayAgentTimeoutError(err)) return "gateway_timeout";
	return isGatewayTransportError(err) && err.kind === "closed" ? "gateway_closed" : void 0;
}
...
function createGatewayFallbackSessionId() {
	return `${GATEWAY_FALLBACK_SESSION_PREFIX}${randomUUID()}`;
}
function createGatewayFallbackSession(agentId) {
...
async function resolveAgentIdForGatewayFallback(opts) {
...
async function agentViaGatewayCommand(opts, runtime, signalBridge) {
	...
	const sessionKey = deferExplicitRecipientSession ? void 0 : classifySessionKeyShape(explicitSessionKey) === "agent" ? explicitSessionKey : (await loadAgentSessionModule()).resolveSessionKeyForRequest({
		cfg,
		agentId,
		to: opts.to,
		sessionId: opts.sessionId,
	...
	const abortSessionKey = deferExplicitRecipientSession ? (await loadAgentSessionModule()).resolveSessionKeyForRequest({
		cfg,
		agentId
	...
	const gatewayIdentity = needsAdminGatewayIdentity ? {
...
async function agentCliCommand(opts, runtime, deps) {
	...
	try {
		...
		try {
			...
			const fallbackSession = createGatewayFallbackSession(await resolveAgentIdForGatewayFallback(dispatchOpts));
			runtime.error?.(`EMBEDDED FALLBACK: Gateway agent ${fallbackReason === "gateway_timeout" ? "timed out" : "connection closed"}; continuity was intentionally not preserved. Running embedded agent with fresh session ${fallbackSession.sessionId} to avoid double-driving the original session: ${String(err)}`);
