...
import { f as buildLegacyBundledRootPath, t as discoverConfiguredPluginLoadPaths, u as listBundledSourceOverlayDirs } from "./discovery-Bhd5Zo0N.js";
...
import { r as getPackageManifestMetadata } from "./manifest-FKjShfr0.js";
...
import { t as resolvePluginCacheInputs } from "./roots-Bilo_HpA.js";
...
function resolvePluginRegistrySnapshotMemoKey(params, env) {
	...
	const persistedRegistryFingerprint = params.preferPersisted !== false ? hashJson(readPersistedInstalledPluginIndexSync({
		env,
...
function directoryChildFingerprint(directoryPath) {
	try {
		return fs.readdirSync(directoryPath, { withFileTypes: true }).map((entry) => [entry.name, entry.isDirectory() ? "dir" : entry.isFile() ? "file" : "other"]).toSorted(([left], [right]) => expectDefined(left, "plugin registry snapshot left").localeCompare(expectDefined(right, "plugin registry snapshot right")));
	} catch {
		return "unreadable";
	}
}
...
function isAllowedPersistedBundledPluginRoot(plugin, bundledPluginsDir, sourceOverlayDirs) {
	...
	if (isPathInsideOrEqual(pluginRootDir, bundledPluginsDir)) {
		...
		const relativePluginRoot = path.relative(resolveComparablePath(bundledPluginsDir), resolveComparablePath(pluginRootDir));
	...
	if (sourcePluginOptsOutOfBundledDist(path.join(legacyRoot, relativePluginRoot))) return false;
...
function sourcePluginOptsOutOfBundledDist(pluginRootDir) {
	return getPackageManifestMetadata(tryReadJsonSync(path.join(pluginRootDir, "package.json")) ?? void 0)?.build?.bundledDist === false;
}
...
//#endregion
export { resolveManifestContractPluginIds as a, inspectPluginRegistry as c, loadPluginRegistrySnapshotWithMetadata as d, refreshPluginRegistry as f, resolveManifestContractOwnerPluginId as i, isPluginEnabled as l, createPluginRegistryIdNormalizer as m, loadPluginManifestRegistryForPluginRegistry as n, resolvePluginContributionOwners as o, fileFingerprint as p, normalizePluginsConfigWithRegistry as r, getPluginRecord as s, listPluginContributionIds as t, loadPluginRegistrySnapshot as u };
