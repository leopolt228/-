...
import { n as getWindowsInstallRoots } from "./windows-install-roots-BTRBDwn4.js";
import { n as resolveLsofCommandSync } from "./ports-lsof-BmdLddJi.js";
import { c as parseProcCmdline, i as readWindowsProcessArgsSync, n as readWindowsListeningPidsResultSync, o as isGatewayArgv, r as readWindowsProcessArgsResultSync, t as readWindowsListeningPidsOnPortSync } from "./windows-port-pids-CPr8M1HP.js";
import { readFileSync } from "node:fs";
import path from "node:path";
import { spawnSync } from "node:child_process";
//#region src/infra/spawn-ps.ts
/** Run a bounded ps probe without letting an ignored SIGTERM extend the synchronous wait. */
function spawnPsSync(args, timeoutMs) {
...
function getExcludedGatewayPidsSync(spawnTimeoutMs, protectedPid) {
	const excluded = getSelfAndAncestorPidsSync(spawnTimeoutMs);
	if (typeof protectedPid === "number" && Number.isSafeInteger(protectedPid) && protectedPid > 0) excluded.add(protectedPid);
	return excluded;
}
/**
...
function parseLsofEntries(stdout) {
	...
	for (const line of stdout.split(/\r?\n/).filter(Boolean)) if (line.startsWith("p")) {
		flush();
		currentPid = parseStrictPositiveInteger(line.slice(1)) ?? void 0;
		currentCmd = void 0;
...
function parsePidsFromLsofOutput(stdout, spawnTimeoutMs, protectedPid) {
...
*/
function filterVerifiedWindowsGatewayPids(rawPids, protectedPid) {
...
function filterVerifiedWindowsGatewayPidsResult(rawPids, processArgsResult, protectedPid) {
	const excluded = getExcludedGatewayPidsSync(SPAWN_TIMEOUT_MS, protectedPid);
	const verified = [];
...
function findVerifiedWindowsGatewayPidsOnPortSync(port, protectedPid) {
	return filterVerifiedWindowsGatewayPids(readWindowsListeningPidsOnPortSync(port), protectedPid);
}
function findVerifiedWindowsGatewayPidsOnPortResultSync(port, protectedPid) {
	const result = readWindowsListeningPidsResultSync(port);
	if (!result.ok) return result;
	return filterVerifiedWindowsGatewayPidsResult(result.pids, (pid) => readWindowsProcessArgsResultSync(pid), protectedPid);
}
function findGatewayPidsOnPortWithProtectedPidSync(port, spawnTimeoutMs, protectedPid) {
	if (process.platform === "win32") return findVerifiedWindowsGatewayPidsOnPortSync(port, protectedPid);
...
function pollPortOnce(port) {
	...
	try {
		...
		if (res.status === 1) return res.stdout.trim() ? { free: false } : { free: true };
...
function terminateStaleProcessesSync(pids) {
	...
	const killed = [];
	for (const pid of pids) if (trySignalStaleProcess(pid, "SIGTERM")) killed.push(pid);
	if (killed.length === 0) return killed;
	sleepSync(STALE_SIGTERM_WAIT_MS);
	for (const pid of killed) if (isProcessAlive(pid)) trySignalStaleProcess(pid, "SIGKILL");
	sleepSync(STALE_SIGKILL_WAIT_MS);
	return killed;
}
function trySignalStaleProcess(pid, signal) {
	try {
		process.kill(pid, signal);
		return true;
	} catch (error) {
		if (!hasErrnoCode(error, "ESRCH")) restartLog.warn(`failed to send ${signal} to stale gateway process ${pid}: ${formatErrorMessage(error)}`);
		return false;
	}
}
/**
...
function cleanStaleGatewayProcessesSync(portOverride, options) {
	try {
...
//#endregion
export { spawnPsSync as i, findGatewayPidsOnPortSync as n, getSelfAndAncestorPidsSync as r, cleanStaleGatewayProcessesSync as t };
