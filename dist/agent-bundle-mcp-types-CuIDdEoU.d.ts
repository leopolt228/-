...
type BundleMcpToolRuntime = {
  tools: AnyAgentTool[]; /** All MCP tool-call projections, including App-only tools, for policy evaluation. */
...
type SessionMcpRuntimeManager = {
  getOrCreate: (params: {
    ...
    agentDir?: string;
    cfg?: OpenClawConfig;
    manifestRegistry?: Pick<PluginManifestRegistry, "plugins">; /** Trusted sender id; required to materialize requester-scoped MCP servers. */
  ...
  resolveSessionId: (sessionKey: string) => string | undefined; /** Looks up an existing runtime only; must not create runtimes or connect transports. */
  peekSession: (params: {
    sessionId?: string;
    sessionKey?: string;
  }) => SessionMcpRuntime | undefined;
  disposeSession: (sessionId: string) => Promise<void>; /** Required retirement stays armed when a stopping run creates or reuses a runtime. */
  deferRetirement: (sessionId: string, opts?: {
  ...
  listSessionIds: () => string[]; /** All managed cache keys (session ids and requester composite keys). */
  listRuntimeKeys: () => string[]; /** Sum of active leases across every runtime key for this session. */
  totalActiveLeasesForSession: (sessionId: string) => number;
};
//#endregion
export { SessionMcpRuntime as a, SessionMcpRequesterScope as i, McpCatalogTool as n, SessionMcpRuntimeManager as o, McpToolCatalog as r, BundleMcpToolRuntime as t };