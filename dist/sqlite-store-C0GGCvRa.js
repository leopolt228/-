...
import { a as migrateSqliteSchemaToStrict, t as configureSqliteConnectionPragmas } from "./sqlite-wal-jkTlXxi6.js";
...
const WORKBOARD_SQLITE_BUSY_TIMEOUT_MS = 5e3;
const WORKBOARD_SQLITE_DIR_MODE = 448;
const WORKBOARD_SQLITE_FILE_MODE = 384;
...
function requiredString(row, key) {
	...
	if (!value) throw new Error(`workboard sqlite row missing ${key}`);
	return value;
}
function requiredNumber(row, key) {
	const value = numberValue(row, key);
	if (value === void 0) throw new Error(`workboard sqlite row missing ${key}`);
	return value;
}
...
function tableColumns(db, tableName) {
	return new Set(db.prepare(`PRAGMA table_info(${tableName})`).all().flatMap((row) => typeof row.name === "string" ? [row.name] : []));
}
...
const WORKBOARD_SCHEMA_SQL = `
  ...
  `;
function ensureWorkboardSchema(db) {
	db.exec(WORKBOARD_SCHEMA_SQL);
	ensureColumn(db, "workboard_cards", "lifecycle_status_source_updated_at", "lifecycle_status_source_updated_at INTEGER");
	const migrationId = `schema-${SCHEMA_VERSION}`;
	if (!db.prepare("SELECT 1 AS found FROM workboard_schema_migrations WHERE id = ?").get(migrationId)) {
		migrateSqliteSchemaToStrict(db, WORKBOARD_SCHEMA_SQL, { databaseLabel: "workboard database" });
		db.prepare("INSERT OR IGNORE INTO workboard_schema_migrations (id, applied_at) VALUES (?, ?)").run(migrationId, Date.now());
	}
}
...
function hardenWorkboardDatabaseFiles(dbPath) {
...
function createDatabase(dbPath) {
	fs.mkdirSync(path.dirname(dbPath), {
		recursive: true,
		mode: WORKBOARD_SQLITE_DIR_MODE
	});
	chmodIfExists(path.dirname(dbPath), WORKBOARD_SQLITE_DIR_MODE);
	if (!fs.existsSync(dbPath)) fs.closeSync(fs.openSync(dbPath, "a", WORKBOARD_SQLITE_FILE_MODE));
...
function readMetadata(db, row) {
	...
	const attempts = childRows(db, "workboard_card_attempts", cardId).map((child) => {
		...
		if (endedAt !== void 0) entry.endedAt = endedAt;
	...
	const links = childRows(db, "workboard_card_links", cardId).map((child) => {
		...
		if (targetCardId) entry.targetCardId = targetCardId;
	...
	const notifications = childRows(db, "workboard_card_notifications", cardId).map((child) => {
		...
		const sessionKey = stringValue(child, "session_key");
		const runId = stringValue(child, "run_id");
		if (sequence !== void 0) entry.sequence = sequence;
		if (sessionKey) entry.sessionKey = sessionKey;
		if (runId) entry.runId = runId;
		return entry;
	});
	const protocol = db.prepare("SELECT * FROM workboard_worker_protocol WHERE card_id = ?").get(cardId);
...
function bindNull(value) {
	if (value === void 0 || value === null || typeof value === "string" || typeof value === "number" || typeof value === "bigint" || value instanceof Uint8Array) return value ?? null;
...
function insertCard(db, card) {
	...
	insertChildren(db, "workboard_card_labels", card.id, card.labels, (label, ordinal) => {
		db.prepare("INSERT INTO workboard_card_labels (card_id, ordinal, label) VALUES (?, ?, ?)").run(card.id, ordinal, label);
	});
	insertChildren(db, "workboard_card_events", card.id, card.events, (event, ordinal) => {
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(event.id, card.id, ordinal, event.kind, event.at, bindNull(event.fromStatus), bindNull(event.toStatus), bindNull(event.sessionKey), bindNull(event.runId));
	});
	insertChildren(db, "workboard_card_attempts", card.id, metadata?.attempts, (entry, ordinal) => {
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(entry.id, card.id, ordinal, entry.status, entry.startedAt, bindNull(entry.endedAt), bindNull(entry.engine), bindNull(entry.mode), bindNull(entry.model), bindNull(entry.sessionKey), bindNull(entry.runId), bindNull(entry.error));
	});
	...
	insertChildren(db, "workboard_card_links", card.id, metadata?.links, (entry, ordinal) => {
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `).run(entry.id, card.id, ordinal, entry.type, bindNull(entry.targetCardId), bindNull(entry.title), bindNull(entry.url), entry.createdAt);
	});
	insertChildren(db, "workboard_card_proof", card.id, metadata?.proof, (entry, ordinal) => {
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(entry.id, card.id, ordinal, entry.status, bindNull(entry.label), bindNull(entry.command), bindNull(entry.url), bindNull(entry.note), entry.createdAt);
	});
	insertChildren(db, "workboard_card_artifacts", card.id, metadata?.artifacts, (entry, ordinal) => {
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `).run(entry.id, card.id, ordinal, bindNull(entry.label), bindNull(entry.url), bindNull(entry.path), bindNull(entry.mimeType), entry.createdAt);
	});
	insertChildren(db, "workboard_card_attachments", card.id, metadata?.attachments, (entry, ordinal) => {
		db.prepare(`
          INSERT INTO workboard_card_attachments
            (id, card_id, ordinal, file_name, byte_size, mime_type, note, created_at)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        `).run(entry.id, entry.cardId, ordinal, entry.fileName, entry.byteSize, bindNull(entry.mimeType), bindNull(entry.note), entry.createdAt);
	});
	insertChildren(db, "workboard_card_diagnostics", card.id, metadata?.diagnostics, (entry, ordinal) => {
		db.prepare(`
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(card.id, ordinal, entry.kind, entry.severity, entry.title, entry.detail, entry.firstSeenAt, entry.lastSeenAt, entry.count, JSON.stringify(entry.actions));
	});
	insertChildren(db, "workboard_card_notifications", card.id, metadata?.notifications, (entry, ordinal) => {
		db.prepare(`
          INSERT INTO workboard_card_notifications
            (id, card_id, ordinal, kind, message, created_at, sequence, session_key, run_id)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(entry.id, card.id, ordinal, entry.kind, entry.message, entry.createdAt, bindNull(entry.sequence), bindNull(entry.sessionKey), bindNull(entry.runId));
	});
	insertChildren(db, "workboard_worker_logs", card.id, metadata?.workerLogs, (entry, ordinal) => {
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `).run(entry.id, card.id, ordinal, entry.level, entry.message, entry.createdAt, bindNull(entry.sessionKey), bindNull(entry.runId));
	});
	db.prepare("DELETE FROM workboard_worker_protocol WHERE card_id = ?").run(card.id);
	if (metadata?.workerProtocol) db.prepare(`
        INSERT INTO workboard_worker_protocol (card_id, state, updated_at, detail)
        VALUES (?, ?, ?, ?)
      `).run(card.id, metadata.workerProtocol.state, metadata.workerProtocol.updatedAt, bindNull(metadata.workerProtocol.detail));
}
var WorkboardSqliteCardStore = class {
	constructor(db) {
		this.db = db;
	}
	async register(key, value) {
		if (value.version !== 1 || value.card.id !== key) throw new Error("invalid workboard card payload");
	...
	async entries() {
		return this.db.prepare("SELECT * FROM workboard_cards ORDER BY created_at ASC, id ASC").all().map((row) => ({
...
var WorkboardSqliteBoardStore = class {
	constructor(db) {
		this.db = db;
	}
	async register(key, value) {
		if (value.version !== 1 || value.board.id !== key) throw new Error("invalid workboard board payload");
		...
		this.db.prepare(`
          ...
          ON CONFLICT(id) DO UPDATE SET
            ...
            updated_at = excluded.updated_at,
            archived_at = excluded.archived_at
        `).run(board.id, bindNull(board.name), bindNull(board.description), bindNull(board.icon), bindNull(board.color), jsonValue(board.defaultWorkspace), jsonValue(board.orchestration), board.createdAt, board.updatedAt, bindNull(board.archivedAt));
	}
	async lookup(key) {
		const row = this.db.prepare("SELECT * FROM workboard_boards WHERE id = ?").get(key);
		if (!row) return;
	...
	async delete(key) {
		return this.db.prepare("DELETE FROM workboard_boards WHERE id = ?").run(key).changes > 0;
	}
	async entries() {
		const rows = this.db.prepare("SELECT id FROM workboard_boards ORDER BY id ASC").all();
		const entries = [];
...
var WorkboardSqliteSubscriptionStore = class {
	constructor(db) {
		this.db = db;
	}
	async register(key, value) {
		if (value.version !== 1 || value.subscription.id !== key) throw new Error("invalid workboard notification subscription payload");
		...
		this.db.prepare(`
          ...
          ON CONFLICT(id) DO UPDATE SET
            ...
            created_at = excluded.created_at,
            updated_at = excluded.updated_at
        `).run(subscription.id, subscription.boardId, bindNull(subscription.cardId), bindNull(subscription.sessionKey), bindNull(subscription.runId), bindNull(subscription.target), jsonValue(subscription.eventKinds), bindNull(subscription.lastEventAt), bindNull(subscription.lastEventId), bindNull(subscription.lastEventSequence), jsonValue(subscription.deliveredEventIds), subscription.createdAt, subscription.updatedAt);
	}
	async lookup(key) {
		const row = this.db.prepare("SELECT * FROM workboard_notification_subscriptions WHERE id = ?").get(key);
		if (!row) return;
	...
	async delete(key) {
		return this.db.prepare("DELETE FROM workboard_notification_subscriptions WHERE id = ?").run(key).changes > 0;
	}
	async entries() {
		const rows = this.db.prepare("SELECT id FROM workboard_notification_subscriptions ORDER BY created_at ASC, id ASC").all();
		const entries = [];
		for (const row of rows) {
			const key = requiredString(row, "id");
			const value = await this.lookup(key);
...
var WorkboardSqliteAttachmentStore = class {
	constructor(db) {
		this.db = db;
	}
	async register(key, value) {
		if (value.version !== 1 || value.attachment.id !== key) throw new Error("invalid workboard attachment payload");
...
function createWorkboardSqliteStores(options = {}) {
	const { db, maintenance } = createDatabase(options.dbPath ?? resolveWorkboardSqlitePath(options.env));
	return {
...
//#endregion
export { resolveWorkboardSqlitePath as n, createWorkboardSqliteStores as t };
