...
import { a as resolveAgentDir, c as resolveDefaultAgentId, n as listAgentIds, s as resolveDefaultAgentDir } from "./agent-scope-config-S7z_Yn4H.js";
import { i as resolveAuthStorePathForDisplay } from "./path-resolve-Crj4m2cc.js";
import "./persisted-BCOBzYGx.js";
import { r as hasLocalAuthProfileStoreSource, t as hasAnyAuthProfileStoreSource } from "./source-check-D7qXDGS3.js";
import { i as ensureAuthProfileStore } from "./store-BTcmQtbp.js";
import "./auth-profiles-D9OcwMed.js";
import { i as formatAuthDoctorHint, n as resolveApiKeyForProfile } from "./oauth-t9_FvpLo.js";
import { n as buildOAuthRefreshFailureLoginCommand, o as formatOAuthRefreshFailureLoginCommandMarkdown, r as classifyOAuthRefreshFailure } from "./oauth-refresh-failure-c7zpMeHO.js";
...
if (process.env.VITEST || false) globalThis[Symbol.for("openclaw.doctorAuthTestApi")] = { legacyCodexProviderOverrideToHealthFinding };
...
function isAuthProfileHealthIssue(profile) {
	...
	if (profile.profileId === "anthropic:claude-cli" && profile.provider === CLAUDE_CLI_PROVIDER_ID && profile.type === "oauth" && profile.status === "expiring") return false;
...
async function noteAuthProfileHealth(params) {
	...
	const labelAgents = activeTargets.length > 1;
	const agentsByIssueLine = /* @__PURE__ */ new Map();
	for (const target of activeTargets) for (const line of await noteAuthProfileHealthForTarget({
		...params,
		target,
		labelAgents
	})) {
		const agentIds = agentsByIssueLine.get(line) ?? /* @__PURE__ */ new Set();
		agentsByIssueLine.set(line, agentIds.add(target.agentId));
	}
	if (agentsByIssueLine.size === 0) return;
	note([...agentsByIssueLine.entries()].toSorted(([left], [right]) => left.localeCompare(right)).map(([line, agentIds]) => agentIds.size === activeTargets.length ? line : `${line} (agents: ${[...agentIds].toSorted().join(", ")})`).join("\n"), "Model auth");
}
//#endregion
