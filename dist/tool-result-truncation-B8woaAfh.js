...
import { i as resolveAgentContextLimits } from "./agent-scope-config-S7z_Yn4H.js";
...
import { p as formatFullOutputFooter } from "./sessions-Coo3M9oK.js";
import { t as log } from "./logger-DTutvtjM.js";
import { c as readTranscriptFileState, i as resolveRuntimeTranscriptReadTarget, n as rewriteTranscriptEntriesInSessionManager, o as createTranscriptFileStateFromPersistedEntries, r as rewriteTranscriptEntriesInState, s as persistTranscriptStateMutation, t as rewriteTranscriptEntriesInRuntimeTranscript } from "./transcript-rewrite-BPF01I6h.js";
import { o as resolveSessionWriteLockOptions, t as acquireSessionWriteLock } from "./session-write-lock-CndgqGyM.js";
...
async function readRuntimeTranscriptFileState(scope) {
	...
	if (!sqliteMarker) throw new Error("runtime transcript target is not SQLite-backed");
	return createTranscriptFileStateFromPersistedEntries(await loadTranscriptEvents({
...
function getToolResultSpillDetails(message) {
	...
	const nested = details.spill;
	const nestedSpill = nested && typeof nested === "object" && !Array.isArray(nested) ? nested : void 0;
	const path = nestedSpill?.path ?? details.fullOutputPath;
	if (typeof path !== "string" || path.length === 0) return;
	const truncated = nestedSpill?.truncated === true || details.spillTruncated === true;
...
function resolveAggregateElisionMarkers(message) {
	...
	if (!spill) return;
	if (!toolResultTextContainsFullOutputFooter(message, spill.path)) return;
	if (!existsSync(spill.path)) return;
	if (spill.truncated) {
		const count = spill.chars === void 0 ? "capped content" : `first ${spill.chars} chars`;
		return {
			...
			truncationSuffix: (truncatedChars) => `[... ${Math.max(1, Math.floor(truncatedChars))} chars truncated; partial output at ${spill.path}]`
		};
	}
	return {
		full: `[tool result elided: full output preserved at ${spill.path}; read it if the output is needed]`,
		compact: `[read ${spill.path}]`,
		truncationSuffix: (truncatedChars) => `[... ${Math.max(1, Math.floor(truncatedChars))} chars truncated; full output at ${spill.path}]`
	};
}
...
function buildRecoveryToolResultReplacementPlan(params) {
	...
	const aggregateBudgetChars = calculateRecoveryAggregateToolResultChars(params.contextWindowTokens, maxChars, params.aggregateMaxCharsOverride);
	return {
		maxChars,
		aggregateBudgetChars,
...
async function truncateOversizedToolResultsInActiveTarget(params) {
	if (parseSqliteSessionFileMarker(params.scope.sessionFile)) return await truncateOversizedToolResultsInRuntimeTranscript(params);
	if (!params.scope.sessionFile) return {
		truncated: false,
		truncatedCount: 0,
...
//#endregion
export { resolveLiveToolResultAggregateMaxChars as a, toolResultWarningDedupe as c, truncateOversizedToolResultsInSessionManager as d, truncateToolResultMessage as f, resolveAutoLiveToolResultMaxChars as i, truncateOversizedToolResultsInActiveTarget as l, calculateMaxToolResultCharsWithCap as n, resolveLiveToolResultMaxChars as o, formatContextLimitTruncationNotice as p, estimateToolResultReductionPotential as r, sessionLikelyHasOversizedToolResults as s, DEFAULT_MAX_LIVE_TOOL_RESULT_CHARS as t, truncateOversizedToolResultsInMessages as u };
