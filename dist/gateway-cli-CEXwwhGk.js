...
import { t as addGatewayServiceCommands } from "./register-service-commands-BE9Ntzat.js";
...
import { t as addGatewayRunCommand } from "./run-command-CYCM6Zqc.js";
...
function addGatewayRestartHandoffCommands(gateway) {
	...
	restartHandoff.command("consume").description("Atomically consume a gateway restart handoff").helpOption(false).allowUnknownOption().allowExcessArguments().option("--expected-pid [pid]", "PID of the exited gateway process", collectExpectedPid, []).option("--json", "Output JSON", false).action(async (opts, command) => {
		...
		try {
			const { consumeGatewayRestartHandoffSync } = await import("./restart-handoff-o7Kcgugk.js");
...
const gatewayStatusModuleLoader = createLazyImportLoader(() => import("./gateway-status-CMpiFEw6.js"));
const gatewayHealthModuleLoader = createLazyImportLoader(() => import("./health-BO8rqUdj.js"));
const bonjourDiscoveryModuleLoader = createLazyImportLoader(() => import("./bonjour-discovery-yB3bwB1O.js"));
const wideAreaDnsModuleLoader = createLazyImportLoader(() => import("./widearea-dns-DqfbfRYu.js"));
const healthStyleModuleLoader = createLazyImportLoader(() => import("./terminal-core/health-style.js"));
const usageFormatModuleLoader = createLazyImportLoader(() => import("./usage-format-BrVCsMM9.js"));
const stabilityBundleModuleLoader = createLazyImportLoader(() => import("./diagnostic-stability-bundle-3kwE9r3L.js"));
const supportExportModuleLoader = createLazyImportLoader(() => import("./diagnostic-support-export-B1cEH1_h.js"));
const daemonStatusGatherModuleLoader = createLazyImportLoader(() => import("./status.gather-CNKInhr2.js"));
const DEFAULT_GATEWAY_RPC_TIMEOUT_MS = 1e4;
const DEFAULT_USAGE_COST_TIMEOUT_MS = 5 * 6e4;
const USAGE_COST_SETTLE_INITIAL_POLL_MS = 250;
const USAGE_COST_SETTLE_MAX_POLL_MS = 5e3;
...
async function callGatewayCli(method, opts, params) {
	return (await import("./call-BhvmFvUS.js")).callGatewayCli(method, opts, params);
}
...
async function loadSettledCostUsageSummary(rpcOpts, params) {
	const timeoutMs = parseTimeoutMsWithFallback(rpcOpts.timeout, DEFAULT_USAGE_COST_TIMEOUT_MS, { invalidType: "error" });
	const deadline = Date.now() + timeoutMs;
	let lastSummary;
	let pollMs = USAGE_COST_SETTLE_INITIAL_POLL_MS;
	for (;;) {
		const remainingBeforeCallMs = deadline - Date.now();
		if (remainingBeforeCallMs <= 0) throw createUsageCostSettleTimeoutError(lastSummary);
		const summary = await callGatewayCli("usage.cost", {
			...rpcOpts,
			timeout: String(Math.min(DEFAULT_GATEWAY_RPC_TIMEOUT_MS, remainingBeforeCallMs))
		}, params);
		lastSummary = summary;
		const status = summary.cacheStatus?.status;
		if (!status || status === "fresh") return summary;
		const remainingMs = deadline - Date.now();
		if (remainingMs <= 0) throw createUsageCostSettleTimeoutError(summary);
		await sleep(Math.min(pollMs, remainingMs));
		pollMs = Math.min(pollMs * 2, USAGE_COST_SETTLE_MAX_POLL_MS);
	}
}
function createUsageCostSettleTimeoutError(summary) {
	const cachedFiles = summary?.cacheStatus?.cachedFiles ?? 0;
	const pendingFiles = summary?.cacheStatus?.pendingFiles ?? 0;
	return /* @__PURE__ */ new Error(`Timed out waiting for usage cost cache refresh (${cachedFiles} cached, ${pendingFiles} pending)`);
}
async function runGatewayCommand(action, label, opts) {
	try {
		...
		if (opts?.json) {
			const { formatGatewayClientRequestErrorJson, formatGatewayTransportErrorJson } = await import("./call-Au-Dq1sZ.js");
...
function registerGatewayCli(program) {
	...
	gatewayCallOpts(gateway.command("stability").description("Fetch payload-free Gateway stability diagnostics").option("--limit <limit>", "Maximum number of recent events", "25").option("--type <type>", "Filter by diagnostic event type").option("--since-seq <seq>", "Only include events after this sequence").option("--bundle [path]", "Read a persisted stability bundle instead of calling Gateway; pass \"latest\" for newest").option("--export", "Write a shareable support diagnostics export", false).option("--output <path>", "Diagnostics export output .zip path").action(async (opts, command) => {
		await runGatewayCommand(async () => {
			const { normalizeDiagnosticStabilityQuery, selectDiagnosticStabilitySnapshot } = await import("./diagnostic-stability-BGHyxjOG.js");
			const rpcOpts = resolveGatewayRpcOptions(opts, command);
	...
	gateway.command("discover").description("Discover gateways via Bonjour (local + wide-area if configured)").option("--timeout <ms>", "Per-command timeout in ms", "2000").option("--json", "Output JSON", false).action(async (opts) => {
		await runGatewayCommand(async () => {
			const [{ readSourceConfigBestEffort }, { discoverGatewayBeacons }, { resolveWideAreaDiscoveryDomain }, { dedupeBeacons, parseDiscoverTimeoutMs, pickBeaconHost, pickGatewayPort, renderBeaconLines }, { withProgress }] = await Promise.all([
				...
				import("./discover-xPaehvTA.js"),
				import("./progress-Dct1gCkK.js")
			]);
