import { j as resolveTimerTimeoutMs, l as asPositiveSafeInteger } from "./number-coercion-Crk_c9KW.js";
...
import { f as readProviderJsonArrayFieldResponse, g as readResponseTextLimited, m as readProviderJsonResponse } from "./provider-http-errors-DrOMjuGn.js";
...
import { i as normalizeApiKeyConfig } from "./models-config.providers.secrets-M8Q2_pbW.js";
import "./string-coerce-runtime-DBMkn-gE.js";
import "./logging-core-DZYwpRgj.js";
import "./number-runtime-C6TGSEc_.js";
import "./provider-auth-Bnib2g6h.js";
import { c as SELF_HOSTED_DEFAULT_COST, l as SELF_HOSTED_DEFAULT_MAX_TOKENS } from "./provider-setup-OGFypbFK.js";
...
function normalizeLmstudioVariantIds(value) {
	if (!Array.isArray(value)) return [];
	return uniqueStrings(value.flatMap((variant) => typeof variant === "string" && variant.trim().length > 0 ? variant.trim() : []));
}
/**
...
*/
function resolveLmstudioCanonicalModelKey(params) {
	const modelKey = params.modelKey.trim();
	if (!modelKey) return modelKey;
	const normalizedModelKey = modelKey.toLowerCase();
	for (const entry of params.models) if (entry.key?.trim() === modelKey) return modelKey;
	for (const entry of params.models) {
		const key = entry.key?.trim();
		if (!key) continue;
		const selectedVariant = typeof entry.selected_variant === "string" ? entry.selected_variant.trim() : "";
		const variants = normalizeLmstudioVariantIds(entry.variants);
		if (selectedVariant.toLowerCase() === normalizedModelKey || variants.some((variant) => variant.toLowerCase() === normalizedModelKey)) return key;
	}
	return modelKey;
}
/**
...
async function resolveLmstudioConfiguredApiKeyForProvider(params) {
	const apiKeyInput = (params.config?.models?.providers?.[params.providerId])?.apiKey;
...
function withResolvedLmstudioModelKey(error, resolvedModelKey) {
	if (error instanceof Error) return Object.assign(error, { resolvedModelKey });
...
async function ensureLmstudioModelLoaded(params) {
	...
	if (loadedContextWindow !== null && loadedContextWindow >= contextLengthForLoad) return canonicalModelKey;
	try {
		const { response, release } = await fetchLmstudioEndpoint({
		...
		try {
			...
			const payload = await readProviderJsonResponse(response, "LM Studio model load");
...
//#endregion
export { LMSTUDIO_DEFAULT_MODEL_ID as A, resolveLmstudioServerBase as C, LMSTUDIO_DEFAULT_EMBEDDING_MODEL as D, LMSTUDIO_DEFAULT_BASE_URL as E, LMSTUDIO_PROVIDER_ID as F, LMSTUDIO_PROVIDER_LABEL as I, LMSTUDIO_DOCKER_HOST_INFERENCE_BASE_URL as M, LMSTUDIO_LOCAL_API_KEY_PLACEHOLDER as N, LMSTUDIO_DEFAULT_INFERENCE_BASE_URL as O, LMSTUDIO_MODEL_PLACEHOLDER as P, resolveLmstudioReasoningCompat as S, LMSTUDIO_DEFAULT_API_KEY_ENV_VAR as T, normalizeLmstudioConfiguredCatalogEntries as _, resolveLmstudioConfiguredApiKey as a, resolveLmstudioInferenceBase as b, resolveLmstudioRequestContext as c, resolveLmstudioProviderAuthMode as d, shouldUseLmstudioApiKeyPlaceholder as f, mapLmstudioWireModelsToConfig as g, mapLmstudioWireEntry as h, buildLmstudioAuthHeaders as i, LMSTUDIO_DOCKER_HOST_BASE_URL as j, LMSTUDIO_DEFAULT_LOAD_CONTEXT_LENGTH as k, resolveLmstudioRuntimeApiKey as l, buildLmstudioModelName as m, ensureLmstudioModelLoaded as n, resolveLmstudioConfiguredApiKeyForProvider as o, shouldUseLmstudioSyntheticAuth as p, fetchLmstudioModels as r, resolveLmstudioProviderHeaders as s, discoverLmstudioModels as t, hasLmstudioAuthorizationHeader as u, normalizeLmstudioConfiguredCatalogEntry as v, resolveLoadedContextWindow as w, resolveLmstudioReasoningCapability as x, normalizeLmstudioProviderConfig as y };
