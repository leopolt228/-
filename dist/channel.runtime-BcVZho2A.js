...
import { r as formatErrorMessage$1, t as collectErrorGraphCandidates } from "./errors-DdbcjW1Y.js";
...
import { i as saveRemoteMedia, r as readRemoteMediaBuffer } from "./fetch-Mq4HGaV9.js";
...
function markdownToStory(markdown) {
	...
	while (i < lines.length) {
		...
		while (true) {
			...
			if (paragraphLine === void 0 || paragraphLine.trim() === "" || paragraphLine.startsWith("#") || paragraphLine.startsWith("```") || paragraphLine.startsWith("> ") || /^(-{3,}|\*{3,})$/.test(paragraphLine.trim())) break;
...
var UrbitSSEClient = class {
	...
	*/
	...
	async attemptReconnect() {
		...
		if (this.aborted || !this.autoReconnect) return;
		try {
...
const TLON_INGRESS_PRUNE_INTERVAL_MS = 3600 * 1e3;
const TLON_INGRESS_FAILED_TTL_MS = 720 * 60 * 60 * 1e3;
const TLON_INGRESS_TOMBSTONE_MAX_ENTRIES = 2e3;
var TlonIngressPermanentError = class extends Error {
...
var TlonIngressShutdownError = class extends Error {
	constructor() {
...
function resolveTlonIngressNonRetryableFailure(error) {
	...
	for (const candidate of collectErrorGraphCandidates(error, (current) => [current.cause])) if (candidate instanceof UrbitAuthError || candidate instanceof UrbitHttpError && (candidate.status === 401 || candidate.status === 403)) return {
		reason: "tlon-auth",
		message: formatErrorMessage$1(candidate)
	};
	return null;
}
function createTlonIngressMonitor(options) {
	const monitor = createChannelIngressMonitor({
		...
		onError: (error) => options.runtime.error?.(`tlon ingress drain failed: ${formatErrorMessage$1(error)}`)
	});
	return {
...
async function monitorTlonProvider(opts = {}) {
	...
	*/
	...
	const processMessage = async (params) => {
		const { messageId, senderShip, isGroup, channelNest, hostShip: _hostShip, channelName: _channelName, timestamp, parentId, isThreadReply, messageContent, turnAdoptionLifecycle } = params;
...
4. Notable participants`;
		...
		await core.channel.inbound.dispatch({
			...
			...turnAdoptionLifecycle ? { replyOptions: bindIngressLifecycleToReplyOptions(turnAdoptionLifecycle) } : {},
...
var init_HttpProtocol = __esmMin((() => {
	...
	HttpProtocol = class extends SerdeContext {
		...
		async loadEventStreamCapability() {
			const { EventStreamSerde, eventStreamSerdeProvider } = await import("./event-streams-xeqAkkiV.js");
