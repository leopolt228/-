...
function isSafeMigrationConfigPath(path) {
	return path.length > 0 && path.every((segment) => segment.length > 0 && !isBlockedObjectKey(segment));
}
function cloneMigrationConfigValue(value) {
	...
	const next = {};
	for (const [key, entry] of Object.entries(value)) if (!isBlockedObjectKey(key)) next[key] = cloneMigrationConfigValue(entry);
	return next;
}
/** Reads a nested config value, returning undefined when a parent is not an object. */
function readMigrationConfigPath(root, path) {
	if (!isSafeMigrationConfigPath(path)) return;
	let current = root;
...
function mergeMigrationConfigValue(left, right) {
	if (!isRecord(left) || !isRecord(right)) return cloneMigrationConfigValue(right);
	const next = { ...left };
...
function hasMigrationConfigPatchConflict(config, path, value) {
	...
	return Object.keys(value).some((key) => !isBlockedObjectKey(key) && Object.hasOwn(existing, key) && existing[key] !== void 0);
}
...
async function applyMigrationConfigPatchItem(ctx, item) {
	...
	if (!isSafeMigrationConfigPath(details.path)) return markMigrationItemError(item, MIGRATION_REASON_UNSAFE_CONFIG_PATCH_PATH);
...
function redactMigrationValueInternal(value, seen) {
	...
	const record = value;
	const next = {};
	const redactSensitiveDetailsValue = record.sensitive === true && isRecord(record.details) && Object.hasOwn(record.details, "value");
