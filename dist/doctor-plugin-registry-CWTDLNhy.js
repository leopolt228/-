...
import { l as normalizeWindowsPathForComparison } from "./path-DILYn_gk.js";
...
import { l as clearLoadInstalledPluginIndexInstallRecordsCache, n as loadInstalledPluginIndexInstallRecords, t as listRecoveredManagedNpmInstallCandidates } from "./installed-plugin-index-record-reader-DjVucfOz.js";
import { i as markRetainedManagedNpmInstall, l as listManagedPluginNpmRootsSync, r as hasRetainedManagedNpmInstallMarker } from "./managed-npm-retention-BDvRhUup.js";
import { r as loadInstalledPluginIndex } from "./installed-plugin-index-DlWmC2dq.js";
import { f as refreshPluginRegistry } from "./plugin-registry-2gpKUE2T.js";
import "./installed-plugin-index-records-D6eYE-Kv.js";
import { n as saveJsonFile } from "./json-file-B4SF82MN.js";
import { n as preflightPluginRegistryInstallMigration, t as migratePluginRegistryForInstall } from "./plugin-registry-migration-Dv4_YNDa.js";
import { t as note } from "./note-AoV1Tth-.js";
import { i as relinkOpenClawPeerDependenciesInManagedNpmRoot, t as auditOpenClawPeerDependenciesInManagedNpmRoot } from "./plugin-peer-link-ClR6nlYm.js";
import { t as listStaleLocalBundledPluginInstallRecords } from "./stale-local-bundled-plugin-install-records-DDLDNcj0.js";
...
const formatPackageReadFailure = (error) => error instanceof Error ? error.message : String(error);
...
async function maybeRepairManagedNpmOpenClawPeerLinks(params) {
	...
	if (!params.prompter.shouldRepair) {
		...
		const issues = (await Promise.all(npmRoots.map((npmRoot) => auditOpenClawPeerDependenciesInManagedNpmRoot({
			npmRoot,
		...
		if (packageReadFailures.length > 0) note(["Managed npm plugin packages could not be inspected:", ...packageReadFailures.map((failure) => `- ${shortenHomePath(failure.packageDir)}: ${failure.reason}`)].join("\n"), "Plugin registry");
		return false;
	}
	const messages = [];
	const logger = {
	...
	const repaired = (await Promise.all(npmRoots.map((npmRoot) => relinkOpenClawPeerDependenciesInManagedNpmRoot({
		...
		logger,
		onPackageReadError: (error, packageDir) => {
			logger.warn(`Could not inspect managed npm package ${shortenHomePath(packageDir)}: ${formatPackageReadFailure(error)}`);
		}
...
async function listManagedNpmOpenClawPeerLinkIssues(params) {
	const packageReadFailures = [];
	return {
		peerLinkIssues: (await Promise.all(listManagedPluginNpmRoots(params).map((npmRoot) => auditOpenClawPeerDependenciesInManagedNpmRoot({
			npmRoot,
			onPackageReadError: (error, packageDir) => {
				packageReadFailures.push({
					packageDir,
...
async function detectPluginRegistryHealthIssues(params) {
	...
	for (const issue of managedNpmAudit.peerLinkIssues) issues.push({
		kind: "managed-npm-openclaw-peer-link",
		packageName: issue.packageName,
		packageDir: issue.packageDir,
		reason: issue.reason
	});
	for (const failure of managedNpmAudit.packageReadFailures) issues.push({
...
async function maybeRepairPluginRegistryState(params) {
	...
	if (preflight.action === "migrate") {
		const result = await migratePluginRegistryForInstall({
			...migrationParams,
			...shouldPersistRepairedInstallRecords ? { installRecords: await loadInstallRecordsWithoutPluginIds(params, stalePluginIdsToRemove) } : {}
		});
		if (result.migrated) {
	...
	if (preflight.action === "skip-existing" || removedStaleManagedNpmBundledPlugins || removedStaleLocalBundledPluginIds.length > 0 || retiredStaleManagedNpmInstallGenerations || repairedManagedNpmOpenClawPeerLinks) {
...
//#endregion
export { pluginRegistryIssueToHealthFinding as a, maybeRepairStaleManagedNpmBundledPlugins as i, maybeRepairManagedNpmOpenClawPeerLinks as n, pluginRegistryIssueToRepairEffect as o, maybeRepairPluginRegistryState as r, detectPluginRegistryHealthIssues as t };
