...
import { M as resolveChannelStreamingBlockCoalesce } from "./streaming-CeN4qI3u.js";
...
function resolveScopedBlockStreamingCoalesce(config) {
	return config ? resolveChannelStreamingBlockCoalesce(config) : void 0;
}
function resolveProviderBlockStreamingCoalesce(params) {
	const { cfg, providerKey, accountId } = params;
	if (!cfg || !providerKey) return;
	const providerCfg = cfg.channels?.[providerKey] ?? cfg[providerKey];
	...
	if (channelCoalesce || accountCoalesce) return {
		...channelCoalesce,
		...accountCoalesce
	};
}
...
//#endregion
export { resolveEffectiveBlockStreamingConfig as n, resolveBlockStreamingChunking as t };
