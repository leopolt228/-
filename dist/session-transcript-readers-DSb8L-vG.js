...
import { D as resolveIntegerOption, O as resolveNonNegativeIntegerOption } from "./number-coercion-Crk_c9KW.js";
...
import { Kt as normalizeUsage, Ut as deriveSessionTotalTokens, Wt as hasNonzeroUsage, a as readSessionTranscriptMessageEventById, c as readSessionTranscriptMessageEvents, g as resolveSessionTranscriptReadTarget, o as readSessionTranscriptMessageEventCount, r as readRecentSessionTranscriptMessageEvents, s as readSessionTranscriptMessageEventPage } from "./session-accessor-Mu3lv_Tl.js";
import { n as parseSqliteSessionFileMarker } from "./sqlite-marker-BejbySI1.js";
import { l as selectSessionTranscriptTreePathNodes, o as parseSessionTranscriptTreeEntry, s as scanSessionTranscriptTree } from "./transcript-tree-DuZTyiYZ.js";
import { n as extractAssistantVisibleText } from "./chat-message-content-CeBHi_A4.js";
import { t as extractInboundSenderLabel } from "./strip-inbound-meta-CbJ4Y6Dq.js";
import { n as readFileWindowFullySync, t as readFileWindowFully } from "./file-read-DtMn74uz.js";
import { a as hasInterSessionUserProvenance } from "./input-provenance-B6vSIOBi.js";
...
import { n as extractToolCallNames, r as hasToolCall } from "./transcript-tools-CZ1jNJJI.js";
import { t as stripEnvelope } from "./chat-envelope-br4jVgj4.js";
import { n as stripUserEnvelopeForDisplay, t as stripInternalMetadataForDisplay } from "./display-text-sanitize-wW1-W6iE.js";
import { a as resolveSessionTranscriptCandidates, o as resolveSessionTranscriptResetArchiveCandidatesAsync } from "./session-transcript-files.fs-BccomQRm.js";
...
async function buildSessionTranscriptIndex(filePath, stat) {
	...
	await visitTranscriptJsonLines(filePath, (line, offset, byteLength) => {
		...
		const parentId = parsed.parentId === null ? null : readNonBlankStringPreservingWhitespace(parsed.parentId) ?? void 0;
...
async function yieldTranscriptScan() {
	await new Promise((resolve) => {
		setImmediate(resolve);
	});
}
...
async function visitTranscriptLinesAsync(filePath, visit) {
	const handle = await fs.promises.open(filePath, "r");
	try {
		const decoder = new StringDecoder("utf8");
...
async function readSessionMessagesAsync$1(sessionId, storePath, sessionFile, opts, agentId) {
	return (await readSessionMessagesWithSourceAsync$1(sessionId, storePath, sessionFile, opts, agentId)).messages;
}
async function readSessionMessagesWithSourceAsync$1(sessionId, storePath, sessionFile, opts, agentId) {
	if (opts.mode === "recent") {
	...
	const filePath = opts.allowResetArchiveFallback === true ? await findExistingTranscriptHistoryPathAsync(sessionId, storePath, sessionFile, agentId, { resetArchiveOnly: opts.resetArchiveOnly === true }) : findExistingTranscriptPath(sessionId, storePath, sessionFile, agentId);
...
async function readSessionMessageByIdAsync$1(sessionId, storePath, sessionFile, messageId, opts) {
	const filePath = opts?.allowResetArchiveFallback === true ? await findExistingTranscriptHistoryPathAsync(sessionId, storePath, sessionFile, opts.agentId, { resetArchiveOnly: opts.resetArchiveOnly === true }) : findExistingTranscriptPath(sessionId, storePath, sessionFile, opts?.agentId);
	if (!filePath) return {
		oversized: false,
		found: false
	};
...
async function visitSessionMessagesAsync$1(sessionId, storePath, sessionFile, visit, opts, agentId) {
	const filePath = findExistingTranscriptPath(sessionId, storePath, sessionFile, agentId);
...
async function readSessionMessageCountAsync$1(sessionId, storePath, sessionFile, agentId) {
	const filePath = findExistingTranscriptPath(sessionId, storePath, sessionFile, agentId);
	if (!filePath) return 0;
...
async function readRecentSessionMessagesWithStatsAsync$1(sessionId, storePath, sessionFile, opts, agentId) {
	const filePath = opts.allowResetArchiveFallback === true ? await findExistingTranscriptHistoryPathAsync(sessionId, storePath, sessionFile, agentId, { resetArchiveOnly: opts.resetArchiveOnly === true }) : findExistingTranscriptPath(sessionId, storePath, sessionFile, agentId);
	if (!filePath) return {
		messages: [],
		totalMessages: 0
...
async function readSessionMessagesPageWithStatsAsync$1(sessionId, storePath, sessionFile, opts, agentId) {
	...
	return {
		messages: index.entries.slice(start, endExclusive).flatMap((entry) => indexedTranscriptEntryToMessages(entry)),
		totalMessages,
		transcriptPath: filePath
	};
}
...
function readSessionTitleFieldsFromTranscript$1(sessionId, storePath, sessionFile, agentId, opts) {
	const filePath = findExistingTranscriptPath(sessionId, storePath, sessionFile, agentId);
	if (!filePath) return {
...
async function readSessionTitleFieldsFromTranscriptAsync$1(sessionId, storePath, sessionFile, agentId, opts) {
	const filePath = findExistingTranscriptPath(sessionId, storePath, sessionFile, agentId);
	if (!filePath) return {
		firstUserMessage: null,
		lastMessagePreview: null
...
async function findExistingTranscriptHistoryPathAsync(sessionId, storePath, sessionFile, agentId, opts) {
...
function readLastMessagePreviewFromOpenTranscript(params) {
	...
	if (bytesRead <= 0) return null;
	return extractLastMessagePreviewFromTranscriptLines(buf.toString("utf-8", 0, bytesRead).split(/\r?\n/).filter((l) => l.trim()).slice(-20));
}
...
function extractUsageSnapshotFromTranscriptLine(line) {
	...
	try {
		...
		if (usage?.contextUsage) snapshot.contextUsage = usage.contextUsage;
...
function extractAggregateUsageFromTranscriptLines(lines) {
	...
	for (const line of lines) {
		...
		else delete snapshot.contextUsage;
...
async function readLatestSessionUsageFromTranscriptAsync$1(sessionId, storePath, sessionFile, agentId) {
	const filePath = findExistingTranscriptPath(sessionId, storePath, sessionFile, agentId);
...
function readRecentSessionUsageFromTranscript$1(sessionId, storePath, sessionFile, agentId, maxBytes) {
	const filePath = findExistingTranscriptPath(sessionId, storePath, sessionFile, agentId);
	if (!filePath) return null;
...
function readRecentMessagesFromTranscript(filePath, maxMessages, readBytes) {
	...
	try {
		...
		const buf = Buffer.alloc(readLen);
		const bytesRead = readFileWindowFullySync(fd, buf, readStart);
		const tailLines = buf.toString("utf-8", 0, bytesRead).split(/\r?\n/).filter((l) => l.trim()).slice(-200);
		const collected = [];
...
function readSessionPreviewItemsFromTranscript$1(sessionId, storePath, sessionFile, agentId, maxItems, maxChars) {
	...
	for (const readSize of PREVIEW_READ_SIZES) {
		...
		if (messages.length > 0 || readSize === PREVIEW_READ_SIZES[PREVIEW_READ_SIZES.length - 1]) return buildSessionPreviewItems(messages, boundedItems, boundedChars);
	}
	return [];
}
//#endregion
...
function isSqliteReadTarget(target) {
	return parseSqliteSessionFileMarker(target.sessionFile) !== void 0;
}
...
function readTranscriptRecordTimestampMs(event) {
	...
	const timestampMs = typeof raw === "string" ? Date.parse(raw) : typeof raw === "number" ? raw : NaN;
...
function sqliteMessageEventWithSeq(entry) {
	const record = extractMessageRecord(entry.event);
	return record ? sqliteRecordMessageWithSeq({
		...record,
		seq: entry.seq
	}) : void 0;
}
...
function readSqliteTitleFields(target, opts) {
	...
	const firstUser = messages.find((message) => {
		...
		return opts?.includeInterSession === true || !hasInterSessionUserProvenance(message);
	});
...
async function readSessionMessagesAsync(scope, opts) {
	...
	if (isSqliteReadTarget(target)) {
		if (opts.mode === "recent") {
			...
			if (records.length === 0 && opts.allowResetArchiveFallback === true) return await readRecentSessionMessagesAsync(target.sessionId, target.storePath, void 0, {
				...opts,
		...
		if (records.length === 0 && opts.allowResetArchiveFallback === true) return await readSessionMessagesAsync$1(target.sessionId, target.storePath, void 0, opts, target.agentId);
		return records.map(sqliteRecordMessageWithSeq);
	}
...
async function readSessionMessagesWithSourceAsync(scope, opts) {
	const target = resolveTranscriptReadTarget(scope);
	if (isSqliteReadTarget(target)) {
		const records = opts.mode === "recent" ? (await readRecentSqliteMessageRecords(target, opts)).records : await readSqliteMessageRecords(target);
		if (records.length === 0 && opts.allowResetArchiveFallback === true) return await readSessionMessagesWithSourceAsync$1(target.sessionId, target.storePath, void 0, {
			...opts,
			resetArchiveOnly: true
		}, target.agentId);
		return {
...
async function readSessionMessageByIdAsync(scope, messageId, opts) {
	const target = resolveTranscriptReadTarget(scope);
	if (isSqliteReadTarget(target)) {
		const foundEvent = readSessionTranscriptMessageEventById(toTranscriptReadScope(target), messageId);
		const found = foundEvent ? extractMessageRecordsFromEventEntries([foundEvent]).at(0) : void 0;
		if (found) return {
			found: true,
		...
		if (opts?.allowResetArchiveFallback === true) return await readSessionMessageByIdAsync$1(target.sessionId, target.storePath, void 0, messageId, {
			...opts,
...
async function readSessionMessageCountAsync(scope) {
	const target = resolveTranscriptReadTarget(scope);
	if (isSqliteReadTarget(target)) return readSessionTranscriptMessageEventCount(toTranscriptReadScope(target));
...
async function readRecentSessionMessagesWithStatsAsync(scope, opts) {
	...
	if (isSqliteReadTarget(target)) {
		...
		if (totalMessages === 0 && records.length === 0 && opts.allowResetArchiveFallback === true) return await readRecentSessionMessagesWithStatsAsync$1(target.sessionId, target.storePath, void 0, {
			...opts,
			resetArchiveOnly: true
		}, target.agentId);
		return {
...
async function readSessionMessagesPageWithStatsAsync(scope, opts) {
	...
	if (isSqliteReadTarget(target)) {
		...
		if (page.totalMessages === 0 && opts.allowResetArchiveFallback === true) return await readSessionMessagesPageWithStatsAsync$1(target.sessionId, target.storePath, void 0, {
			...opts,
			resetArchiveOnly: true
		}, target.agentId);
		return {
...
function readSessionTitleFieldsFromTranscript(scope, opts) {
	const target = resolveTranscriptReadTarget(scope);
	if (isSqliteReadTarget(target)) return readSqliteTitleFields(target, opts);
...
async function readLatestSessionUsageFromTranscriptAsync(scope) {
	const target = resolveTranscriptReadTarget(scope);
	if (isSqliteReadTarget(target)) return readSqliteAggregateUsageSnapshot(target);
	return await readLatestSessionUsageFromTranscriptAsync$1(target.sessionId, target.storePath, target.sessionFile, target.agentId);
}
...
function readRecentSessionUsageFromTranscript(scope, maxBytes) {
	const target = resolveTranscriptReadTarget(scope);
	if (isSqliteReadTarget(target)) return aggregateSqliteUsageSnapshots(readRecentSqliteUsageMessages(target, maxBytes));
...
function readSessionPreviewItemsFromTranscript(scope, maxItems, maxChars) {
	const target = resolveTranscriptReadTarget(scope);
	if (isSqliteReadTarget(target)) return buildSqlitePreviewItems(target, maxItems, maxChars);
...
//#endregion
export { resolveSessionHistoryTranscriptPathAsync as C, stripEnvelopeFromMessages as E, readSessionMessagesAsync$1 as S, stripEnvelopeFromMessage as T, attachOpenClawTranscriptMeta as _, readSessionMessageByIdAsync as a, indexedTranscriptEntryToMessages as b, readSessionMessagesPageWithStatsAsync as c, readSessionTitleFieldsFromTranscript as d, readSessionTitleFieldsFromTranscriptAsync as f, visitSessionMessagesAsync as g, toTranscriptReadScope as h, readRecentSessionUsageFromTranscript as i, readSessionMessagesWithSourceAsync as l, sqliteMessageEventWithSeq as m, readLatestSessionUsageFromTranscriptAsync as n, readSessionMessageCountAsync as o, resolveTranscriptReadTarget as p, readRecentSessionMessagesWithStatsAsync as r, readSessionMessagesAsync as s, isSqliteReadTarget as t, readSessionPreviewItemsFromTranscript as u, capArrayByJsonBytes as v, readSessionTranscriptIndex as w, readSessionMessages as x, findExistingTranscriptPath as y };
