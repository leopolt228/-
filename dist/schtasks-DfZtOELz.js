...
import { a as getWindowsSystem32ExePath, r as getWindowsPowerShellExePath, t as getWindowsCmdExePath } from "./windows-install-roots-BTRBDwn4.js";
import { c as resolveWindowsOemCodePageForEncoding, l as resolveWindowsOemEncoding, s as resolveWindowsOemCodePage } from "./spawn-utils-bQOZkqhj.js";
import { n as inspectPortUsage } from "./ports-inspect-BTAghQ0E.js";
import "./ports-BSfVrBR-.js";
import { o as isGatewayArgv } from "./windows-port-pids-CPr8M1HP.js";
...
import { a as writeFormattedLines, n as parseKeyValueOutput, r as formatLine, t as createGatewayLifecycleMutationReporter } from "./service-mutation-PgzR3-XQ.js";
import { t as findVerifiedGatewayListenerPidsOnPortSync } from "./gateway-processes-c8np4yhp.js";
import { t as execSchtasks } from "./schtasks-exec-DTXY9p-S.js";
...
async function launchFallbackTaskScript(env, installedCommand) {
	...
	if (command?.programArguments.length) {
		...
		spawn(expectDefined(executable, "schtasks executable"), args, {
...
async function resolveScheduledTaskProcess(env, matchesProcess) {
	...
	const port = parseTcpPortFromArgs(installedArguments) ?? parsePositivePort(command?.environment?.OPENCLAW_GATEWAY_PORT) ?? resolveConfiguredGatewayPort(env);
	if (!port) return null;
...
async function resolveScheduledTaskGatewayProcess(env) {
	return await resolveScheduledTaskProcess(env, (argv) => isGatewayArgv(argv, { allowGatewayBinary: true }));
}
...
async function resolveScheduledTaskPort(env) {
	const command = await readScheduledTaskCommand(env).catch(() => null);
	return parseTcpPortFromArgs(command?.programArguments) ?? parsePositivePort(command?.environment?.OPENCLAW_GATEWAY_PORT) ?? resolveConfiguredGatewayPort(env);
}
...
async function resolveFallbackRuntime(env, installedCommand, mode = "observe") {
	...
	const port = parseTcpPortFromArgs(command?.programArguments) ?? parsePositivePort(command?.environment?.OPENCLAW_GATEWAY_PORT) ?? resolveConfiguredGatewayPort(env);
	if (!port) return {
		status: "unknown",
	...
	if (requireCommandOwnership) {
		...
		if (verifiedPids.length > 0) return {
			status: "running",
			pid: verifiedPids[0],
	...
	if (diagnostics.status !== "busy") {
		...
		const status = diagnostics.status === "free" && !processInspectionUnavailable ? "stopped" : "unknown";
		return {
			status,
	...
	if (matchedGatewayPids.length > 0) {
		if (requireCommandOwnership) return {
			status: "unknown",
...
async function assertReplacementPortAvailableForTakeover(params) {
	...
	const port = parseTcpPortFromArgs(params.programArguments) ?? parsePositivePort(params.environment?.OPENCLAW_GATEWAY_PORT) ?? resolveConfiguredGatewayPort(params.env);
...
async function readWindowsStartupFallbackRuntimeForUpdate(env) {
	...
	return await resolveFallbackRuntime(env, void 0, "control");
}
...
async function shouldFallbackScheduledTaskLaunch(params) {
	const readLaunchObservation = async () => {
		...
		if (normalizedResult === "0x0") return {
	...
	const hasLaunchEvidence = async () => {
		...
		const installedArguments = command?.programArguments;
		const taskPort = parseTcpPortFromArgs(installedArguments) ?? parsePositivePort(command?.environment?.OPENCLAW_GATEWAY_PORT) ?? resolveConfiguredGatewayPort(params.env);
		...
		if (!manageGatewayPort) return installedArguments?.length ? findInstalledProcessPid(entries, taskPort, installedArguments, isNodeHostArgv) != null : false;
		return entries.some((entry) => {
			if (!normalizeLowercaseStringOrEmpty(entry.CommandLine ?? "")) return false;
			const argv = parseCmdScriptCommandLine(entry.CommandLine ?? "");
			return isGatewayArgv(argv, { allowGatewayBinary: true }) && parseTcpPortFromArgs(argv) === taskPort;
		});
	};
...
async function suspendScheduledTaskAutoStartForUpdate(env = process.env) {
	return await changeScheduledTaskEnabledState({
		env,
		enabled: false
	});
}
...
async function restartRegisteredScheduledTask(params) {
	...
	if (params.mode.kind === "standard") {
		if (manageGatewayPort) await terminateScheduledTaskGatewayListeners(params.env);
...
async function restartScheduledTask({ stdout, env, onMutation }) {
	...
	try {
		...
		if (await isStartupEntryInstalled(effectiveEnv)) return await restartStartupEntry(effectiveEnv, stdout, (kind) => reportMutation(kind === "stop" ? "startup-entry-stop" : "startup-entry-restart"));
		throw err;
	}
	if (!await isRegisteredScheduledTask(effectiveEnv)) {
		if (await isStartupEntryInstalled(effectiveEnv)) return await restartStartupEntry(effectiveEnv, stdout, (kind) => reportMutation(kind === "stop" ? "startup-entry-stop" : "startup-entry-restart"));
...
//#endregion
export { readWindowsStartupFallbackRuntimeForUpdate as a, resumeScheduledTaskAutoStartAfterUpdate as c, stopScheduledTask as d, suspendScheduledTaskAutoStartForUpdate as f, readScheduledTaskRuntime as i, stageScheduledTask as l, encodeWindowsLauncherScript as m, isScheduledTaskInstalled as n, resolveTaskScriptPath as o, uninstallScheduledTask as p, readScheduledTaskCommand as r, restartScheduledTask as s, installScheduledTask as t, startScheduledTask as u };
