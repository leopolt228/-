...
import { E as parseAgentSessionKey, d as resolveAgentIdFromSessionKey, v as toAgentStoreSessionKey } from "../../session-key-Drrs61Fd.js";
...
import { i as isOpenClawDeliveryMirrorAssistantMessage } from "../../transcript-only-openclaw-assistant-ByevblQR.js";
import { a as hasInterSessionUserProvenance } from "../../input-provenance-B6vSIOBi.js";
import { r as sanitizeModelSpecialTokens } from "../../external-content-DkHx38wP.js";
import { t as generateSlugViaLLM } from "../../llm-slug-generator-CKcxCq-C.js";
import { i as resolveHookConfig } from "../../config-DHK8km67.js";
...
function renderSessionMemoryMessage(entry) {
	if (!entry || typeof entry !== "object") return;
	const record = entry;
	if (record.type !== "message" || !record.message) return;
	const role = record.message.role;
	if (role !== "user" && role !== "assistant" || !("content" in record.message)) return;
	if (role === "user" && hasInterSessionUserProvenance(record.message)) return;
	...
	if (!sanitized) return;
	if (sanitized.startsWith("/")) return role === "user" ? {
...
function getRecentSessionContentFromEvents(events, messageCount = 15) {
	const allMessages = [];
...
async function getRecentSessionContent(sessionFilePath, messageCount = 15) {
	try {
		return getRecentSessionContentFromEvents((await fs.readFile(sessionFilePath, "utf-8")).trim().split("\n").flatMap((line) => {
			try {
...
async function getRecentSessionContentWithResetFallback(sessionFilePath, messageCount = 15) {
	...
	try {
		...
		if (latestReset === void 0) return primary;
		return await getRecentSessionContent(path.join(dir, latestReset), messageCount) || primary;
	} catch {
		return primary;
	}
}
...
async function findPreviousSessionFile(params) {
	try {
		...
		const currentBaseName = params.currentSessionFile ? path.basename(params.currentSessionFile) : void 0;
		const baseFromReset = currentBaseName ? stripResetSuffix(currentBaseName) : void 0;
		if (baseFromReset && fileSet.has(baseFromReset)) return path.join(params.sessionsDir, baseFromReset);
		if (currentBaseName?.includes(".reset.") && fileSet.has(currentBaseName)) return path.join(params.sessionsDir, currentBaseName);
		const trimmedSessionId = params.sessionId?.trim();
		if (trimmedSessionId) {
			const canonicalFile = `${trimmedSessionId}.jsonl`;
			if (fileSet.has(canonicalFile)) return path.join(params.sessionsDir, canonicalFile);
			const [canonicalResetVariant] = files.filter((name) => name.startsWith(`${canonicalFile}.reset.`)).toSorted().toReversed();
			if (canonicalResetVariant !== void 0) return path.join(params.sessionsDir, canonicalResetVariant);
			const [topicVariant] = files.filter((name) => name.startsWith(`${trimmedSessionId}-topic-`) && name.endsWith(".jsonl") && !name.includes(".reset.")).toSorted().toReversed();
			if (topicVariant !== void 0) return path.join(params.sessionsDir, topicVariant);
			const [topicResetVariant] = files.filter((name) => name.startsWith(`${trimmedSessionId}-topic-`) && name.includes(".jsonl.reset.")).toSorted().toReversed();
			if (topicResetVariant !== void 0) return path.join(params.sessionsDir, topicResetVariant);
		}
		if (!params.currentSessionFile) return;
		const [nonResetFile] = files.filter((name) => name.endsWith(".jsonl") && !name.includes(".reset.")).toSorted().toReversed();
		if (nonResetFile !== void 0) return path.join(params.sessionsDir, nonResetFile);
		const [resetFile] = files.filter((name) => name.includes(".jsonl.reset.")).toSorted().toReversed();
		if (resetFile !== void 0) return path.join(params.sessionsDir, resetFile);
	} catch {}
}
//#endregion
...
async function getRecentSqliteSessionContent(marker, messageCount) {
	try {
		return getRecentSessionContentFromEvents(selectVisibleTranscriptEvents(await loadTranscriptEvents({
...
async function saveSessionMemoryNow(event) {
	try {
		...
		if (sessionFile) {
			...
			sessionContent = sqliteMarker ? await getRecentSqliteSessionContent(sqliteMarker, messageCount) : await getRecentSessionContentWithResetFallback(sessionFile, messageCount);
