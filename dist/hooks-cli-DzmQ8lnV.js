...
import { r as evaluateEntryRequirementsForCurrentPlatform } from "./status-BKR_g98B.js";
...
import { a as isNativeHookRelayBridgeStaleRegistrationError, i as invokeNativeHookRelayBridge, s as renderNativeHookRelayUnavailableResponse } from "./native-hook-relay-6mIkwkRz.js";
import { a as resolveHookEnableState, i as resolveHookConfig, n as isHookEnvSatisfied, o as resolveHookEntries, t as isHookConfigPathTruthy } from "./config-DHK8km67.js";
...
import { n as isKnownInternalHookEventKey, t as loadWorkspaceHookEntries } from "./workspace-C_mCNxFl.js";
import { t as runPluginInstallCommand } from "./plugins-install-command-CVMo1U5P.js";
import { t as runPluginUpdateCommand } from "./plugins-update-command-BLdWuD2D.js";
...
function registerHooksCli(program) {
	...
	hooks.command("list").description("List all hooks").option("--eligible", "Show only eligible hooks", false).option("--json", "Output as JSON", false).option("-v, --verbose", "Show more details including missing requirements", false).action(async (opts) => runOneShotHooksCliAction(async () => {
		writeHooksOutput(formatHooksList(buildHooksReport(getRuntimeConfig()), opts), opts.json);
	}));
	hooks.command("info <name>").description("Show detailed information about a hook").option("--json", "Output as JSON", false).action(async (name, opts) => runOneShotHooksCliAction(async () => {
		writeHooksOutput(formatHookInfo(buildHooksReport(getRuntimeConfig()), name, opts), opts.json);
	}));
	hooks.command("check").description("Check hooks eligibility status").option("--json", "Output as JSON", false).action(async (opts) => runOneShotHooksCliAction(async () => {
	...
	hooks.command("install").description("Deprecated: install a hook pack via `openclaw plugins install`").argument("<path-or-spec>", "Path to a hook pack or npm package spec").option("-l, --link", "Link a local path instead of copying", false).option("--pin", "Record npm installs as exact resolved <name>@<version>", false).option("--force", "Confirm non-ClawHub sources and overwrite an existing hook pack", false).action(async (raw, opts) => {
	...
	hooks.action(async () => runOneShotHooksCliAction(async () => {
