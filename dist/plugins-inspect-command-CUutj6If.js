...
async function runPluginsInspectCommand(id, opts) {
	const { buildAllPluginInspectReports, buildPluginDiagnosticsReport, buildPluginInspectReport, buildPluginSnapshotReport, formatPluginCompatibilityNotice } = await import("./status-B-9UuJnN.js");
	const { loadInstalledPluginIndexInstallRecords } = await import("./installed-plugin-index-records-CBud6Uko.js");
	...
	if (!targetPlugin) {
		if (id === "skill-workshop") {
			const { detectSkillWorkshopToolPolicyDiagnostic } = await import("./tool-policy-diagnostic-CW_iSjFm.js");
			const diagnostic = detectSkillWorkshopToolPolicyDiagnostic({
				config: cfg,
				workshopEnabled: true
			});
			const lines = ["Skill Workshop is built into OpenClaw, not a plugin; configure it under skills.workshop."];
