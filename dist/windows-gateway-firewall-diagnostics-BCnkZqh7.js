import { a as getWindowsSystem32ExePath, r as getWindowsPowerShellExePath } from "./windows-install-roots-BTRBDwn4.js";
import { r as runCommandWithTimeout } from "./exec-Cb0CNQNz.js";
//#region src/infra/windows-gateway-firewall-diagnostics.ts
const DEFAULT_WINDOWS_GATEWAY_FIREWALL_TIMEOUT_MS = 5e3;
const QUICK_WINDOWS_GATEWAY_FIREWALL_TIMEOUT_MS = 5e3;
...
function buildWindowsNetSecurityFirewallRulesCommand(port, policyStore, policyStoreSourceTypes) {
...
function buildWindowsPersistentFirewallRulesCommand(port) {
	return buildWindowsNetSecurityFirewallRulesCommand(port, "PersistentStore");
}
function buildWindowsManagedActiveFirewallRulesCommand(port) {
	return buildWindowsNetSecurityFirewallRulesCommand(port, "ActiveStore", WINDOWS_MANAGED_FIREWALL_POLICY_SOURCE_TYPES);
}
function buildWindowsFirewallRulesCommand(port) {
	return `
$ErrorActionPreference = 'Stop'
$ProgressPreference = 'SilentlyContinue'
$targetPort = ${port}
...
function classifyWindowsGatewayFirewallState(state) {
	...
	if (programAgnosticMatchingRules.length > 0) {
		...
		if (programAgnosticMatchingRules.filter(isLocalRule).length === programAgnosticMatchingRules.length && !localRulesAreAllowed(state)) {
			const policyDetail = /gpo-store only/i.test(state.netshOutput) ? "Windows reports LocalFirewallRules as N/A (GPO-store only)." : "Local firewall rules are not explicitly enabled for the active profile.";
			return {
				applies: true,
				severity: "warning",
	...
	if (programAgnosticLocalRules.length > 0 && !localRulesAreAllowed(state)) {
		const policyDetail = /gpo-store only/i.test(state.netshOutput) ? "Windows reports LocalFirewallRules as N/A (GPO-store only)." : "Local firewall rules are disabled for the active profile.";
		return {
			applies: true,
			severity: "warning",
			code: "windows_firewall_local_rules_ignored",
	...
	if (blockingProfiles.length > 0 || activeProfiles.length === 0) return {
		applies: true,
		severity: "warning",
...
function buildClassifiedState(stateJson, netshOutput, activeRules, localRules) {
...
function shouldProbeManagedActiveRules(diagnostic) {
	return diagnostic.severity === "warning" && diagnostic.code !== "windows_firewall_inbound_rules_disabled";
}
...
async function inspectWindowsGatewayFirewall(params) {
	...
	const timeoutMs = params.timeoutMs ?? (mode === "quick" ? QUICK_WINDOWS_GATEWAY_FIREWALL_TIMEOUT_MS : DEFAULT_WINDOWS_GATEWAY_FIREWALL_TIMEOUT_MS);
	if (mode === "quick") {
		...
		if (quickJson === null) return {
			...
			details: ["Run `openclaw gateway status --deep` again, or verify the advertised LAN URL from another device."]
		};
	...
	const [stateJson, rulesJson, netshOutput] = await Promise.all([
		runBestEffortCommand(runCommandWithTimeout$1, powershell(WINDOWS_FIREWALL_STATE_COMMAND), timeoutMs),
		runBestEffortCommand(runCommandWithTimeout$1, powershell(buildWindowsFirewallRulesCommand(params.port)), timeoutMs),
		runBestEffortCommand(runCommandWithTimeout$1, [
	...
	if (stateJson === null || rulesJson === null) return {
		applies: true,
		severity: "warning",
		code: "windows_firewall_inspection_failed",
		message: "OpenClaw could not inspect Windows Firewall policy for LAN Gateway reachability.",
		details: ["Run `openclaw gateway status --deep` from a normal PowerShell session and verify the advertised LAN URL from another device."]
	};
	...
	if (shouldProbeManagedActiveRules(classifyWindowsGatewayFirewallState(state))) {
		const managedRulesJson = await runBestEffortCommand(runCommandWithTimeout$1, powershell(buildWindowsManagedActiveFirewallRulesCommand(params.port)), timeoutMs);
		...
		} else if (!localRulesAreAllowed(state)) return {
			applies: true,
			severity: "warning",
			code: "windows_firewall_inspection_failed",
			message: "OpenClaw could not inspect managed Windows Firewall rules for LAN Gateway reachability.",
			details: ["Run `openclaw gateway status --deep` from a normal PowerShell session and verify Group Policy or administrator-managed allow rules for the Gateway port."]
		};
	}
	...
	if (!localRulesAreAllowed(state) && diagnosticBeforeLocalDetail.severity !== "info") {
		const localRulesJson = await runBestEffortCommand(runCommandWithTimeout$1, powershell(buildWindowsPersistentFirewallRulesCommand(params.port)), Math.max(timeoutMs, 1e4));
		if (localRulesJson !== null) state.localMatchingRules = parseFirewallRules(parseJsonPayload(localRulesJson));
	}
	return classifyWindowsGatewayFirewallState(state);
}
function formatWindowsGatewayFirewallGuidance(params) {
	if ((params.platform ?? process.platform) !== "win32" || params.bind !== "lan") return [];
	return ["Windows firewall: if another device cannot connect to the LAN URL, run `openclaw gateway status --deep` from this Windows host."];
}
//#endregion
