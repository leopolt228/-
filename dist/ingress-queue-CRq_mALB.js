...
function countFailedChannelIngressQueueEntries(stateDir) {
	...
	]).where("status", "=", "failed").groupBy(["channel_id", "account_id"]).orderBy("channel_id", "asc").orderBy("account_id", "asc")).rows.map((row) => ({
		channelId: row.channel_id,
		accountId: row.account_id,
...
function createChannelIngressQueue(options) {
	...
	const listPending = async (listOptions) => {
		...
		while (records.length < limit) {
			...
			const rows = executeSqliteQuerySync(db, (listOptions?.orderBy === "id" ? pageQuery.orderBy("event_id", "asc") : pageQuery.orderBy("received_at", "asc").orderBy("event_id", "asc")).limit(LIST_PENDING_BATCH_SIZE)).rows;
	...
	const listClaims = async () => {
		const { db } = openStateDatabase(options.stateDir);
		return executeSqliteQuerySync(db, getChannelIngressKysely(db).selectFrom("channel_ingress_events").selectAll().where("queue_name", "=", queueName).where("status", "=", "claimed").orderBy("claimed_at", "asc").orderBy("received_at", "asc").orderBy("event_id", "asc")).rows.map((row) => claimedRecord(row)).filter((rec) => rec !== null);
	};
	...
	const recoverStaleClaims = async (recoverOptions) => {
		...
		const claimedRows = executeSqliteQuerySync(database.db, getChannelIngressKysely(database.db).selectFrom("channel_ingress_events").selectAll().where("queue_name", "=", queueName).where("status", "=", "claimed").where("claimed_at", "<=", cutoff)).rows;
		let recovered = 0;
		for (const row of claimedRows) {
			...
			if (claimRec === null) {
				...
				if (runOpenClawStateWriteTransaction((tx) => tombstoneCorruptPayloadRow({
					db: tx.db,
					row,
...
//#endregion
export { createChannelIngressQueue as n, countFailedChannelIngressQueueEntries as t };
