...
import { n as getPath, r as setPathCreateStrict } from "./path-utils-BQoJTFAB.js";
import { n as t } from "./i18n-CX_FBkXY.js";
//#region src/wizard/setup.plugin-config.ts
const loadPluginMetadataSnapshotModule = createLazyRuntimeModule(() => import("./plugin-metadata-snapshot-CVRn3wG7.js"));
...
function parseJsonNumberInput(value) {
	try {
...
async function promptPluginFields(params) {
	...
	for (const [key, hint] of Object.entries(plugin.uiHints)) {
		...
		if (trimmed !== currentStr) if (schemaProp?.type === "number" || schemaProp?.type === "integer") if (trimmed === "") {
			...
			if (parsed !== void 0 && (schemaProp.type === "number" || Number.isInteger(parsed))) {
