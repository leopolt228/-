...
import { n as createHttp1EnvHttpProxyAgent, r as createHttp1ProxyAgent, t as createHttp1Agent } from "./undici-runtime-CvoyIVwn.js";
import { _ as resolvePinnedHostnameWithPolicy, a as createPinnedDispatcher, i as closeDispatcher, n as assertHostnameAllowedWithPolicy, p as matchesHostnameAllowlist, t as SsrFBlockedError, v as resolveSsrFPolicyForUrl } from "./ssrf-eKWXIRoD.js";
import { t as retainSafeHeadersForCrossOriginRedirect$1 } from "./redirect-headers-CivNCvUj.js";
import { i as isMockedFetch, n as fetchWithRuntimeDispatcher } from "./runtime-fetch-DldMe-lf.js";
import { o as globalUndiciStreamTimeoutMs } from "./undici-global-dispatcher-CyLdv3rm.js";
...
async function runAbortablePreflight(run, signal) {
	...
	if (signal.aborted) throw signal.reason ?? createAbortError("Guarded fetch aborted during network preflight");
	return await new Promise((resolve, reject) => {
		...
		const onAbort = () => settle(() => reject(toErrorObject(signal.reason ?? createAbortError("Guarded fetch aborted during network preflight"), "Guarded fetch aborted during network preflight")));
		signal.addEventListener("abort", onAbort, { once: true });
		if (signal.aborted) {
			onAbort();
			return;
		}
		run().then((value) => settle(() => resolve(value)), (error) => settle(() => reject(toErrorObject(error, "Network preflight failed"))));
	});
}
...
async function captureGuardedFetchExchange(params) {
	...
	const { captureHttpExchange, isDebugProxyGlobalFetchPatchInstalled } = await import("./runtime-B6f6awlL.js");
