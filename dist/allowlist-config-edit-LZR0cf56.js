...
function applyAccountScopedAllowlistConfigEdit(params) {
	...
	if (!hasStoredList) for (const entry of params.resolveEffectiveEntries?.() ?? []) {
		const value = String(entry).trim();
		if (!value || existing.includes(value)) continue;
		existing.push(value);
	}
...
function buildAccountScopedAllowlistConfigEditor(params) {
	return ({ cfg, parsedConfig, accountId, scope, action, entry }) => {
		...
		return applyAccountScopedAllowlistConfigEdit({
			...
			resolveEffectiveEntries: () => params.resolveEffectiveEntries?.({
				cfg,
				accountId,
				scope
			}),
...
function buildAccountAllowlistAdapter(params) {
	return {
		...
		applyConfigEdit: buildAccountScopedAllowlistConfigEditor({
			...
			resolveEffectiveEntries: ({ cfg, accountId, scope }) => params.resolveEntries(params.resolveAccount({
				cfg,
				accountId
			}), scope, {
				cfg,
...
function buildDmGroupAccountAllowlistAdapter(params) {
	return buildAccountAllowlistAdapter({
		...
		resolveEntries: (account, scope, context) => scope === "dm" ? params.resolveDmAllowFrom(account, context) : params.resolveGroupAllowFrom(account),
