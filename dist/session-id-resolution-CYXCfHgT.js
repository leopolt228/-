...
import { _ as toAgentRequestSessionKey } from "./session-key-Drrs61Fd.js";
...
function collapseAliasMatches(matches) {
	...
	return Array.from(grouped.values(), (group) => {
		if (group.length === 1) return expectDefined(group[0], "normalized session id match");
		return expectDefined([...group].toSorted((a, b) => {
...
function resolveSessionIdMatchSelection(matches, sessionId) {
	...
	if (canonicalMatches.length === 1) return {
		kind: "selected",
		sessionKey: expectDefined(canonicalMatches[0], "canonical matches capture group 0").sessionKey
	};
