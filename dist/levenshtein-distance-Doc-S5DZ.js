...
function levenshteinDistance(left, right) {
	...
	for (let leftIndex = 0; leftIndex < left.length; leftIndex += 1) {
		...
		for (let rightIndex = 0; rightIndex < right.length; rightIndex += 1) {
			...
			current[rightIndex + 1] = Math.min(expectDefined(current[rightIndex], "current entry at right index") + 1, expectDefined(previous[rightIndex + 1], "previous entry at right index + 1") + 1, expectDefined(previous[rightIndex], "previous entry at right index") + cost);
		}
