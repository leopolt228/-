...
import { l as listOfficialExternalPluginCatalogEntries, p as resolveOfficialExternalPluginId } from "./official-external-plugin-catalog-D3_jWsTb.js";
...
function collectPluginRegistryState(cfg, env) {
	...
	const officialIds = new Set(listOfficialExternalPluginCatalogEntries().map((entry) => normalizePluginId(resolveOfficialExternalPluginId(entry) ?? "")).filter(Boolean));
