...
import { r as resolveBundledPluginGeneratedPath } from "./bundled-plugin-metadata-CrWS3bPy.js";
import { c as isJavaScriptModulePath, n as getCachedPluginModuleLoader } from "./plugin-module-loader-cache-BmNGbwiD.js";
...
function resolveBundledChannelBoundaryRoot(params) {
	...
	const resolveMatchingRoot = (root) => {
	...
	if (overrideBoundaryRoot) boundaryRoot = overrideBoundaryRoot;
	else {
		const distBoundaryRoot = resolveMatchingRoot(path.resolve(params.packageRoot, "dist", "extensions", params.metadata.dirName));
		if (distBoundaryRoot) boundaryRoot = distBoundaryRoot;
		else boundaryRoot = resolveMatchingRoot(path.resolve(params.packageRoot, "dist-runtime", "extensions", params.metadata.dirName)) ?? resolveCanonicalPathOrAbsolute(path.resolve(params.packageRoot, "extensions", params.metadata.dirName));
	}
...
function resolveGeneratedBundledChannelModulePath(params) {
	...
	const generatedPath = resolveBundledChannelGeneratedPath(params.rootScope.packageRoot, params.entry, params.metadata.dirName, resolveBundledChannelScanDir(params.rootScope));
