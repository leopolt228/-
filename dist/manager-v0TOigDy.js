...
import { C as MEMORY_INDEX_PATHS_FTS_TABLE, D as dropMemoryPathFtsTriggers, E as MEMORY_INDEX_VECTOR_TABLE, O as ensureMemoryIndexSchema, h as ensureOpenClawAgentDatabaseSchema, k as ensureMemoryPathFtsTriggers, x as MEMORY_INDEX_FTS_TABLE, y as MEMORY_EMBEDDING_CACHE_TABLE } from "./openclaw-agent-db-BZ3-lIlN.js";
import { S as parseUsageCountedSessionIdFromFileName, _ as isSessionArchiveArtifactName, b as isUsageCountedSessionTranscriptFileName, c as resolveSessionTranscriptsDirForAgent } from "./paths-BpMRJ7TJ.js";
...
import { t as extractKeywords } from "./query-expansion-DzoKGtnD.js";
import { r as listRegisteredMemoryEmbeddingProviderAdapters, t as getMemoryEmbeddingProvider } from "./memory-embedding-provider-runtime-C8U8ZwXj.js";
import { a as classifyMemoryMultimodalPath, t as resolveMemorySearchConfig } from "./memory-search-Do8IpoGY.js";
...
import "./embedding-providers-5hwSgDcz.js";
import "./sqlite-runtime-CCJF5_XA.js";
import "./memory-core-host-embedding-registry-KdLBY3BW.js";
import { A as isEmbeddingBatchUnavailableError, u as enforceEmbeddingMaxInputTokens } from "./memory-core-host-engine-embeddings-BgaDotZ3.js";
import { a as ensureMemoryHostDir, c as normalizeExtraMemoryPaths, d as runMemoryHostTasksWithConcurrency, f as hasNonTextEmbeddingParts, i as cosineSimilarity, l as parseEmbedding, n as buildMultimodalChunkForIndexing, r as chunkMarkdown, s as listMemoryFiles, t as buildFileEntry, u as remapChunkLines } from "./internal-B2SueKGX.js";
...
import { d as resolveSessionFileForSyncTarget, g as listSessionTranscriptCorpusEntriesForAgent, h as statSessionEntrySync, l as buildSessionEntry, m as sessionPathForSessionIdentity, p as sessionPathForFile, u as parseCanonicalSessionSyncTargetFromPath } from "./engine-qmd-M1vAwevo.js";
import "./memory-core-host-engine-qmd-B1m1U0B5.js";
import { n as readMemoryFile } from "./read-file-BRuwlKvD.js";
import { i as loadSqliteVecExtension, n as configureMemorySqliteWalMaintenance, r as requireMemoryHostNodeSqlite, t as closeMemorySqliteWalMaintenance } from "./engine-storage-BXrWdYvs.js";
import "./memory-core-host-engine-storage-BItALzrQ.js";
import "./dreaming-shared-COCFY4u9.js";
import { r as tokenize, t as jaccardSimilarity } from "./tokenize-CHt3MKP-.js";
...
const LOCAL_EMBEDDING_RUNTIME_FACTS$1 = Symbol.for("openclaw.localEmbeddingRuntimeFacts");
...
function adaptGenericEmbeddingProvider(provider) {
	...
	const getRuntimeFacts = Reflect.get(provider, LOCAL_EMBEDDING_RUNTIME_FACTS$1);
	if (typeof getRuntimeFacts === "function") Object.defineProperty(adapted, LOCAL_EMBEDDING_RUNTIME_FACTS$1, {
		enumerable: false,
...
async function mergeHybridResults(params) {
	...
	const rankable = (await applyTemporalDecayToHybridResults({
	...
	const nonExact = rankable.filter((entry) => entry.exactPathSpecificity === 0).toSorted((a, b) => b.score - a.score);
	const mmrConfig = {
		...DEFAULT_MMR_CONFIG,
		...params.mmr
	};
	const rerankExactGroup = (entries) => {
		...
		return applyMMRToHybridResults(entries.map((entry) => Object.assign(entry, { score: entry.exactPathTieScore })), mmrConfig).map((entry) => Object.assign(entry, { score: 1 }));
	};
...
const VECTOR_TABLE$3 = MEMORY_INDEX_VECTOR_TABLE;
...
var MemoryManagerSyncBase = class {
	...
	async ensureVectorReady(dimensions) {
		...
		try {
			...
			this.vectorReady = null;
			log$7.warn(`sqlite-vec unavailable: ${message}`);
			return false;
		}
	...
	ensureVectorTable(dimensions) {
		if (this.vector.dims === dimensions && memoryTableExists(this.db, VECTOR_TABLE$3)) return;
		if (!this.dropVectorTable()) throw new Error(`Failed to reset ${VECTOR_TABLE$3} before rebuilding vector dimensions`);
		this.db.exec(`CREATE VIRTUAL TABLE IF NOT EXISTS ${VECTOR_TABLE$3} USING vec0(\n  id TEXT PRIMARY KEY,\n  embedding FLOAT[${dimensions}]\n)`);
	...
	dropLegacyVectorTable() {
		...
		try {
			...
			log$7.debug(`Failed to drop ${LEGACY_VECTOR_TABLE}: ${formatErrorMessage(err)}`);
			return false;
		}
	}
	dropVectorTable() {
		try {
			this.db.exec(`DROP TABLE IF EXISTS ${VECTOR_TABLE$3}`);
			return true;
		} catch (err) {
			const message = formatErrorMessage(err);
			log$7.debug(`Failed to drop ${VECTOR_TABLE$3}: ${message}`);
			return false;
		}
	}
	...
	ensureSchema() {
		...
		if (result.ftsError) {
			...
			if (this.fts.enabled) log$7.warn(`fts unavailable: ${result.ftsError}`);
		}
	}
	readMeta() {
...
function runDetachedMemorySync(sync, reason) {
	sync().catch((err) => {
		log$6.warn(`memory sync failed (${reason}): ${String(err)}`);
	});
}
var MemoryManagerWatchOps = class extends MemoryManagerSyncBase {
	...
	ensureWatcher() {
		...
		const fileWatchPaths = /* @__PURE__ */ new Set([path.join(this.workspaceDir, "MEMORY.md")]);
		const dirWatchPaths = /* @__PURE__ */ new Set([path.join(this.workspaceDir, "memory")]);
		...
		if (fileWatchPaths.size > 0) {
			...
			if (existingWatcher) existingWatcher.add(Array.from(fileWatchPaths));
				...
				watcher.on("error", (err) => {
					const message = err instanceof Error ? err.message : String(err);
					log$6.warn(`memory watcher error: ${message}`);
				});
				watcher.once("ready", () => {
	...
	warnIfMemoryWatchPressure(count, unit) {
		warnIfMemoryWatchPressureHigh(this.memoryWatchPressureWarning, count, unit, "Large memory folders or extraPaths can make OpenClaw run out of file watchers or open files.", "Remove large extraPaths, or set memorySearch.sync.watch to false and refresh memory manually.", (message) => log$6.warn(message));
	}
	...
	attachNativeMemoryWatchForDir(dir, markDirty) {
		...
		try {
			...
			log$6.warn(`failed to start native recursive watcher on ${dir}: ${String(err)}; falling back to chokidar`);
			return false;
		}
		const pair = {
			dir,
		...
		mainWatcher.on("error", (err) => {
			const message = err instanceof Error ? err.message : String(err);
			log$6.warn(`memory native watcher error on ${dir}: ${message}`);
		...
		try {
			...
			parentWatcher.on("error", (err) => {
				const message = err instanceof Error ? err.message : String(err);
				log$6.warn(`memory native parent watcher error on ${path.dirname(dir)}: ${message}`);
				try {
					parentWatcher.close();
				} catch {}
			...
			log$6.warn(`memory native parent watcher could not start on ${path.dirname(dir)}: ${String(err)}`);
		}
		return true;
	}
	attachLinuxMemoryDirectoryTreeWatchForDir(dir, markDirty) {
		...
		const attachDirectory = (watchDir) => {
			...
			try {
				...
				if (watchDir === dir) log$6.warn(`failed to start Linux memory directory watcher on ${watchDir}: ${String(err)}; falling back to chokidar`);
				return null;
			}
		...
		try {
			...
			parentWatcher.on("error", (err) => {
				const message = err instanceof Error ? err.message : String(err);
				log$6.warn(`memory Linux parent watcher error on ${path.dirname(dir)}: ${message}`);
				try {
					parentWatcher.close();
				} catch {}
				this.removeNativeMemoryParentWatch(parentWatcher);
			...
			log$6.warn(`memory Linux parent watcher could not start on ${path.dirname(dir)}: ${String(err)}`);
		}
		return true;
	}
	...
	attachMemoryChokidarFallback(dir, markDirty) {
		...
		try {
			...
			log$6.warn(`failed to attach chokidar fallback for ${dir}: ${String(err)}`);
		}
	}
	ensureIntervalSync() {
...
var MemoryManagerSessionSyncOps = class extends MemoryManagerWatchOps {
	...
	ensureSessionListener() {
		...
		this.sessionUnsubscribe = this.subscribeSessionTranscriptUpdates((update) => {
			...
			if (sessionFile) this.scheduleCorpusSessionFileDirty(sessionFile).catch((err) => {
				log$5.warn(`memory session corpus update failed: ${String(err)}`);
			});
		});
	}
	...
	ensureSessionStartupCatchup() {
		...
		this.runSessionStartupCatchup().catch((err) => {
			log$5.warn("memory session startup catch-up failed: " + String(err));
		});
	}
	async markSessionStartupCatchupDirtyFiles() {
		...
		if (corpusEntries.length === 0 || this.closed) return [];
	...
	async runSessionStartupCatchup() {
		...
		this.sync({ reason: "session-startup-catchup" }).catch((err) => {
			log$5.warn("memory sync failed (session-startup-catchup): " + String(err));
		});
		return dirtyFiles;
	}
	scheduleSessionDirty(target) {
		...
		this.sessionWatchTimer = setTimeout(() => {
			...
			this.processSessionDeltaBatch().catch((err) => {
				log$5.warn(`memory session delta failed: ${String(err)}`);
			});
	...
	async processSessionDeltaBatch() {
		...
		if (shouldSync) this.sync({ reason: "session-delta" }).catch((err) => {
			log$5.warn(`memory sync failed (session-delta): ${String(err)}`);
		});
	}
	...
	normalizeTargetArchiveFiles(archiveFiles, corpusEntries = []) {
		if (!archiveFiles || archiveFiles.length === 0) return null;
		const normalized = /* @__PURE__ */ new Set();
		const corpusMarkers = new Set(corpusEntries.filter((entry) => entry.transcriptSource === "sqlite").map((entry) => entry.sessionFile));
		const corpusPaths = new Set(corpusEntries.filter((entry) => entry.transcriptSource !== "sqlite").map((entry) => path.resolve(entry.sessionFile)));
		for (const sessionFile of archiveFiles) {
			...
			if (sqliteMarker && normalizeAgentId(sqliteMarker.agentId) === normalizeAgentId(this.agentId)) {
				normalized.add(trimmed);
				continue;
			}
	...
	async resolveArchiveFilesForSyncTargets(sessions, knownCorpusEntries) {
		...
		for (const session of targets) {
			...
			for (const entry of corpusEntries) {
				...
				files.add(entry.transcriptSource === "sqlite" ? entry.sessionFile : path.resolve(entry.sessionFile));
	...
	async combineTargetArchiveFiles(params) {
		const files = /* @__PURE__ */ new Set();
		const corpusEntries = await listSessionTranscriptCorpusEntriesForAgent(this.agentId);
		for (const file of this.normalizeTargetArchiveFiles(params.archiveFiles, corpusEntries) ?? []) files.add(file);
		for (const file of await this.resolveArchiveFilesForSyncTargets(this.normalizeTargetSessions(params.sessions)?.values(), corpusEntries)) files.add(file);
...
var MemoryManagerSourceSyncOps = class extends MemoryManagerSessionSyncOps {
	async syncMemoryFiles(params) {
		...
		log$4.debug("memory sync: indexing memory files", {
	...
	async syncArchiveFiles(params) {
		...
		const deleteFtsRowsByPathAndSource = this.fts.enabled && this.fts.available ? this.db.prepare(`DELETE FROM ${FTS_TABLE$2} WHERE path = ? AND source = ?`) : null;
		const corpusEntries = await listSessionTranscriptCorpusEntriesForAgent(this.agentId);
		const targetArchiveFiles = params.needsFullReindex ? null : this.normalizeTargetArchiveFiles(params.targetArchiveFiles, corpusEntries);
		...
		const { activePaths, existingRows, existingHashes, indexAll } = resolveMemorySessionSyncPlan({
			...
			sessionPathForFile: (file) => {
				...
				if (corpusEntry) return this.sessionPathForCorpusEntry(corpusEntry);
				const sqliteMarker = parseSqliteSessionFileMarker(file);
				return sqliteMarker ? sessionPathForSessionIdentity(sqliteMarker.agentId, sqliteMarker.sessionId) : sessionPathForFile(file);
			}
		});
		log$4.debug("memory sync: indexing session files", {
			files: files.length,
			indexAll,
		...
		const deleteTargetArchiveStaleLiveRows = () => {
			...
			for (const file of targetArchiveFiles) {
				const corpusEntry = corpusEntryByPath.get(file);
				const sqliteMarker = parseSqliteSessionFileMarker(file);
				const sessionId = corpusEntry?.sessionId ?? sqliteMarker?.sessionId ?? parseUsageCountedSessionIdFromFileName(path.basename(file));
				if (!sessionId) continue;
				const staleAgentId = corpusEntry?.agentId ?? sqliteMarker?.agentId ?? this.agentId;
				const staleLivePaths = [sessionPathForSessionIdentity(staleAgentId, sessionId), this.legacyExtensionlessSessionPathForIdentity(staleAgentId, sessionId)];
			...
			for (let start = 0; start < files.length; start += SOURCE_WIDE_SESSION_INDEX_FLUSH_FILES) {
				const dirtyEntries = (await runMemoryHostTasksWithConcurrency(files.slice(start, start + SOURCE_WIDE_SESSION_INDEX_FLUSH_FILES).map((absPath) => async () => {
					try {
						...
						const entry = await buildSessionEntry(absPath, corpusEntry ? this.buildSessionEntryOptions(corpusEntry) : void 0);
						if (!entry) {
							if (params.progress) {
								params.progress.completed += 1;
								params.progress.report({
...
var MemoryManagerSyncOps = class extends MemoryManagerSourceSyncOps {
	...
	async runSync(params) {
		...
		const needsFullReindex = params?.force && !hasTargetArchiveFiles || needsInitialIndex || needsMissingIdentityReindex || needsExplicitIdentityReindex || this.memoryFullRetryDirty && canRunRetryFullReindex || this.sessionsFullRetryDirty && indexIdentity.status !== "valid" && canRunRetryFullReindex;
		...
		try {
			...
			if (this.shouldFallbackOnError(err) && await this.activateFallbackProvider(reason)) {
				if (needsFullReindex && !hasTargetArchiveFiles) await this.runInPlaceReindex({
		...
		try {
			return await runMemoryEmbeddingBatchRetryWithSplit({
				...
				run: async (batchTexts) => {
					...
					log$2.debug("memory embeddings: batch completed", {
						provider: provider.id,
						items: batchTexts.length
					});
					return result;
			...
			log$2.debug("memory embeddings: batch failed", {
				provider: provider.id,
	...
	*/
	...
	async indexFiles(items) {
		...
		if (!provider || !this.batch.enabled || !batchEmbed || this.providerRuntime?.sourceWideBatchEmbed !== true) {
			await runMemoryHostTasksWithConcurrency(items.map((item) => async () => await this.indexFile(item.entry, { source: item.source })), this.getIndexConcurrency());
			return;
		}
...
//#endregion
//#region extensions/memory-core/src/memory/manager-local-worker-errors.ts
...
//#endregion
export { createEmbeddingProvider as a, formatMemoryVectorDegradedWriteReason as i, closeAllMemoryIndexManagers as n, resolveMemoryCoreLocalServiceHostIdentity as o, closeMemoryIndexManagersForAgent as r, MemoryIndexManager as t };
