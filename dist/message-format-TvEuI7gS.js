...
import { n as getLoadedChannelPlugin, t as getChannelPlugin } from "./registry-DqyhCDsQ.js";
import "./plugins-CJcRWm9n.js";
import { t as formatTargetDisplay } from "./target-resolver-BOFYQsKD.js";
...
function renderPaginationHint(payload, muted) {
	if (!payload || typeof payload !== "object") return null;
	const obj = payload;
	if (obj.hasMore === true) return muted("More results available. Use --limit to fetch more, or --json for the raw cursor.");
	if (typeof obj.nextBatch === "string" && obj.nextBatch) return muted("More results available. Use --limit to fetch more, or --json for the raw cursor.");
	if (typeof obj["@odata.nextLink"] === "string" && obj["@odata.nextLink"]) return muted("More results available. Use --limit to fetch more, or --json for the raw cursor.");
	return null;
}
	...
	if (result.kind === "poll") {
		if (result.handledBy === "core" && result.pollResult) {
			...
			if (poll.via === "direct") {
				...
				const lines = [ok(formatOutboundDeliverySummary(poll.channel, directResult, { action: "Poll sent" }))];
	...
	if (result.action === "search") {
		...
		if (list) {
			...
			const hint = renderPaginationHint(payload, muted) ?? renderPaginationHint(results, muted) ?? (() => {
				if (!results || typeof results !== "object") return null;
				const r = results;
				if (typeof r.total_results === "number" && r.total_results > list.length) return muted("More results available. Use --limit to fetch more, or --json for the raw cursor.");
				return null;
			})();
			if (hint) lines.push(hint);
			return lines;
