...
import { r as stripAnsi, t as sanitizeForLog } from "./ansi-BEaQ2G9r.js";
...
import { h as shortenHomePath, t as CONFIG_DIR } from "./utils-K2PjeLaV.js";
import { _ as fetchClawHubSkillVerification, p as fetchClawHubSkillCard } from "./clawhub-B8a59qSy.js";
import { n as isImmutableGitCommitRef, r as parseGitPluginSpec } from "./git-install-Cf8XLLO2.js";
...
import { s as withTempDir } from "./install-source-utils-D71Lc61M.js";
...
import { n as resolveSkillStatusEntry } from "./status-BKR_g98B.js";
import { t as CLAWHUB_TRUST_ERROR_CODE } from "./clawhub-install-trust-DyTzSv3F.js";
import { a as readVerifiedClawHubSkillSourceUrl, d as updateSkillsFromClawHub, f as installExtractedSkillRoot, i as readTrackedClawHubSkillSlugs, l as searchSkillsFromClawHub, m as validateRequestedSkillSlug, s as resolveClawHubSkillVerificationTarget, t as installSkillFromClawHub, u as untrackClawHubSkill } from "./clawhub-DZSPq6t8.js";
import { a as quarantineSkillProposal, c as rejectSkillProposal, d as inspectSkillProposal, f as listSkillProposals, i as proposeUpdateSkill, l as reviseSkillProposal, o as readSkillProposalDraftDirectory, r as proposeCreateSkill, s as readSkillProposalDraftFile, t as applySkillProposal } from "./service-4WfHAV4N.js";
import { t as resolveOptionFromCommand } from "./cli-utils-B33Avitx.js";
import { n as decorativePrefix, t as decorativeEmoji } from "./decorative-emoji-Cn977Iie.js";
import { n as renderTable, t as getTerminalTableWidth } from "./table-1bwnCkN2.js";
import { r as parseStrictPositiveIntOption } from "./helpers-CXUPqDHh.js";
...
async function loadSkillsStatusReport(options) {
	...
	const { buildWorkspaceSkillStatus } = await import("./status-Ca5DmmuD.js");
...
function formatSkillCuratorStatus(status) {
	...
	for (const overlap of status.overlaps) lines.push(`Possible overlap: ${overlap.left} ~ ${overlap.right} — merge via /learn if desired`);
	return `${lines.join("\n")}\n`;
}
async function loadGatewaySkillCuratorStatus(config) {
	try {
		const { callGateway } = await import("./call-Au-Dq1sZ.js");
		return await callGateway({
			config,
		...
		if (config.gateway?.mode === "remote") throw err;
		return null;
	}
}
async function loadSkillCuratorStatus() {
	return await loadGatewaySkillCuratorStatus(getRuntimeConfig()) ?? getSkillCuratorStatus();
}
async function runSkillCuratorMutation(method, skill) {
	...
	if (method === "pin") return pinCuratedSkill(skill);
	if (method === "unpin") return unpinCuratedSkill(skill);
...
function registerSkillsCli(program) {
	...
	const curator = skills.command("curator").description("Inspect and manage skill lifecycle curation").option("--json", "Output as JSON", false);
	const showCuratorStatus = async () => {
		try {
			const status = await loadSkillCuratorStatus();
			if (curator.opts().json) {
	...
	curator.command("status").description("Show curator run and lifecycle status").action(showCuratorStatus);
	for (const action of [
		"pin",
		"unpin",
		"restore"
	]) curator.command(action).description(`${action} a curated skill`).argument("<skill>", "Skill name or key").action(async (skill) => {
		try {
			...
			defaultRuntime.writeStdout(`${action[0]?.toUpperCase()}${action.slice(1)} ${result.skillKey}\n`);
		} catch (err) {
			defaultRuntime.error(String(err));
			defaultRuntime.exit(1);
		}
