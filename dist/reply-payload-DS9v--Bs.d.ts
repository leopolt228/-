import { r as ReplyPayload$1 } from "./reply-payload-Cz6pe8eB.js";
...
declare function sendPayloadMediaSequence<TResult>(params: {
  ...
  send: (input: {
    ...
    index: number; /** Whether this is the first non-empty media entry sent. */
    isFirst: boolean;
  }) => Promise<TResult>; /** Called after each successful media send and before the next send starts. */
  onResult?: (result: TResult) => Promise<void> | void;
}): Promise<TResult | undefined>;
/** Sends text chunks sequentially and returns the last send result. */
declare function sendPayloadTextChunkSequence<TResult>(params: {
  /** Ordered text chunks to send. */chunks: readonly string[];
  send: (input: {
    text: string;
    index: number;
    isFirst: boolean;
  }) => Promise<TResult>; /** Called after each successful chunk send and before the next send starts. */
  onResult?: (result: TResult) => Promise<void> | void;
}): Promise<TResult | undefined>;
...
declare function sendPayloadMediaSequenceOrFallback<TResult>(params: {
  ...
  onResult?: (result: TResult) => Promise<void> | void; /** Result returned when no media item is sent. */
  fallbackResult: TResult; /** Optional callback used instead of `fallbackResult` when no media item is sent. */
...
declare function sendPayloadMediaSequenceAndFinalize<TMediaResult, TResult>(params: {
  ...
  onResult?: (result: TMediaResult) => Promise<void> | void; /** Final callback whose result is returned after optional media sends. */
...
//#endregion
export { sendPayloadMediaSequenceOrFallback as C, MediaPayload as D, sendTextMediaPayload as E, MediaPayloadInput as O, sendPayloadMediaSequenceAndFinalize as S, sendPayloadWithChunkedTextAndMedia as T, resolvePayloadMediaUrls as _, countOutboundMedia as a, sendMediaWithLeadingCaption as b, deliverTextOrMediaReply as c, hasOutboundReplyContent as d, hasOutboundText as f, resolveOutboundMediaUrls as g, normalizeOutboundReplyPayload as h, SendableOutboundReplyParts as i, buildMediaPayload as k, formatTextWithAttachmentLinks as l, isReasoningReplyPayload as m, ReasoningReplyPayload as n, createNormalizedOutboundDeliverer as o, isNumericTargetId as p, ReplyPayload as r, deliverFormattedTextWithAttachments as s, OutboundReplyPayload as t, hasOutboundMedia as u, resolveSendableOutboundReplyParts as v, sendPayloadTextChunkSequence as w, sendPayloadMediaSequence as x, resolveTextChunksWithFallback as y };