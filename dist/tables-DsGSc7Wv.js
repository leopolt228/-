...
import { a as HTML_TAG_RE, i as isWhiteSpace, n as isMdAsciiPunct, r as isPunctChar, t as MarkdownIt } from "./markdown-it-CuMGuate.js";
...
function chunkTextRanges(text, options) {
	if (!text) return [];
	if (options.limit <= 0 || text.length <= options.limit) return [{
		start: 0,
		end: text.length
	}];
	const ranges = [];
...
function renderTokens(tokens, state) {
	...
	for (const [tokenIndex, token] of tokens.entries()) switch (token.type) {
		...
		case "code_block":
		case "fence":
			renderCodeBlock(state, token.content ?? "", token.info, sourceBlockNewlineCount(state.preserveSourceBlockSpacing, nextMappedBlockStarts[tokenIndex], token.map?.[1]));
			break;
		case "html_block":
		case "html_inline":
			appendText(state, token.content ?? "");
...
function renderMarkdownWithMarkers(ir, options) {
	...
	if (options.buildLink) {
		const links = ir.links.flatMap((span) => subtractRanges(span, dominantAnnotationRanges).flatMap((piece) => splitAtBoundaries(piece, annotationBoundaries)).map((piece) => copyMarkdownLinkSpan(span, {
			start: piece.start,
			end: piece.end,
			href: piece.href
		})));
...
//#endregion
export { markdownToIRWithMeta as a, copyMarkdownLinkSpan as c, findAssistantTranscriptRoleHeaderSpans as d, tokenizeHtmlTags as f, markdownToIR as i, isAutoLinkedMarkdownLink as l, renderMarkdownWithMarkers as n, sliceMarkdownIR as o, chunkTextRanges as p, chunkMarkdownIR as r, annotateAssistantTranscriptRoleMessageBoundary as s, convertMarkdownTables as t, mergeAnnotationSpans as u };
