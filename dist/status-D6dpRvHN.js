...
import { g as classifySystemdUnavailableDetail } from "./systemd-iYtBw5_g.js";
import { p as resolveGatewaySystemdServiceName, u as resolveGatewayLaunchAgentLabel } from "./constants-obO8goqF.js";
import { t as formatGatewayHeapLimitReport } from "./gateway-heap-CaeyR8CS.js";
import { p as isWSLEnv } from "./undici-runtime-CvoyIVwn.js";
import { t as formatConfigIssueLine } from "./issue-format-BfBp97Wi.js";
import { a as resolveGatewayRestartLogPath, o as resolveGatewaySupervisorLogPaths } from "./restart-logs-BENYnzNc.js";
import { a as isSystemdUnavailableDetail, i as resolveDaemonContainerContext, o as renderSystemdUnavailableHints, r as formatRuntimeStatus } from "./runtime-hints-CjbqWfI_.js";
import { a as filterDaemonEnv, d as resolveRuntimeStatusColor, f as safeDaemonEnv, t as createCliStatusTextStyles, u as renderRuntimeHints } from "./shared-B2j_B0O6.js";
import { r as isSystemdStartLimitHit } from "./service-runtime-Cy_yLpbt.js";
import { n as resolveControlUiLinks } from "./control-ui-links-CzaYlpy_.js";
import { n as formatGatewayRestartHandoffDiagnostic } from "./restart-handoff-Dgbh21G4.js";
import { n as renderPortDiagnosticsForCli, r as resolvePortListeningAddresses, t as gatherDaemonStatus } from "./status.gather-CGS91LFy.js";
import { n as resolvePluginVersionDriftUpdateCommand } from "./plugin-version-drift-BUXHgZJ7.js";
import { i as renderGatewayServiceCleanupHints } from "./inspect-P2j2IdTG.js";
...
function printDaemonStatus(status, opts) {
	...
	if (rpc && !rpc.ok && service.loaded && service.runtime?.status === "running") if (status.health?.healthy === true && status.health.staleGatewayPids.length === 0) defaultRuntime.log(warnText("Gateway process is running and owns the gateway port, so this is not a warm-up delay. Check the probe credentials/config, or restart the gateway and inspect its logs if it stays unresponsive."));
	...
	if (service.runtime?.missingUnit) {
		...
		defaultRuntime.error(errorText(startLimitHit ? `systemd stopped restarting the gateway after repeated crashes; run ${formatCliCommand("openclaw gateway restart")} or inspect logs.` : "Service is loaded but not running (likely exited immediately)."));
		for (const hint of renderRuntimeHints(service.runtime, service.command?.environment ?? process.env, status.logFile)) defaultRuntime.error(errorText(hint));
		spacer();
	}
