...
import { l as generatePkceVerifierChallenge } from "./provider-auth-Bnib2g6h.js";
...
async function waitForOpenRouterOAuthCallback(params) {
	...
	return new Promise((resolve, reject) => {
		...
		const server = createServer((req, res) => {
			try {
				...
				if (error) {
					...
					finish(/* @__PURE__ */ new Error(`OpenRouter OAuth error: ${detail}`));
					return;
				}
		...
		const onAbort = () => finish(/* @__PURE__ */ new Error("OpenRouter OAuth cancelled"));
