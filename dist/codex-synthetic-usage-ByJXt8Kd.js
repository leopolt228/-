...
function shouldUseCodexSyntheticUsageForRuntime(params) {
	...
	return (harness === CODEX_SYNTHETIC_USAGE_HOOK_PROVIDER || sessionHarness === CODEX_SYNTHETIC_USAGE_HOOK_PROVIDER) && (provider === CODEX_SYNTHETIC_USAGE_PROVIDER || provider === "codex");
}
...
function byPrecedence(candidate, existing, rank) {
	const candidateRank = rank(candidate);
	const existingRank = rank(existing);
	return candidateRank >= existingRank && !(candidateRank === 0 && existingRank === 0) ? [candidate, existing] : [existing, candidate];
}
...
function mergeUsageSnapshots(precedence) {
...
function mergeUsageSummaries(base, extra) {
	...
	for (const provider of extra.providers) {
		...
		providersById.set(provider.provider, mergeUsageSnapshots(byPrecedence(provider, existing, usageSnapshotRank)));
	}
	return {
