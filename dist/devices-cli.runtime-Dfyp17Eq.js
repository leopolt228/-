...
import { c as callGateway, o as buildGatewayConnectionDetails, p as formatGatewayTransportErrorJson } from "./call-ChM1o8yU.js";
...
import { a as READ_SCOPE, r as PAIRING_SCOPE, t as ADMIN_SCOPE } from "./operator-scopes-BHrNTqoH.js";
import { t as quoteCliArg } from "./quote-cli-arg-BriMa9wW.js";
import { t as parseNodeList } from "./node-list-parse-BlhiSeiu.js";
...
import { i as formatDevicePairingForbiddenMessage, l as listDevicePairing, n as approveDevicePairing, v as summarizeDeviceTokens } from "./device-pairing-DUA4LHep.js";
...
function isPendingNodeApprovalState(state) {
...
function formatNodeConnectionFlagReminder(opts) {
...
async function tryReadPendingNodeApprovals(opts) {
	try {
		return parseNodeList(await callGatewayCli("node.list", opts, {}, { scopes: [READ_SCOPE, PAIRING_SCOPE] })).filter((node) => isPendingNodeApprovalState(node.approvalState) && Boolean(normalizeOptionalString(node.pendingRequestId)));
	} catch {
		return [];
	}
}
function pairedDeviceCanBeNode(device) {
	return [device.role, ...device.roles ?? []].some((role) => normalizeOptionalString(role) === "node");
}
...
function nodeMatchesPairedDevice(node, device) {
	return stringsMatch(node.nodeId, device.deviceId) || stringsMatch(node.remoteIp, device.remoteIp);
}
function nodeMatchesQuery(node, query) {
	return stringsMatch(node.nodeId, query) || stringsMatch(node.remoteIp, query) || stringsMatch(node.pendingRequestId, query);
}
function pairedDeviceMatchesQuery(device, query) {
	return stringsMatch(device.deviceId, query) || stringsMatch(device.remoteIp, query);
}
async function tryReadGatewayPairingList(opts) {
	try {
...
function buildPendingNodeApprovalNoticesForOpts(nodes, opts) {
...
function formatNodeApprovalNotice(notice) {
	const lines = [`Node ${notice.node.approvalState === "pending-reapproval" ? "reapproval" : "approval"} pending for ${sanitizeForLog(normalizeOptionalString(notice.node.displayName) ?? notice.node.nodeId)}. Run ${sanitizeForLog(notice.command)}`];
	if (notice.connectionReminder) lines.push(notice.connectionReminder);
	return lines.join("\n");
}
async function findPairedDevicePendingNodeApprovalNotices(opts, paired) {
	const nodeDevices = (paired ?? []).filter(pairedDeviceCanBeNode);
	if (nodeDevices.length === 0) return [];
	return buildPendingNodeApprovalNoticesForOpts((await tryReadPendingNodeApprovals(opts)).filter((node) => nodeDevices.some((device) => nodeMatchesPairedDevice(node, device))), opts);
}
async function findQueryPendingNodeApprovalNotices(opts, query) {
	const nodes = await tryReadPendingNodeApprovals(opts);
	const directMatches = nodes.filter((node) => nodeMatchesQuery(node, query));
	if (directMatches.length > 0) return buildPendingNodeApprovalNoticesForOpts(directMatches, opts);
	const pairedMatches = ((await tryReadGatewayPairingList(opts))?.paired ?? []).filter((device) => pairedDeviceMatchesQuery(device, query));
	return buildPendingNodeApprovalNoticesForOpts(nodes.filter((node) => pairedMatches.some((device) => nodeMatchesPairedDevice(node, device))), opts);
}
...
function isUnknownRequestIdError(error) {
	...
	return normalizeLowercaseStringOrEmpty(typeof maybeGatewayError?.message === "string" ? maybeGatewayError.message : normalizeErrorMessage(error)).includes("unknown requestid");
}
...
//#endregion
export { runDevicesApproveCommand, runDevicesClearCommand, runDevicesListCommand, runDevicesRejectCommand, runDevicesRemoveCommand, runDevicesRenameCommand, runDevicesRevokeCommand, runDevicesRotateCommand };
