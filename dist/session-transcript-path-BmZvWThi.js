...
import { l as asPositiveSafeInteger, s as asFiniteNumber } from "./number-coercion-Crk_c9KW.js";
...
import "./stream-message-shared-DKS8UMJ_.js";
import "./internal-runtime-context-BW7WOTKc.js";
import { f as stripInterSessionPromptPrefixForDisplay, t as INTER_SESSION_PROMPT_PREFIX_BASE, u as normalizeInputProvenance } from "./input-provenance-B6vSIOBi.js";
import { i as stripInlineDirectiveTagsForDisplay } from "./directive-tags-DnwgHzaK.js";
import { E as stripEnvelopeFromMessages } from "./session-transcript-readers-DSb8L-vG.js";
...
import { n as isSuppressedControlReplyText, r as stripSuppressedControlReplyToken } from "./control-reply-text-MP4oFRZ6.js";
...
function coerceCanvasPreview(record) {
	...
	const boardWidgetName = requestedBoardWidgetName && /^[a-z0-9][a-z0-9._-]{0,63}$/u.test(requestedBoardWidgetName) ? requestedBoardWidgetName : void 0;
...
function extractCanvasFromText(outputText, _toolName) {
	return coerceCanvasPreview(outputText ? asOptionalRecord(safeParseJson(outputText)) : void 0);
}
//#endregion
...
function projectToolResultDetails(details, maxChars) {
	...
	if (typeof record.diff === "string" && record.diff.trim()) projected.diff = truncateChatHistoryText(record.diff, maxChars).text;
...
function sanitizeChatHistoryMessage(message, maxChars = DEFAULT_CHAT_HISTORY_TEXT_MAX_CHARS) {
	...
	if ("details" in entry) {
		const projectedDetails = messageHasToolResultShape(entry) ? projectToolResultDetails(entry.details, maxChars) : void 0;
		if (projectedDetails) entry.details = projectedDetails;
		else delete entry.details;
		changed = true;
	}
	...
	if (typeof entry.content === "string") {
		const stripped = stripInlineDirectiveTagsForDisplay(entry.content);
		const controlStripped = stripAssistantControlTokens ? stripSuppressedControlReplyToken(stripped.text) : stripped.text;
...
function shouldPreserveAssistantControlReplyText(message) {
	...
	return texts.length > 0 && texts.every((text) => isSuppressedControlReplyText(stripInlineDirectiveTagsForDisplay(text).text)) && hasAssistantDisplayableNonTextContent(message);
}
...
function shouldDropAssistantHistoryMessage(message) {
	...
	const text = extractAssistantTextForSilentCheck(message);
	const displayText = text === void 0 ? void 0 : stripInlineDirectiveTagsForDisplay(text).text;
	if (displayText === void 0 || !isSuppressedControlReplyText(displayText)) return false;
...
function filterVisibleProjectedHistoryMessages(messages, turnBoundaryPending = false) {
	...
	for (let i = 0; i < messages.length; i++) {
		...
		if (isDuplicateAcpGatewayInjectedMessage(current, visible.at(-1)) || isDuplicateChannelFinalDeliveryMirror(current, messages[i - 1])) {
			changed = true;
			continue;
		}
...
function projectChatDisplayMessagesWithState(messages, options) {
	...
	return {
		messages: sanitizeChatHistoryMessages(mergeTtsSupplementMessages(filtered.messages), options?.maxChars ?? 8e3),
...
//#endregion
export { isHeartbeatHistoryTurnBoundaryMessage as a, projectChatDisplayMessagesWithState as c, sanitizeChatHistoryMessages as d, dropPreSessionStartAnnouncePairs as i, projectRecentChatDisplayMessages as l, DEFAULT_CHAT_HISTORY_TEXT_MAX_CHARS as n, projectChatDisplayMessage as o, augmentChatHistoryWithCanvasBlocks as r, projectChatDisplayMessages as s, resolveTranscriptPathForComparison as t, resolveEffectiveChatHistoryMaxChars as u };
