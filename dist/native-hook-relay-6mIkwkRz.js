...
import { i as listAgentToolResultMiddlewares } from "./registry-BSBtFA2q.js";
...
import { l as payloadTextResult } from "./common-C39GdgQ7.js";
import { m as normalizeToolName } from "./tool-policy-GYMCyycR.js";
import { d as requestDeferredPluginToolApproval, n as cancelDeferredPluginToolApproval, o as hasBeforeToolCallPolicy, p as runBeforeToolCallHook } from "./agent-tools.before-tool-call-CvBO0Qc6.js";
...
function readNativeHookRelayBridgeSnapshot(row) {
	if (!row || typeof row.relay_id !== "string" || row.relay_id.length === 0 || !Number.isSafeInteger(row.pid) || row.hostname !== "127.0.0.1" || !Number.isSafeInteger(row.port) || row.port <= 0 || row.port > 65535 || typeof row.token !== "string" || row.token.length === 0 || !Number.isSafeInteger(row.expires_at_ms) || !Number.isSafeInteger(row.updated_at_ms)) return;
	const { token } = row;
	return {
		record: {
...
function readNativeHookRelayBridgeRecord$1(params) {
...
function registerNativeHookRelay(params) {
	...
	unregisterNativeHookRelay(relayId, void 0, { deferBridgeRecordRemovalMs: NATIVE_HOOK_BRIDGE_REPLACEMENT_RECORD_GRACE_MS });
	const registration = {
		relayId,
	...
	const handle = {
		...
		renew: (ttlMs) => {
			...
			if (bridge && bridge.server.listening) {
				...
				if (!record) return;
				try {
					if (!renewOrRestoreNativeHookRelayBridgeRecord({
						record,
...
function registerNativeHookRelayBridge(registration, stateDbPath) {
	try {
		...
		for (const row of pruned) log.debug("pruned stale native hook relay bridge record", {
			relayId: row.relayId,
			stalePid: row.pid,
			currentPid: process.pid,
			reason: row.reason
...
function readNativeHookRelayBridgeRecord(relayId, stateDbPath) {
...
function readNativeHookRelayBridgeRecordIfExists(relayId, stateDbPath) {
	try {
		return readNativeHookRelayBridgeRecord$1({
			relayId,
			stateDbPath
		});
	} catch (error) {
		log.debug("failed to read native hook relay bridge record", {
			error,
			relayId
		});
	}
