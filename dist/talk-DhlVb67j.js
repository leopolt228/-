...
import { C as resolveExpiresAtMsFromDurationMs, S as resolveDateTimestampMs, g as parseFiniteNumber, m as isFutureDateTimestampMs, o as asDateTimestampMs } from "./number-coercion-Crk_c9KW.js";
...
import { d as resolveAgentIdFromSessionKey, i as buildAgentMainSessionKey } from "./session-key-Drrs61Fd.js";
...
import { i as resolveActiveTalkProviderConfig, r as normalizeTalkSection, t as buildTalkConfigResponse } from "./talk-CRJyHUve.js";
import "./config-BOMcY2yX.js";
import { a as READ_SCOPE, o as TALK_SECRETS_SCOPE, t as ADMIN_SCOPE } from "./operator-scopes-BHrNTqoH.js";
import { t as ErrorCodes } from "./gateway-error-details-CLDhuP4F.js";
import { d as getVoiceProviderConfig, f as providerMatchesId, m as resolveSupportedVoiceModelRefs } from "./registry-BSBtFA2q.js";
import { A as resolveOpenClientVoiceSessionId, C as assertClientVoiceSessionOpen, D as ensureClientVoiceAgentSessionEntry, E as createOrResumeClientVoiceSession, M as bindAuthorizedClientVoiceConfirmation, O as registerClientVoiceConsultRun, S as appendRelayVoiceTranscript, T as closeStaleClientVoiceSessions, j as authorizeClientVoiceConfirmation, k as resolveClientVoiceSessionOrigin, w as closeClientVoiceSession, x as appendClientVoiceTranscript } from "./agent-tools.before-tool-call-CvBO0Qc6.js";
import { a as missingScopeErrorShape, i as errorShape } from "./error-codes-DKVDGU7l.js";
import { $r as validateTalkConfigParams, Gr as validateTalkClientCloseParams, Kr as validateTalkClientCreateParams, Qr as validateTalkClientTranscriptParams, Wr as validateTalkCatalogParams, Xr as validateTalkClientToolCallParams, Yr as validateTalkClientSteerParams, ai as validateTalkSessionCancelTurnParams, ci as validateTalkSessionJoinParams, di as validateTalkSessionTurnParams, fi as validateTalkSpeakParams, ii as validateTalkSessionCancelOutputParams, li as validateTalkSessionSteerParams, ni as validateTalkSessionAcknowledgeMarkParams, oi as validateTalkSessionCloseParams, ri as validateTalkSessionAppendAudioParams, si as validateTalkSessionCreateParams, ti as validateTalkModeParams, ui as validateTalkSessionSubmitToolResultParams } from "./src-Cy32TawB.js";
import { t as formatValidationErrors } from "./validation-errors-B9K6VbD7.js";
import { _ as CODE_HEAVY_SPOKEN_FALLBACK, f as synthesizeSpeech, n as getResolvedSpeechProviderConfig, v as isCodeHeavySpeechText } from "./runtime-api-IUluPrEw.js";
import { S as withSpeakerSelectionFallbackCompat, g as resolveTtsConfig, x as withSpeakerSelectionCompat } from "./tts-settings-Cunm4eSy.js";
...
import { t as abortChatRunById } from "./chat-abort-BKKIixKZ.js";
import { t as formatForLog } from "./ws-log-Bj-6Do--.js";
import { n as getRealtimeTranscriptionProvider, r as listRealtimeTranscriptionProviders, t as canonicalizeRealtimeTranscriptionProviderId } from "./provider-registry-Cyk6R62G.js";
import { n as resolveRealtimeBootstrapContextInstructions } from "./realtime-bootstrap-context-myab7ma-.js";
import { A as readSpeakableRealtimeVoiceToolResult, E as createRealtimeVoiceForcedConsultCoordinator, N as recordTalkObservabilityEvent, S as parseRealtimeVoiceAgentConsultArgs, V as REALTIME_VOICE_AUDIO_FORMAT_PCM16_24KHZ, c as resolveConfiguredRealtimeVoiceProvider, d as listRealtimeVoiceProviders, g as buildRealtimeVoiceAgentConsultChatMessage, i as isLikelyRealtimeVoiceAssistantEchoTranscript, j as createTalkSessionController, l as canonicalizeRealtimeVoiceProviderId, m as REALTIME_VOICE_AGENT_CONSULT_TOOL_NAME, o as recordRealtimeVoiceTranscript, p as REALTIME_VOICE_AGENT_CONSULT_TOOL, s as createRealtimeVoiceBridgeSession, y as buildRealtimeVoiceAgentConsultWorkingResponse } from "./session-log-runtime-GBoG4Ecc.js";
import { a as buildRealtimeVoiceAgentCancelProviderResult, d as shouldAutoControlRealtimeVoiceAgentText, i as REALTIME_VOICE_AGENT_CONTROL_TOOL_NAME, o as buildRealtimeVoiceAgentControlSpeechMessage, r as REALTIME_VOICE_AGENT_CONTROL_TOOL, t as controlRealtimeVoiceAgentRun } from "./agent-run-control-DE7KbzhN.js";
import { n as resolveProviderRawConfig } from "./provider-selection-runtime-DGqZaqbE.js";
import { t as chatHandlers } from "./chat-DWq-Qjl0.js";
...
function enqueueRelayVoiceTranscript(session, role, text) {
	if (!session.sessionKey) {
		...
		if (session.pendingVoiceTranscripts.length > MAX_PENDING_VOICE_TRANSCRIPTS) session.pendingVoiceTranscripts.shift();
		return;
	}
	if (!ensureRelayVoiceSession(session)) return;
	session.voiceTranscriptSeq += 1;
	const entryId = String(session.voiceTranscriptSeq);
	const sessionKey = session.sessionKey;
	session.voiceTranscriptWrites = session.voiceTranscriptWrites.then(async () => {
		let lastError;
...
function closeRelayVoiceSession(session) {
	if (!session.sessionKey || !ensureRelayVoiceSession(session)) return;
	const sessionKey = session.sessionKey;
	session.voiceTranscriptWrites.then(async () => {
...
function ensureTalkRealtimeRelayVoiceSession(params) {
	...
	if (session.sessionKey && session.sessionKey !== params.sessionKey) throw new Error("Realtime relay session belongs to another agent session");
...
function submitFinalProviderToolResult(params) {
	...
	const submitAfterWorking = async () => {
		...
		if (params.session.toolResultEpoch !== epoch) {
			...
			await params.session.bridge.submitToolResult(params.callId, buildRealtimeVoiceAgentCancelProviderResult("OpenClaw cancelled this consult before completion. Do not restart it."), suppressedToolResultOptions(params.session));
			...
			if (relay && toolCall.name === "openclaw_agent_consult") {
				...
				if (forcedConsult.kind === "in_flight" || forcedConsult.kind === "already_delivered") {
					if (forcedConsult.kind === "already_delivered") {
						const result = relay.forcedConsults.isCancelled(forcedConsult.handle) ? buildRealtimeVoiceAgentCancelProviderResult("OpenClaw cancelled this consult before completion. Do not restart it.") : buildAlreadyDeliveredToolResult();
...
async function flushTalkRealtimeRelayVoiceWrites(params) {
	await getRelaySession(params.relaySessionId, params.connId).voiceTranscriptWrites;
}
...
async function resolveTalkRealtimeProviderInstructions(params) {
	...
	const agentId = requestedSessionKey ? resolveAgentIdFromSessionKey(requestedSessionKey) : resolveDefaultAgentId(params.config);
...
const talkClientHandlers = {
	"talk.client.create": async ({ params, respond, context, client }) => {
		...
		try {
			...
			if (resolution.provider.createBrowserSession && transport !== "gateway-relay") {
				...
				if (wantsCameraFrames) tools.push(REALTIME_VOICE_DESCRIBE_VIEW_TOOL);
	...
	"talk.client.transcript": async ({ params, respond, context }) => {
		if (!validateTalkClientTranscriptParams(params)) {
			respond(false, void 0, errorShape(ErrorCodes.INVALID_REQUEST, `invalid talk.client.transcript params: ${formatValidationErrors(validateTalkClientTranscriptParams.errors)}`));
			return;
		}
		try {
	...
	"talk.client.close": async ({ params, respond, context, client }) => {
		if (!validateTalkClientCloseParams(params)) {
			respond(false, void 0, errorShape(ErrorCodes.INVALID_REQUEST, `invalid talk.client.close params: ${formatValidationErrors(validateTalkClientCloseParams.errors)}`));
			return;
		}
		try {
			const agentId = resolveAgentIdFromSessionKey(params.sessionKey);
			if (resolveClientVoiceSessionOrigin({
				...
				sessionKey: params.sessionKey,
				voiceSessionId: params.voiceSessionId
			}) === "relay") throw new Error("relay-owned voice sessions close through talk.session.stop");
			await closeClientVoiceSession({
				agentId,
				sessionKey: params.sessionKey,
				voiceSessionId: params.voiceSessionId,
	...
	"talk.session.create": async ({ params, respond, context, client }) => {
		...
		try {
			...
			if (mode === "realtime") {
				...
				if (realtimeContext.requestedSessionKey) await ensureClientVoiceAgentSessionEntry({
