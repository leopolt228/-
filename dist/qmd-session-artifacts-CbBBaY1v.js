import { a as migrateSqliteSchemaToStrict } from "./sqlite-wal-jkTlXxi6.js";
...
function ensureQmdSessionArtifactSchema(db) {
	...
	const table = db.prepare("SELECT strict FROM pragma_table_list WHERE schema = 'main' AND name = ?").get(QMD_SESSION_ARTIFACT_TABLE);
	if (Number(table?.strict ?? 0) !== 1) migrateSqliteSchemaToStrict(db, QMD_SESSION_ARTIFACT_SCHEMA, { databaseLabel: "QMD session artifact mappings" });
...
function openQmdSessionArtifactDb(indexPath, readOnly = false) {
	const { DatabaseSync: SqliteDatabase } = requireMemoryHostNodeSqlite();
