...
import { n as resolveEffectiveTtsConfig } from "./tts-config-Cl3uWggE.js";
...
function collectConfiguredSpeechProviderIds(config) {
	...
	if (isRecord(agents) && Array.isArray(agents.list)) {
		for (const agent of agents.list) if (isRecord(agent)) if (typeof agent.id === "string") addConfiguredTtsProviderIds(configured, resolveEffectiveTtsConfig(config, { agentId: agent.id }));
...
//#endregion
export { normalizeConfiguredSpeechProviderIdForStartup as n, collectConfiguredSpeechProviderIds as t };
