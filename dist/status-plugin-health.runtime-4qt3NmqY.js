...
import { r as listLoadedRuntimePluginIdsAcrossSurfaces, t as getActiveRuntimePluginRegistry } from "./active-runtime-registry-zZ4JR6Tx.js";
import { r as resolveReadOnlyChannelPluginsForConfig } from "./read-only-CTGMGkDz.js";
import { n as listPersistedRuntimeToolSchemaQuarantines } from "./tool-schema-quarantine-health-CLLzAl9P.js";
...
async function collectInstalledPluginHealthSnapshot(params) {
	const { buildPluginCompatibilityNotices, buildPluginSnapshotReport } = await import("./status-B-9UuJnN.js");
...
async function resolveUnregisteredMemoryEmbeddingProviders(params) {
	...
	try {
		const { collectRegisteredEmbeddingProviderIds, collectUnregisteredConfiguredMemoryEmbeddingProviders } = await import("./gateway-startup-plugin-ids-CRZHOxhx.js");
...
async function resolveEagerShouldRunPluginIds(params) {
	...
	try {
		const { loadGatewayStartupPluginPlan } = await import("./gateway-startup-plugin-ids-CRZHOxhx.js");
		const { resolvePluginActivationSourceConfig } = await import("./activation-source-config-BHsKTqoW.js");
		const { resolveGatewayStartupPluginActivationConfig } = await import("./plugin-activation-runtime-config-BE_1Jxty.js");
