...
import { _ as findStructuredAuthParamRanges, d as HTTP_AUTH_LEGACY_VALUE_WHITESPACE_PATTERN, f as HTTP_AUTH_OPAQUE_CREDENTIAL_PATTERN, g as HTTP_AUTH_SERIALIZED_QUOTE_PATTERN, h as HTTP_AUTH_SCHEME_PATTERN, l as CREDENTIAL_STYLE_HEADER_REDACT_PATTERN, m as HTTP_AUTH_REQUIRED_VALUE_WHITESPACE_PATTERN, p as HTTP_AUTH_OPTIONAL_VALUE_WHITESPACE_PATTERN, u as HTTP_AUTH_HEADER_BOUNDARY_PATTERN, v as redactStructuredAuthHeaders } from "./errors-C7_LR8fF.js";
import { o as isRecord } from "./record-coerce-DHZ4bFlT.js";
import "./src-Bg3_o3L-.js";
import { t as expectDefined } from "./expect-CyE8FADM.js";
import "./src-COWbwBfI.js";
import { n as getCommandPathWithRootOptions } from "./argv-D4LdWdQQ.js";
...
function registerOneSecretValue(value) {
	...
	if (registeredValues.size > MAX_SECRET_VALUES) {
		...
		if (oldest !== void 0) registeredValues.delete(oldest);
	}
	rebuildProbe();
}
...
function redactSensitiveText(text, options) {
	...
	if (!resolvedOptions.patterns?.length && !couldMatchDefaultRedactPatterns(exactRedacted)) return exactRedacted;
...
function redactStructuredSecretValue(key, value, seen, options, path = key ? [key] : []) {
	...
	if (typeof value === "object") {
		...
		seen.add(value);
		const out = {};
		for (const [nestedKey, nestedValue] of Object.entries(value)) out[nestedKey] = redactStructuredSecretValue(nestedKey, nestedValue, seen, options, [...path, nestedKey]);
		seen.delete(value);
		return out;
	}
	return value;
...
function redactSensitiveLines(lines, resolved) {
	...
	let redacted = (resolved.redactFormBodies ? lines.map((line) => redactFormBody(redactUrlQueryPairs(line))) : lines).join("\n");
...
//#endregion
export { testRegexWithBoundedInput as S, readLoggingConfig as _, redactSensitiveFieldValue as a, compileSafeRegex as b, redactSensitiveText as c, redactToolPayloadTextWithConfig as d, resolveRedactOptions as f, registerSecretValueForRedaction as g, redactRegisteredSecretValues as h, redactSecrets as i, redactToolDetail as l, isSecretValueRegisteredForRedaction as m, getDefaultRedactPatterns as n, redactSensitiveFieldValueWithConfig as o, hasRegisteredSecretValuesForRedaction as p, isSensitiveFieldKey as r, redactSensitiveLines as s, computeSensitiveRedactionBitmap as t, redactToolPayloadText as u, shouldSkipMutatingLoggingConfigRead as v, compileSafeRegexDetailed as x, compileConfigRegexes as y };
