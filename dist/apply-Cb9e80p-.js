...
import { t as resolveConcurrency } from "./resolve-CMSY74Kr.js";
import { T as resolveAttachmentKind, f as extractFileContentFromSource, g as resolveInputFileLimits, h as normalizeMimeType } from "./runner.entries-B5B9dOb9.js";
import { i as wrapExternalContent } from "./external-content-DkHx38wP.js";
import { a as createMediaAttachmentCache, i as runCapability, o as normalizeMediaAttachments, r as resolveMediaAttachmentLocalRoots, t as buildProviderRegistry } from "./runner-CAPzTqmg.js";
import { n as sendTranscriptEcho } from "./echo-transcript-Dvotos5w.js";
import { t as finalizeInboundContext } from "./inbound-context-DpKaYErg.js";
import { t as renderFileContextBlock } from "./file-context-V_MaV_rQ.js";
...
function positiveExtractionLimit(value) {
	return typeof value === "number" && Number.isFinite(value) && value > 0 ? value : void 0;
}
function resolveInboundFileExtractionMaxBytes(defaults) {
	const maxMb = positiveExtractionLimit(defaults?.mediaMaxMb) ?? INBOUND_FILE_EXTRACTION_DEFAULT_MAX_MB;
...
function resolveInboundFileExtractionMaxPages(defaults) {
	const pages = positiveExtractionLimit(defaults?.pdfMaxPages) ?? INBOUND_FILE_EXTRACTION_DEFAULT_MAX_PAGES;
...
const TEXT_EXT_MIME = /* @__PURE__ */ new Map([
	[".csv", "text/csv"],
...
function hasSuspiciousBinarySignal(buffer) {
	...
	const signature = expectDefined(sample[2], "sample entry at 2") << 8 | expectDefined(sample[3], "sample entry at 3");
...
async function extractFileContext(params) {
	...
	for (const attachment of attachments) {
		...
		if (extracted?.images && extracted.images.length > 0) images.push(...extracted.images.map((image) => ({
			...image,
			attachmentIndex: attachment.index
		})));
		if (!blockText) if (extracted?.images && extracted.images.length > 0) blockText = "[PDF content rendered to images]";
...
async function applyMediaUnderstanding(params) {
	...
	try {
		...
		if (fileContext.blocks.length > 0) ctx.Body = appendFileBlocks(ctx.Body, fileContext.blocks);
		if (outputs.length > 0 || fileContext.blocks.length > 0) finalizeInboundContext(ctx, {
