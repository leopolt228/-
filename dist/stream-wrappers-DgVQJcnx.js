...
import { F as resolveAnthropicPayloadPolicy, N as applyAnthropicPayloadPolicyToParams, k as streamWithPayloadPatch, n as composeProviderStreamWrappers, r as createAnthropicThinkingPrefillPayloadWrapper } from "./provider-stream-shared-BiURRLUJ.js";
...
function isAnthropic1MModel(modelId) {
	if (resolveClaudeFable5ModelIdentity({ id: modelId }) !== void 0 || resolveClaudeSonnet5ModelIdentity({ id: modelId }) !== void 0) return true;
...
function createAnthropicServiceTierWrapper(baseStreamFn, serviceTier) {
	const underlying = baseStreamFn ?? streamSimple;
	return (model, context, options) => {
		if (isAnthropicOAuthApiKey(options?.apiKey) || resolveClaudeSonnet5ModelIdentity(model) !== void 0) return underlying(model, context, options);
		...
		return streamWithPayloadPatch(underlying, model, context, options, (payloadObj) => applyAnthropicPayloadPolicyToParams(payloadObj, payloadPolicy, /* @__PURE__ */ new Set()));
	};
}
