...
import { n as hasAuthProfileStoreSourceForProvider, t as hasAnyAuthProfileStoreSource } from "./source-check-D7qXDGS3.js";
import { t as isConfiguredAwsSdkAuthProfileForProvider } from "./order-FUfwr_5s.js";
import { t as resolveEnvApiKey } from "./model-auth-env-COYR71VZ.js";
import "./auth-profiles-D9OcwMed.js";
import { f as resolveApiKeyForProvider, g as resolveUsableCustomProviderApiKey } from "./model-auth-919iJVmy.js";
import { i as resolveActiveMemoryBackendConfig, r as getActiveMemorySearchManager } from "./memory-runtime-Cx64GvXS.js";
...
import { r as resolveQmdBinaryUnavailableReason, t as checkQmdBinaryAvailability } from "./engine-qmd-M1vAwevo.js";
import { d as repairShortTermPromotionArtifacts, n as auditShortTermPromotionArtifacts, t as auditDreamingArtifacts, u as repairDreamingArtifacts } from "./memory-core-bundled-runtime-BXqYfd4f.js";
import "./legacy-config-record-shared-B7RruIeK.js";
import { i as maybeRepairWorkspaceMemoryHealth, o as noteWorkspaceMemoryHealth } from "./doctor-workspace-BznxWOMq.js";
		...
		try {
			...
			if ((await auditShortTermPromotionArtifacts({
				...
				if (await params.prompter.confirmRuntimeRepair({
					message: formatAgentMessage(scope.agentId, labelAgents, "Normalize memory recall artifacts and remove stale promotion locks?"),
					initialValue: true
				})) {
...
function noteRememberAcrossConversationsHealth(params) {
	...
	if (!activeMemoryAvailable) params.noteFn(`Remember across conversations is effectively enabled for agent "${params.agentId}", but the Active Memory plugin is disabled. Enable the plugin or set memorySearch.rememberAcrossConversations to false.`, "Memory search");
	...
	else if (activeMemoryAvailable && !conversationRecallSupport.memorySearchAllowed) params.noteFn(`Remember across conversations is effectively enabled for agent "${params.agentId}", but Active Memory does not allow memory_search. Add memory_search to the plugin toolsAllow list or set memorySearch.rememberAcrossConversations to false.`, "Memory search");
	return { enabled: true };
}
...
async function noteMemorySearchHealthForAgent(cfg, scope, opts) {
	...
	if (!resolved) {
		noteFn(recallHealth.enabled ? `Remember across conversations is effectively enabled for agent "${agentId}", but memory search is disabled. Enable memory search or set memorySearch.rememberAcrossConversations to false.` : "Memory search is explicitly disabled (enabled: false).", "Memory search");
		return;
	}
	...
	if (backendConfig.backend === "qmd") {
		...
		if (resolved.sources?.includes("sessions") && !resolved.rememberAcrossConversations && cfg.memory?.qmd?.sessions?.enabled !== true) noteFn([
			...
			"",
			"Fix (pick one):",
			`- Enable QMD session export: ${formatCliCommand("openclaw config set memory.qmd.sessions.enabled true")}`,
