...
import { r as discoverConfigSecretTargetsByIds, s as listSecretTargetRegistryEntries } from "./target-registry-query-DKaR_5Cb.js";
import "./target-registry-B8VdrXt8.js";
import { i as resolveChannelDefaultAccountId } from "./helpers-BzNF0htn.js";
import { d as sortWebFetchProvidersForAutoDetect, r as sortWebSearchProvidersForAutoDetect } from "./web-search-providers.shared-C3OtHWMV.js";
...
function getCapabilityWebSearchTargetIds() {
	cachedCapabilityWebSearchTargetIds ??= sortUniqueStrings(listSecretTargetRegistryEntries().map((entry) => entry.id).filter(isPluginWebSearchCredentialTargetId));
	return cachedCapabilityWebSearchTargetIds;
}
function getCapabilityWebFetchTargetIds() {
	cachedCapabilityWebFetchTargetIds ??= sortUniqueStrings(listSecretTargetRegistryEntries().map((entry) => entry.id).filter(isPluginWebFetchCredentialTargetId));
