...
import { x as isTimeoutErrorMessage } from "./sanitize-user-facing-text-sWgeyF-a.js";
import { a as classifyFailoverSignal, d as inferSignalStatus, s as extractFailoverSignalDetails, w as isUnclassifiedNoBodyHttpSignal } from "./errors-DMOgb-Rt.js";
...
function findCliMaxTurnsError(err, seen = /* @__PURE__ */ new Set()) {
	if (isFailoverError(err) && err.code === "cli_max_turns") return err;
...
function findCliTimeoutError(err, seen = /* @__PURE__ */ new Set()) {
	if (isFailoverError(err) && hasCliTimeoutContext(err)) return err;
	if (!err || typeof err !== "object" || seen.has(err)) return;
	seen.add(err);
	const candidate = err;
	const nested = [
...
function hasPreservedTakeoverPromptError(err) {
	return Boolean(isEmbeddedAttemptSessionTakeover(err) && err && typeof err === "object" && Object.hasOwn(err, "promptError"));
}
function resolveFailoverSourceError(err) {
...
function coerceToFailoverError(err, context) {
	...
	if (isFailoverError(sourceError)) {
		if (context?.authMode && !sourceError.authMode) return new FailoverError(typeof sourceError.message === "string" ? sourceError.message : String(sourceError), {
...
function resolveModelFallbackError(err, context) {
	...
	if (isEmbeddedAttemptSessionTakeover(err) && !hasPreservedTakeoverPromptError(err)) return {
		kind: "coordination",
		error: err
	};
	...
	if (hasSessionWriteLockContention(err) || hasEmbeddedAttemptSessionTakeover(err) || hasMissingToolResultFailure(err)) return {
		kind: "coordination",
		error: err
	};
	return {
...
//#endregion
export { isMissingAgentHarnessError as _, describeFailoverError as a, isFailoverError as c, isTimeoutError as d, resolveFailoverReasonFromError as f, MissingAgentHarnessError as g, AgentHarnessSessionSupersededError as h, coerceToFailoverError as i, isNonProviderRuntimeCoordinationError as l, resolveModelFallbackError as m, buildFailoverRemediationHint as n, findCliMaxTurnsError as o, resolveFailoverStatus as p, buildProviderReauthCommand as r, findCliTimeoutError as s, FailoverError as t, isSignalTimeoutReason as u };
