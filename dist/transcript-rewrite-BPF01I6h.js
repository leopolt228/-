...
import { I as replaceTranscriptEvents, O as loadTranscriptEvents, _ as resolveSessionTranscriptRuntimeReadTarget } from "./session-accessor-Mu3lv_Tl.js";
import { n as parseSqliteSessionFileMarker } from "./sqlite-marker-BejbySI1.js";
import { r as isSessionTranscriptSideAppendEntry } from "./transcript-tree-DuZTyiYZ.js";
...
*/
const sessionEntryTypes = /* @__PURE__ */ new Set([
	"branch_summary",
	"compaction",
	"custom",
	"custom_message",
...
const repairableToolCallContentTypes = /* @__PURE__ */ new Set([
	"functionCall",
	"function_call",
	"toolCall",
	"toolUse",
...
function createTranscriptFileStateFromPersistedEntries(entries) {
	const fileEntries = entries.map(fileEntryOrMigrationSlot);
	return createReadableTranscriptFileState({
		fileEntries,
...
function isTranscriptEventRecord(event) {
...
async function rewriteSqliteRuntimeTranscript(params) {
	...
	const replacementsById = new Map(params.request.replacements.map((replacement) => [replacement.entryId, replacement.message]));
	let bytesFreed = 0;
	let rewrittenEntries = 0;
	const nextEvents = (await loadTranscriptEvents({
		agentId: marker.agentId,
		sessionId: marker.sessionId,
		sessionKey: params.target.sessionKey,
		storePath: marker.storePath
	})).map((event) => {
		if (!isTranscriptEventRecord(event)) return event;
		const eventId = typeof event.id === "string" ? event.id : void 0;
		const replacement = eventId ? replacementsById.get(eventId) : void 0;
		if (!replacement || event.type !== "message") return event;
		bytesFreed += Math.max(0, Buffer.byteLength(JSON.stringify(event.message), "utf8") - Buffer.byteLength(JSON.stringify(replacement), "utf8"));
		rewrittenEntries += 1;
		return Object.assign({}, event, { message: replacement });
	});
	if (rewrittenEntries === 0) return {
		changed: false,
		bytesFreed: 0,
		rewrittenEntries: 0,
...
function rewriteTranscriptEntriesInState(params) {
	...
	if (params.allowedRewriteSuffixEntryIds) {
		const allowedIds = new Set(params.allowedRewriteSuffixEntryIds);
		if (branch.slice(firstMatchedIndex).some((entry) => typeof entry.id === "string" && !allowedIds.has(entry.id))) return {
			changed: false,
			bytesFreed: 0,
			rewrittenEntries: 0,
...
async function rewriteTranscriptEntriesInRuntimeTranscript(params) {
	...
	try {
		...
		if (parseSqliteSessionFileMarker(target.sessionFile)) return await rewriteSqliteRuntimeTranscript({
			target,
...
//#endregion
export { TranscriptFileState as a, readTranscriptFileState as c, setRawSessionAppendMessage as d, resolveRuntimeTranscriptReadTarget as i, writeTranscriptFileAtomic as l, rewriteTranscriptEntriesInSessionManager as n, createTranscriptFileStateFromPersistedEntries as o, rewriteTranscriptEntriesInState as r, persistTranscriptStateMutation as s, rewriteTranscriptEntriesInRuntimeTranscript as t, getRawSessionAppendMessage as u };
