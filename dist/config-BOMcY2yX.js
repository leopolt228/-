...
import { c as getPublishedConfigRuntimeEnvState, n as cloneEnvWithPlatformSemantics, s as createConfigRuntimeEnvBase, t as applyConfigEnvVars } from "./config-env-vars-9fUuyise.js";
...
import { A as resolveManagedUnsetPathsForWrite, C as restoreEnvVarRefs, E as applyUnsetPathsForWrite, I as validateConfigObjectWithPlugins, Q as ConfigMutationConflictError, T as maintainConfigBackups, b as createConfigIO, d as readConfigFileSnapshotForWrite, j as resolveWriteEnvSnapshotForPath, nt as restoreEnvChangesIfUnchanged, tt as resolveConfigSnapshotHash, x as warnIfJSON5CommentsWillBeStripped, y as writeConfigFile } from "./io-CEgS2K9F.js";
import { t as GATEWAY_CONFIG_SELECTION_ENV_KEYS } from "./gateway-env-selection-DmX_wdxL.js";
import { a as hashConfigIncludeRaw, n as ConfigIncludeError, r as INCLUDE_KEY, s as resolveConfigIncludeWritePath } from "./includes-BC9UdEVK.js";
import { a as getRuntimeConfigSnapshot, c as getRuntimeConfigSourceSnapshot, f as notifyRuntimeConfigWriteListeners, l as hasManagedRuntimeConfigWriteOwner, m as preflightRuntimeSnapshotWrite, n as createRuntimeConfigWriteNotification, p as preflightManagedRuntimeConfigWrite, r as finalizeRuntimeSnapshotWrite, s as getRuntimeConfigSnapshotRefreshHandler, v as resolveConfigWriteAfterWrite, y as resolveConfigWriteFollowUp } from "./runtime-snapshot-BW7iP5ad.js";
import { n as formatInvalidConfigDetails, t as createInvalidConfigError } from "./io.invalid-config-FF36ME2X.js";
...
function assertManagedRuntimeEnvGeneration(generation) {
	if (getPublishedConfigRuntimeEnvState().generation !== generation) throw new ConfigMutationConflictError("active config environment changed while preparing write", { currentHash: null });
}
...
function assertExpectedConfigPathMatches(snapshot, expectedConfigPath) {
	if (expectedConfigPath !== void 0 && expectedConfigPath !== snapshot.path) throw new ConfigMutationConflictError("config path changed since last load", {
...
async function withConfigMutationLock(params, fn) {
	...
	return await configMutationQueue.enqueue(configPath, () => activeConfigMutationLocks.run(nextActiveLocks, async () => await withFileLock(configPath, CONFIG_MUTATION_LOCK_OPTIONS, fn)));
}
...
async function resolveExpectedRootBoundIncludeFile(params) {
	...
	try {
		...
		if (error instanceof ConfigIncludeError || error instanceof Error && error.message.startsWith("Config include write path has no approved existing root:")) throw new ConfigMutationConflictError("included config target changed since last load", { currentHash: null });
		throw error;
	}
	if (path.normalize(target.absolutePath) !== path.normalize(params.expectedAbsolutePath)) throw new ConfigMutationConflictError("included config target changed since last load", { currentHash: null });
	return target;
}
...
async function assertRootConfigStillMatchesSnapshot(snapshot) {
	...
	if (currentHash !== hashConfigIncludeRaw(snapshot.exists ? snapshot.raw ?? null : null)) throw new ConfigMutationConflictError("config changed while preparing include write", { currentHash });
}
async function rollbackJsonFileWriteIfUnchanged(params) {
	if (hashConfigIncludeRaw(await readRootBoundFileRawIfExists(params.target)) !== params.committedHash) return false;
...
async function writeRootBoundJsonFile(params) {
	...
	const currentHash = hashConfigIncludeRaw(currentRaw);
	if (currentHash !== hashConfigIncludeRaw(params.expectedRaw)) throw new ConfigMutationConflictError("included config changed while preparing write", { currentHash });
...
async function tryWriteSingleTopLevelIncludeMutation(params) {
	...
	if (!expectedIncludeTarget) throw new ConfigMutationConflictError("included config target changed since last load", { currentHash: null });
	const assertConfigPathForWrite = params.writeOptions?.assertConfigPathForWrite;
	if (!assertConfigPathForWrite) return null;
	assertConfigPathForWrite();
	if (!isPathInside(await fs.realpath(path.dirname(params.snapshot.path)), expectedIncludeTarget)) throw new Error(`Config mutation cannot update external $include target ${includePath}; edit the included file directly or move it under the config directory.`);
	...
	if (expectedIncludeHash !== void 0 && expectedIncludeHash !== previousIncludeHash) throw new ConfigMutationConflictError("included config changed since last load", { currentHash: previousIncludeHash });
	...
	if (previousIncludeRaw === null && (!snapshotHasBrokenInclude || expectedIncludeHash === void 0)) throw new ConfigMutationConflictError("included config changed since last load", { currentHash: previousIncludeHash });
	...
	if (previousIncludeRaw !== null) {
		...
		try {
			...
			if (!snapshotHasBrokenInclude || expectedIncludeHash === void 0) throw new ConfigMutationConflictError("included config changed since last load", { currentHash: previousIncludeHash });
		}
		if (parsedInclude) {
			...
			if (!isDeepStrictEqual(currentIncludedValue, snapshotIncludedValue)) throw new ConfigMutationConflictError("included config changed since last load", { currentHash: previousIncludeHash });
	...
	if (hashConfigIncludeRaw(includeRawAtCommit) !== hashConfigIncludeRaw(previousIncludeRaw)) throw new ConfigMutationConflictError("included config changed while preparing write", { currentHash: hashConfigIncludeRaw(includeRawAtCommit) });
