...
import { D as resolveMemoryRemDreamingConfig, E as resolveMemoryLightDreamingConfig, S as resolveMemoryDreamingConfig } from "./dreaming-BmyNO7Dv.js";
import { b as isUsageCountedSessionTranscriptFileName, c as resolveSessionTranscriptsDirForAgent } from "./paths-BpMRJ7TJ.js";
...
import { c as normalizeExtraMemoryPaths, s as listMemoryFiles } from "./internal-B2SueKGX.js";
...
import { n as resolveShortTermPromotionDreamingConfig } from "./dreaming-xl2Me5i6.js";
import { a as removeBackfillDiaryEntries, s as writeBackfillDiaryEntries } from "./dreaming-narrative-B5zCquwr.js";
import { n as previewGroundedRemMarkdown, t as previewRemHarness } from "./rem-harness-C_cazWqF.js";
import { C as resolveShortTermRecallStorePath, S as resolveShortTermRecallLockPath, _ as recordGroundedShortTermCandidates, b as removeGroundedShortTermCandidates, o as applyShortTermPromotions, p as rankShortTermPromotionCandidates, s as auditShortTermPromotionArtifacts, x as repairShortTermPromotionArtifacts, y as recordShortTermRecalls } from "./short-term-promotion-DpDDtSH8.js";
import { c as seedHistoricalDailyMemorySignals } from "./dreaming-phases-DqKOEtvS.js";
import { i as formatMemoryVectorDegradedWriteReason } from "./manager-v0TOigDy.js";
...
function getMemoryCommandSecretTargetIds() {
	return /* @__PURE__ */ new Set(["agents.defaults.memorySearch.remote.apiKey", "agents.list[].memorySearch.remote.apiKey"]);
}
...
async function withMemoryManagerForAgent(params) {
	...
	if (params.withLease) managerParams.withLease = params.withLease;
	await withManager({
...
async function runMemoryStatus(opts, hostOptions) {
	...
	for (const result of allResults) {
		...
		if (llamaCppRuntime) {
			...
			if (runtime.deviceNames?.length) lines.push(`${label("Devices")} ${info(runtime.deviceNames.join(", "))}`);
			if (runtime.memory) {
				const unified = runtime.memory.unifiedBytes > 0 ? ` · ${formatRuntimeBytes(runtime.memory.unifiedBytes)} unified` : "";
				lines.push(`${label("VRAM snapshot")} ${info(`${formatRuntimeBytes(runtime.memory.usedBytes)} used · ${formatRuntimeBytes(runtime.memory.freeBytes)} free · ${formatRuntimeBytes(runtime.memory.totalBytes)} total${unified}`)} ${muted(`(${new Date(runtime.memory.observedAtMs).toISOString()})`)}`);
			}
			if (runtime.offload) {
				const layers = typeof runtime.offload.offloadedLayers === "number" && typeof runtime.offload.totalLayers === "number" ? `${runtime.offload.offloadedLayers}/${runtime.offload.totalLayers} layers` : runtime.offload.supported ? "supported" : "unsupported";
				lines.push(`${label("GPU offload")} ${info(layers)}`);
			}
			if (runtime.context) lines.push(`${label("Requested context")} ${info(`${runtime.context.requestedSize} tokens`)}`);
...
async function runMemorySearch(queryArg, opts, hostOptions) {
	...
	await withMemoryManagerForAgent({
		...
		run: async (manager) => {
			...
			const identityWarning = typeof manager.status === "function" ? formatMemoryIndexIdentityWarning(manager.status(), agentId) : null;
			if (identityWarning) defaultRuntime.error(`Memory index warning: ${identityWarning.reason}. Vector memory search is paused until the index is rebuilt. ${identityWarning.fix}`);
			if (results.length === 0) {
			...
			for (const result of results) {
				lines.push(`${success(result.score.toFixed(3))} ${accent(`${shortenHomePath(result.path)}:${result.startLine}-${result.endLine}`)}`);
	...
	await withMemoryManagerForAgent({
		...
		run: async (manager) => {
			...
			for (const candidate of candidates) {
				lines.push(`${success(candidate.score.toFixed(3))} ${accent(`${shortenHomePath(candidate.path)}:${candidate.startLine}-${candidate.endLine}`)}`);
				lines.push(muted(`signals=${candidate.signalCount} recalls=${candidate.recallCount} avg=${candidate.avgScore.toFixed(3)} queries=${candidate.uniqueQueries} age=${candidate.ageDays.toFixed(1)}d consolidate=${candidate.components.consolidation.toFixed(2)} conceptual=${candidate.components.conceptual.toFixed(2)}`));
				if (candidate.conceptTags.length > 0) lines.push(muted(`concepts=${candidate.conceptTags.join(", ")}`));
				if (candidate.snippet) lines.push(muted(candidate.snippet));
				lines.push("");
			}
			if (audit.issues.length > 0) {
				lines.push(warn("Audit issues:"));
				for (const issue of audit.issues) lines.push((issue.severity === "error" ? warn : muted)(issue.message));
				lines.push("");
			}
			if (applyResult) if (applyResult.applied > 0) {
				lines.push(success(`Processed ${applyResult.applied} candidate(s) for ${shortenHomePath(applyResult.memoryPath)}.`));
				lines.push(muted(`appended=${applyResult.appended} reconciledExisting=${applyResult.reconciledExisting}`));
			} else lines.push(warn("No candidates met apply criteria."));
			defaultRuntime.log(lines.join("\n").trim());
		}
	});
}
	...
	await withMemoryManagerForAgent({
		...
		run: async (manager) => {
			...
			const lines = [
				...
				muted(`${shortenHomePath(candidate.path)}:${String(candidate.startLine)}-${String(candidate.endLine)}`),
				candidate.snippet,
				muted(`score=${candidate.score.toFixed(3)} signals=${candidate.signalCount} recalls=${candidate.recallCount} uniqueQueries=${candidate.uniqueQueries} ageDays=${candidate.ageDays.toFixed(1)}`),
				...
				muted(`thresholds: minScore=${thresholds.minScore} minRecallCount=${thresholds.minRecallCount} minUniqueQueries=${thresholds.minUniqueQueries} maxAgeDays=${thresholds.maxAgeDays ?? "none"}`)
			];
			if (candidate.conceptTags.length > 0) lines.push(muted(`concepts=${candidate.conceptTags.join(", ")}`));
			defaultRuntime.log(lines.join("\n"));
		}
			...
			try {
				...
				const lines = [
					...
					...opts.path ? [
						...
						muted(`historicalFiles=${sourceFiles.length} importedFiles=${importedFileCount} importedSignals=${importedSignalCount}`),
						...skippedPaths.length > 0 ? [warn(`skipped=${skippedPaths.map((entry) => shortenHomePath(entry)).join(", ")}`)] : []
					] : [],
					...opts.grounded ? [muted(`groundedInputs=${groundedInputPaths.length > 0 ? groundedInputPaths.map((entry) => shortenHomePath(entry)).join(", ") : "none"}`)] : [],
					muted(`recentRecallEntries=${preview.recallEntryCount} deepCandidates=${deepCandidates.length}`),
					"",
					heading("REM Preview"),
		...
		run: async (manager) => {
			...
			if (opts.rollback || opts.rollbackShortTerm) {
				...
				defaultRuntime.log([
					...
					muted(`workspace=${shortenHomePath(workspaceDir)}`),
					...diaryRollback ? [muted(`dreamsPath=${shortenHomePath(diaryRollback.dreamsPath)}`), muted(`removedEntries=${diaryRollback.removed}`)] : [],
					...shortTermRollback ? [muted(`shortTermStorePath=${shortenHomePath(shortTermRollback.storePath)}`), muted(`removedShortTermEntries=${shortTermRollback.removed}`)] : []
				].join("\n"));
				return;
			}
			if (!opts.path) {
			...
			try {
				...
				defaultRuntime.log([
					...
					muted(`workspace=${shortenHomePath(workspaceDir)}`),
					muted(`sourcePath=${shortenHomePath(path.resolve(opts.path))}`),
					muted(`historicalFiles=${sourceFiles.length} writtenEntries=${written.written} replacedEntries=${written.replaced}`),
					...opts.stageShortTerm ? [muted(`stagedShortTermEntries=${stagedShortTermEntries} replacedShortTermEntries=${replacedShortTermEntries}`)] : [],
