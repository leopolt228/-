...
const TEXT_ONLY_MODELS = /* @__PURE__ */ new Set([
	"gen4.5",
	"veo3.1",
	"veo3.1_fast",
	"veo3"
...
const VIDEO_MODELS = /* @__PURE__ */ new Set(["gen4_aleph"]);
...
async function pollRunwayTask(params) {
	...
	return await pollProviderOperationJson({
		...
		getFailureMessage: (payload) => {
			...
			return status === "FAILED" || status === "CANCELLED" ? readRunwayFailureMessage(payload.failure) || `Runway video generation ${normalizeLowercaseStringOrEmpty(status)}` : void 0;
		}
	});
}
async function downloadRunwayVideos(params) {
	...
	for (const [index, url] of params.urls.entries()) {
		...
		const buffer = await readResponseWithLimit(response, params.maxBytes, {
			timeoutMs,
			onTimeout: ({ timeoutMs: bodyTimeoutMs }) => /* @__PURE__ */ new Error(`Runway generated video download timed out after ${deadline.timeoutMs ?? bodyTimeoutMs}ms`),
			onOverflow: ({ maxBytes }) => /* @__PURE__ */ new Error(`Runway generated video download exceeds ${maxBytes} bytes`)
		});
		videos.push({
			buffer,
			mimeType,
