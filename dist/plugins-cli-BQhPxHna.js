...
const loadPluginsAuthoringCommands = createModuleLoader(() => import("./plugins-authoring-command-BOQEAdjc.js"));
function registerPluginsCli(program) {
	...
	plugins.command("inspect").alias("info").description("Inspect plugin details").argument("[id]", "Plugin id").option("--all", "Inspect all plugins").option("--runtime", "Load plugin runtime for hooks/tools/diagnostics").option("--json", "Print JSON").action(async (id, opts) => {
		const { runPluginsInspectCommand } = await import("./plugins-inspect-command-CUutj6If.js");
	...
	plugins.command("uninstall").description("Uninstall a plugin").argument("<id>", "Plugin id").option("--keep-files", "Keep installed files on disk", false).option("--keep-config", "Deprecated alias for --keep-files", false).option("--force", "Skip confirmation prompt", false).option("--dry-run", "Show what would be removed without making changes", false).action(async (id, opts) => {
		...
		await runPluginUninstallCommand(id, {
			...opts,
			invalidateRuntimeCache: false
		});
	});
	plugins.command("install").description("Install a plugin or hook pack (path, archive, npm spec, git repo, clawhub:package, or marketplace entry)").argument("<path-or-spec-or-plugin>", "Path (.ts/.js/.zip/.tgz/.tar.gz), npm package spec, or marketplace plugin name").option("-l, --link", "Link a local path instead of copying", false).option("--force", "Confirm non-ClawHub sources and overwrite an existing plugin or hook pack", false).option("--pin", "Record npm installs as exact resolved <name>@<version>", false).option("--dangerously-force-unsafe-install", "Deprecated no-op; security.installPolicy may still block", false).option("--acknowledge-clawhub-risk", "Acknowledge ClawHub release trust warnings without prompting", false).option("--marketplace <source>", "Install a Claude marketplace plugin from a local repo/path or git/GitHub source").action(async (raw, opts) => {
	...
	plugins.command("update").description("Update installed plugins and tracked hook packs").argument("[id]", "Plugin or hook-pack id (omit with --all)").option("--all", "Update all tracked plugins and hook packs", false).option("--dry-run", "Show what would change without writing", false).option("--dangerously-force-unsafe-install", "Deprecated no-op; security.installPolicy may still block", false).option("--acknowledge-clawhub-risk", "Acknowledge ClawHub release trust warnings without prompting", false).action(async (id, opts) => {
		const { runPluginUpdateCommand } = await import("./plugins-update-command-DnoSqSBI.js");
	...
	plugins.command("init").description("Create a plugin project").argument("<id>", "Plugin id").option("--directory <path>", "Output directory").option("--name <name>", "Display name").option("--type <type>", "Scaffold type (tool or provider)", "tool").option("--force", "Overwrite an existing output directory", false).action(async (id, opts) => {
	...
	marketplace.command("entries").description("List entries from the configured OpenClaw marketplace feed").option("--feed-profile <name>", "Configured marketplace feed profile to list").option("--feed-url <url>", "Explicit hosted marketplace feed URL").option("--offline", "Read the latest accepted snapshot without fetching the feed", false).option("--json", "Print JSON").action(async (opts) => {
		const { runPluginMarketplaceEntriesCommand } = await loadPluginsRuntime();
		await runPluginMarketplaceEntriesCommand(opts);
	});
	marketplace.command("refresh").description("Refresh the configured OpenClaw marketplace feed snapshot").option("--feed-profile <name>", "Configured marketplace feed profile to refresh").option("--feed-url <url>", "Explicit hosted marketplace feed URL").option("--expected-sha256 <hash>", "Expected hosted feed SHA-256 payload checksum").option("--json", "Print JSON").action(async (opts) => {
