...
async function discoverHuggingfaceModels(apiKey, timeoutMs = HUGGINGFACE_DISCOVERY_TIMEOUT_MS) {
	...
	try {
		...
		const { response, release } = await fetchWithSsrFGuard(withTrustedEnvProxyGuardedFetchMode({
		...
		try {
			...
			const data = (await readProviderJsonResponse(response, "huggingface.model-discovery"))?.data;
