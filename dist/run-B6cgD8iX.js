...
import { m as resolveGatewayPort, o as normalizeStateDirEnv, t as CONFIG_PATH } from "./paths-CHQRdQZ3.js";
...
import { c as setConsoleTimestampPrefix, s as setConsoleSubsystemFilter } from "./console-DvVy2coK.js";
import { t as createSubsystemLogger } from "./subsystem-Dogzi5wG.js";
import { p as ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS_ENV } from "./config-env-vars-9fUuyise.js";
...
import { i as isLoopbackHost, p as resolveGatewayBindHost, t as defaultGatewayBindMode } from "./net-DBokCmJs.js";
import { r as CONFIG_AUDIT_STORE_LABEL } from "./io.audit-ChVTQVyd.js";
import { t as clearRuntimeConfigSnapshot } from "./runtime-snapshot-BW7iP5ad.js";
import { i as isInvalidConfigError, r as isDoctorRecoverableInvalidConfigError } from "./io.invalid-config-FF36ME2X.js";
import { n as formatGatewayPidList, t as findVerifiedGatewayListenerPidsOnPortSync } from "./gateway-processes-c8np4yhp.js";
...
import { r as isTerminalInteractive, t as NON_INTERACTIVE_GATEWAY_RUN_FORCE_MESSAGE } from "./terminal-interactivity-Bmck99HR.js";
...
import { a as markGatewayRestartTrace, f as startGatewayRestartTrace, o as measureGatewayRestartTrace, r as createGatewayRestartTraceHandoffEnv, t as captureGatewayRestartTraceHandoff } from "./restart-trace-CzbQ5gIb.js";
import { n as setGatewayWsLogStyle } from "./ws-logging-86BGsxSJ.js";
import { t as printClawBanner } from "./claw-banner-C4ERcHln.js";
import { n as isGatewayRunFutureConfigAllowed, t as enforceGatewayRunFutureConfigGuard } from "./future-config-guard-BOi_RVwh.js";
...
async function runGatewayLoop(params) {
	...
	const completeForcedStop = (reason) => {
		params.completeBoot?.({
			outcome: "forced_stop",
			reason
		});
	...
	const handleRestartAfterServerClose = async () => {
		...
		const { detectGatewayRespawnSupervisor, markUpdateRestartSentinelFailure, respawnGatewayProcessForUpdate, restartGatewayProcessWithFreshPid, writeGatewayRestartHandoffSync } = await loadGatewayLifecycleRuntimeModule();
		const restartReason = activeRestartRequest?.restartReason;
		params.completeBoot?.({
			outcome: "planned_restart",
			reason: restartReason ?? "gateway.restart"
		});
		if (isUpdateProcessRestartReason(restartReason)) {
			...
			if (respawn.mode === "supervised") {
				...
				if (supervisorMode === "external" && !handoff) {
					...
					await markUpdateRestartSentinelFailure("restart-handoff-unavailable").catch((err) => {
						gatewayLog$1.warn(`failed to mark update restart handoff unavailable: ${String(err)}`);
					});
					if (!await reacquireLockForInProcessRestart()) return;
					activeRestartRequest = null;
					shuttingDown = false;
		...
		if (respawn.mode === "spawned" || respawn.mode === "supervised") {
			const supervisorMode = respawn.mode === "supervised" ? detectGatewayRespawnSupervisor(process.env, process.platform) : null;
			...
			forceExitTimer = setTimeout(() => {
				...
				(async () => {
					try {
						...
						completeForcedStop(isRestart ? "gateway.restart_shutdown_timeout" : "gateway.stop_shutdown_timeout");
						exitProcess(1);
					}
				})();
			}, forceExitMs);
				...
				if (isRestart) {
					...
					await measureGatewayRestartTrace("restart.drain", async () => {
						...
						const markActiveMainSessionsForRestart = async (reason) => {
							const sessionKeys = /* @__PURE__ */ new Set([...activeRestartSessionKeysAtDrainStart, ...collectActiveRestartSessionKeys()]);
							const sessionIds = /* @__PURE__ */ new Set([...activeRestartSessionIdsAtDrainStart, ...collectActiveRestartSessionIds()]);
						...
						markRestartDraining();
						const rootDrainTimeoutMs = restartDrainDeadlineAt === void 0 ? void 0 : Math.max(0, restartDrainDeadlineAt - Date.now());
						const rootDrainPromise = restartIntent?.force ? Promise.resolve({
							drained: true,
							active: 0
						...
						if (!rootDrain.drained) {
							drainTimedOut = true;
							gatewayLog$1.warn(`gateway root transaction drain timeout reached with ${rootDrain.active} root(s) still active; proceeding with restart`);
						}
					}, () => [
	...
	const request = (action, signal, restartReason, restartIntent) => {
		...
		if (shuttingDown) {
			...
			if (action === "restart" && isUpdateProcessRestartReason(restartReason) && currentRestartRequest?.action === "restart" && !isUpdateProcessRestartReason(currentRestartRequest.restartReason)) {
		...
		const isRestart = action === "restart";
		if (isRestart) markRestartDraining();
		shuttingDown = true;
...
async function resolveGatewayPasswordOption(opts) {
	...
	if (file) {
		const { readSecretFromFile } = await import("./secret-file-Bn6JLBR-.js");
...
async function resolveGatewayRunShellEnvFallbackPlan(cfg) {
	const { createConfigRuntimeEnv } = await import("./env-vars-B7lN3JgG.js");
	const { resolveShellEnvFallbackTimeoutMs, shouldDeferShellEnvFallback, shouldEnableShellEnvFallback } = await import("./shell-env-C7hkMq_E.js");
	...
	const { resolveShellEnvExpectedKeys } = await import("./shell-env-expected-keys-EAHmPOYg.js");
	return {
		enabled: true,
...
async function loadGatewayRunShellEnvFallback(plan) {
	const { loadShellEnvFallback } = await import("./shell-env-C7hkMq_E.js");
...
async function clearGatewayRunShellEnvFallback(values) {
	...
	const { clearShellEnvAppliedKeys } = await import("./shell-env-C7hkMq_E.js");
...
function resolveGatewayLockErrorExitCode(err, supervisor, healthyGatewayConfirmed) {
	...
	return await new Promise((resolve) => {
		...
		const req = (params.tlsFingerprint ? request$1 : request)({
			...
			if (params.tlsFingerprint) {
				if ((res.socket instanceof TLSSocket ? normalizeFingerprint(res.socket.getPeerCertificate().fingerprint256 ?? "") : "") !== normalizeFingerprint(params.tlsFingerprint)) {
					res.resume();
					finish(false);
					return;
				}
...
async function maybeWriteGatewayStartupFailureBundle(err, reason = "gateway.startup_failed") {
	const { writeDiagnosticStabilityBundleForFailureSync } = await import("./diagnostic-stability-bundle-3kwE9r3L.js");
...
async function runGatewayCommandOnce(opts, hooks = {}) {
	...
	const { clearGatewayRunConfigEnvironment } = await import("./pre-bootstrap-CLju9IvY.js");
	...
	const serverImportMeasurement = startupTrace.startMeasure("cli.server-import", () => import("./server-ReArp9C2.js"));
	...
	if (devMode) {
		if (opts.reset) {
			const { recheckGatewayRunReset } = await import("./pre-bootstrap-CLju9IvY.js");
			if (!await recheckGatewayRunReset({
				opts,
				runtime: defaultRuntime
			})) return;
		}
		const { ensureDevGatewayConfig } = await import("./dev-oBkLrjGv.js");
	...
	if (snapshot) {
		const { applyFinalGatewayRunConfigEnv } = await import("./pre-bootstrap-CLju9IvY.js");
	...
	if (process.env.OPENCLAW_SERVICE_MARKER?.trim()) {
		const { cleanStaleGatewayProcessesSync } = await import("./restart-stale-pids-CjMwpGXr.js");
	...
	if (opts.force) {
		...
		try {
			const { forceFreePortAndWait, waitForPortBindable } = await import("./ports-I99V7hkd.js");
	...
	const beginBoot = async (startedAtMs) => {
		...
		const message = `gateway restart-loop breaker tripped: ${crashLoopDecision.uncleanBoots} unclean boot(s) within ${crashLoopDecision.windowMs}ms; suppressing channel/provider account auto-start. Inspect the stability bundle and fix the startup crash before restarting the service.`;
	...
	const { detectRespawnSupervisor } = await import("./supervisor-markers-Cu9lUPdq.js");
	...
	try {
		...
		if (isGatewayLockError(err)) {
			...
			try {
				const { formatPortDiagnostics, inspectPortUsage } = await import("./ports-D4MaiwGZ.js");
				const diagnostics = await inspectPortUsage(port);
				if (diagnostics.status === "busy") for (const line of formatPortDiagnostics(diagnostics)) defaultRuntime.error(line);
			} catch {}
			const { maybeExplainGatewayServiceStop } = await import("./shared-C-JtaQ01.js");
			await maybeExplainGatewayServiceStop();
			const gatewayTls = isGatewayAlreadyRunningLockError(err) && cfg.gateway?.tls?.enabled === true ? await import("./gateway-BaqZivxF.js").then(({ loadGatewayTlsRuntime }) => loadGatewayTlsRuntime(cfg.gateway?.tls)).catch(() => void 0) : void 0;
			const canProbeGateway = cfg.gateway?.tls?.enabled !== true || Boolean(gatewayTls?.fingerprintSha256);
			const healthyGatewayConfirmed = isGatewayAlreadyRunningLockError(err) && canProbeGateway && await probeGatewayHealthz({
				host: healthHost,
				port,
				...gatewayTls?.fingerprintSha256 ? { tlsFingerprint: gatewayTls.fingerprintSha256 } : {}
			});
			defaultRuntime.exit(resolveGatewayLockErrorExitCode(err, supervisor, healthyGatewayConfirmed));
			return;
		}
