...
import { h as validateApprovalPresentation } from "./src-Cy32TawB.js";
//#region src/gateway/operator-approval-store.ts
const OPERATOR_APPROVAL_TERMINAL_RETENTION_MS = 720 * 60 * 6e4;
...
function inputMatchesExistingRow(input, row, serialized) {
	...
	return row.status === "pending" && row.kind === input.kind && row.presentation_json === serialized.presentationJson && row.requested_by_device_id === normalizeString(input.requester?.deviceId) && row.requested_by_client_id === normalizeString(input.requester?.clientId) && row.requested_by_device_token_auth === (input.requester?.deviceTokenAuth === true ? 1 : 0) && row.reviewer_device_ids_json === serialized.reviewerDeviceIdsJson && row.source_agent_id === normalizeString(source.agentId) && row.source_session_key === normalizeString(source.sessionKey) && row.source_session_id === normalizeString(source.sessionId) && row.source_run_id === normalizeString(source.runId) && row.source_tool_call_id === normalizeString(source.toolCallId) && row.source_tool_name === normalizeString(source.toolName) && row.audience_session_keys_json === serialized.audienceSessionKeysJson && row.runtime_epoch === input.runtimeEpoch.trim() && row.created_at_ms === input.createdAtMs && row.expires_at_ms === input.expiresAtMs;
}
...
function getOperatorApprovalDetailedByLocator(params) {
...
//#endregion
export { forceDenyOperatorApproval as a, insertOperatorApproval as c, pruneTerminalOperatorApprovals as d, resolveOperatorApproval as f, expireDueOperatorApprovals as i, listPendingOperatorApprovals as l, closeOrphanedOperatorApprovals as n, getOperatorApprovalDetailed as o, consumeOperatorApprovalAllowOnce as r, getOperatorApprovalDetailedByLocator as s, OperatorApprovalHistoryCursorError as t, listTerminalOperatorApprovals as u };
