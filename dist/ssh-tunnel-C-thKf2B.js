...
import { o as isErrno, r as formatErrorMessage } from "./errors-DdbcjW1Y.js";
import { r as ensurePortAvailable, t as PortInUseError } from "./ports-BSfVrBR-.js";
import { spawn } from "node:child_process";
import net from "node:net";
//#region src/infra/ssh-tunnel.ts
function isMalformedHost(host) {
...
function parseSshTarget(raw) {
	...
	if (colonIdx > 0 && colonIdx < hostPart.length - 1) {
		...
		if (isMalformedHost(host)) return null;
		return {
			user: userPart,
			host,
			port
	...
	if (isMalformedHost(hostPart)) return null;
	return {
		user: userPart,
		host: hostPart,
		port: 22
...
async function startSshPortForward(opts) {
	...
	try {
		...
		if (err instanceof PortInUseError || isErrno(err) && err.code === "EADDRINUSE") localPort = await pickEphemeralPort();
		else throw err;
	}
	...
	const stop = async () => {
		if (child.killed || !child.kill("SIGTERM")) return;
