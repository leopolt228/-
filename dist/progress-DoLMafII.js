		...
		progress: {
			...
			onStepComplete: (step) => {
				...
				if (isAdvisoryStep(step) && step.stderrTail) {
					...
					for (const line of lines) if (line.trim()) defaultRuntime.log(`    ${theme.warn(line)}`);
...
function formatStepStatus(step) {
	if (isAdvisoryStep(step)) return theme.warn("!");
	if (step.exitCode === 0) return theme.success("✓");
	if (step.exitCode === null) return theme.warn("?");
	return theme.error("✗");
}
