...
const MOONSHOT_K2_7_CODE_MODEL_IDS = ["kimi-k2.7-code", "kimi-k2.7-code-highspeed"];
...
function sanitizeKimiK3Payload(payloadObj) {
	delete payloadObj.thinking;
	delete payloadObj.reasoningEffort;
	payloadObj.reasoning_effort = "max";
	for (const field of MOONSHOT_FIXED_SAMPLING_FIELDS) delete payloadObj[field];
}
function sanitizeAlwaysThinkingPayload(payloadObj, modelId) {
	if (modelId === MOONSHOT_K3_MODEL_ID) sanitizeKimiK3Payload(payloadObj);
	else sanitizeKimiK27Payload(payloadObj);
}
function sanitizeAlwaysThinkingAfterCaller(value, fallbackPayload, modelId) {
...
function resolveAlwaysThinkingModelId(modelId, directMoonshotModel) {
	if (MOONSHOT_K2_7_CODE_MODEL_IDS.includes(modelId)) return modelId;
	return directMoonshotModel && modelId === MOONSHOT_K3_MODEL_ID ? MOONSHOT_K3_MODEL_ID : void 0;
}
...
function createMoonshotThinkingWrapper(baseStreamFn, thinkingType, thinkingKeep) {
	const wrap = (underlying) => (model, context, options) => {
		...
		const directMoonshotModel = normalizeOptionalLowercaseString(model.provider) === MOONSHOT_PROVIDER_ID;
		const alwaysThinkingModel = resolveAlwaysThinkingModelId(modelId, directMoonshotModel);
		const streamModel = alwaysThinkingModel ? {
			...model,
			reasoning: true
		} : model;
		const streamOptions = alwaysThinkingModel ? {
			...options,
			reasoning: alwaysThinkingModel === MOONSHOT_K3_MODEL_ID ? "max" : "low"
		} : options;
		...
		return underlying(streamModel, context, {
			...
			onPayload(payload, payloadModel) {
				...
				const payloadAlwaysThinkingModel = resolveAlwaysThinkingModelId(payloadModelId, directMoonshotModel);
				if (payloadAlwaysThinkingModel) {
					...
					if (result && typeof result.then === "function") return Promise.resolve(result).then((resolved) => sanitizeAlwaysThinkingAfterCaller(resolved, payloadObj, payloadAlwaysThinkingModel));
					return sanitizeAlwaysThinkingAfterCaller(result, payloadObj, payloadAlwaysThinkingModel);
				}
