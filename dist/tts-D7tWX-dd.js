...
async function seedSpeechTTS(params) {
	...
	try {
		const frames = parseSeedTtsFrames(new TextDecoder().decode(await readResponseWithLimit(response, VOLCENGINE_TTS_RESPONSE_MAX_BYTES, { onOverflow: ({ maxBytes }) => /* @__PURE__ */ new Error(`BytePlus Seed Speech TTS response exceeds ${maxBytes} bytes`) })));
		const chunks = [];
...
async function legacyVolcengineTTS(params) {
	...
	try {
		const body = parseLegacyTtsResponse(new TextDecoder().decode(await readResponseWithLimit(response, VOLCENGINE_TTS_RESPONSE_MAX_BYTES, { onOverflow: ({ maxBytes }) => /* @__PURE__ */ new Error(`Volcengine TTS response exceeds ${maxBytes} bytes`) })));
