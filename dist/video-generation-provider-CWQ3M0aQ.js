...
import { t as TOGETHER_BASE_URL } from "./models-CpGWOTUK.js";
...
async function downloadTogetherVideo(params) {
	...
	return {
		buffer: await readResponseWithLimit(response, params.maxBytes, {
			timeoutMs,
			onTimeout: ({ timeoutMs: bodyTimeoutMs }) => /* @__PURE__ */ new Error(`Together generated video download timed out after ${deadline.timeoutMs ?? bodyTimeoutMs}ms`),
			onOverflow: ({ maxBytes }) => /* @__PURE__ */ new Error(`Together generated video download exceeds ${maxBytes} bytes`)
		}),
		mimeType,
