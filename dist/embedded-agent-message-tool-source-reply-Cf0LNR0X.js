...
import { i as isMessagingToolDeliveryAction, n as isMessageToolSendActionName, t as isMessageToolConversationCreateActionName } from "./embedded-agent-messaging-6-R0iczA.js";
...
function recordHasDeliveredMessageId(record) {
	...
	if (hasDeliveredId(record.messageId) || hasDeliveredId(record.pollId) || hasDeliveredId(message.id)) return true;
...
function isDeliveredMessageToolOnlySourceReplyResult(params) {
	...
	const hasConfirmedExplicitSourceRoute = params.allowExplicitSourceRoute === true || resultConfirmsCurrentSourceRoute(params.result);
	if (hasExplicitMessageRoute(args) && !hasConfirmedExplicitSourceRoute) return false;
