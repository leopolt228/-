...
function buildRawChannelAdapterResult(channel, result) {
	if (!result.ok) throw new Error(result.error?.trim() || `Channel send failed for ${channel}`);
...
function createRawChannelSendResultAdapter(params) {
	return {
		sendText: params.sendText ? async (ctx) => buildRawChannelAdapterResult(params.channel, await params.sendText(ctx)) : void 0,
		sendMedia: params.sendMedia ? async (ctx) => buildRawChannelAdapterResult(params.channel, await params.sendMedia(ctx)) : void 0
	};
}
