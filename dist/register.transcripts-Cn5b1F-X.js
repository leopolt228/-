...
function assertRequestedSession(entry, sessionId) {
	if (entry.session.sessionId !== sessionId && safeSegment(entry.session.sessionId) !== sessionId) throw new Error(`transcripts metadata mismatch for ${sessionId}: found ${entry.session.sessionId}`);
	return entry;
}
async function requireStoredSession(selector) {
	...
	const matches = (await listStoredSessions()).filter((entry) => entry.session.sessionId === selector || safeSegment(entry.session.sessionId) === selector);
