...
import { V as isPluginRegistryRetired, t as collectLivePluginRegistries } from "./runtime-BapEso0o.js";
...
function collectHookRegistrySources(lastInitialized) {
	...
	if (!liveRegistries.some((registry) => registry === lastInitialized)) add(lastInitialized);
	for (const registry of liveRegistries) add(registry);
	return ordered;
}
function composeLiveHookRegistry(lastInitialized) {
	...
	const liveRegistries = collectLivePluginRegistries();
	if (lastInitialized && !liveRegistries.includes(lastInitialized)) {
		const isolatedSourceIndex = sources.indexOf(lastInitialized);
		if (isolatedSourceIndex >= 0) for (const pluginId of expectDefined(hookPluginIdsBySource[isolatedSourceIndex], "isolated hook plugin ids")) claimOwner(pluginId, isolatedSourceIndex);
	}
	const claimToolOwners = (registry) => {
		if (!registry) return;
		const sourceIndex = sources.indexOf(registry);
		if (sourceIndex < 0) return;
		for (const tool of registry.tools) claimOwner(tool.pluginId, sourceIndex);
	};
	...
	sources.forEach((registry, index) => {
		for (const plugin of registry.plugins) if (plugin.status === "loaded" && expectDefined(hookPluginIdsBySource[index], "hook plugin ids by source entry at index").has(plugin.id)) claimOwner(plugin.id, index);
	});
	sources.forEach((registry, index) => {
	...
	sources.forEach((registry, index) => {
		for (const plugin of registry.plugins) if (plugin.status === "loaded" && expectDefined(trustedPolicyPluginIdsBySource[index], "trusted policy plugin ids by source entry at index").has(plugin.id)) claimPolicyOwner(plugin.id, index);
	});
	sources.forEach((registry, index) => {
...
function createHookRunner(registry, options = {}) {
	...
	*/
	async function runChannelPairingRequested(event, ctx) {
	...
	*/
	...
	async function runSubagentProgress(event, ctx) {
	...
	*/
	async function runCronReconciled(event, ctx) {
...
//#endregion
export { resetGlobalHookRunner as a, joinPresentTextSegments as c, getGlobalHookRunnerRegistry as d, initializeGlobalHookRunner as i, fireAndForgetBoundedHook as l, getGlobalPluginRegistry as n, runGlobalGatewayStopSafely as o, hasGlobalHooks as r, resolveBlockMessage as s, getGlobalHookRunner as t, fireAndForgetHook as u };
