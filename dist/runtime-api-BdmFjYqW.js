...
import { C as resolveExpiresAtMsFromDurationMs, b as parseStrictPositiveInteger, m as isFutureDateTimestampMs, t as MAX_DATE_TIMESTAMP_MS } from "./number-coercion-Crk_c9KW.js";
...
function containsWorkboardWorkspaceMutation(value) {
	...
	return containsWorkboardWorkspaceMutation(record.patch) || containsWorkboardWorkspaceMutation(readRecord(record.metadata)?.automation) || Array.isArray(record.children) && record.children.some((child) => containsWorkboardWorkspaceMutation(child));
}
...
function withoutWorkboardWorkspaceAccess(value) {
	const { workspaceAccess: _untrustedWorkspaceAccess, ...rest } = readRecord(value) ?? {};
	return rest;
}
...
async function assertWorkboardWorkspaceMutationAccess(value, access) {
	...
	const automation = readRecord(readRecord(record.metadata)?.automation);
...
//#endregion
//#region extensions/workboard/src/store-constants.ts
...
function normalizeWorkspace(value, fallback) {
	...
	const sourcePath = normalizeBoundedString(record.sourcePath, fallback?.sourcePath, 2e3, "workspace source path");
	if (sourcePath && !isAbsoluteWorkspacePath(sourcePath)) throw new Error("workspace source path must be absolute.");
	const sourceBranch = normalizeBoundedString(record.sourceBranch, fallback?.sourceBranch, 160, "workspace source branch");
	return {
		kind,
...
function normalizeMetadata(value, fallback = {}, options = {}) {
	if (!value || typeof value !== "object" || Array.isArray(value)) return trimMetadataToBudget(fallback, options);
	const record = value;
...
function removeUndefinedExecutionFields(execution) {
	...
	if (next.engine === void 0) delete next.engine;
	if (next.model === void 0) delete next.model;
...
var WorkboardCoreStore = class {
	...
	async createDirect(input, scope) {
		...
		const position = Number.isFinite(normalizedPosition) ? normalizedPosition : Math.max(0, ...cards.filter((card) => card.status === status && cardBoardId$1(card) === boardId).map((card) => card.position)) + POSITION_STEP;
		let card = {
			id: randomUUID(),
...
var WorkboardWorkflowStore = class extends WorkboardPromoteStore {
	async claim(id, input, options = {}) {
		...
		return await this.enqueueMutation(async () => {
			...
			if (expectedAuthority && (guarded.agentId !== expectedAuthority.agentId || !isDeepStrictEqual(guarded.metadata?.automation?.workspace, expectedAuthority.workspace) || !isDeepStrictEqual(guarded.metadata?.automation?.workspaceAccess, expectedAuthority.workspaceAccess))) throw new Error("card workspace authority changed before claim.");
			const existingClaim = guarded.metadata?.claim;
			const activeClaim = existingClaim && isFutureDateTimestampMs(existingClaim.expiresAt, { nowMs: now }) ? existingClaim : void 0;
			...
			const metadata = clearDiagnostics((options.adoptWorkspaceAccess && !guarded.metadata?.automation?.workspaceAccess ? await this.updateCard(id, { workspaceAccess: options.adoptWorkspaceAccess }) : guarded).metadata, ["stranded_ready"]);
	...
	async dispatch(input = Date.now()) {
		...
		return await this.enqueueMutation(async () => {
			...
			for (const card of await this.list({ boardId })) {
				...
				const claimExpired = Boolean(claim?.expiresAt && now - claim.expiresAt > 3e5);
...
async function dispatchAndStartWorkboardCards(params) {
	...
	for (const card of selectStartableCards(cards, maxStarts, candidates)) {
		...
		try {
			...
			if (materializedWorkspace) await params.store.update(card.id, {
			...
			if (!runStarted && materializedWorkspace?.kind === "worktree" && materializedWorkspace.path && params.worktrees) {
				...
				if (sourceWorkspace) await params.store.update(card.id, { workspace: sourceWorkspace }).catch(() => void 0);
			}
...
function createWorkboardDispatchHandler(params) {
	return async ({ params: requestParams, respond, client, context }, options) => {
		try {
			...
			const workspaceAccess = resolveGatewayWorkboardWorkspaceAccess({
				context,
				client
			});
...
function registerWorkboardWorkspaceCardMethods(params) {
	...
	api.registerGatewayMethod("workboard.cards.update", async (request) => {
		...
		try {
			...
			respond(true, { card: redactCard(await store.update(readId(requestParams), containsWorkboardWorkspaceMutation(patch) ? withWorkboardWorkspaceAccess(patch, access) : patch)) });
		} catch (error) {
			respondError(respond, error);
		}
	}, { scope: WRITE_SCOPE$1 });
...
//#endregion
export { WORKBOARD_TOOL_NAMES as a, resolveAgentWorkboardWorkspaceRuntime as c, cleanupWorkboardRunWorktree as i, resolveCommandWorkboardWorkspaceAccess as l, dispatchAndStartWorkboardCards as n, canonicalizeWorkboardWorkspaceAccess as o, WorkboardStore as r, guardWorkboardToolsForWorkspaceAccess as s, registerWorkboardGatewayMethods as t, resolveWorkboardAgentWorkspace as u };
