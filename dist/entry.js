...
import { d as isRootHelpInvocation, f as isRootVersionInvocation, r as getCommandPositionalsWithRootOptions } from "./argv-D4LdWdQQ.js";
import { t as isMainModule } from "./is-main-CH4EEB_R.js";
import { t as resolveCliArgvInvocation } from "./argv-invocation-DyflwSWk.js";
import { i as tryOutputPrecomputedCommandHelp, n as applyCliProfileEnv, o as parseCliContainerArgs, r as parseCliProfileArgs, s as resolveCliContainerTarget, t as createGatewayStartupTrace } from "./startup-trace-BraDUmmm.js";
...
import { n as ensureOpenClawExecMarkerOnProcess } from "./openclaw-exec-env-BmbZ1aqS.js";
...
if (process$1.env.VITEST || process$1.env.NODE_ENV === "test") globalThis[Symbol.for("openclaw.entryCompileCacheTestApi")] = {
...
function hasNodeRuntimeOption(params) {
	...
	return params.execArgv.includes(params.option) || nodeOptions.some((token) => {
		if (token === params.option) return true;
		const quote = token[0];
		return (quote === "\"" || quote === "'") && token.at(-1) === quote && token.slice(1, -1) === params.option;
	});
}
...
function buildCliRespawnPlan(params = {}) {
	...
	if (platform === "darwin" && env.NODE_USE_SYSTEM_CA === "1" && !isTerminalInteractiveRespawnArgv(argv) && !hasNodeRuntimeOption({
		env,
		execArgv,
		option: SYSTEM_CA_FLAG
	}) && !hasNodeRuntimeOption({
		env,
		execArgv,
		option: OPENSSL_CA_FLAG
	})) {
		childEnv.NODE_USE_SYSTEM_CA = "0";
		if (!hasNodeRuntimeOption({
			env,
			execArgv,
			option: BUNDLED_CA_FLAG
...
function tryHandleRootVersionFastPath(argv, deps = {}) {
	...
	(deps.resolveVersion ?? (async () => {
		const [{ VERSION }, { resolveCommitHash }] = await Promise.all([import("./version-BmsGkjsI.js"), import("./git-commit-B_-zaMR3.js")]);
		return {
			VERSION,
			resolveCommitHash
		};
...
if (!isMainModule({
	...
	if (!respawnWithoutOpenClawCompileCacheIfNeeded({
		...
		normalizeEnv();
		const { assertSupportedRuntime } = await import("./runtime-guard-C1WpjZFt.js");
...
async function tryHandleRootHelpFastPath(argv, deps = {}) {
	...
	try {
		...
		await (deps.outputRootHelp ?? (await import("./root-help-7aHXui0K.js")).outputRootHelp)(liveRootHelpOptions ?? void 0);
		return true;
	} catch (error) {
		handleError(error);
		return true;
...
async function runMainOrRootHelp(argv) {
	...
	try {
		...
		const { formatCliFailureLines } = await import("./failure-output-Du-XOOhH.js");
