...
import { t as resolveGoogleGenerativeAiHttpRequestConfig } from "./api-BhdQaxwo.js";
...
async function synthesizeGoogleTtsPcmOnce(params) {
	...
	try {
		...
		try {
			return extractGoogleSpeechPcm(await readProviderJsonResponse(res, "Google TTS response"));
		} catch (err) {
...
async function synthesizeConfiguredGoogleTts(req) {
...
function buildGoogleSpeechProvider() {
	return {
		...
		synthesizeTelephony: async (req) => {
			return {
				audioBuffer: await synthesizeConfiguredGoogleTts(req),
				outputFormat: "pcm",
...
//#endregion
export { buildGoogleSpeechProvider as t };
