...
import { i as setPathExistingStrict, n as getPath } from "./path-utils-BQoJTFAB.js";
...
import { Cn as validateSecretsResolveResult } from "./src-Cy32TawB.js";
...
const WEB_RUNTIME_SECRET_TARGET_ID_PREFIXES = ["plugins.entries."];
const WEB_RUNTIME_SECRET_PATH_PREFIXES = ["plugins.entries."];
...
if (process.env.VITEST || false) globalThis[Symbol.for("openclaw.commandSecretGatewayTestApi")] = testing;
...
function classifyRuntimeWebTargetPathState(params) {
	...
	if (pluginId) {
		if (params.path.endsWith(".config.webFetch.apiKey")) {
			...
			if (!configuredPluginId) return "unknown";
...
function collectActiveGatewayExecSecretRefCredentialPaths(config) {
	...
	return ALL_GATEWAY_SECRET_INPUT_PATHS.filter((path) => {
		...
		return ref?.source === "exec" && gatewaySecretInputPathCanWin({
			config,
			path,
			env: process.env
		});
...
async function resolveCommandSecretRefsViaGateway(params) {
	...
	if (analyzed.unresolved.length > 0) try {
		...
		const locallyResolvedPaths = /* @__PURE__ */ new Set();
		for (const unresolved of analyzed.unresolved) {
			...
			if (localState !== "resolved_local") continue;
		...
		if (stillUnresolved.length > 0) {
			...
			diagnostics = dedupeDiagnostics([...diagnostics, ...buildUnresolvedDiagnostics(params.commandName, stillUnresolved, mode)]);
			for (const unresolved of stillUnresolved) targetStatesByPath[unresolved.path] = "unresolved";
		} else if (locallyResolvedPaths.size > 0) diagnostics = dedupeDiagnostics([...diagnostics, `${params.commandName}: resolved ${locallyResolvedPaths.size} secret ${locallyResolvedPaths.size === 1 ? "path" : "paths"} locally after the gateway snapshot was incomplete.`]);
	} catch (error) {
