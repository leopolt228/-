...
import { f as normalizeResolvedSecretInputString, p as normalizeSecretInputString } from "./types.secrets-BgE_Zq2x.js";
import { s as sleepWithAbort } from "./src-DKBD8PDy.js";
import { o as warn } from "./globals-DBBT7Ru5.js";
...
import { i as isProviderAuthProfileConfigured, s as resolveProviderAuthProfileApiKey } from "./provider-auth-Bnib2g6h.js";
...
import { a as resolveOpenAIProviderConfigRecord, i as readRealtimeErrorDetail, n as createOpenAIRealtimeClientSecret, o as trimToUndefined, t as captureOpenAIRealtimeWsClose } from "./realtime-provider-shared-DQQjj1_9.js";
...
async function resolveOpenAIRealtimePlatformAuth(params) {
	...
	if (hasConfiguredApiKeyProfile || hasOpenAIRealtimeApiKeyInput(void 0)) return { status: "missing" };
	return { status: "missing" };
}
...
var OpenAIRealtimeVoiceBridge = class OpenAIRealtimeVoiceBridge {
	...
	acknowledgeMark(markName) {
		const index = markName === void 0 ? 0 : this.markQueue.indexOf(markName);
	...
	async doConnect() {
		await new Promise((resolve, reject) => {
			...
			const openWebSocket = (connection) => {
				...
				const ws = new WebSocket$1(connection.url, {
	...
	buildAzureDeploymentSessionUpdate() {
		...
		const tools = normalizeOpenAIRealtimeTools(cfg.tools, AZURE_OPENAI_REALTIME_TOOL_NAME_MAX_LENGTH);
		return {
			type: "session.update",
			session: {
	...
	buildTurnDetectionConfig(options) {
		...
		return {
			type: "server_vad",
			threshold: this.config.vadThreshold ?? .5,
			...
			...options?.includeInterruptResponse ? { interrupt_response: this.config.interruptResponseOnInputAudio ?? configuredAutoResponse } : {}
		};
	}
	...
	handleEvent(event) {
		...
		switch (event.type) {
			...
			case "error": {
				...
				if (rejectsManualResponseCreate && detail.startsWith(OPENAI_REALTIME_ACTIVE_RESPONSE_ERROR_PREFIX)) {
					this.responseActive = true;
					this.responseCreateInFlight = false;
					this.manualResponseCreateEventId = null;
...
async function createOpenAIRealtimeBrowserSession(req) {
	...
	if (isOpenAIGptLiveModel(model)) throw new Error(OPENAI_GPT_LIVE_BROWSER_SESSION_UNSUPPORTED_MESSAGE);
	const auth = await requireOpenAIRealtimePlatformAuth({
...
function buildOpenAIRealtimeVoiceProvider() {
	return {
		...
		createBridge: (req) => {
			const config = normalizeProviderConfig(req.providerConfig);
			if (isOpenAIGptLiveModel(config.model)) throw new Error(OPENAI_GPT_LIVE_BRIDGE_UNSUPPORTED_MESSAGE);
