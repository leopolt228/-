...
function getDiagnosticEventListenerPresence() {
	...
	if (existing && typeof existing === "object" && existing.marker === DIAGNOSTIC_EVENT_LISTENER_PRESENCE_KEY) return existing;
	const state = {
...
function dispatchDiagnosticEvent(state, enriched, metadata, privateData, options = {}) {
	...
	try {
		if (!options.trustedListenersOnly) for (const listener of state.listeners) try {
...
function scheduleAsyncDiagnosticDrain(state) {
	...
	setImmediate(() => {
		...
		for (const entry of batch) dispatchDiagnosticEvent(state, entry.event, entry.metadata, entry.privateData, { trustedListenersOnly: entry.trustedListenersOnly });
...
//#endregion
export { parseDiagnosticTraceparent as A, createDiagnosticTraceContextFromActiveScope as C, isValidDiagnosticSpanId as D, getActiveDiagnosticTraceContext as E, hasInternalDiagnosticEventListeners as M, isValidDiagnosticTraceFlags as O, createDiagnosticTraceContext as S, freezeDiagnosticTraceContext as T, onTrustedToolExecutionEvent as _, emitInternalDiagnosticEvent as a, waitForDiagnosticEventsDrained as b, emitTrustedSecurityEvent as c, hasPendingInternalDiagnosticEvent as d, isDiagnosticsEnabled as f, onTrustedInternalDiagnosticEvent as g, onInternalDiagnosticEvent as h, emitFailoverEvent as i, runWithDiagnosticTraceContext as j, isValidDiagnosticTraceId as k, emitTrustedSkillUsedDiagnosticEvent as l, onDiagnosticEvent as m, emitDiagnosticEvent as n, emitTrustedDiagnosticEvent as o, isInternalDiagnosticEventMetadata as p, emitDiagnosticEventWithTrustedTraceContext as r, emitTrustedDiagnosticEventWithPrivateData as s, areDiagnosticsEnabledForProcess as t, getInternalDiagnosticEventSequence as u, resetDiagnosticEventsForTest as v, formatDiagnosticTraceparent as w, createChildDiagnosticTraceContext as x, setDiagnosticsEnabledForProcess as y };
