...
import { d as markMigrationItemSkipped, v as summarizeMigrationItems } from "./migration-nGWjmzKy.js";
import { t as backupCreateCommand } from "./backup-CtKaSHMU.js";
import { n as resolvePluginMigrationProvider, r as resolvePluginMigrationProviders, t as ensureStandaloneMigrationProviderRegistryLoaded } from "./migration-provider-runtime-DxNZRKbi.js";
import { n as buildMigrationReportDir, t as buildMigrationContext } from "./context-D-0Vtt7A.js";
import { n as assertApplySucceeded, o as writeApplyResult, r as assertConflictFreePlan, t as MIGRATION_CONFLICT_REASON_PHRASES } from "./output-CUkJyk6c.js";
...
function applyMigrationItemSelection(plan, selectedItemIds) {
	...
	const items = plan.items.map((item) => selectableIds.has(item.id) && !selected.has(item.id) ? markMigrationItemSkipped(item, MIGRATION_NOT_SELECTED_REASON$1) : item);
	return {
		...plan,
		items,
...
async function runMigrationApply(params) {
	...
	if (!params.opts.allowPartialResult) assertApplySucceeded(withBackup);
	return withBackup;
}
//#endregion
