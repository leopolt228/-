...
import { l as resolveConfigPath, x as resolveStateDir } from "./paths-CHQRdQZ3.js";
import { t as formatCliCommand } from "./command-format-H_Arqann.js";
import { a as inspectPathPermissions, i as formatPermissionRemediation, r as formatPermissionDetail } from "./permissions-sY2quqHz.js";
import { c as emitTrustedSecurityEvent } from "./diagnostic-events-Dt41CZkD.js";
...
import { o as listRiskyConfiguredSafeBins } from "./exec-safe-bin-trust-BWRYCL4x.js";
import { i as resolveSandboxConfigForAgent } from "./config-ZQnZeh2f.js";
import { l as isInterpreterLikeAllowlistPattern } from "./risks-BpC_U-rz.js";
import { K as resolveExecApprovalsFromFile, S as minSecurity, b as maxAsk, v as loadExecApprovals } from "./exec-approvals-BWcbplqx.js";
import { t as DEFAULT_GATEWAY_HTTP_TOOL_DENY } from "./dangerous-tools-CKOgTnGD.js";
import { i as resolveMergedSafeBinProfileFixtures, n as listInterpreterLikeSafeBins } from "./exec-safe-bin-runtime-policy-Dxbvzdgj.js";
import { t as resolveExecDefaults } from "./exec-defaults-Bk6w9ufW.js";
import { r as materializeGatewayAuthSecretRefs, t as canMaterializeGatewayAuthSecretRefsWithoutExec } from "./auth-config-utils-D4OWSnZE.js";
import { n as normalizeConfiguredTrustedSafeBinDirs, t as normalizeConfiguredSafeBins } from "./exec-safe-bin-config-CKyThdf2.js";
import { t as resolveGatewayAuthTokenSourceConflict } from "./auth-token-source-conflict-Z8iaw20w.js";
import { r as collectCoreInsecureOrDangerousFlags } from "./dangerous-config-flags-current-ExokZh8J.js";
import { t as collectEnabledInsecureOrDangerousFlags } from "./dangerous-config-flags-BhfD548e.js";
import { t as collectExecFilesystemPolicyDriftHits } from "./exec-filesystem-policy-D5D1_LHz.js";
...
function collectGatewayConfigFindings$1(cfg, sourceConfig, env, options = {}) {
	...
	if (cfg.mcp?.apps?.enabled === true) findings.push({
		...
		remediation: "Keep this enabled only for MCP servers you trust. Disable with `openclaw config set mcp.apps.enabled false --strict-json` when it is not needed."
	});
	...
	if (auth.mode === "trusted-proxy") {
		...
		if (trustedProxiesLocal.length === 0) findings.push({
		...
		if (trustedProxyConfig?.deviceAutoApprove?.enabled === true) {
			findings.push({
				...
				detail: "gateway.auth.trustedProxy.deviceAutoApprove.enabled=true delegates new Control UI and WebChat device pairing entirely to the reverse-proxy identity.",
			...
			if (trustedProxyConfig.deviceAutoApprove.scopes?.some((scope) => scope.trim() === "operator.admin")) findings.push({
				...
				detail: "gateway.auth.trustedProxy.deviceAutoApprove.scopes includes operator.admin, so every proxy-authenticated user can auto-approve a new browser device with full admin; requests without scopes receive full admin automatically.",
				remediation: "Remove operator.admin and approve admin access manually, or use per-identity roles when they become available."
			});
		}
...
async function readBoundedMcporterRegistry(stateDir) {
	...
	try {
		...
		const chunks = [];
		const scratch = Buffer.allocUnsafe(Math.min(READ_CHUNK_SIZE, 16777217));
		let total = 0;
		while (true) {
			const { bytesRead } = await handle.read(scratch, 0, scratch.length, null);
			if (bytesRead === 0) break;
			total += bytesRead;
			if (total > MAX_MCPORTER_REGISTRY_BYTES) return {
				status: "rejected",
				reason: "oversized"
			};
...
const loadReadOnlyChannelPlugins = createLazyRuntimeModule(() => import("./read-only-BUP9acPr.js"));
...
const loadPluginMetadataRegistryLoaderModule = createLazyRuntimeModule(() => import("./metadata-registry-loader-F74ZGomi.js"));
const loadPluginAutoEnableModule = createLazyRuntimeModule(() => import("./plugin-auto-enable-CetzpwqE.js"));
const loadChannelPluginIdsModule = createLazyRuntimeModule(() => import("./channel-plugin-ids-BVmsqdeo.js"));
const loadPluginRuntimeModule = createLazyRuntimeModule(() => import("./runtime-BITw4f07.js"));
const loadGatewayProbeDeps = createLazyRuntimeModule(() => Promise.all([
	import("./call-Au-Dq1sZ.js"),
	import("./probe-auth-D9cCKY-K.js"),
	import("./probe-XViXBYH9.js")
