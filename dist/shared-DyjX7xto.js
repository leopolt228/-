...
import { n as createProviderOperationDeadline, p as resolveProviderHttpRequestConfig, r as createProviderOperationTimeoutResolver, s as pollProviderOperationJson } from "./shared-CpiwWgfg.js";
...
function resolveVydraBaseUrlFromConfig(cfg) {
	const vydra = asObject(asObject(asObject(asObject(cfg)?.models)?.providers)?.vydra);
...
async function downloadVydraAsset(params) {
	...
	const response = await fetchWithTimeout(params.url, { method: "GET" }, resolveTimeoutMs(), params.fetchFn);
