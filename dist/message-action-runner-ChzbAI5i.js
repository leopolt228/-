...
import { a as actionHasResourceReference, c as resolveActionDeliveryTargetAlias, l as hasPotentialPluginActionParam, o as actionHasTarget, r as applyTargetToParams, s as actionRequiresTarget } from "./channel-target-BqlEv3Xv.js";
import { r as shouldAttemptTtsPayload } from "./tts-config-Cl3uWggE.js";
...
import { _ as readStringParam, h as readStringArrayParam, p as readPositiveIntegerParam, y as readSnakeCaseParamRaw } from "./common-C39GdgQ7.js";
import { s as stripPlainTextToolCallBlocks } from "./src-CeNsIwRl.js";
...
import { O as normalizeOutboundLocation, m as resolveSendableOutboundReplyParts } from "./reply-payload-CPcXnHho.js";
import { _ as renderMessagePresentationFallbackText, d as normalizeLegacyInteractiveReply, f as normalizeMessagePresentation, n as hasLegacyInteractiveReplyBlocks, o as hasReplyPayloadContent, r as hasMessagePresentationBlocks } from "./payload-Br8oiJ5V.js";
import { u as stripUnsupportedCitationControlMarkers } from "./payloads-BfQIm4rr.js";
...
import { a as reconcileTerminalSourceReplyDelivery, c as hydrateAttachmentParamsForAction, d as parseInteractiveParam, f as parseJsonMessageParam, h as resolveExtraActionMediaSourceParamKeys, l as normalizeSandboxMediaList, m as resolveAttachmentMediaPolicy, n as cancelTerminalSourceReplyDelivery, o as dispatchChannelMessageAction, p as readBooleanParam, r as isDeliveredCurrentSourceReply, s as collectActionMediaSourceHints, t as beginTerminalSourceReplyDelivery, u as normalizeSandboxMediaParams } from "./source-reply-mirror-B-2zRtLs.js";
import { i as enforceMessageActionAllowlist, n as buildCrossContextDecoration, o as resolveEffectiveMessageToolsConfig, r as enforceCrossContextPolicy, s as shouldApplyCrossContextMarker, t as applyCrossContextDecoration } from "./outbound-policy-CUvR3Gsw.js";
import { n as resolveAgentIdentity, o as resolveResponsePrefix } from "./identity-DV846zOa.js";
import { i as throwIfAborted } from "./deliver-Cui1uOGS.js";
...
import { n as listConfiguredMessageChannels, r as resolveMessageChannelSelection } from "./channel-selection-DA0nSDDM.js";
import { n as sendPoll, r as resolveOutboundMessageGatewayOptions, t as sendMessage } from "./message-CA46AR6r.js";
...
function normalizeMessageActionInput(params) {
	...
	if (actionRequiresTarget(action) && (!actionHasTarget(action, normalizedArgs, { channel: inferredChannel }) || hasResourceReference && !hasCanonicalTarget)) throw new Error(`Action ${action} requires a target.`);
...
function markDeliveredCurrentSourceReply(result, params) {
	if (result.kind !== "send" || params.input.sourceReplyDeliveryMode !== "message_tool_only") return result;
	const authorization = params.input.messageActionAuthorization;
	if (!authorization?.toolContext || !isDeliveredCurrentSourceReply({
		action: "send",
		channel: params.channel,
...
async function resolveChannel(cfg, params, toolContext, action) {
...
function hasPotentialActionTargetInput(input, params) {
	return Boolean(hasExplicitSingularTargetParam(params) || normalizeOptionalString(input.toolContext?.currentChannelId) || normalizeOptionalString(input.toolContext?.currentMessagingTarget) || hasPotentialPluginActionParam(params));
}
...
async function runGatewayPluginMessageActionOrNull(params) {
	...
	const terminalDeliveryReceipt = callerOwnsTerminalReceipt ? await beginTerminalSourceReplyDelivery(sourceReplyMirror) : void 0;
...
async function buildSendPayloadParts(params) {
	...
	if (location && (message.trim() || mergedMediaUrls.length > 0 || hasPresentation || hasInteractive)) throw new Error("Location sends cannot be combined with message text or media.");
...
async function handleSendAction(ctx) {
	...
	const gatewayPluginAction = requiresCoreDelivery ? null : await runGatewayPluginMessageActionOrNull({
		cfg,
		params,
		channel,
		action,
	...
	if (gatewayPluginAction) return markDeliveredCurrentSourceReply(gatewayPluginAction, {
		cfg,
		actionParams: params,
		channel,
		accountId,
	...
	const useCorePresentationDelivery = Boolean(sendPayload.payload.presentation && hasCorePresentationDelivery(resolveOutboundChannelPlugin({
		channel,
		cfg
	})?.outbound));
...
async function runMessageAction(input) {
	...
	if (actionRequiresTarget(action) && !hasPotentialActionTargetInput(input, params)) throw new Error(`Action ${action} requires a target.`);
