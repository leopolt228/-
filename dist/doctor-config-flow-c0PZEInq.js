...
import { t as CONFIG_PATH } from "./paths-CHQRdQZ3.js";
import { t as formatCliCommand } from "./command-format-H_Arqann.js";
import "./utils-K2PjeLaV.js";
import { h as parseLegacySecretRefEnvMarker, i as LEGACY_SECRETREF_ENV_MARKER_PREFIX } from "./types.secrets-BgE_Zq2x.js";
...
import { n as discoverConfigSecretTargets } from "./target-registry-query-DKaR_5Cb.js";
import "./target-registry-B8VdrXt8.js";
import { J as resolveNormalizedProviderModelMaxTokens } from "./io-CEgS2K9F.js";
import { r as collectConfiguredModelRefs } from "./configured-model-refs-wBBEGQ5a.js";
import { t as splitTrailingAuthProfile } from "./model-ref-profile-BIKs-96s.js";
import { n as getBundledChannelPlugin, o as hasBundledChannelPackageSetupFeature } from "./bundled-CX_lU3gw.js";
import { r as normalizeTalkSection } from "./talk-CRJyHUve.js";
import { n as formatConfigIssueLines } from "./issue-format-BfBp97Wi.js";
import { c as callGateway } from "./call-ChM1o8yU.js";
import { a as runPluginSetupConfigMigrations } from "./setup-registry-DIXn9uxN.js";
...
import { n as isCommonSingleAccountPromotionKey, t as collectSingleAccountPromotionEntries } from "./setup-promotion-keys-8uS1jFgo.js";
import { a as isBlockedLegacyCodexModelRef } from "./codex-route-model-ref-DLaEiamX.js";
import { c as mergeMissing, r as isLegacyModelsAddCodexMetadataModel } from "./legacy-config-migrations.runtime.models-Cm95tnMp.js";
import { a as noteSandboxOriginProxyWarning, i as noteOpencodeProviderOverrides, n as noteImplicitFallbackClobberWarnings, s as stripUnknownConfigKeys } from "./doctor-config-analysis-BfL8Qb8f.js";
import { a as legacyRuntimeModelAliasRequiresRuntimePolicy, i as migrateLegacyXSearchConfig, o as listLegacyRuntimeModelProviderAliases, r as migrateLegacyWebSearchConfig, s as migrateLegacyRuntimeModelRef } from "./legacy-config-migrations-Blf5WtS0.js";
import { n as ensureRecord, r as hasOwnKey, t as cloneRecord } from "./legacy-config-record-shared-B7RruIeK.js";
import { n as applyChannelDoctorCompatibilityMigrations, t as migrateLegacyConfig } from "./legacy-config-migrate-CwWuLgY9.js";
import { t as runDoctorConfigPreflight } from "./doctor-config-preflight-B32hMZGR.js";
import { n as cronCodexRuntimePolicyTargetKey } from "./store-migration-9VgEllds.js";
...
function modelEntryWithRuntimePolicy(entry, runtime) {
	...
	const currentRuntime = isRecord(base.agentRuntime) ? normalizeOptionalLowercaseString(base.agentRuntime.id) : void 0;
...
function resolveLegacyFetchConfig(raw) {
	...
	return isRecord(web?.fetch) ? web.fetch : void 0;
}
...
function migratePluginWebFetchConfig(params) {
	...
	const existing = isRecord(config.webFetch) ? cloneRecord(config.webFetch) : void 0;
...
function migrateLegacyWebFetchConfig(raw) {
	if (!isRecord(raw) || !hasMappedLegacyWebFetchConfig(raw)) return {
		config: raw,
		changes: []
	};
...
function normalizeLegacyWebFetchConfigRecord(raw) {
	...
	for (const [key, value] of Object.entries(fetch)) {
		if (key === "firecrawl" && isRecord(value)) continue;
...
async function loadAndMaybeMigrateDoctorConfig(params) {
	...
	const { collectBlockedLegacyOpenAICodexProviderPlan } = await import("./legacy-config-migrations.runtime.models-BSR1R2E2.js");
	...
	if (preflight.cronCodexRuntimePolicyTargets?.length) {
		const { repairCronCodexRuntimePolicies } = await import("./runtime-policy-migration-DOhCsO3Z.js");
	...
	const { applyPluginAutoEnable } = await import("./plugin-auto-enable-CetzpwqE.js");
	...
	const { repairStaleAgentModelRefs } = await import("./stale-agent-model-ref-repair-Bkm-hpls.js");
	...
	if (staleAgentModelRepair.warnings.length > 0) emitDoctorNotes({
		note,
		warningNotes: staleAgentModelRepair.warnings
	});
	({cfg, candidate, pendingChanges, fixHints} = applyDoctorConfigMutation({
		state: {
			cfg,
	...
	const { collectPluginToolAllowlistWarnings } = await import("./plugin-tool-allowlist-warnings-BFijJlHh.js");
	...
	if (hasConfiguredChannels) {
		const channelDoctor = await import("./channel-doctor-DovKDfVn.js");
	...
	const { repairHooksTokenReuseGatewayAuth } = await import("./hooks-token-reuse-repair-BbYrtg8J.js");
	...
	if (shouldRepair) {
		const { runDoctorRepairSequence } = await import("./repair-sequencing-IYGmiVxy.js");
		...
		const { collectDoctorPreviewNotes } = await import("./preview-warnings-zb-XkfAY.js");
