...
import { i as mergeDeliveryContext, o as normalizeDeliveryContext } from "./delivery-context.shared-D6zu5SGz.js";
...
function consumeSystemEventEntries(sessionKey, consumedEntries) {
	...
	if (consumedEntries.length > entry.queue.length || !consumedEntries.every((event, index) => areSystemEventsEqual(expectDefined(entry.queue[index], "queue entry at index"), event))) return consumeSelectedSystemEventEntries(key, consumedEntries);
