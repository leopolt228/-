import { d as resolveHomePath, i as isDirectory, r as exists, u as readText } from "./helpers-C5lweg-X.js";
...
async function discoverOpenCodeAuthPath(params) {
	...
	const candidates = Array.from(new Set([
		...
		...params.includeHomeFallback ? [path.join(path.resolve(params.env.HOME?.trim() || params.env.USERPROFILE?.trim() || resolveHomePath("~")), OPENCODE_AUTH_RELATIVE_PATH)] : []
	].filter((candidate) => Boolean(candidate))));
	for (const candidate of candidates) if (await exists(candidate)) return candidate;
}
async function discoverHermesSource(input, options = {}) {
	const env = options.env ?? process.env;
	const platform = options.platform ?? process.platform;
	const explicitInput = input?.trim();
	const root = explicitInput ? resolveHomePath(explicitInput) : await resolveImplicitHermesRoot(env, platform);
	...
	const globalRoot = !explicitInput && path.basename(profileParent) === "profiles" ? path.dirname(profileParent) : void 0;
	const globalAuthPath = globalRoot ? path.join(globalRoot, "auth.json") : void 0;
	const archivePaths = [];
...
async function resolveImplicitHermesRoot(env, platform) {
	...
	if (configuredHome) {
		...
		if (supervisedChild || path.basename(path.dirname(configuredRoot)) === "profiles") return configuredRoot;
		return await resolveActiveHermesProfile(configuredRoot);
	}
	const userHome = (platform === "win32" ? env.USERPROFILE?.trim() : env.HOME?.trim()) || resolveHomePath("~");
	let root;
	if (platform === "win32") {
	...
	} else root = path.resolve(userHome, ".hermes");
	return supervisedChild ? root : await resolveActiveHermesProfile(root);
}
async function resolveActiveHermesProfile(root) {
	const activeProfile = (await readText(path.join(root, "active_profile")))?.trim();
	if (!activeProfile || activeProfile === "default" || !HERMES_PROFILE_RE.test(activeProfile)) return root;
	const profileRoot = path.join(root, "profiles", activeProfile);
	return await isDirectory(profileRoot) ? profileRoot : root;
}
async function hasHermesState(root) {
	for (const marker of HERMES_STATE_MARKERS) if (await exists(path.join(root, marker))) return true;
	return false;
}
function hasHermesSource(source) {
	return Boolean(source.configPath || source.envPath || source.authPath || source.globalAuthPath || source.soulPath || source.agentsPath || source.memoryPath || source.userPath || source.skillsDir || source.archivePaths.length > 0);
}
//#endregion
