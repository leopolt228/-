...
import { _ as randomIdempotencyKey } from "./call-ChM1o8yU.js";
import { t as quoteCliArg } from "./quote-cli-arg-BriMa9wW.js";
import { a as writeScreenRecordToFile, c as parseCameraClipPayload, d as resolveCameraSnapTargets, f as writeCameraClipPayloadToFile, l as parseCameraSnapPayload, p as writeCameraPayloadToFile, r as screenRecordTempPath, s as cameraTempPath, t as parseScreenRecordPayload, u as resolveCameraClipTarget } from "./nodes-screen-C1SJ14fK.js";
import { n as parsePairingList, t as parseNodeList } from "./node-list-parse-BlhiSeiu.js";
...
import { n as withConsoleLogsRoutedToStderrForJson } from "./json-output-mode-DgaytQ3L.js";
...
function registerNodesCameraCommands(nodes) {
	...
	nodesCallOpts(camera.command("snap").description("Capture a photo from a node camera (prints the saved path)").requiredOption("--node <idOrNameOrIp>", "Node id, name, or IP").option("--facing <front|back|both>", "Camera facing", "both").option("--device-id <id>", "Camera device id (from nodes camera list)").option("--max-width <px>", "Max width in px (optional)").option("--quality <0-1>", "JPEG quality (optional; platform-specific default)").option("--delay-ms <ms>", "Delay before capture in ms (optional; platform-specific default)").option("--invoke-timeout <ms>", "Node invoke timeout in ms (default 20000)", "20000").action(async (opts) => {
		await runNodesCommand("camera snap", async () => {
			...
			const facing = facingOpt === "both" || facingOpt === "front" || facingOpt === "back" ? facingOpt : (() => {
	...
	nodesCallOpts(camera.command("clip").description("Capture a short video clip from a node camera (prints the saved path)").requiredOption("--node <idOrNameOrIp>", "Node id, name, or IP").option("--facing <front|back>", "Camera facing", "front").option("--device-id <id>", "Camera device id (from nodes camera list)").option("--duration <ms|10s|1m>", "Duration (default 3000ms; supports ms/s/m, e.g. 10s)", "3000").option("--no-audio", "Disable audio capture").option("--invoke-timeout <ms>", "Node invoke timeout in ms (default 90000)", "90000").action(async (opts) => {
		await runNodesCommand("camera clip", async () => {
			...
			const target = resolveCameraClipTarget({
				facing: parseFacing(opts.facing ?? "front"),
				platform: node.platform
			});
...
function registerNodesNotifyCommand(nodes) {
	nodesCallOpts(nodes.command("notify").description("Send a local notification on a node").requiredOption("--node <idOrNameOrIp>", "Node id, name, or IP").option("--title <text>", "Notification title").option("--body <text>", "Notification body").option("--sound <name>", "Notification sound").option("--priority <passive|active|timeSensitive>", "Notification priority").option("--delivery <system|overlay|auto>", "Delivery mode", "system").option("--invoke-timeout <ms>", "Node invoke timeout in ms (default 15000)", "15000").action(async (opts) => {
...
function formatLastActive(now, lastActiveAtMs) {
	return typeof lastActiveAtMs === "number" && Number.isFinite(lastActiveAtMs) ? formatTimeAgo(Math.max(0, now - lastActiveAtMs)) : null;
}
...
async function registerNodesCli(program, argv = process.argv) {
	const nodes = program.command("nodes").description("Manage gateway-owned nodes (pairing, status, invoke, and media)").addHelpText("after", () => `\n${theme.heading("Examples:")}\n${formatHelpExamples([
		...
		["openclaw nodes invoke --node <id> --command system.which --params '{\"bins\":[\"uname\"]}'", "Invoke a node command directly."],
