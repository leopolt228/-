...
import { D as resolveMemoryRemDreamingConfig, E as resolveMemoryLightDreamingConfig, T as resolveMemoryDreamingWorkspaces, v as formatMemoryDreamingDay } from "./dreaming-BmyNO7Dv.js";
import { S as parseUsageCountedSessionIdFromFileName } from "./paths-BpMRJ7TJ.js";
...
import { g as listSessionTranscriptCorpusEntriesForAgent, l as buildSessionEntry, p as sessionPathForFile } from "./engine-qmd-M1vAwevo.js";
import "./memory-core-host-engine-qmd-B1m1U0B5.js";
import "./memory-core-host-status-C_IY4Tnv.js";
import { _ as writeMemoryCoreWorkspaceEntries, g as readMemoryCoreWorkspaceEntries, m as normalizeMemoryCoreWorkspaceKey, n as DREAMING_SESSION_INGESTION_FILES_NAMESPACE, r as DREAMING_SESSION_INGESTION_SEEN_NAMESPACE, t as DREAMING_DAILY_INGESTION_NAMESPACE } from "./dreaming-state-B_O8tXV-.js";
import "./dreaming-shared-COCFY4u9.js";
import { t as appendFailedDreamingEvent } from "./dreaming-events-lb_bQi34.js";
import { i as readRecentDreamDiaryEntries, o as runDetachedDreamNarrative, r as generateAndAppendDreamNarrative } from "./dreaming-narrative-B5zCquwr.js";
import { t as writeDailyDreamingPhaseBlock } from "./dreaming-markdown-CtlMBhLk.js";
import { n as textSimilarity } from "./tokenize-CHt3MKP-.js";
import { c as filterFreshLightDreamingEntries, g as recordDreamingPhaseSignals, h as readShortTermRecallEntries, l as filterLiveShortTermRecallEntries, m as readLightStagedKeys, v as recordRemConsideredPhaseSignals, y as recordShortTermRecalls } from "./short-term-promotion-DpDDtSH8.js";
...
function buildSessionScopeKey(agentId, sessionId) {
	return `${agentId}:${parseUsageCountedSessionIdFromFileName(`${sessionId}.jsonl`) ?? sessionId}`;
}
function buildSessionFileScopeKey(agentId, absolutePath) {
	const fileName = path.basename(absolutePath);
	return buildSessionScopeKey(agentId, parseUsageCountedSessionIdFromFileName(fileName) ?? fileName);
}
...
function buildSessionStateKey(agentId, sessionPath) {
	return `${agentId}:${sessionPath}`;
}
function buildSqliteDreamingSessionPath(agentId, sessionId) {
...
function buildSessionRenderedLine(params) {
	return truncateUtf16Safe(`[${`${params.agentId}/${params.sessionPath}#L${params.lineNumber}`}] ${params.snippet}`, 344);
}
...
async function collectSessionIngestionBatches(params) {
	...
	for (const agentId of agentIds) for (const entry of await listSessionTranscriptCorpusEntriesForAgent(agentId)) {
		...
		sessionFiles.push({
			...
			sessionPath: entry.transcriptSource === "sqlite" ? buildSqliteDreamingSessionPath(entry.agentId, entry.sessionId) : sessionPathForFile(absolutePath),
			...entry.transcriptSource === "sqlite" ? { transcriptSource: "sqlite" } : {},
	...
	for (const file of sortedFiles) {
		...
		if (file.transcriptSource === "sqlite") {
			entry = await buildSessionEntry(file.absolutePath, {
				...
				...file.updatedAtMs !== void 0 ? { updatedAtMs: file.updatedAtMs } : {}
			});
			if (!entry) {
			...
			if (previous !== void 0 && previous.mtimeMs === fingerprint.mtimeMs && previous.size === fingerprint.size && previous.contentHash.length > 0 && cursorAtEnd) {
		...
		const sessionScope = file.transcriptSource === "sqlite" ? `${file.agentId}:${file.sessionPath}` : buildSessionFileScopeKey(file.agentId, file.absolutePath);
		const preFlipSessionScope = file.transcriptSource === "sqlite" ? buildSessionScopeKey(file.agentId, file.sessionId) : void 0;
		const previousSeen = nextSeenMessages[sessionScope] ?? [];
		const seenSet = new Set(previousSeen);
		const preFlipSeenSet = preFlipSessionScope ? new Set(nextSeenMessages[preFlipSessionScope] ?? []) : null;
		const newSeenHashes = [];
		...
		for (let index = cursor; index < lines.length; index += 1) {
			...
			const dedupeBasis = messageTimestampMs > 0 ? `ts:${Math.floor(messageTimestampMs)}` : `line:${lineNumber}`;
			const messageHash = hashSessionMessageId(`${sessionScope}\n${dedupeBasis}\n${snippet}`);
			const preFlipMessageHash = preFlipSessionScope ? hashSessionMessageId(`${preFlipSessionScope}\n${dedupeBasis}\n${snippet}`) : void 0;
			if (seenSet.has(messageHash) || preFlipMessageHash !== void 0 && preFlipSeenSet?.has(preFlipMessageHash)) continue;
...
async function runDreamingSweepPhases(params) {
	...
	if (rem.enabled && rem.limit > 0) try {
...
//#endregion
export { normalizeSessionIngestionState as a, seedHistoricalDailyMemorySignals as c, normalizeDailyIngestionState as i, SESSION_INGESTION_STATE_RELATIVE_PATH as n, previewRemDreaming as o, filterRecallEntriesWithinLookback as r, runDreamingSweepPhases as s, DAILY_INGESTION_STATE_RELATIVE_PATH as t };
