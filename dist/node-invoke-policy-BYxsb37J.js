...
async function persistAllowAlways(input) {
	...
	await mutateConfigFile({
		...
		mutate: (draft) => {
			...
			const candidates = [input.nodeId, input.nodeDisplayName].filter((k) => typeof k === "string" && k.length > 0);
			let entry;
...
async function requestApproval(input) {
	...
	if (approvalDecision === "allow-always") try {
...
async function listDirFetchArchiveEntries(payload) {
	...
	if (entries.length > DIR_FETCH_MAX_ENTRIES) return {
		ok: false,
		code: "ARCHIVE_ENTRIES_TOO_MANY",
		reason: `dir.fetch archive contains more than ${DIR_FETCH_MAX_ENTRIES} entries`
	};
