...
function listExistingAgentIdsFromDisk() {
	...
	try {
		return fs.readdirSync(agentsDir, { withFileTypes: true }).filter((entry) => entry.isDirectory()).map((entry) => normalizeAgentId(entry.name)).filter((id) => id && !isReservedSystemAgentId(id));
	} catch {
		return [];
	}
}
