import { p as chunkTextRanges } from "./tables-DsGSc7Wv.js";
import "./text-chunking-CcRmx-1w.js";
import { t as expectDefined } from "./expect-runtime--WgnKYXT.js";
import { C as createZalouserSendReceipt, _ as sendZaloSeenEvent, g as sendZaloReaction, h as sendZaloLink, m as sendZaloDeliveredEvent, v as sendZaloTextMessage, w as TextStyle, y as sendZaloTypingEvent } from "./zalo-js-CR_CFuBA.js";
...
function parseZalouserTextStyles(input) {
	...
	if (escapeMap.length > 0) {
		...
		plainText = plainText.replace(escapeRegex, (match, index) => escapeMap.at(Number.parseInt(index, 10)) ?? match);
	}
...
function hasClosingFence(lines, startIndex, fence) {
	for (let index = startIndex; index < lines.length; index += 1) {
		...
		if (isClosingFence(fence.quoteIndent > 0 ? stripQuotePrefix(line, fence.quoteIndent).text : line, fence)) return true;
	}
	return false;
}
