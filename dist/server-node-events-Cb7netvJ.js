...
import { K as canonicalizeSessionEntryAliases } from "./session-accessor-Mu3lv_Tl.js";
import { a as normalizeChannelId } from "./registry-DqyhCDsQ.js";
import "./plugins-CJcRWm9n.js";
import { O as resolveAgentHarnessSessionContextError } from "./store-DDuGv_UJ.js";
import { r as deleteMediaBuffer } from "./store-NmJjqmad.js";
...
import { m as resolveGatewayModelSupportsImages, u as loadSessionEntry } from "./session-utils-CEU0rCPC.js";
...
import { $t as validateNodePresenceActivityPayload } from "./src-Cy32TawB.js";
...
import { y as updatePairedDeviceMetadata } from "./device-pairing-DUA4LHep.js";
...
import { a as persistInboundImagesForTranscript, i as parseMessageWithAttachments, o as resolveChatAttachmentMaxBytes, t as normalizeRpcAttachmentsToChatAttachments } from "./attachment-normalize-CgycBNVp.js";
import { m as registerApnsRegistration } from "./push-apns-store-KXfXqjY4.js";
import "./push-apns-Bu5h5nO7.js";
...
function dispatchNodeAgentCommand(ctx, nodeId, input) {
	runWithGatewayIndependentRootWorkContinuation(() => agentCommandFromIngress(input, defaultRuntime, ctx.deps)).catch((err) => {
...
function queueSessionStoreTouch(params) {
	runWithGatewayIndependentRootWorkContinuation(() => touchSessionStore({
...
const handleNodeEvent = async (ctx, nodeId, evt, opts) => {
	switch (evt.event) {
		case "voice.transcript": {
			...
			const { storePath, entry, canonicalKey, storeKeys } = loadSessionEntry(sessionKeyRaw.length > 0 ? sessionKeyRaw : rawMainKey);
		...
		case "agent.request": {
			...
			if (wantsReceipt && deliveryChannel && deliveryTo) runWithGatewayIndependentRootWorkContinuation(() => sendReceiptAck({
				cfg,
				deps: ctx.deps,
				sessionKey: canonicalKey,
			...
			const transcriptMedia = (await persistInboundImagesForTranscript({
				images,
				imageOrder,
				offloadedRefs,
				log: ctx.logGateway,
		...
		case "notifications.changed": {
			...
			const { canonicalKey: sessionKey, entry } = loadSessionEntry(normalizeOptionalString(obj.sessionKey) ?? `node-${nodeId}`);
		...
		case NODE_PRESENCE_ALIVE_EVENT: {
			...
			try {
				if (!await updatePairedDeviceMetadata(deviceId, {
					lastSeenAtMs: now,
					lastSeenReason
				})) return {
					ok: true,
