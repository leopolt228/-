...
import { n as loadPluginManifestRegistry, t as loadBundledPluginManifestRegistry } from "../manifest-registry-DkJa8Tn0.js";
import { a as tryLoadActivatedBundledPluginPublicSurfaceModuleSync, r as loadBundledPluginPublicSurfaceModuleSync } from "../facade-runtime-CZWmCPja.js";
...
function listDeclaredQaRunnerPlugins(env = resolvePrivateQaBundledPluginsEnv()) {
	return (env ? loadBundledPluginManifestRegistry({ env }) : loadPluginManifestRegistry()).plugins.filter((plugin) => Array.isArray(plugin.qaRunners) && plugin.qaRunners.length > 0).toSorted((left, right) => {
...
function listQaRunnerCliContributions() {
	...
	for (const plugin of listDeclaredQaRunnerPlugins(env)) {
		...
		for (const runner of plugin.qaRunners) {
			...
			if (adapterFactory && (adapterFactory.id !== runner.commandName || typeof adapterFactory.matches !== "function" || typeof adapterFactory.create !== "function")) throw new Error(`QA runner plugin "${plugin.id}" exported an invalid transport factory for "${runner.commandName}"`);
			contributions.set(runner.commandName, {
				pluginId: plugin.id,
				commandName: runner.commandName,
				...runner.description ? { description: runner.description } : {},
