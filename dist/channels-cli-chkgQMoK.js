...
import { l as formatUnsupportedChannelActionMessage } from "./error-format-CG7mpTEd.js";
...
import { a as normalizeChannelId, i as listChannelPlugins } from "./registry-DqyhCDsQ.js";
...
import { t as formatCliChannelOptions } from "./channel-options-CKL4O-YY.js";
...
const channelsCommandsLoader = createLazyImportLoader(() => import("./channels-CLQyDOlC.js"));
const bundledPackageChannelMetadataLoader = createLazyImportLoader(() => import("./bundled-package-channel-metadata-vgqBB-uA.js"));
...
function resolveChannelsAddOptions(channelArg, opts) {
	return {
		...opts,
...
async function registerChannelsCli(program, argv = process.argv, options = {}) {
	...
	deadLetters.command("list").description("List failed inbound events for one channel account").requiredOption("--channel <name>", "Channel id").option("--account <id>", "Account id", "default").option("--limit <n>", "Maximum entries", "100").option("--json", "Output JSON", false).action(async (opts) => {
		await runChannelsCommand(async () => {
			const { channelsDeadLettersListCommand } = await import("./dead-letters-BYKkAipY.js");
	...
	deadLetters.command("resubmit").description("Re-enqueue one failed inbound event").argument("<event-id>", "Ingress event id").requiredOption("--channel <name>", "Channel id").option("--account <id>", "Account id", "default").option("--json", "Output JSON", false).action(async (eventId, opts) => {
		await runChannelsCommand(async () => {
			const { channelsDeadLettersResubmitCommand } = await import("./dead-letters-BYKkAipY.js");
	...
	addCommand.action(async (channelArg, opts, command) => {
		await runChannelsCommand(async () => {
			...
			await channelsAddCommand(resolveChannelsAddOptions(channelArg, opts), defaultRuntime, { hasFlags });
		});
	});
