...
import { l as findCatalogTemplate, n as buildManifestModelProviderConfig } from "./provider-catalog-shared-CVTyEDNG.js";
import { a as getCachedLiveProviderModelRows, n as buildLiveModelProviderConfig } from "./provider-catalog-live-runtime-xz7nWUaf.js";
import { c as resolveOpenAIDefaultBaseUrl, i as classifyOpenAIBaseUrl, n as OPENAI_CODEX_RESPONSES_BASE_URL, o as isOpenAICodexBaseUrl, s as isOpenAIHttpsApiBaseUrl } from "./base-url-DuhOo9rF.js";
import { a as OPENAI_DEFAULT_MODEL, c as applyOpenAIConfig, t as OPENAI_CODEX_DEFAULT_MODEL } from "./default-models-C99FJ72C.js";
...
import { a as OPENAI_GPT_54_MINI_MODEL_ID, b as normalizeOpenAIModelRouteId, c as OPENAI_GPT_54_PRO_MODEL_ID, g as OPENAI_PROVIDER_MODERN_MODEL_IDS, l as OPENAI_GPT_55_MODEL_ID, o as OPENAI_GPT_54_MODEL_ID, s as OPENAI_GPT_54_NANO_MODEL_ID, u as OPENAI_GPT_55_PRO_MODEL_ID, v as isOpenAIPlatformOnlyRouteModelId, y as isOpenAISubscriptionOnlyRouteModelId } from "./model-route-contract-DQrf9Dsy.js";
import { n as resolveUnifiedOpenAIThinkingProfile } from "./thinking-policy-Cv-MVerm.js";
import { n as buildOpenAICodexProviderHooks, t as buildOpenAIChatGPTAuthMethods } from "./openai-chatgpt-provider-CbM6GnLo.js";
...
const OPENAI_CODEX_MODELS_ENDPOINT = `${OPENAI_CODEX_RESPONSES_BASE_URL}/models?client_version=0.144.6`;
const OPENAI_MODELS_CACHE_TTL_MS = 6e4;
const OPENAI_CODEX_MODELS_CACHE_TTL_MS = 6e4;
const OPENAI_GPT_56_DIRECT_CONTEXT_TOKENS = 105e4;
...
function normalizeOpenAICodexCatalogModel(model) {
	...
	if (modelId === "gpt-5.6") return;
	if (modelId === "gpt-5.6-sol" || modelId === "gpt-5.6-terra" || modelId === "gpt-5.6-luna") {
		const supportsNativeUltra = modelId === "gpt-5.6-sol" || modelId === "gpt-5.6-terra";
		const supportedReasoningEfforts = model.compat?.supportedReasoningEfforts ? [.../* @__PURE__ */ new Set([...model.compat.supportedReasoningEfforts.filter((effort) => effort !== "none"), ...supportsNativeUltra ? ["ultra"] : []])] : void 0;
		return {
			...model,
...
function resolveAuthoredOpenAIConfigRoute(params) {
	if (normalizeProviderId(params.provider) !== PROVIDER_ID) return;
	const providers = Object.entries(params.config?.models?.providers ?? {});
	const requestedProvider = params.provider.trim();
	const providerKey = providers.find(([providerId]) => providerId.trim() === requestedProvider)?.[0].trim() ?? providers.find(([providerId]) => normalizeProviderId(providerId) === PROVIDER_ID)?.[0].trim();
	let providerConfig;
	for (const [providerId, candidate] of providers) {
		if (providerId.trim() !== providerKey || !candidate) continue;
...
function resolveOpenAIGptForwardCompatModel(ctx) {
	...
	} else if (lower === "gpt-5.6" || lower === "gpt-5.6-sol" || lower === "gpt-5.6-terra" || lower === "gpt-5.6-luna") {
		templateIds = OPENAI_GPT_56_TEMPLATE_MODEL_IDS;
		const cost = lower === "gpt-5.6" || lower === "gpt-5.6-sol" ? OPENAI_GPT_56_SOL_COST : lower === "gpt-5.6-terra" ? OPENAI_GPT_56_TERRA_COST : OPENAI_GPT_56_LUNA_COST;
		patch = {
			api: "openai-responses",
			provider: PROVIDER_ID,
			baseUrl: resolveOpenAIDefaultBaseUrl(),
	...
	return {
		...
		prepareExtraParams: (ctx) => {
			...
			return (shouldUseCodexResponsesHooks({
				...
				api: ctx.model?.api,
				baseUrl: ctx.model?.baseUrl
			}) || normalizeProviderId(ctx.provider) === PROVIDER_ID && (!providerConfig?.baseUrl || isOpenAIHttpsApiBaseUrl(providerConfig.baseUrl)) && resolveConfiguredProviderAuthTransport(providerConfig) === "codex" ? codexResponsesHooks : responsesHooks).prepareExtraParams?.(ctx);
		},
