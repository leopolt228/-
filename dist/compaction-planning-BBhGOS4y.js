...
import { a as stripToolResultDetails, n as repairToolUseResultPairing } from "./session-transcript-repair-RGUYmndm.js";
...
function splitMessagesByTokenShare(messages, parts = DEFAULT_PARTS) {
	...
	for (const [index, message] of messages.entries()) {
		...
		if (messageTokens === void 0) throw new Error("Compaction token estimates are out of sync with messages");
...
function buildOversizedFallbackPlan(params) {
	...
	for (const [index, msg] of params.messages.entries()) {
		...
		if (tokens === void 0) throw new Error("Compaction token estimates are out of sync with messages");
