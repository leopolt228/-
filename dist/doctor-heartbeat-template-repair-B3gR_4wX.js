...
import { r as DEFAULT_HEARTBEAT_FILENAME, v as resolveWorkspaceTemplateDir } from "./workspace-GYctLxSN.js";
...
function heartbeatTemplateAnalysisToHealthFinding(agentId, labelAgent, heartbeatPath, analysis) {
...
async function collectHeartbeatTemplateHealthFindings(cfg, deps) {
	...
	for (const { agentId, heartbeatPath } of targets) {
		...
		if (analysis.status !== "clean") findings.push(heartbeatTemplateAnalysisToHealthFinding(agentId, labelAgents, heartbeatPath, analysis));
	}
	return findings;
}
...
async function maybeRepairHeartbeatTemplate(params) {
	...
	const targets = agentIds.map((agentId) => ({
		agentId,
		heartbeatPath: path.join(resolveAgentWorkspaceDir(params.cfg, agentId), DEFAULT_HEARTBEAT_FILENAME)
	}));
	for (const { agentId, heartbeatPath } of targets) {
		...
		try {
			...
			if (error?.code === "ENOENT") continue;
			note(`${prefix}Could not inspect ${shortenHomePath(heartbeatPath)}: ${formatErrorMessage(error)}`, "Heartbeat template");
			continue;
		}
		const analysis = analyzeHeartbeatTemplateForRepair(content);
		if (analysis.status === "clean") continue;
		if (analysis.status === "dirty-template-with-custom-content") {
			note([`${prefix}${shortenHomePath(heartbeatPath)} contains an older heartbeat template wrapper plus custom or unrecognized content.`, "Doctor left it unchanged so it does not delete user tasks. Remove the fenced template and Related lines manually if they are not intentional."].join("\n"), "Heartbeat template");
			continue;
		}
		if (!params.shouldRepair) {
			note([`${prefix}${shortenHomePath(heartbeatPath)} contains an older heartbeat documentation template.`, "Run \"openclaw doctor --fix\" to replace it with the clean heartbeat template."].join("\n"), "Heartbeat template");
			continue;
		}
		try {
			await writeTextAtomic(heartbeatPath, await readCleanHeartbeatTemplate(), { mode: 384 });
			note(`${prefix}Replaced ${shortenHomePath(heartbeatPath)} with the clean heartbeat template.`, "Doctor changes");
		} catch (error) {
			note(`${prefix}Could not repair ${shortenHomePath(heartbeatPath)}: ${formatErrorMessage(error)}`, "Heartbeat template");
		}
	}
}
//#endregion
