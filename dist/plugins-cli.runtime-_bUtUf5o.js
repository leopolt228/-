...
import { a as formatMissingPluginMessage } from "./error-format-CG7mpTEd.js";
import { a as resolveConfiguredRuntimePluginInstallCandidate, i as collectConfiguredRuntimePluginIds } from "./configured-runtime-plugin-installs-Cs9SeMel.js";
//#region src/cli/plugins-cli.runtime.ts
function createModuleLoader(load) {
	let promise;
	return () => promise ??= load();
}
const loadPluginsConfigState = createModuleLoader(() => import("./config-state-D9__G8-b.js"));
const loadPluginsStatus = createModuleLoader(() => import("./status-B-9UuJnN.js"));
const loadPluginSlotSelection = createModuleLoader(() => import("./slot-selection-BJMauInk.js"));
const loadPluginsCommandHelpers = createModuleLoader(() => import("./plugins-command-helpers-CMSpBaXu.js"));
const loadPluginsRegistryRefresh = createModuleLoader(() => import("./registry-refresh-7ijD11ai.js"));
...
async function runPluginsEnableCommand(idInput) {
	...
	const { enableExplicitlySelectedPluginInConfig } = await import("./enable-D2Vxwd3E.js");
...
async function runPluginsDisableCommand(idInput) {
	...
	const { normalizePluginId } = await loadPluginsConfigState();
	const { buildPluginRegistrySnapshotReport } = await loadPluginsStatus();
	const { setPluginEnabledInConfig } = await import("./plugins-config-BwgKFUoB.js");
	const { refreshPluginRegistryAfterConfigMutation } = await loadPluginsRegistryRefresh();
	const snapshot = await readConfigFileSnapshot();
	const cfg = snapshot.sourceConfig ?? snapshot.config;
	const report = buildPluginRegistrySnapshotReport({ config: cfg });
...
async function runPluginsRegistryCommand(opts) {
	const { inspectPluginRegistry, refreshPluginRegistry } = await import("./plugin-registry-oxJDsekA.js");
...
async function runPluginMarketplaceEntriesCommand(opts) {
	const catalog = await import("./official-external-plugin-catalog-BvK1qIfA.js");
	const cfg = getRuntimeConfig();
	...
	if (summary.trust) lines.push(theme.muted("Trust:") + " " + formatMarketplaceFeedTrust(summary.trust));
...
async function runPluginMarketplaceRefreshCommand(opts) {
	const { loadConfiguredHostedOfficialExternalPluginCatalogEntries } = await import("./official-external-plugin-catalog-BvK1qIfA.js");
	const cfg = getRuntimeConfig();
	...
	const { clearManagedPluginOfficialCatalogCache } = await import("./management-service-5-bn1pes.js");
...
async function runPluginMarketplaceListCommand(source, opts) {
	const { listMarketplacePlugins } = await import("./marketplace-D8UO7Qc-.js");
