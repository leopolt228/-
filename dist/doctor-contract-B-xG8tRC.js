...
import { a as isSupportedRealtimeVoiceActivationName, c as normalizeRealtimeVoiceActivationNamePrefix } from "./realtime-voice-D9eMvxKo.js";
...
function stripRetiredTuningKnobs(value) {
	...
	const next = { ...record };
	let changed = false;
	for (const key of RETIRED_TUNING_KEYS) if (Object.hasOwn(next, key)) {
		delete next[key];
		changed = true;
	}
	return {
...
function stripRetiredDiscordTuningKnobs(value) {
	const rootResult = stripRetiredTuningKnobs(asObjectRecord(value) ?? {});
	const nextRoot = rootResult.value;
...
function hasLegacyDiscordGuildChannelAgentId(value) {
	...
	return Object.values(guilds).some((guildValue) => {
		const channels = asObjectRecord(asObjectRecord(guildValue)?.channels);
		if (!channels) return false;
		return Object.values(channels).some((channel) => Object.hasOwn(asObjectRecord(channel) ?? {}, "agentId"));
	});
}
...
function normalizeDiscordGuildChannelAgentIds(params) {
	...
	for (const [guildId, guildValue] of Object.entries(guilds)) {
		...
		for (const [channelId, channelValue] of Object.entries(channels)) {
			const channel = asObjectRecord(channelValue);
			if (!channel || !Object.hasOwn(channel, "agentId")) continue;
			const nextChannel = { ...channel };
