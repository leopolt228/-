...
function resolveThreadBindingThreadName(params) {
	return truncateUtf16Safe(`🤖 ${normalizeOptionalString(params.label) || normalizeOptionalString(params.agentId) || "agent"}`.replace(/\s+/g, " ").trim(), 100);
}
...
function resolveThreadBindingIntroText(params) {
	const normalized = truncateUtf16Safe((normalizeOptionalString(params.label) || normalizeOptionalString(params.agentId) || "agent").replace(/\s+/g, " ").trim(), 100) || "agent";
