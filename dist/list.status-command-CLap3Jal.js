...
import { a as restoreCurrentPluginMetadataSnapshotState, o as setCurrentPluginMetadataSnapshot, r as getCurrentPluginMetadataSnapshot, t as captureCurrentPluginMetadataSnapshotState } from "./current-plugin-metadata-snapshot-BQju0mzJ.js";
import { b as createConfigIO } from "./io-CEgS2K9F.js";
import { c as shouldEnableShellEnvFallback, n as getShellEnvAppliedKeys } from "./shell-env-4g6GM0d2.js";
import { A as resolveMergedModelProviderConfig, n as OPENAI_PROVIDER_ID } from "./openai-routing-Cq9SwNpx.js";
...
import { S as resolveModelRefFromString, f as modelCatalogLogicalKey, i as buildModelAliasIndex, k as parseModelPolicyWildcardRef, v as resolveConfiguredModelPolicyAllow, y as resolveConfiguredModelRef } from "./model-selection-shared-CPPxIJAX.js";
import { s as loadManifestMetadataSnapshot } from "./manifest-contract-eligibility-DbVdNqi2.js";
import { i as modelKey, o as normalizeProviderId } from "./model-selection-normalize-D7Dhjaxs.js";
...
import { c as isNonSecretApiKeyMarker, g as resolveProviderEnvAuthLookupMaps, h as listProviderEnvAuthLookupKeys, l as isOAuthApiKeyMarker } from "./model-auth-markers-Bqpoo9x7.js";
import { k as resolveProviderSyntheticAuthWithPlugin } from "./provider-runtime-BE5KxvKF.js";
...
import { s as resolveProfileUnusableUntilForDisplay } from "./usage-DaLssncS.js";
import { g as resolveUsableCustomProviderApiKey, s as getCustomProviderApiKey } from "./model-auth-919iJVmy.js";
...
import { n as resolveAgentHarnessOwnerPluginIds, r as resolveAgentHarnessRuntimeAvailability } from "./runtime-plugin-f-lb12_n.js";
...
import { n as resolveUtilityModelRefForAgent, t as readUtilityModelSetting } from "./utility-model-Fw1rnMGN.js";
import { n as ensureFlagCompatibility, s as resolveKnownAgentId } from "./shared-Dys0_Ah-.js";
import { n as buildAuthHealthSummary, r as formatRemainingShort, t as DEFAULT_OAUTH_WARN_MS } from "./auth-health-1Cq6fRMd.js";
import { t as maskApiKey } from "./secret-mask-CWRecbVB.js";
import { t as loadModelsConfig } from "./load-config-BLpHbz3i.js";
import { n as isRich } from "./list.format-D9RXa4-v.js";
...
const providerUsageRuntimeLoader = createLazyImportLoader(() => import("./provider-usage-D0za0v_C.js"));
const progressRuntimeLoader = createLazyImportLoader(() => import("./progress-Dct1gCkK.js"));
const terminalTableRuntimeLoader = createLazyImportLoader(() => import("./terminal-core/table.js"));
const listProbeRuntimeLoader = createLazyImportLoader(() => import("./list.probe-BUZ9oTl8.js"));
...
async function modelsStatusCommand(opts, runtime) {
	...
	const { runPluginPayloadSmokeCheckForManifestRecords } = await import("./plugin-payload-validation-Dx6ipXzS.js");
	...
	try {
		...
		const createStatusAuthResolver = (authStore) => createModelAuthAvailabilityResolver({
			cfg,
			authStore,
			agentDir,
			workspaceDir,
			env: process.env,
			syntheticAuthProviderRefs: [...syntheticAuthProviderRefs].filter((provider) => provider !== "codex"),
			metadataSnapshot
		});
		...
		const resolveProviderUses = async (resolver) => await Promise.all(providerUseRefs.map(async (usage) => {
			const observedRoutes = routeSourcesByModel.get(modelCatalogLogicalKey({
			...
			const rawEvaluation = usage.routeScope === "text" ? resolver.evaluateModelAuth(usage.provider, ref) : {
				availability: resolver.resolveProviderAuthAvailability(usage.provider, ref),
				routeResolution: null
			};
			const routeAuth = await (async () => {
				if (rawEvaluation.routeResolution?.kind === "incompatible") return {
					kind: "incompatible",
				...
				const usesCodexRuntimeAuth = usage.allowCodexRuntimeFallback && resolveAgentHarnessPolicy({
					provider: usage.provider,
					modelId: usage.model,
				...
				if (usesCodexRuntimeAuth && usage.provider !== "openai" && usage.provider !== "codex") return {
					kind: "incompatible",
				...
				if (evaluation.selectedRoute) return {
					kind: "route",
				...
				if (evaluation.routeResolution?.kind === "routes" || evaluation.routeResolution?.kind === "indeterminate") return {
					kind: "indeterminate",
					evaluation,
					usesCodexRuntimeAuth,
					runtimeAvailability
		...
		const cliRuntimeAuthUsages = providerUses.filter((usage) => usage.allowCodexRuntimeFallback && !usage.routeAuth.usesCodexRuntimeAuth).map((usage) => {
		...
		let codexRuntimeAuthUsages = providerUses.filter((usage) => usage.routeAuth.usesCodexRuntimeAuth);
		...
		const hasUsableAuthForProviderInUse = (usage) => {
			...
			if (usage.routeAuth.kind === "incompatible") return true;
		...
		const runtimeAuthRouteEntries = [...Array.from(codexRuntimeUsagesByProvider.entries()).map(([provider, usages]) => {
			const representative = usages.find((usage) => usage.routeAuth.evaluation.availability === true) ?? usages[0];
			const effective = resolveRuntimeAuthRouteEffective(codexProvider, representative?.routeAuth.evaluation);
		...
		const modelRouteIssues = providerUses.flatMap((usage) => {
			const cliRuntimeAuthProvider = resolveCliRuntimeAuthProvider(usage);
			const evaluation = cliRuntimeAuthProvider ? authResolver.evaluateModelAuth(cliRuntimeAuthProvider) : usage.routeAuth.evaluation;
			if (usage.routeAuth.kind === "incompatible") return [{
				kind: "incompatible",
				provider: usage.provider,
				model: usage.model,
			...
			if (usage.routeAuth.kind !== "route" || evaluation.availability) return [];
		...
		const checkStatus = (() => {
			const resolveRouteAuthHealth = (usage) => {
				if (usage.routeAuth.kind === "incompatible") return "missing";
				const cliRuntimeAuthProvider = resolveCliRuntimeAuthProvider(usage);
				const evaluation = cliRuntimeAuthProvider ? authResolver.evaluateModelAuth(cliRuntimeAuthProvider) : usage.routeAuth.evaluation;
		...
		if (missingProvidersInUse.length > 0) {
			const { buildProviderAuthRecoveryHint } = await import("./provider-auth-recovery-hint-BiZIyNJi.js");
			runtime.log("");
			...
			for (const provider of missingProvidersInUse) {
				const hint = buildProviderAuthRecoveryHint({
					provider,
					config: cfg,
					includeEnvVar: !dedupedModelRouteIssues.some((issue) => issue.kind === "missing-auth" && issue.provider === provider && issue.authRequirement === "subscription")
				});
