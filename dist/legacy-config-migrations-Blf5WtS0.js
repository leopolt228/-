...
import { n as DEFAULT_GATEWAY_PORT } from "./paths-CHQRdQZ3.js";
...
import { n as loadBundledPluginPublicArtifactModuleSync } from "./public-surface-loader-DKFjs6ns.js";
...
import { c as mergeAlsoAllowPolicy, f as expandToolGroups, h as resolveToolProfilePolicy } from "./tool-policy-GYMCyycR.js";
import { n as isToolAllowedByPolicyName } from "./tool-policy-match-gf5E9Psx.js";
import { i as resolveOpenClawMcpTransportAlias, n as isKnownCliMcpTypeAlias } from "./mcp-config-normalize-C4v_5S7O.js";
import { n as normalizeToolProviderPolicyKey, t as isCanonicalToolProviderPolicyKey } from "./provider-tool-policy-KyAGXjuf.js";
import { a as ensureRecord$1, c as mergeMissing, i as defineLegacyConfigMigration, o as getRecord, s as mapLegacyAudioTranscription, t as LEGACY_CONFIG_MIGRATIONS_RUNTIME_MODELS } from "./legacy-config-migrations.runtime.models-Cm95tnMp.js";
import { a as resolveGatewayPortWithDefault, i as isGatewayNonLoopbackBindMode, r as hasConfiguredControlUiAllowedOrigins, t as buildDefaultControlUiAllowedOrigins } from "./gateway-control-ui-origins-Wcq5mZWn.js";
import { n as ensureRecord$2, r as hasOwnKey$1, t as cloneRecord$1 } from "./legacy-config-record-shared-B7RruIeK.js";
...
const LEGACY_MEMORY_SEARCH_FLAT_KEY_RULES = [{
	...
	message: "agents.defaults.memorySearch uses legacy flat chunkSize, chunkOverlap, or maxResults fields. Run \"openclaw doctor --fix\".",
...
function hasAgentListLegacyMemorySearchFlatKeys(value) {
	return Array.isArray(value) && value.some((agent) => hasLegacyMemorySearchFlatKeys(getRecord(agent)?.memorySearch));
}
...
const LEGACY_CONFIG_MIGRATIONS_RUNTIME_AGENTS = [
	...
	defineLegacyConfigMigration({
		...
		apply: (raw, changes) => {
			const agents = getRecord(raw.agents);
			migrateLegacyMemorySearchFlatKeys(getRecord(getRecord(agents?.defaults)?.memorySearch), "agents.defaults.memorySearch", changes);
			if (!Array.isArray(agents?.list)) return;
			for (const [index, agent] of agents.list.entries()) migrateLegacyMemorySearchFlatKeys(getRecord(getRecord(agent)?.memorySearch), `agents.list.${index}.memorySearch`, changes);
		}
	}),
	defineLegacyConfigMigration({
...
const LEGACY_CONFIG_MIGRATIONS_RUNTIME_DIAGNOSTICS = [];
//#endregion
...
function deleteRetiredPath(owner, path, index = 0) {
	...
	if (key === "*") {
		let changed = false;
		for (const value of Object.values(record)) changed = deleteRetiredPath(value, path, index + 1) || changed;
		return changed;
	}
	...
	if (!child || !deleteRetiredPath(child, path, index + 1)) return false;
...
const LEGACY_CONFIG_MIGRATIONS_RUNTIME_RETIRED = [defineLegacyConfigMigration({
	...
	legacyRules: [
		...
		rule(["messages", "messagePrefix"], "messages.messagePrefix moved to channels.whatsapp.messagePrefix."),
		rule([
			"tools",
			"media",
			"asyncCompletion"
	...
	apply: (raw, changes) => {
		...
		if (messages && Object.hasOwn(messages, "messagePrefix")) {
			const whatsapp = ensureRecord$1(ensureRecord$1(raw, "channels"), "whatsapp");
			if (whatsapp.messagePrefix === void 0) {
				whatsapp.messagePrefix = messages.messagePrefix;
				changes.push("Moved messages.messagePrefix → channels.whatsapp.messagePrefix.");
			} else changes.push("Removed messages.messagePrefix (channels.whatsapp.messagePrefix already set).");
