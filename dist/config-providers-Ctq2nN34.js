...
import { a as resolveHermesConfiguredProviderId, n as normalizeHermesCustomProviderId } from "./model-Biappd0R.js";
import { c as readProviderHeaders, d as resolveHermesProviderApiKeyEnv, f as resolveHermesProviderBaseUrlEnv, i as readEnvReference, l as resolveHermesEndpointApiKeyEnv, n as collectProviderModels, o as readProviderApiKeyEnv, p as resolveProviderApi, s as readProviderBaseUrl, t as HERMES_TRANSPORTS, u as resolveHermesImplicitBaseUrl } from "./config-provider-contract-C5vqCjFF.js";
...
function collectHermesProviderSecretBindings(config, env = {}) {
	...
	if (Array.isArray(config.custom_providers)) for (const raw of config.custom_providers) {
		...
		if (sourceProvider && envVar) bindings.push({
			envVar,
			provider: resolveHermesConfiguredProviderId(config, sourceProvider, env)
		});
	}
...
function providerManualItems(config, env, includeSecrets) {
	...
	for (const [id, raw] of Object.entries(childRecord(config, "providers"))) if (isRecord(raw)) entries.push({
		id,
		raw,
	...
	if (Array.isArray(config.custom_providers)) for (const raw of config.custom_providers) {
		if (!isRecord(raw)) continue;
		const id = readString(raw.name) ?? readString(raw.id);
		if (id && !currentProviderIds.has(normalizeHermesCustomProviderId(id))) entries.push({
			id,
			raw,
	...
	if (isRecord(config.model)) {
		...
		if (readString(config.model.base_url) ?? readString(config.model.baseUrl)) entries.push({
	...
	for (const { id, raw, source } of entries) {
		...
		if (readString(raw.api_key) && !readEnvReference(raw.api_key)) items.push(createMigrationManualItem({
