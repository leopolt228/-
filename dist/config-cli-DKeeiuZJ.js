...
import { l as resolveConfigPath, t as CONFIG_PATH } from "./paths-CHQRdQZ3.js";
import { a as hasErrnoCode } from "./errors-DdbcjW1Y.js";
import { t as formatCliCommand } from "./command-format-H_Arqann.js";
import { t as formatDocsLink } from "./links-ClIwBcy4.js";
import { r as theme } from "./theme-vjDs9tao.js";
import { i as writeRuntimeJson, r as defaultRuntime, t as ExitError } from "./runtime-ZHfN2VLf.js";
...
import { d as isValidEnvSecretRefId, g as resolveSecretInputRef, l as isSecretRef, s as coerceSecretRef } from "./types.secrets-BgE_Zq2x.js";
import { n as readFileDescriptorBoundedSync } from "./boundary-file-read-BgBHxIxZ.js";
import { a as isValidFileSecretRefId, i as isValidExecSecretRefId, l as secretRefKey, o as isValidSecretProviderAlias, r as formatExecSecretRefIdValidationMessage, u as validateExecSecretRefId } from "./ref-contract-DzV1H2nk.js";
import { a as isPluginIntegrationSecretProviderConfig, n as resolveSecretRefValue, s as resolveSecretProviderIntegrationConfig } from "./resolve-DhgogJwd.js";
import { i as resolveAgentModelPrimaryValue, n as normalizeAgentModelRefForConfig, t as normalizeAgentModelMapForConfig } from "./model-input-B7OGjVYg.js";
import { c as resolveAgentExplicitModelPrimary, d as resolveAgentModelFallbacksOverride } from "./agent-scope-CrBA-6Gx.js";
import { n as normalizeAgentId } from "./agent-id-DDgUze4y.js";
import "./session-key-Drrs61Fd.js";
import { c as resolveDefaultAgentId } from "./agent-scope-config-S7z_Yn4H.js";
import { F as SecretProviderSchema } from "./zod-schema.agent-runtime-CeKzEtj8.js";
...
import { c as resolveConfigSecretTargetByPath, n as discoverConfigSecretTargets } from "./target-registry-query-DKaR_5Cb.js";
import "./target-registry-B8VdrXt8.js";
import { a as success, n as info, o as warn, t as danger } from "./globals-DBBT7Ru5.js";
import { $ as AUTO_MANAGED_CONFIG_META_PATHS, F as validateConfigObjectRawWithPlugins, M as collectUnsupportedSecretRefPolicyIssues, l as readConfigFileSnapshot } from "./io-CEgS2K9F.js";
...
import { i as normalizeConfigIssues, n as formatConfigIssueLines } from "./issue-format-BfBp97Wi.js";
...
import { s as formatStrictJsonParseFailure } from "./error-format-CG7mpTEd.js";
...
import { n as readBestEffortRuntimeConfigSchema } from "./runtime-schema-vxnKajqw.js";
import { n as setCommandJsonMode } from "./json-mode-BcwbsF88.js";
...
function collectTextModelConfigRefs(params) {
	if (typeof params.model === "string") {
		...
		return [{
			path: params.path,
			value,
			...params.agentIndex === void 0 ? {} : { agentIndex: params.agentIndex },
...
function collectTextModelRefs(config) {
	...
	if (Array.isArray(agentList)) for (const [agentIndex, agent] of agentList.entries()) {
...
function modelRefComparisonKey(ref) {
	if (ref.agentId && ref.agentIndex !== void 0) {
...
function collectTouchedTextModelRefs(params) {
	...
	const defaultPrimaryProviderChanged = defaultPrimaryTouched && (!previousRefs || resolveDefaultModelForAgent({ cfg: params.config }).provider !== resolveDefaultModelForAgent({ cfg: params.previousConfig ?? {} }).provider);
	const touchedRefs = refs.filter((ref) => {
		...
		const agentIdPath = ref.agentIndex === void 0 ? void 0 : [
			"agents",
			"list",
			String(ref.agentIndex),
			"id"
		];
		if (agentIdPath && params.touchedPaths.some((touchedPath) => isPathPrefix(agentIdPath, touchedPath))) {
			ref.dependency = true;
			return true;
		}
	...
	const defaultRefs = refs.filter((ref) => ref.agentIndex === void 0);
	const agentList = params.config.agents?.list;
	if (defaultRefs.length === 0) return touchedRefs;
	if (!Array.isArray(agentList) || agentList.length === 0) {
		const listPath = ["agents", "list"];
		const listTouched = params.touchedPaths.some((touchedPath) => isPathPrefix(touchedPath, listPath) || isPathPrefix(listPath, touchedPath));
		const previousList = params.previousConfig?.agents?.list;
		if (!listTouched || !Array.isArray(previousList) || previousList.length === 0) return touchedRefs;
		const previousDefaultAgentId = resolveDefaultAgentId(params.previousConfig ?? {});
		for (const defaultRef of defaultRefs) {
			const previouslyInherited = normalizeAgentId(previousDefaultAgentId) === "main" && params.previousConfig ? defaultRef.fallback ? resolveAgentModelFallbacksOverride(params.previousConfig, previousDefaultAgentId) === void 0 : resolveAgentExplicitModelPrimary(params.previousConfig, previousDefaultAgentId) === void 0 : false;
			const alreadySelected = touchedRefs.some((ref) => ref.agentIndex === void 0 && ref.path === defaultRef.path);
			if (!previouslyInherited && !alreadySelected) touchedRefs.push({
				...defaultRef,
				dependency: true
			});
		}
	...
	for (const [agentIndex, agent] of agentList.entries()) {
		...
		if (!params.touchedPaths.some((touchedPath) => isPathPrefix(touchedPath, agentEntryPath) || isPathPrefix(touchedPath, agentIdPath) || isPathPrefix(agentIdPath, touchedPath) || isPathPrefix(touchedPath, agentModelPath) || isPathPrefix(agentModelPath, touchedPath))) continue;
		for (const defaultRef of defaultRefs) {
			...
			const previouslyInherited = Boolean(params.previousConfig?.agents?.list?.some((entry) => normalizeAgentId(entry?.id) === normalizeAgentId(agentId))) && params.previousConfig ? defaultRef.fallback ? resolveAgentModelFallbacksOverride(params.previousConfig, agentId) === void 0 : resolveAgentExplicitModelPrimary(params.previousConfig, agentId) === void 0 : false;
...
function hasUnresolvedInheritedFallbackProvider(config, ref) {
...
function expandInheritedDefaultRefs(config, refs) {
	const agentList = config.agents?.list;
	if (!Array.isArray(agentList)) return refs;
	...
	for (const ref of refs) {
		if (ref.agentIndex !== void 0) {
			push(ref);
			continue;
		}
		if (!agentList.some((agent) => normalizeAgentId(agent?.id) === normalizeAgentId(defaultAgentId)) || (ref.fallback ? resolveAgentModelFallbacksOverride(config, defaultAgentId) === void 0 : resolveAgentExplicitModelPrimary(config, defaultAgentId) === void 0)) push(ref);
		for (const [agentIndex, agent] of agentList.entries()) {
			const agentId = typeof agent?.id === "string" ? agent.id : "";
			if (!agentId || normalizeAgentId(agentId) === normalizeAgentId(defaultAgentId)) continue;
...
function validateModelRefSyntax(config, ref) {
	if (!ref.value) return "Model reference is empty";
	if (containsEnvVarReference(ref.value)) return "Model reference contains an unresolved environment variable";
...
async function createRuntimeModelRefResolver() {
	const [agentScope, modelSelection] = await Promise.all([import("./agent-scope-RIXtZ2Lu.js"), import("./model-selection-Su90IYNf.js")]);
	const preparedByAgent = /* @__PURE__ */ new Map();
	let modelModules;
	const loadModelModules = () => modelModules ??= Promise.all([import("./model-DnXblBcP.js"), import("./prepared-model-catalog-C7ceMjSu.js")]);
	return async ({ config, ref }) => {
		const configuredAgent = ref.agentIndex === void 0 ? void 0 : config.agents?.list?.[ref.agentIndex];
		const targetAgentId = typeof configuredAgent?.id === "string" ? configuredAgent.id : agentScope.resolveDefaultAgentId(config);
...
async function checkTouchedTextModelRefs(params) {
	...
	try {
		...
		validationConfig = resolveConfigEnvVars(params.config, env, { onMissing: () => {} });
	...
	const validatedRefs = refs.filter((ref) => !hasUnresolvedInheritedFallbackProvider(validationConfig, ref)).map((ref) => ({
		ref,
...
function readConfigMutationFileSync(filePath) {
...
function parseBatchSource(opts) {
	...
	try {
		...
		if (hasErrnoCode(err, "ENOENT")) throw new Error(`--batch-file not found: ${pathname}`, { cause: err });
		throw err;
	}
...
function normalizeConfigMutationExplicitSetPath(path) {
	if (path.length >= 4 && path[0] === "agents" && path[1] === "defaults" && path[2] === "models") {
		const normalizedModelId = normalizeAgentModelRefForConfig(expectDefined(path[3], "path entry at 3"));
...
async function loadValidConfig(runtime = defaultRuntime) {
	...
	for (const line of formatConfigIssueLines(displayIssues, "-", { normalizeRoot: true })) runtime.error(line);
...
function configApplyHintForOperations(operations, beforeConfig, afterConfig) {
	...
	return configApplyHintForPaths(expandActualChangedPathsWithRequestedDescendants(diffConfigPaths(beforeConfig, afterConfig), requestedPaths, beforeConfig, afterConfig), afterConfig);
}
...
async function readConfigPatchInput(opts) {
	...
	if (stdin) raw = await readStdinText();
		...
		raw = readConfigMutationFileSync(file);
	} catch (err) {
		if (hasErrnoCode(err, "ENOENT")) throw new Error(`--file not found: ${file}`, { cause: err });
		throw err;
	}
	try {
		return JSON5.parse(raw);
...
async function runConfigOperations(params) {
	...
	if (options.dryRun) {
		...
		const dryRunResult = {
			...
			checks: {
				...
				resolvabilityComplete: (hasJsonMode || hasBuilderMode || hasUnsetMode || modelRefCheck.refsTotal > 0) && selectedDryRunRefs.skippedExecRefs.length === 0 && modelRefCheck.refsChecked === modelRefCheck.refsTotal
			},
	...
	const firstModelError = (await checkTouchedTextModelRefs({
		config: nextConfig,
		previousConfig: currentConfigForApplyHint,
		touchedPaths: operations.map((operation) => operation.setPath),
		redactDependencyValues: true
	})).errors[0];
	if (firstModelError) throw new Error(firstModelError);
...
async function runConfigUnset(opts) {
	...
	try {
		...
		await replaceConfigFile({
			nextConfig,
			...snapshot.hash !== void 0 ? { baseHash: snapshot.hash } : {},
			...unsetResult.leafContainer === "array" ? { writeOptions: { auditOrigin: "cli" } } : { writeOptions: {
				auditOrigin: "cli",
				unsetPaths: [parsedPath]
			} }
		});
		const hint = configApplyHintForOperations([buildUnsetOperation(parsedPath)], currentConfigForApplyHint, nextConfig);
	...
	try {
		...
		if (!snapshot.valid) {
			...
			if (opts.json) writeRuntimeJson(runtime, {
				...
				for (const line of formatConfigIssueLines(displayIssues, danger("×"), { normalizeRoot: true })) runtime.error(`  ${line}`);
				runtime.error("");
...
function registerConfigCli(program) {
	const cmd = program.command("config").description("Non-interactive config helpers (get/set/patch/unset/file/schema/validate). Run without subcommand for guided setup.").addHelpText("after", () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.openclaw.ai/cli/config")}\n`).option("--section <section>", "Configuration sections for guided setup (repeatable). Use with no subcommand.", (value, previous) => [...previous, value], []).action(async (opts) => {
		const { configureCommandFromSectionsArg } = await import("./configure-BEF_4mtd.js");
...
//#endregion
export { parseConfigSetPath, registerConfigCli, runConfigGet, runConfigPatch, runConfigSet, runConfigUnset };
