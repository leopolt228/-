...
import { n as createFileLockManager } from "./file-lock-3_2flBo_.js";
import { n as getProcessStartTime, r as isPidAlive } from "./pid-alive-3LhI2apQ.js";
import { r as readGatewayProcessArgsSync } from "./gateway-processes-c8np4yhp.js";
...
function resolveSessionWriteLockMs(params) {
	...
	return readPositiveMsEnv(params.env ?? process.env, SESSION_WRITE_LOCK_ENV[params.key], opts) ?? params.fallback;
}
...
function resolveSessionWriteLockTarget(sessionFile) {
	...
	if (!sqliteMarker) return path.resolve(sessionFile);
	...
	return path.join(path.dirname(path.resolve(sqliteMarker.storePath)), "session-locks", `${safeAgentId}-${safeSessionId}.sqlite-transcript`);
}
...
//#endregion
export { resolveSessionWriteLockAcquireTimeoutMs as a, resolveSessionLockMaxHoldFromTimeout as i, cleanStaleLockFiles as n, resolveSessionWriteLockOptions as o, drainSessionWriteLockStateForTest as r, resolveSessionWriteLockStaleMs as s, acquireSessionWriteLock as t };
