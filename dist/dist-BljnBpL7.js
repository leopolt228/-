...
import { t as require_src$1 } from "./src-Ch5YakSI.js";
import { i as require_axios, n as require_lib$2, r as require_dist$3, t as require_dist$2 } from "./dist-CDMt8XJf.js";
import { t as require_jws } from "./jws-yRRSvXJf.js";
...
var require_webcrypto = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	exports.default = crypto$4.webcrypto;
...
var require_aesgcmkw = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	async function wrap(alg, key, cek, iv) {
		...
		const { ciphertext: encryptedKey, tag } = await (0, encrypt_js_1.default)(jweAlgorithm, cek, key, iv, /* @__PURE__ */ new Uint8Array(0));
		return {
			encryptedKey,
...
var require_sign$4 = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	let oneShotSign;
	if (crypto$3.sign.length > 3) oneShotSign = (0, util_1$2.promisify)(crypto$3.sign);
	else oneShotSign = crypto$3.sign;
	const sign = async (alg, key, data) => {
		const keyObject = (0, get_sign_verify_key_js_1.default)(alg, key, "sign");
		if (alg.startsWith("HS")) {
			const hmac = crypto$3.createHmac((0, hmac_digest_js_1.default)(alg), keyObject);
			hmac.update(data);
			return hmac.digest();
		}
...
var require_verify$4 = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	const flags_js_1 = require_flags();
	let oneShotVerify;
	if (crypto$2.verify.length > 4 && flags_js_1.oneShotCallback) oneShotVerify = (0, util_1$1.promisify)(crypto$2.verify);
	else oneShotVerify = crypto$2.verify;
	const verify = async (alg, key, signature, data) => {
		...
		if (alg.startsWith("HS")) {
			...
			try {
				return crypto$2.timingSafeEqual(actual, expected);
			} catch {
				return false;
			}
		}
...
var require_request = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	module.exports.default = (options) => {
		...
		return new Promise((resolve, reject) => {
			...
			const httpRequest = (protocol === "https:" ? https : http$1).request(requestOptions, (res) => {
				let rawData = "";
	...
	exports.RemoteFunctionValidator = class RemoteFunctionValidator {
		...
		*/
		static create(tenantId, clientId, logger, options) {
			const jwtValidator = new jwt_validator_1.JwtValidator({
				clientId,
				tenantId,
				validateIssuer: { allowedTenantIds: options?.allowedTenantIds },
				validateScope: options?.requiredScope ? { requiredScope: options.requiredScope } : void 0,
	...
	var HttpServer = class {
		...
		*/
		async initialize(deps) {
			...
			if (!this.credentials && !this.skipAuth) this.logger.warn("No credentials configured and skipAuth is not enabled. All incoming requests will be rejected. Configure client authentication to securely receive messages, or set skipAuth: true for local development.");
			...
			else if (!this.credentials && this.skipAuth) this.logger.warn(`No credentials configured (CLIENT_ID / CLIENT_SECRET / TENANT_ID), but skipAuth is enabled. Bot will accept unauthenticated requests on ${this._messagingEndpoint}.`);
...
var require_msal_node = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	*/
	const CodeChallengeMethodValues = { S256: "S256" };
	/**
	...
	*/
	...
	const AUTHORITY_METADATA_REFRESH_TIME_SECONDS = 3600 * 24;
	...
	const SERVER_TELEM_MAX_LAST_HEADER_BYTES = 330;
	const SERVER_TELEM_MAX_CACHED_ERRORS = 50;
	...
	*/
	function addClaims(parameters, correlationId, claims, clientCapabilities, skipBrokerClaims) {
		const mergedClaims = buildMergedClaims(claims, skipBrokerClaims && parameters.has(BROKER_CLIENT_ID) ? void 0 : clientCapabilities, correlationId);
	...
	*/
	function buildMergedClaims(claims, clientCapabilities, correlationId = "") {
		...
		if (!claims) mergedClaims = {};
			...
			if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) throw new Error("Claims must be a JSON object");
			mergedClaims = parsed;
		} catch (e) {
	...
	*/
	...
	const correlationCache = /* @__PURE__ */ new Map();
	/**
		...
		getAccessToken(account, request, tokenKeys, targetRealm) {
			...
			accessTokenKeys.forEach((key) => {
				if (this.accessTokenKeyMatchesFilter(key, accessTokenFilter, true)) {
					...
					if (accessToken && this.credentialMatchesFilter(accessToken, accessTokenFilter, correlationId)) accessTokens.push(accessToken);
				}
			});
	...
	var ServerTelemetryManager = class ServerTelemetryManager {
		...
		*/
		generateCurrentRequestHeaderValue() {
			...
			const requestWithRegionDiscoveryFields = [request, this.getRegionDiscoveryFields()].join(SERVER_TELEM_VALUE_SEPARATOR);
			return [
	...
	*/
	function computeAdditionalCacheKeyHash(components) {
		const input = Object.keys(components).sort().map((k) => k + components[k]).join("");
		return crypto$1.createHash("sha256").update(input, "utf8").digest("base64url");
	}
	function generateCredentialKey(credential) {
		...
		if (credential.additionalCacheKeyComponents && Object.keys(credential.additionalCacheKeyComponents).length > 0) credentialKey.push(computeAdditionalCacheKeyHash(credential.additionalCacheKeyComponents));
	...
	var ClientCredentialClient = class extends BaseClient {
		...
		*/
		async acquireToken(request) {
			...
			if (request.fmiPath) additionalCacheKeyComponents = { fmi_path: request.fmiPath };
			if (request.skipCache || request.claims) return this.executeTokenRequest(request, this.authority, void 0, additionalCacheKeyComponents);
...
var require_token_manager = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	var TokenManager = class {
		...
		async getTokenWithFederatedCredentials(credentials, scope, tenantId) {
			const result = await this.getFederatedIdentityClient(credentials, tenantId).acquireTokenByClientCredential({ scopes: [scope] });
			return this.handleTokenResponse(result);
		}
			...
			const client = new msal_node_1.ConfidentialClientApplication({
				auth: {
					...
					clientAssertion: async () => {
						...
						if (!managedIdentityTokenRes) throw new Error("Failed to acquire managed identity exchange token for federated identity credential");
