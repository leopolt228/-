...
import { c as resolveTextChunkLimit, o as chunkTextWithMode, s as resolveChunkMode } from "./chunk-B-Yo_muw.js";
...
import { a as resolveEnvelopeFormatOptions, i as formatInboundFromLabel, r as formatInboundEnvelope } from "./envelope-BfKEFEwi.js";
...
import { n as filterChannelInboundQuoteContext, o as resolveInboundSupplementalSenderAllowed, t as buildChannelInboundEventContext } from "./context-CGmpW7gY.js";
...
import { t as waitForTransportReady } from "./transport-ready-D4SBj6lc.js";
import "./transport-ready-runtime-DdACrY9z.js";
...
import { c as rememberIMessageReplyCache, s as isKnownFromMeIMessageMessageId } from "./monitor-reply-cache-D1matHtv.js";
...
import { c as parseIMessageTarget, o as normalizeIMessageHandle, r as isAllowedIMessageReplyContextSender, s as parseIMessageAllowTarget, t as formatIMessageChatTarget } from "./targets-B8U82l9l.js";
import { t as getIMessageRuntime } from "./runtime-D2WDA3RL.js";
import { n as resolveIMessageInboundConversationId, o as probeIMessage, t as sanitizeOutboundText } from "./sanitize-outbound-3VTQEqXF.js";
import { c as handleIMessageApprovalReaction, d as registerIMessageApprovalReactionTarget, l as listPendingIMessageApprovalReactionPollTargets, s as extractIMessageApprovalPromptBinding, u as maybeResolveIMessageApprovalReaction } from "./approval-reactions-B8WHau0l.js";
...
import { a as resolveCatchupConfig, i as performIMessageCatchup, n as advanceIMessageCatchupCursor } from "./catchup-DAnhrpkW.js";
import { c as stripLeadingEchoTextCorruptionMarkers, i as hasPersistedIMessageEcho, o as resolveIMessageEchoMediaKey } from "./persisted-echo-cache-BB9qxaU5.js";
import { n as hasIMessageQuestionReactionTarget, r as maybeResolveIMessageQuestionReaction } from "./question-reactions-X3ikoQaq.js";
...
function combineIMessagePayloads(payloads) {
	...
	const boundedPayloads = payloads.length > MAX_COALESCED_ENTRIES ? [...payloads.slice(0, MAX_COALESCED_ENTRIES - 1), last] : payloads;
	...
	if (combinedText.length > MAX_COALESCED_TEXT_CHARS) combinedText = `${sliceUtf16Safe(combinedText, 0, MAX_COALESCED_TEXT_CHARS)}…[truncated]`;
	const allAttachments = boundedPayloads.flatMap((p) => p.attachments ?? []).slice(0, MAX_COALESCED_ATTACHMENTS);
...
function hasUsableConversationAnchor(projection) {
	return hasPositiveChatId(projection.chat_id) || isNonEmptyString(projection.chat_guid) || isNonEmptyString(projection.chat_identifier);
}
...
function extractAuthoritativeRecoveryProjection(entry) {
	...
	if (!isNonEmptyString(entry.sender)) return null;
	const projection = {
		sender: entry.sender.trim(),
		is_from_me: entry.is_from_me,
		is_group: entry.is_group
	};
	if (isNonEmptyString(entry.destination_caller_id)) projection.destination_caller_id = entry.destination_caller_id.trim();
	if (hasPositiveChatId(entry.chat_id)) projection.chat_id = entry.chat_id;
	if (isNonEmptyString(entry.chat_guid)) projection.chat_guid = entry.chat_guid;
	if (isNonEmptyString(entry.chat_identifier)) projection.chat_identifier = entry.chat_identifier;
...
async function resolveIMessageInboundDecision(params) {
	...
	if (params.message.is_from_me) {
		...
		if (isSelfChat) {
			...
			if (params.echoCache && (bodyText || inboundMessageId || mediaFacts.length > 0) && hasIMessageEchoMatch({
...
async function buildIMessageInboundContext(params) {
	...
	const media = toInboundMediaFacts(params.media?.facts?.map((entry) => ({
		...entry,
...
const IMESSAGE_INGRESS_PRUNE_INTERVAL_MS = 3600 * 1e3;
const IMESSAGE_INGRESS_COMPLETED_TTL_MS = 14400 * 1e3;
const IMESSAGE_INGRESS_COMPLETED_MAX_ENTRIES = 1e4;
const IMESSAGE_INGRESS_FAILED_TTL_MS = 720 * 60 * 60 * 1e3;
const IMESSAGE_INGRESS_FAILED_MAX_ENTRIES = 1e3;
var IMessageIngressPayloadError = class extends Error {
...
function buildIMessageFlushIngressLifecycle(lifecycles) {
...
function createIMessageDurableIngress(options) {
	...
	const monitor = createChannelIngressMonitor({
		...
		deliver: async (event, lifecycle, record) => await dispatchAdmissionQueue.enqueue(record.laneKey ?? record.id, async () => {
			...
			return await options.dispatch(event.message, lifecycle, event.receivedAt, event.catchup ? { catchup: true } : {});
		}),
...
function renderIMessagePollBody(poll) {
...
function readRecoveryCursor(accountId, dbIdentity) {
	try {
...
function migrateLegacyCatchupCursor(accountId, dbIdentity) {
	try {
		...
		if (rowid !== null) advanceIMessageRecoveryCursor(accountId, dbIdentity, rowid);
		return rowid;
	} catch {
		return null;
	}
...
* different database).
*/
function loadIMessageRecoveryCursor(accountId, dbIdentity, options = {}) {
...
function advanceIMessageRecoveryCursor(accountId, dbIdentity, rowid) {
...
const IMESSAGE_THROTTLED_DIAGNOSTIC_DROP_REASONS = /* @__PURE__ */ new Set(["from me"]);
...
async function monitorIMessageProvider(opts = {}) {
	...
	const recoveryCursorRowid = loadIMessageRecoveryCursor(accountInfo.accountId, recoveryCursorDbIdentity, { migrateLegacyCatchup: !catchupCfg.enabled });
	...
	function advanceRecoveryCursorAfterDurableEnqueue(rowid) {
		...
		if (nextCursorRowid >= 0 && nextCursorRowid > latestAdvancedRecoveryCursorRowid) {
			advanceIMessageRecoveryCursor(accountInfo.accountId, recoveryCursorDbIdentity, nextCursorRowid);
	...
	const { debouncer: inboundDebouncer } = createChannelInboundDebouncer({
		...
		onFlush: async (entries) => {
			...
			const dispatchUnit = async (unitEntries, message) => {
				const { lifecycle, settle, abandon } = buildIMessageFlushIngressLifecycle(unitEntries.flatMap((entry) => entry.ingressLifecycle ? [entry.ingressLifecycle] : []));
				try {
			...
			if (entries.length === 1) {
				await dispatchUnit(entries, expectDefined(entries[0], "single iMessage dispatch entry").message);
				return;
			}
	...
	async function handleMessageNow(message, ingressLifecycle) {
	...
	function resolveIMessageInboundBodyText(message) {
		const messageText = ((message.poll ? renderIMessagePollBody(message.poll) : null) ?? message.text ?? "").trim();
	...
	async function handleMessageNowInner(rawMessage, ingressLifecycle) {
		...
		if (decision.kind === "drop") {
			...
			if (diagnostic) {
				...
				if (!shouldThrottleIMessageInboundDropDiagnostic(decision.reason) || !loggedThrottledDropDiagnostics.check(throttleKey)) runtime.log?.(warn(diagnostic));
			}
