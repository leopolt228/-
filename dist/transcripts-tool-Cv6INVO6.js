...
import { n as listTranscriptSourceProviders, t as getTranscriptSourceProvider } from "./provider-registry-DCZypLo7.js";
...
var TranscriptsStore = class {
	...
	async readUtterancesFromDir(dir, options = {}) {
		...
		if (maxUtterances !== void 0) return await new Promise((resolve, reject) => {
			const utterances = [];
			...
			const settle = () => {
				...
				else if (emptyForENOENT) resolve([]);
				else resolve(utterances);
			};
			const setError = (err) => {
				if (!pendingError) pendingError = err instanceof Error ? err : new Error(String(err));
			};
