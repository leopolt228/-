...
import { i as assertOkOrThrowProviderError } from "./provider-http-errors-DrOMjuGn.js";
...
import { a as isDebugProxyGlobalFetchPatchInstalled, t as captureHttpExchange } from "./runtime-ByW-vknB.js";
...
async function openaiTTS(params) {
	...
	try {
		...
		return await readResponseWithLimit(response, maxBytes, { onOverflow: ({ maxBytes: maxBytesLocal }) => /* @__PURE__ */ new Error(`OpenAI TTS audio response exceeds ${maxBytesLocal} bytes`) });
	} finally {
		await release();
	}
}
