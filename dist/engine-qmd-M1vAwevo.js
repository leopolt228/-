...
import { o as statRegularFile, r as readRegularFile } from "./regular-file-D9KgyI-A.js";
import { t as createSubsystemLogger } from "./subsystem-Dogzi5wG.js";
import { _ as resolveSessionAgentId } from "./agent-scope-CrBA-6Gx.js";
import { n as normalizeAgentId } from "./agent-id-DDgUze4y.js";
import { x as isCronRunSessionKey } from "./session-key-Drrs61Fd.js";
...
import { S as parseUsageCountedSessionIdFromFileName, _ as isSessionArchiveArtifactName, b as isUsageCountedSessionTranscriptFileName, c as resolveSessionTranscriptsDirForAgent, p as isCompactionCheckpointTranscriptFileName, r as resolveSessionFilePath } from "./paths-BpMRJ7TJ.js";
...
import { n as HEARTBEAT_PROMPT } from "./heartbeat-Bkwxbekw.js";
import { r as isHeartbeatUserMessage } from "./heartbeat-filter-heuIP_Mh.js";
import "./query-expansion-DzoKGtnD.js";
import { h as resolveTranscriptSessionKeyBySessionId, i as listSessionEntries, m as resolveStorePath, o as loadTranscriptEventsSync, u as readTranscriptStatsSync } from "./session-store-runtime-yTK-eEl-.js";
import { i as isExecCompletionEvent } from "./heartbeat-events-filter-CwG3RmMF.js";
import { p as readTranscriptContentRevisionSync } from "./memory-search-Do8IpoGY.js";
import "./fs-utils-BADRUEfU.js";
import { r as retryTransientMemoryRead, t as hashText } from "./hash-CLsGYYJA.js";
import { n as extractAgentIdFromSessionsDir, r as isDreamingNarrativeSessionStoreKey } from "./openclaw-runtime-session-BX1eTSHK.js";
...
function resolveSessionStoreTranscriptCorpusSource(agentId, sessionsDir, storePath, entry) {
	const sessionFile = typeof entry?.sessionFile === "string" && entry.sessionFile.trim().length > 0 ? entry.sessionFile.trim() : void 0;
	const sqliteMarker = sessionFile ? parseSqliteSessionFileMarker(sessionFile) : void 0;
	const sessionId = (typeof entry?.sessionId === "string" && entry.sessionId.trim().length > 0 ? entry.sessionId.trim() : null) ?? sqliteMarker?.sessionId ?? (sessionFile ? parseUsageCountedSessionIdFromFileName(path.basename(sessionFile)) : null);
	if (!sessionId) return null;
	if (sqliteMarker) {
		if (!sessionFile) return null;
		if (sqliteMarker.sessionId !== sessionId || normalizeAgentId(sqliteMarker.agentId) !== normalizeAgentId(agentId) || normalizeComparablePath$1(sqliteMarker.storePath) !== normalizeComparablePath$1(storePath)) return null;
		return {
			sessionFile,
			sessionId,
	...
	try {
		if (!sessionFile) return {
			sessionFile: resolveSessionFilePath(sessionId, void 0, {
				agentId,
				sessionsDir
			}),
			sessionId,
...
function classifySessionEntry(sessionKey, entry, cronGeneratedSessionKeys) {
	return {
		generatedByDreamingNarrative: isDreamingNarrativeSessionStoreKey(sessionKey) || isDreamingNarrativeSessionKeyLike(entry.spawnedBy),
...
function toSessionStoreCorpusEntry(agentId, sessionsDir, storePath, summary, cronGeneratedSessionKeys) {
	const source = resolveSessionStoreTranscriptCorpusSource(agentId, sessionsDir, storePath, summary.entry);
	if (!source) return null;
	if (source.transcriptSource === "file" && !isUsageCountedSessionTranscriptFileName(path.basename(source.sessionFile))) return null;
	...
	const contentRevision = source.transcriptSource === "sqlite" ? sqliteContentRevision({
		agentId,
	...
	return {
		...
		...source.transcriptSource === "sqlite" ? { transcriptSource: "sqlite" } : {},
		...source.transcriptSource === "sqlite" && Number.isFinite(summary.entry.updatedAt) ? { updatedAtMs: summary.entry.updatedAt } : {},
		...sessionKey ? { sessionKey } : {},
...
function classifyTranscriptArtifact(artifactPath, activeEntriesByPath, activeEntriesBySessionId) {
	...
	const primaryEntry = primarySessionId && isSessionArchiveArtifactName(path.basename(artifactPath)) ? activeEntriesByPath.get(normalizeRealComparablePath(path.join(sessionsDir, `${primarySessionId}.jsonl`))) ?? activeEntriesBySessionId.get(primarySessionId) : void 0;
	return {
...
function toArtifactCorpusEntry(agentId, artifactPath, activeEntriesByPath, activeEntriesBySessionId) {
	const sessionId = parseUsageCountedSessionIdFromFileName(path.basename(artifactPath));
	if (!sessionId) return null;
	if (!isSessionArchiveArtifactName(path.basename(artifactPath))) return null;
	const classification = classifyTranscriptArtifact(artifactPath, activeEntriesByPath, activeEntriesBySessionId);
...
function listSessionTranscriptCorpusEntriesForAgentSync(agentId) {
	...
	for (const summary of sessionEntries) {
		...
		const entry = toSessionStoreCorpusEntry(ownerAgentId, sessionsDir, storePath, summary, cronGeneratedSessionKeys);
		if (!entry) continue;
		const normalizedEntryPath = entry.transcriptSource === "sqlite" ? null : normalizeRealComparablePath(entry.sessionFile);
		...
		if (ownerAgentId === normalizedAgentId) {
			...
			if (normalizedEntryPath) activeEntriesByPath.set(normalizedEntryPath, entry);
		}
	}
	const includeUnownedArtifacts = !isSharedFixedStore;
	const corpusEntries = [...activeEntriesBySessionId.values()].filter((entry) => entry.transcriptSource === "sqlite");
	...
	for (const artifactDir of artifactDirsByPath.values()) for (const artifactPath of listSessionTranscriptArtifactFiles(artifactDir)) {
		...
		const entry = toArtifactCorpusEntry(normalizedAgentId, artifactPath, activeEntriesByPath, activeEntriesBySessionId);
...
async function buildSessionEntry(absPath, opts = {}) {
	try {
		...
		const rawSource = sqliteMarker ? (() => {
			...
			const raw = serializeTranscriptEvents(loadTranscriptEventsSync({
				agentId: sqliteMarker.agentId,
				sessionId: sqliteMarker.sessionId,
				...opts.sessionKey ? { sessionKey: opts.sessionKey } : {},
				storePath: sqliteMarker.storePath
		...
		if (rawSource) {
			...
			raw = (await retryTransientMemoryRead(() => readRegularFile({ filePath: absPath }), `read session transcript ${absPath}`)).buffer.toString("utf-8");
			mtimeMs = stat.mtimeMs;
			size = stat.size;
		...
		const sqliteSessionKey = sqliteMarker && !opts.sessionKey ? resolveTranscriptSessionKeyBySessionId({
			agentId: sqliteMarker.agentId,
			sessionId: sqliteMarker.sessionId,
			storePath: sqliteMarker.storePath
		}) : void 0;
		const sessionStoreClassification = !sqliteMarker && (opts.generatedByDreamingNarrative === void 0 || opts.generatedByCronRun === void 0) ? classifySessionTranscriptFromSessionStore(absPath) : null;
		...
		for (let jsonlIdx = 0, lineStart = 0; lineStart <= raw.length; jsonlIdx++) {
			...
			const isHeartbeatUser = message.role === "user" && provenance?.kind === "internal_system" && provenance.sourceTool === "heartbeat";
			if (message.role === "user") insideHeartbeatTurn = isHeartbeatUser;
			...
			if (!text) continue;
			if (insideHeartbeatTurn) continue;
...
//#endregion
export { deriveQmdScopeChannel as a, parseQmdQueryJson as c, resolveSessionFileForSyncTarget as d, resolveSessionIdentityForTranscriptFile as f, listSessionTranscriptCorpusEntriesForAgent as g, statSessionEntrySync as h, runCliCommand as i, buildSessionEntry as l, sessionPathForSessionIdentity as m, resolveCliSpawnInvocation as n, deriveQmdScopeChatType as o, sessionPathForFile as p, resolveQmdBinaryUnavailableReason as r, isQmdScopeAllowed as s, checkQmdBinaryAvailability as t, parseCanonicalSessionSyncTargetFromPath as u };
