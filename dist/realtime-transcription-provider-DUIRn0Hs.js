...
function buildXaiRealtimeTranscriptionProvider() {
	return {
		...
		isConfigured: ({ providerConfig, cfg }) => Boolean(normalizeProviderConfig(providerConfig).apiKey ?? normalizeOptionalString(process.env.XAI_API_KEY)) || isProviderAuthProfileConfigured({
			provider: "xai",
			cfg
		}),
		createSession: (req) => {
