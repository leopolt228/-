...
import "./commands-text-routing-CE3L-xl5.js";
...
function findCommandByNativeName(name, provider, options) {
	...
	return getChatCommands().find((command) => command.scope !== "text" && supportsNativeProvider(command, provider) && [resolveNativeName(command, provider, options), ...command.nativeAliases ?? []].some((nameLocal) => normalizeOptionalLowercaseString(nameLocal) === normalized));
}
