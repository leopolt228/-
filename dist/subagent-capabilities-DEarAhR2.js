...
import { D as resolveIntegerOption, O as resolveNonNegativeIntegerOption, y as parseStrictNonNegativeInteger } from "./number-coercion-Crk_c9KW.js";
import { _ as uniqueStrings } from "./string-normalization-CRyoFBPt.js";
import "./parse-finite-number-CG8VFQF4.js";
import "./agent-scope-CrBA-6Gx.js";
import { C as isSubagentSessionKey, E as parseAgentSessionKey, b as isAcpSessionKey, y as getSubagentDepth } from "./session-key-Drrs61Fd.js";
...
function getSubagentDepthFromSessionStore(sessionKey, opts) {
	...
	const depthFromStore = (key) => {
		...
		const parentKey = normalizeOptionalString(entry?.spawnedBy) ?? normalizeOptionalString(entry?.parentSessionKey);
		if (!parentKey) return;
...
function resolveSessionCapabilityEntry(params) {
	...
	const store = readSessionStore(resolveStorePath(params.cfg.session?.store, { agentId: parsed.agentId }), parsed.agentId);
...
function resolveSubagentCapabilityStore(sessionKey, opts) {
	...
	if (!parsed?.agentId) return;
	return readSessionStore(resolveStorePath(opts.cfg.session?.store, { agentId: parsed.agentId }), parsed.agentId);
}
