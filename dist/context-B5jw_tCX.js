...
const loadPreparedModelCatalogRuntime = () => import("./prepared-model-catalog-C7ceMjSu.js");
...
function ensureContextWindowCacheLoaded(cfgOverride) {
	...
	CONTEXT_WINDOW_RUNTIME_STATE.loadPromise = Promise.resolve().then(async () => {
		...
		try {
			...
			if (CONTEXT_WINDOW_RUNTIME_STATE.generation !== generation) return;
			const models = catalogResult.status === "fulfilled" ? catalogResult.value.modelCatalog.entries : [];
			const providerStaticModels = catalogResult.status === "fulfilled" ? catalogResult.value.modelCatalog.staticEntries ?? [] : [];
