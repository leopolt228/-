...
import { i as chunkAudio, n as mapProviderStatusToEndReason, r as normalizeProviderStatus, t as isProviderStatusTerminal } from "./call-status-Dp0hAJ00.js";
import { n as getHeader } from "./proxy-ip-bnHmy_VL.js";
import { c as verifyTwilioWebhook, i as readProviderJsonResponseText, n as cancelProviderResponseBody, r as readProviderErrorResponseSnippet, t as guardedJsonApiRequest } from "./guarded-json-api-D-hrta2M.js";
	...
	*/
	async playTtsViaStream(text, streamSid) {
		...
		const sendAudioChunk = (audio) => {
			const raw = handler.sendAudio(streamSid, audio);
			return normalizeSendResult(raw);
		};
		const sendPlaybackMark = (name) => {
			const raw = handler.sendMark(streamSid, name);
			return normalizeSendResult(raw);
		};
