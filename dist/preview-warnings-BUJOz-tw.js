...
import { n as isToolAllowedByPolicyName, t as isToolAllowedByPolicies } from "./tool-policy-match-gf5E9Psx.js";
import { i as resolveProviderToolPolicyEntry, n as normalizeToolProviderPolicyKey, r as resolveProviderToolPolicy } from "./provider-tool-policy-KyAGXjuf.js";
...
import { t as resolveDoctorPrimaryModelRef } from "./primary-model-ref-m1O9Iq-i.js";
...
const channelDoctorModuleLoader = createLazyImportLoader(() => import("./channel-doctor-DovKDfVn.js"));
...
async function resolveDoctorChannelPreviewConfig(params) {
	const [{ resolveCommandSecretRefsViaGateway }, { getConfiguredChannelsCommandSecretTargetIds }] = await Promise.all([import("./command-secret-gateway-fazON44C.js"), import("./command-secret-targets-fLyHx5aP.js")]);
...
async function collectDoctorPreviewNotes(params) {
	...
	const { collectActiveToolSchemaProjectionWarnings } = await import("./active-tool-schema-warnings-DDEF5Zkd.js");
	warnings.push(...collectActiveToolSchemaProjectionWarnings({
		cfg: params.cfg,
		env
	}));
	const channelPluginRuntime = await import("./channel-plugin-blockers-PObpgN2f.js");
	...
	if (hasChannelConfig) {
		...
		const { collectOpenPolicyAllowFromWarnings, maybeRepairOpenPolicyAllowFrom } = await import("./open-policy-allowfrom-CD4CQib6.js");
	...
	if ((hasPluginConfig || hasChannelConfig) && params.cfg.plugins?.enabled !== false) {
		const { collectStalePluginConfigWarnings, isStalePluginAutoRepairBlocked, scanStalePluginConfig } = await import("./stale-plugin-config-BW3yTTnq.js");
	...
	const { collectCodexRouteWarnings } = await import("./codex-route-warnings-D6kY2AjJ.js");
	...
	if (hasPluginConfig) {
		const { collectContextEngineHostCompatibilityWarnings } = await import("./context-engine-host-compat-whuAyXUr.js");
	...
	if (hasSubagentAllowlistConfig(params.cfg)) {
		const { collectStaleSubagentAllowlistWarnings, scanStaleSubagentAllowlistReferences } = await import("./stale-subagent-allowlist-G4ptcH5q.js");
	...
	const { collectCodexNativeAssetInfoNotes } = await import("./codex-native-assets-B99wLC94.js");
	infoNotes.push(...await collectCodexNativeAssetInfoNotes({
		cfg: params.cfg,
		env
	}));
	if (hasPluginLoadPaths(params.cfg)) {
		const { collectBundledPluginLoadPathWarnings, scanBundledPluginLoadPathMigrations } = await import("./bundled-plugin-load-paths-BQSHfKpt.js");
	...
	if (hasChannelConfig) {
		...
		const { scanEmptyAllowlistPolicyWarnings } = await import("./empty-allowlist-scan-XaBtiX1w.js");
	...
	if (hasToolsBySenderKey(params.cfg)) {
		const { collectLegacyToolsBySenderWarnings, scanLegacyToolsBySenderKeys } = await import("./legacy-tools-by-sender-DNxFvlpK.js");
	...
	if (hasConfiguredSafeBins(params.cfg)) {
		const { collectExecSafeBinCoverageWarnings, collectExecSafeBinTrustedDirHintWarnings, scanExecSafeBinCoverage, scanExecSafeBinTrustedDirHints } = await import("./exec-safe-bins-DPLEAWBb.js");
	...
	const { collectStaleOAuthProfileShadowWarnings, scanStaleOAuthProfileShadows } = await import("./stale-oauth-profile-shadows-D1ONF5xM.js");
	...
	const { collectStaleConfiguredAuthOrderWarnings } = await import("./stale-auth-order-CoUYcy6g.js");
