...
import { i as normalizePluginConfigId, r as isWorkspacePluginAllowedByConfig, t as resolveProviderAuthAliasMap } from "./provider-auth-aliases-DqR_mVNH.js";
import { n as listSetupProviderIds } from "./setup-descriptors-CBhOf5ID.js";
...
function resolveManifestProviderUsageAuthEnvVarNames(params) {
	return uniqueStrings(resolveProviderMetadataSnapshot(params).plugins.filter((plugin) => shouldUsePluginProviderEnvVars(plugin, params)).flatMap((plugin) => Object.values(plugin.providerUsageAuthEnvVars ?? {}).flat()));
}
...
function listKnownSecretEnvVarNames(params) {
	return uniqueStrings([...Object.values(resolveProviderEnvVars(params)).flat(), ...resolveManifestProviderUsageAuthEnvVarNames(params)]);
}
