import { r as isExactSemverVersion, s as parseRegistryNpmSpec } from "./npm-registry-spec-CqBTTiC9.js";
...
function resolveNpmInstallSpecsForUpdateChannel(params) {
	if (params.updateChannel === "extended-stable") {
		...
		if (target && params.officialPackageName === target.name) {
			...
			if (!coreVersion || !isExactSemverVersion(coreVersion)) throw new Error(`Extended-stable plugin resolution for ${target.name} requires an exact core version.`);
			return {
