...
import { n as createStreamingBinaryOutputSanitizer, o as getShellConfig } from "./shell-utils-BbCh5CHM.js";
...
import { c as readEnvInt, i as clampWithDefault, r as chunkString, t as buildDockerExecArgs } from "./bash-tools.shared-BoXDo33n.js";
import { f as markExited, m as tail, n as appendOutput, r as createSessionSlug, t as addSession } from "./bash-process-registry-BrIqJ2bV.js";
import { t as getProcessSupervisor } from "./supervisor-Da_-xdZV.js";
import { V as resolveExecApprovalAllowedDecisions, r as DEFAULT_EXEC_APPROVAL_TIMEOUT_MS } from "./exec-approvals-BWcbplqx.js";
import { i as scopedHeartbeatWakeOptionsForPolicy, t as resolveEventSessionKeyForPolicy } from "./event-session-routing-CyZ_0PGe.js";
import "./bash-tools.schemas-Ct4x1Tq6.js";
...
function compactNotifyOutput(value, maxChars = DEFAULT_NOTIFY_SNIPPET_CHARS) {
	...
	return `${truncateUtf16Safe(normalized, Math.max(1, maxChars - 1))}…`;
}
...
function maybeNotifyOnExit(session, status) {
	...
	const eventText = appendExecTimeoutRetryGuidance(output ? `Exec ${status} (${session.id.slice(0, 8)}, ${exitLabel}) :: ${output}` : `Exec ${status} (${session.id.slice(0, 8)}, ${exitLabel})`, session.exitReason);
...
function formatExecFailureReason(params) {
	switch (params.failureKind) {
		...
		case "overall-timeout": return `${appendExecTimeoutRetryGuidance(typeof params.timeoutSec === "number" && params.timeoutSec > 0 ? `Command timed out after ${params.timeoutSec} seconds.` : "Command timed out.", params.failureKind)}\n\nIf it should keep running, start it with exec background=true or yieldMs so OpenClaw can register a pollable process session. Do not rely on shell backgrounding with a trailing &.`;
...
async function runExecProcess(opts) {
	...
	const finalizeAndSettleSession = async (outcome) => {
		...
		try {
			...
			else logWarn(`exec: sandbox finalize after process failure failed (${String(error)}).`);
		} finally {
...
//#endregion
export { renderExecOutputText as _, DEFAULT_PENDING_MAX_OUTPUT as a, buildExecRuntimeErrorOutcome as c, normalizeNotifyOutput as d, renderExecTargetLabel as f, appendExecTimeoutRetryGuidance as g, runExecProcess as h, DEFAULT_PATH as i, createApprovalSlug as l, resolveExecTarget as m, DEFAULT_APPROVAL_TIMEOUT_MS as n, applyShellPath as o, resolveApprovalRunningNoticeMs as p, DEFAULT_MAX_OUTPUT as r, buildApprovalPendingMessage as s, DEFAULT_APPROVAL_REQUEST_TIMEOUT_MS as t, isRequestedExecTargetAllowed as u, renderExecUpdateText as v };
