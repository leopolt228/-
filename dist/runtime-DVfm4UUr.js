...
import { C as resolveExpiresAtMsFromDurationMs, P as timestampMsToIsoString, s as asFiniteNumber } from "./number-coercion-Crk_c9KW.js";
...
import { c as upsertInferredCommitments, i as listPendingCommitmentsForScope, l as resolveCommitmentTimezone, u as resolveCommitmentsConfig } from "./store-tti8aokW.js";
import { randomUUID } from "node:crypto";
//#region src/commitments/extraction.ts
const KIND_VALUES = /* @__PURE__ */ new Set([
	"event_check_in",
	"deadline_check",
	"care_check_in",
	"open_loop"
]);
const SENSITIVITY_VALUES = /* @__PURE__ */ new Set([
	"routine",
	"personal",
	"care"
]);
const SOURCE_VALUES = /* @__PURE__ */ new Set(["inferred_user_context", "agent_promise"]);
...
function formatExtractionNow(valueMs) {
	return timestampMsToIsoString(valueMs) ?? timestampMsToIsoString(Date.now()) ?? "1970-01-01T00:00:00.000Z";
}
...
function parseDueMs(raw) {
	...
	if (!Number.isFinite(parsed) || parseAbsoluteTimeMs(raw) === null) return;
	return parsed;
}
...
if (process.env.VITEST || false) globalThis[Symbol.for("openclaw.commitmentExtractionTestApi")] = { validateCommitmentCandidates };
//#endregion
...
function scheduleDrainSoon(debounceMs) {
	if (timer) return;
	timer = setTimer(() => {
		timer = null;
...
function openTerminalFailureCooldown(agentId, error, nowMs, fallbackNowMs) {
	const cooldownUntil = resolveExpiresAtMsFromDurationMs(TERMINAL_EXTRACTION_FAILURE_COOLDOWN_MS, { nowMs }) ?? resolveExpiresAtMsFromDurationMs(TERMINAL_EXTRACTION_FAILURE_COOLDOWN_MS, { nowMs: fallbackNowMs });
	if (cooldownUntil !== void 0) terminalFailureCooldownUntilByAgent.set(agentId, cooldownUntil);
...
function takeAgentBatch(agentId, maxItems) {
	const batch = [];
		...
		while (queue.length > 0) {
			...
			try {
				...
				if (isTerminalExtractionError(error)) {
					openTerminalFailureCooldown(items[0]?.agentId ?? "", error, Date.now(), items[0]?.nowMs ?? Date.now());
					if (queue.length > 0) scheduleDrainSoon(resolved.extraction.debounceMs);
				} else {
					queue.unshift(...batch);
					scheduleDrainSoon(resolved.extraction.debounceMs);
				}
...
if (process.env.VITEST || process.env.OPENCLAW_COMMITMENTS_SAFETY_E2E === "1") globalThis[Symbol.for("openclaw.commitmentRuntimeTestApi")] = {
