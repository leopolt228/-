...
import { _ as parseStrictFiniteNumber, b as parseStrictPositiveInteger, j as resolveTimerTimeoutMs, o as asDateTimestampMs } from "./number-coercion-Crk_c9KW.js";
...
import { i as withProgressTotals } from "./progress-DY8jzvl0.js";
...
import { d as updateConfig, i as formatTokenK, r as formatMs } from "./shared-Dys0_Ah-.js";
import { t as loadModelsConfig } from "./load-config-BLpHbz3i.js";
import { i as truncate, r as pad } from "./list.format-D9RXa4-v.js";
...
function normalizeCreatedAtMs(value) {
	...
	return asDateTimestampMs(value > 0xe8d4a51000 ? Math.round(value) : Math.round(value * 1e3)) ?? null;
}
...
async function readOpenRouterModelsJson(response, timeoutMs) {
	const buffer = await readResponseWithLimit(response, OPENROUTER_MODELS_BODY_MAX_BYTES, {
		chunkTimeoutMs: timeoutMs,
		onOverflow: ({ size, maxBytes }) => /* @__PURE__ */ new Error(`OpenRouter /models response too large: ${size} bytes (limit ${maxBytes} bytes)`),
...
async function fetchOpenRouterModels(fetchImpl, timeoutMs) {
	...
	try {
		return await withTimeout(timeoutMs, async (signal) => {
			...
			const payload = await readOpenRouterModelsJson(res, timeoutMs);
		...
		if (res && !res.bodyUsed) await res.body?.cancel().catch(() => void 0);
	}
}
