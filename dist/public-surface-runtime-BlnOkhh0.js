...
import { n as resolveBundledPluginsDir, t as areBundledPluginsDisabled } from "./bundled-dir-CNGxEehk.js";
...
function resolvePluginRootPublicSurfacePath(params) {
	...
	for (const candidate of [path.join(pluginRoot, artifactBasename), path.join(pluginRoot, "dist", artifactBasename)]) if (fs.existsSync(candidate)) return candidate;
	const sourceBaseName = artifactBasename.replace(/\.js$/u, "");
	for (const ext of PUBLIC_SURFACE_SOURCE_EXTENSIONS) {
...
function resolvePublicSurfaceFromBundledDir(params) {
	...
	if (fs.existsSync(packageLocalBuiltCandidate)) return packageLocalBuiltCandidate;
	return resolveBundledPluginSourcePublicSurfacePath({
...
//#endregion
export { resolvePluginRootPublicSurfacePath as a, resolveBundledPluginSourcePublicSurfacePath as i, normalizeBundledPluginArtifactSubpath as n, resolveBundledPluginPublicSurfacePath as r, PUBLIC_SURFACE_SOURCE_EXTENSIONS as t };
