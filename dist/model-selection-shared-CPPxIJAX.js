...
import { n as computeModelPolicyAllowlist, r as hasExplicitModelPolicyAllow } from "./model-policy-allowlist-migration-CCPChZ54.js";
...
import { n as normalizeConfiguredProviderCatalogModelId, r as normalizeStaticProviderModelId } from "./model-ref-shared-BlCyhiC_.js";
import { a as normalizeModelRef, c as parseModelRef, i as modelKey, n as findNormalizedProviderValue, o as normalizeProviderId$1 } from "./model-selection-normalize-D7Dhjaxs.js";
...
function listModelAliasCandidates(cfg, agentId) {
	...
	if (agentId) {
		...
		const agentModels = cfg.agents?.list?.find((entry) => normalizeAgentId(entry.id) === normalizedAgentId)?.models;
		modelMaps.push(agentModels);
	}
	return modelMaps.flatMap((models) => Object.entries(models ?? {}).flatMap(([keyRaw, entryRaw]) => {
...
function buildModelCatalogMetadata(params) {
	...
	for (const [rawKey, entryRaw] of Object.entries(configuredModels)) {
		if (parseModelPolicyWildcardRef(rawKey)) continue;
		const key = resolveAllowlistModelKey({
			cfg: params.cfg,
			raw: rawKey,
			defaultProvider: params.defaultProvider,
...
function buildAllowedModelSetWithFallbacks(params) {
	...
	const policyAliasIndex = params.aliasIndex ?? buildModelAliasIndex({
		cfg: params.cfg,
		defaultProvider: params.defaultProvider,
	...
	const selectionAliasIndex = params.agentId && policyAliasAgentId !== params.agentId ? buildModelAliasIndex({
		cfg: params.cfg,
		defaultProvider: params.defaultProvider,
		agentId: params.agentId,
		allowManifestNormalization: params.allowManifestNormalization,
	...
	const resolveSelectionModelRef = (raw, aliasIndex) => {
		const trimmed = raw.trim();
	...
	const automaticFallbackKeys = /* @__PURE__ */ new Set();
	for (const fallback of params.fallbackModels) {
		const parsed = resolveSelectionModelRef(fallback, selectionAliasIndex);
		if (parsed) automaticFallbackKeys.add(modelKey(parsed.provider, parsed.model));
	}
	...
	const addAllowedModelRef = (raw, aliasIndex) => {
	...
	for (const raw of visibility.exactModelRefs) addAllowedModelRef(raw, policyAliasIndex);
...
function resolveConfiguredModelPolicyAllow(params) {
	...
	if (params.agentId) {
		...
		const agentPolicy = (params.cfg?.agents?.list?.find((entry) => normalizeAgentId(entry.id) === normalizedAgentId))?.modelPolicy;
...
function resolveConfiguredWildcardModelKeys(params) {
	...
	for (const raw of resolveConfiguredModelPolicyAllow(params).refs) {
		...
		if (wildcard) wildcardModelKeys.add(wildcard.key);
	}
...
function createModelVisibilityPolicyWithFallbacks(params) {
	...
	for (const raw of visibility.exactModelRefs) addConfiguredRef(raw, false, policyAliasIndex);
...
//#endregion
export { findModelCatalogEntry as C, isModelPolicyCompatSelector as D, resolveConfiguredProviderFallback as E, isValidExactModelPolicyRef as O, resolveModelRefFromString as S, modelSupportsInput as T, resolveBareModelDefaultProvider as _, createModelVisibilityPolicyWithFallbacks as a, resolveConfiguredOpenRouterCompatAlias as b, hasConfiguredProviderModelRows as c, isModelKeyAllowedBySet as d, modelCatalogLogicalKey as f, resolveAllowlistModelKey as g, resolveAllowedModelRefFromAliasIndex as h, buildModelAliasIndex as i, parseModelPolicyWildcardRef as k, inferUniqueProviderFromCatalog as l, parseConfiguredModelVisibilityEntries as m, buildConfiguredAllowlistKeys as n, dedupeModelCatalogEntries as o, normalizeModelSelection as p, buildConfiguredModelCatalog as r, getModelRefStatusWithFallbackModels as s, buildAllowedModelSetWithFallbacks as t, inferUniqueProviderFromConfiguredModels as u, resolveConfiguredModelPolicyAllow as v, findModelInCatalog as w, resolveHooksGmailModel as x, resolveConfiguredModelRef as y };
