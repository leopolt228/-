...
import { i as require_type, n as require_get_intrinsic, r as require_call_bind_apply_helpers, t as require_form_data } from "./form_data-_ckfZIoj.js";
...
var require_activity = /* @__PURE__ */ __commonJSMin(((exports) => {
	exports.Activity = class Activity {
		...
		*/
		addEntities(...value) {
			for (const entity of value) this.addEntity(entity);
			return this;
		}
		/**
		...
		addCitation(position, appearance) {
			...
			messageEntity.citation.push({
				...
				appearance: {
					...
					encodingFormat: appearance.text ? "application/vnd.microsoft.card.adaptive" : void 0,
		...
		*/
		ensureSingleRootLevelMessageEntity() {
			let messageEntity = this.entities?.find((e) => this.isRootLevelMessageEntity(e));
			if (!messageEntity) {
				messageEntity = {
		...
		isRootLevelMessageEntity(entity) {
			return entity.type === "https://schema.org/Message" && entity["@type"] === "Message";
		}
		mergeRootLevelMessageEntity(target, source) {
			const merged = {
				...target,
				...source
			};
			if (target.additionalType || source.additionalType) merged.additionalType = [.../* @__PURE__ */ new Set([...target.additionalType || [], ...source.additionalType || []])];
			if (target.citation || source.citation) {
				const dedupedCitations = /* @__PURE__ */ new Map();
				for (const citation of [...target.citation || [], ...source.citation || []]) dedupedCitations.set(citation.position, citation);
...
var require_strip_mentions_text = /* @__PURE__ */ __commonJSMin(((exports) => {
	function stripMentionsText(activity, { accountId, tagOnly } = {}) {
		...
		for (const mention of activity.entities?.filter((e) => e.type === "mention") || []) {
			...
			} else if (mention.mentioned.name) text = text.replace(`<at>${mention.mentioned.name}</at>`, !tagOnly ? "" : mention.mentioned.name);
		}
		return text.trim();
	}
