...
import { a as removeProviderAuthProfilesWithLock } from "../../profiles-C6oqGGG6.js";
import { n as buildApiKeyCredential } from "../../provider-auth-helpers-DS3RlYgA.js";
import "../../string-coerce-runtime-DBMkn-gE.js";
import "../../number-runtime-C6TGSEc_.js";
import "../../provider-auth-Bnib2g6h.js";
import { t as ensureApiKeyFromEnvOrPrompt } from "../../provider-auth-input-B1415fQi.js";
import { h as withAgentModelAliases } from "../../provider-onboard-BhAeQ_J7.js";
import { n as configureOpenAICompatibleSelfHostedProviderNonInteractive, t as applyProviderDefaultModel } from "../../provider-setup-OGFypbFK.js";
import { t as WizardCancelledError } from "../../prompts-B0iOB1_a.js";
import "../../setup-BBJhG_GE.js";
import { d as selectPreferredLocalModelId } from "../../provider-model-shared-Dzz3IkWT.js";
import { A as LMSTUDIO_DEFAULT_MODEL_ID, C as resolveLmstudioServerBase, D as LMSTUDIO_DEFAULT_EMBEDDING_MODEL, E as LMSTUDIO_DEFAULT_BASE_URL, F as LMSTUDIO_PROVIDER_ID, I as LMSTUDIO_PROVIDER_LABEL, M as LMSTUDIO_DOCKER_HOST_INFERENCE_BASE_URL, N as LMSTUDIO_LOCAL_API_KEY_PLACEHOLDER, O as LMSTUDIO_DEFAULT_INFERENCE_BASE_URL, P as LMSTUDIO_MODEL_PLACEHOLDER, S as resolveLmstudioReasoningCompat, T as LMSTUDIO_DEFAULT_API_KEY_ENV_VAR, _ as normalizeLmstudioConfiguredCatalogEntries, a as resolveLmstudioConfiguredApiKey, b as resolveLmstudioInferenceBase, c as resolveLmstudioRequestContext, d as resolveLmstudioProviderAuthMode, f as shouldUseLmstudioApiKeyPlaceholder, g as mapLmstudioWireModelsToConfig, h as mapLmstudioWireEntry, i as buildLmstudioAuthHeaders, j as LMSTUDIO_DOCKER_HOST_BASE_URL, k as LMSTUDIO_DEFAULT_LOAD_CONTEXT_LENGTH, l as resolveLmstudioRuntimeApiKey, m as buildLmstudioModelName, r as fetchLmstudioModels, s as resolveLmstudioProviderHeaders, t as discoverLmstudioModels, u as hasLmstudioAuthorizationHeader, v as normalizeLmstudioConfiguredCatalogEntry, w as resolveLoadedContextWindow, x as resolveLmstudioReasoningCapability, y as normalizeLmstudioProviderConfig } from "../../models.fetch-CRz8_g9r.js";
...
function selectDefaultLmstudioModelId(discoveredModels) {
	...
	return ids.includes("qwen/qwen3.5-9b") ? LMSTUDIO_DEFAULT_MODEL_ID : selectPreferredLocalModelId(ids) ?? ids[0];
}
function collectAppGuidedLmstudioModelIds(discovery) {
	return new Set(discovery.models.flatMap((entry) => {
		const id = entry.key?.trim();
		return entry.type === "llm" && entry.capabilities?.trained_for_tool_use === true && id ? [id] : [];
	}));
}
...
async function prepareAppGuidedLmstudioSetup(ctx) {
	const existingProvider = ctx.config.models?.providers?.[LMSTUDIO_PROVIDER_ID];
	const baseUrl = resolveLmstudioInferenceBase(existingProvider?.baseUrl ?? resolveLmstudioSetupDefaultInferenceBaseUrl(ctx.env));
	let headers;
	let configuredValue;
	try {
	...
	const setupDiscovery = await discoverLmstudioSetupModels({
		baseUrl,
		apiKey: accessValue ?? "lmstudio-local",
		...headers ? { headers } : {},
		timeoutMs: 5e3
	});
	if ("failure" in setupDiscovery) return null;
	const requestedPrefix = `${LMSTUDIO_PROVIDER_ID}/`;
	const requestedModelId = ctx.modelRef?.startsWith(requestedPrefix) ? ctx.modelRef.slice(requestedPrefix.length) : void 0;
	const appGuidedModelIds = collectAppGuidedLmstudioModelIds(setupDiscovery.value.discovery);
	const selectedModelId = requestedModelId ?? selectDefaultLmstudioModelId(setupDiscovery.value.models.filter((model) => appGuidedModelIds.has(model.id)));
	if (!selectedModelId || !appGuidedModelIds.has(selectedModelId) || !setupDiscovery.value.models.some((model) => model.id === selectedModelId)) return null;
	const providerSetup = {
		apiKey: accessValue ? existingProvider?.apiKey ?? "LM_API_TOKEN" : shouldUseLmstudioApiKeyPlaceholder({
			hasModels: true,
	...
	return {
		profiles: [],
		defaultModel: `${LMSTUDIO_PROVIDER_ID}/${selectedModelId}`,
		configPatch: { models: {
			mode: ctx.config.models?.mode ?? "merge",
			providers: { [LMSTUDIO_PROVIDER_ID]: buildLmstudioSetupProviderConfig(providerSetup) }
		} }
	};
}
...
async function promptAndConfigureLmstudioInteractive(params) {
	const promptText = params.prompter ? params.prompter.text.bind(params.prompter) : params.promptText;
