...
async function resolveGatewayUrl(api) {
	...
	if (bindResult && "url" in bindResult && bindResult.source === "gateway.bind=lan") {
		const { resolveTailscaleServeGatewayUrlsWithRunner, runPluginCommandWithTimeout } = await loadDevicePairApiModule();
		const serveUrls = await resolveTailscaleServeGatewayUrlsWithRunner(port, (argv, opts) => runPluginCommandWithTimeout({
			argv,
			timeoutMs: opts.timeoutMs
		}));
		const urls = [.../* @__PURE__ */ new Set([bindResult.url, ...serveUrls])].slice(0, 8);
		return {
			...bindResult,
			...urls.length > 1 ? { urls } : {}
		};
...
async function resolveMobilePairingGatewayUrl(api) {
	...
	const urls = result.urls?.filter((url) => !validateMobilePairingUrl(url, "tailscale serve status"));
	return {
		...result,
...
function resolveQrChannelSender(channel) {
	return Object.hasOwn(QR_CHANNEL_SENDERS, channel) ? QR_CHANNEL_SENDERS[channel] : void 0;
}
...
function formatGatewayLines(payload) {
	return (payload.urls ?? [payload.url]).map((url, index) => index === 0 ? `Gateway: ${url}` : `Fallback: ${url}`);
}
...
async function issueSetupPayload(params) {
	...
	const hasPlaintextRoute = [.../* @__PURE__ */ new Set([params.url, ...params.urls ?? []])].some((url) => !isFullAccessMobilePairingUrl(url));
	const fullAccess = params.allowFullAccess && !hasPlaintextRoute;
	const accessDowngraded = params.allowFullAccess && hasPlaintextRoute;
	const issuedBootstrap = await issueDeviceBootstrapToken({ profile: fullAccess ? {
