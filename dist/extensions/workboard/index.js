...
import "../../api-DEhiGt8A.js";
import { d as WORKBOARD_STATUSES } from "../../src-BSVWqAG9.js";
import { a as WORKBOARD_TOOL_NAMES, c as resolveAgentWorkboardWorkspaceRuntime, i as cleanupWorkboardRunWorktree, l as resolveCommandWorkboardWorkspaceAccess, n as dispatchAndStartWorkboardCards, o as canonicalizeWorkboardWorkspaceAccess, r as WorkboardStore, s as guardWorkboardToolsForWorkspaceAccess, t as registerWorkboardGatewayMethods, u as resolveWorkboardAgentWorkspace } from "../../runtime-api-BdmFjYqW.js";
	...
	return [
		...
		},
		{
			...
			parameters: Type.Object({
				...
				proof: Type.Optional(Type.Object({
					status: Type.Optional(Type.String({ description: "passed, failed, skipped, or unknown." })),
					label: Type.Optional(Type.String({ description: "Proof label." })),
		...
		},
		{
			...
			parameters: Type.Object({ id: Type.String({ description: "Attachment id." }) }, { additionalProperties: false }),
			execute: async (_toolCallId, rawParams) => {
				const id = readStringParam(rawParams, "id", { required: true });
		...
		},
		{
			...
			parameters: Type.Object({
				...
				defaultWorkspace: Type.Optional(Type.Object({
					kind: Type.String({ description: "scratch, dir, or worktree." }),
					path: Type.Optional(Type.String({ description: "Absolute dir/worktree path." })),
					branch: Type.Optional(Type.String({ description: "Suggested branch." }))
				}, { additionalProperties: false })),
				orchestration: Type.Optional(Type.Object({
		...
		},
		{
			...
			parameters: Type.Object({ id: Type.String({ description: "Board id." }) }, { additionalProperties: false }),
		...
		},
		{
			...
			parameters: Type.Object({ boardId: Type.Optional(Type.String({ description: "Optional board id filter." })) }, { additionalProperties: false }),
			execute: async (_toolCallId, rawParams) => {
				const record = rawParams;
		...
		},
		{
			...
			execute: async (_toolCallId, rawParams) => jsonResult({ subscription: await store.subscribeNotifications(rawParams) })
		},
		{
		...
		},
		{
			...
			parameters: Type.Object({ id: Type.String({ description: "Subscription id." }) }, { additionalProperties: false }),
			execute: async (_toolCallId, rawParams) => {
				const id = readStringParam(rawParams, "id", { required: true });
...
var workboard_default = definePluginEntry({
	...
	register(api) {
		...
		api.on("subagent_ended", async (event) => {
			if (event.runId) await cleanupWorkboardRunWorktree({
				store,
		...
		api.registerCli(async ({ program }) => {
			const { registerWorkboardCli } = await import("../../cli-CUFTM-fX.js");
