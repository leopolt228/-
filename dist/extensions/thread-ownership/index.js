...
var thread_ownership_default = definePluginEntry({
	...
	register(api) {
		const resolveCurrentState = () => {
			...
			return {
				currentConfig,
				forwarderUrl: (pluginCfg.forwarderUrl ?? normalizeOptionalString(process.env.SLACK_FORWARDER_URL) ?? "http://slack-forwarder:8750").replace(/\/$/, ""),
			...
			try {
				...
				try {
					...
					if (resp.status === 409) {
						let owner = "unknown";
						try {
							const body = await readProviderJsonResponse(resp, "thread-ownership forwarder conflict", { maxBytes: THREAD_OWNERSHIP_CONFLICT_BODY_LIMIT_BYTES });
							if (typeof body.owner === "string" && body.owner) owner = body.owner;
						} catch (error) {
							api.logger.warn?.(`thread-ownership: conflict body unreadable (${String(error)}), cancelling send`);
						}
						api.logger.info?.(`thread-ownership: cancelled send to ${channelId}:${threadTs} — owned by ${owner}`);
						return { cancel: true };
					}
					api.logger.warn?.(`thread-ownership: unexpected status ${resp.status}, allowing send`);
				} finally {
					if (!resp.bodyUsed) await resp.body?.cancel().catch(() => void 0);
					await release();
				}
			} catch (err) {
