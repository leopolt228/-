...
import { a as DIR_LIST_TOOL_DESCRIPTOR, c as FILE_FETCH_TOOL_DESCRIPTOR, d as FILE_WRITE_TOOL_DESCRIPTOR, r as DIR_FETCH_TOOL_DESCRIPTOR } from "../../descriptors-BxCusZ_E.js";
//#region extensions/file-transfer/src/shared/lazy-node-invoke-policy.ts
const loadFileTransferNodeInvokePolicy = async () => {
	const { createFileTransferNodeInvokePolicy } = await import("../../node-invoke-policy-BYxsb37J.js");
	...
	nodeHostCommands: [
		{
			...
			handle: async (paramsJSON) => {
				const { handleFileFetch } = await import("../../file-fetch-oy9U2ObJ.js");
		...
		},
		{
			...
			dangerous: true,
			handle: async (paramsJSON) => {
				const { handleDirList } = await import("../../dir-list-CjKUIbQQ.js");
		...
		},
		{
			...
			dangerous: true,
			handle: async (paramsJSON) => {
				const { handleDirFetch } = await import("../../dir-fetch-Bs5mq4al.js");
		...
		},
		{
			...
			dangerous: true,
			handle: async (paramsJSON) => {
				const { handleFileWrite } = await import("../../file-write-BS-nckfa.js");
