...
import { t as MAX_DATE_TIMESTAMP_MS } from "./number-coercion-Crk_c9KW.js";
...
import { t as resolveBundledChannelThreadBindingDefaultPlacement } from "./thread-binding-api-KTuLCSiR.js";
...
function resolveThreadBindingSpawnPolicy(params) {
	...
	return {
		channel,
		accountId,
		enabled: normalizeBoolean(account?.enabled) ?? normalizeBoolean(root?.enabled) ?? normalizeBoolean(params.cfg.session?.threadBindings?.enabled) ?? true,
		spawnEnabled: normalizeBoolean(account?.spawnSessions) ?? normalizeBoolean(root?.spawnSessions) ?? normalizeBoolean(params.cfg.session?.threadBindings?.spawnSessions) ?? true,
