...
import { i as readSecretFileSync } from "./secret-file-TeYT5iMV.js";
...
function readGoogleAdcCredentials(adcPath) {
	const text = readSecretFileSync(adcPath, "Google Vertex ADC credentials", {
		maxBytes: GOOGLE_VERTEX_ADC_FILE_MAX_BYTES,
		rejectHardlinks: false
	});
	const parsed = JSON.parse(text);
	if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) throw new Error(`Google Vertex ADC credentials must be a JSON object: ${adcPath}`);
	return parsed;
}
function resolveGoogleAuthorizedUserCredentials(adcConfig) {
	const record = adcConfig;
...
async function resolveGoogleVertexAccessTokenViaGoogleAuth(adcConfig) {
	if (!cachedGoogleAuthClient) cachedGoogleAuthClient = { promise: import("./src-2zMq7fEn.js").then((m) => /* @__PURE__ */ __toESM(m.default, 1)).then(({ GoogleAuth }) => {
...
async function resolveGoogleVertexAuthorizedUserHeaders(fetchImpl) {
	...
	if (adcPath) {
		...
		if (userAdc) return { Authorization: `Bearer ${await refreshGoogleVertexAuthorizedUserAccessToken({
			credentialsPath: adcPath,
			credentials: userAdc,
			fetchImpl
		})}` };
	}
	return { Authorization: `Bearer ${await resolveGoogleVertexAccessTokenViaGoogleAuth(adcConfig)}` };
}
//#endregion
export { resolveGoogleVertexAuthorizedUserHeaders as n, resolveGoogleVertexConfigApiKey as r, isGoogleVertexCredentialsMarker as t };
