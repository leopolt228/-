...
import { D as resolveIntegerOption, _ as parseStrictFiniteNumber } from "./number-coercion-Crk_c9KW.js";
...
import { At as boolean, Rn as string, Tn as object, dn as literal, wn as number, yt as _enum } from "./schemas-CBJjibl3.js";
import { t as runTasksWithConcurrency } from "./run-with-concurrency-BHgpSCM6.js";
import { r as extractFirstTextBlock } from "./chat-message-content-CeBHi_A4.js";
import { c as readMcpOAuthCredentialsStatus, d as runMcpOAuthLogin, f as buildMcpHttpFetch, m as withoutMcpAuthorizationHeader, o as resolveMcpTransportConfig, p as withSameOriginMcpHttpHeaders, s as clearMcpOAuthCredentials } from "./mcp-auth-profile-B4c7HSD3.js";
import { t as createSessionMcpRuntime } from "./agent-bundle-mcp-runtime-cXylnYqu.js";
import { t as buildBundleMcpToolsFromCatalog } from "./agent-bundle-mcp-materialize-8Ic7kVvm.js";
import { a as updateConfiguredMcpServerTools, i as updateConfiguredMcpServer, n as setConfiguredMcpServer, o as parseConfigValue, r as unsetConfiguredMcpServer, t as listConfiguredMcpServers } from "./mcp-config-BQjQsfx6.js";
...
var OpenClawChannelBridge = class {
	...
	async start() {
		...
		const [{ resolveGatewayClientBootstrap }, { GatewayClient: GatewayClientCtor }, { startGatewayClientWhenEventLoopReady }, { APPROVALS_SCOPE, READ_SCOPE, WRITE_SCOPE }, { GATEWAY_CLIENT_CAPS, GATEWAY_CLIENT_MODES, GATEWAY_CLIENT_NAMES }] = await Promise.all([
			import("./client-bootstrap-CV7agVHr.js"),
			import("./client-CVT_26G9.js"),
			import("./client-start-readiness-BwSkOgd8.js"),
			import("./method-scopes-D6lrNQYT.js"),
			import("./client-info-CglC9SY7.js")
		]);
	...
	async dispatchGatewayEvent(event) {
		try {
			...
			if (this.verbose) process.stderr.write(`openclaw mcp: gateway event ${event.event} error: ${String(error)}\n`);
		}
	}
	...
	if (!disabled) {
		...
		if (resolved?.kind === "http") {
			if (server.auth === "oauth") {
				...
				if (authStatus.requiresAuthorization) issues.push(issue("warning", `OAuth credentials require additional authorization; run ${formatCliCommand(`openclaw mcp login ${name}`)}`));
...
function registerMcpCli(program) {
	...
	mcp.command("status").description("Show configured MCP server transport status without connecting").option("-v, --verbose", "Show transport, auth, timeout, and filter details", false).option("--json", "Print JSON").action(async (opts) => {
		...
		for (const entry of status) {
			...
			const oauth = entry.authStatus?.requiresAuthorization ? " authorization-required" : entry.authStatus?.hasTokens ? " authorized" : "";
			...
			if (opts.verbose) {
				...
				if (entry.auth === "oauth") defaultRuntime.log(`  oauth: tokens=${entry.authStatus?.hasTokens ? "yes" : "no"} authorization=${entry.authStatus?.requiresAuthorization ? "required" : entry.authStatus?.hasTokens ? "ready" : "missing"} client=${entry.authStatus?.hasClientInformation ? "yes" : "no"}`);
	...
	mcp.command("doctor").description("Check configured MCP servers for static setup problems").argument("[name]", "MCP server name").option("--probe", "Also connect to each checked server", false).option("--json", "Print JSON").action(async (name, opts) => {
		...
		const { results: servers, firstError, hasError } = await runTasksWithConcurrency({
			tasks: Object.entries(selected).toSorted(([a], [b]) => a.localeCompare(b)).map(([serverName, server]) => async () => {
	...
	mcp.command("configure").description("Update MCP server operator controls without replacing the server").argument("<name>", "MCP server name").option("--enable", "Enable this saved server", false).option("--disable", "Disable this saved server", false).option("--include <csv>", "Comma-separated MCP tool names or '*' globs to expose").option("--exclude <csv>", "Comma-separated MCP tool names or '*' globs to hide").option("--clear-tools", "Clear this server's MCP tool filter", false).option("--timeout <seconds>", "Per-request timeout in seconds").option("--connect-timeout <seconds>", "Connection timeout in seconds").option("--clear-timeouts", "Clear request and connection timeout overrides", false).option("--parallel", "Mark this server safe for concurrent tool calls").option("--no-parallel", "Clear the concurrent tool-call marker").option("--auth <mode>", "HTTP auth mode: oauth").option("--clear-auth", "Clear auth and OAuth metadata", false).option("--oauth-scope <scope>", "OAuth scope").option("--oauth-redirect-url <url>", "OAuth redirect URL").option("--oauth-client-metadata-url <url>", "OAuth client metadata URL").option("--ssl-verify <boolean>", "Verify HTTPS certificates: true or false").option("--client-cert <path>", "HTTP mutual TLS client certificate path").option("--client-key <path>", "HTTP mutual TLS client key path").option("--clear-tls", "Clear TLS verification and mTLS overrides", false).option("--probe", "Probe the updated server before saving", false).action(async (name, opts) => {
		...
		const requestTimeoutSeconds = parsePositiveNumberOption(opts.timeout, "--timeout");
		setOptionalField(next, "requestTimeoutMs", requestTimeoutSeconds === void 0 ? void 0 : requestTimeoutSeconds * 1e3);
		const connectionTimeoutSeconds = parsePositiveNumberOption(opts.connectTimeout, "--connect-timeout");
		setOptionalField(next, "connectionTimeoutMs", connectionTimeoutSeconds === void 0 ? void 0 : connectionTimeoutSeconds * 1e3);
