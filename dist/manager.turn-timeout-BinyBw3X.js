...
import { b as parseStrictPositiveInteger, f as clampTimerTimeoutMs } from "./number-coercion-Crk_c9KW.js";
...
function buildRuntimeConfigOptionPairs(options, advertisedConfigOptionKeys) {
	...
	if (normalized.thinking && shouldEmitThinkingConfigOption(advertisedConfigOptionKeys)) pairs.set(resolveRuntimeConfigOptionKey("thinking", advertisedConfigOptionKeys), normalized.thinking);
...
function shouldEmitThinkingConfigOption(advertisedConfigOptionKeys) {
	const advertisedKeys = buildAdvertisedConfigOptionKeyMap(advertisedConfigOptionKeys);
	return advertisedKeys.size === 0 || RUNTIME_CONFIG_OPTION_ALIASES.thinking.some((alias) => advertisedKeys.has(normalizeLowercaseStringOrEmpty(alias)));
}
