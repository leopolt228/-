...
import { v as formatMemoryDreamingDay, y as isSameMemoryDreamingDay } from "./dreaming-BmyNO7Dv.js";
...
import { _ as writeMemoryCoreWorkspaceEntries, a as SHORT_TERM_LOCK_NAMESPACE, c as SHORT_TERM_RECALL_NAMESPACE, d as memoryCoreStateReference, g as readMemoryCoreWorkspaceEntries, h as openMemoryCoreStateStore, i as SHORT_TERM_LOCK_MAX_ENTRIES, o as SHORT_TERM_META_NAMESPACE, p as memoryCoreWorkspaceStateKey, s as SHORT_TERM_PHASE_SIGNAL_NAMESPACE, v as writeMemoryCoreWorkspaceEntry } from "./dreaming-state-B_O8tXV-.js";
...
async function recordShortTermRecalls(params) {
	...
	await withShortTermLock(workspaceDir, async () => {
		...
		for (const result of relevant) {
			...
			const lastRecalledAt = Boolean(params.dedupeByQueryPerDay) && queryHashesBase.includes(queryHash) && existing?.snippet === snippet ? existing?.lastRecalledAt ?? nowIso : nowIso;
			store.entries[key] = {
				key,
				path: normalizedPath,
				startLine: Math.max(1, Math.floor(result.startLine)),
...
async function writeExistingMemoryInPlace(filePath, content) {
	let handle;
	try {
...
async function applyShortTermPromotions(options) {
	...
	return await withShortTermLock(workspaceDir, async () => {
		...
		const existingMemory = await fs.readFile(memoryWritePath, "utf-8").catch((err) => {
		...
		if (toAppend.length > 0) {
			...
			const memoryDirMode = (await fs.stat(path.dirname(memoryWritePath))).mode & 4095;
			...
			try {
				...
				if (atomicRenameCommitted || !isAtomicReplacePermissionError(error) || !await writeExistingMemoryInPlace(memoryWritePath, content)) throw error;
			}
		}
...
//#endregion
export { resolveShortTermRecallStorePath as C, resolveShortTermRecallLockPath as S, recordGroundedShortTermCandidates as _, SHORT_TERM_STORE_RELATIVE_PATH as a, removeGroundedShortTermCandidates as b, filterFreshLightDreamingEntries as c, normalizeShortTermPhaseSignalStore as d, normalizeShortTermRecallStore as f, recordDreamingPhaseSignals as g, readShortTermRecallEntries as h, SHORT_TERM_PHASE_SIGNAL_RELATIVE_PATH as i, filterLiveShortTermRecallEntries as l, readLightStagedKeys as m, DEFAULT_PROMOTION_MIN_SCORE as n, applyShortTermPromotions as o, rankShortTermPromotionCandidates as p, DEFAULT_PROMOTION_MIN_UNIQUE_QUERIES as r, auditShortTermPromotionArtifacts as s, DEFAULT_PROMOTION_MIN_RECALL_COUNT as t, loadShortTermPromotionDreamingStats as u, recordRemConsideredPhaseSignals as v, repairShortTermPromotionArtifacts as x, recordShortTermRecalls as y };
