...
import { g as isGatewayTransportError } from "./call-ChM1o8yU.js";
...
function formatGatewayClosedDiagnostic(err) {
	if (!isGatewayTransportError(err) || err.kind !== "closed") return;
...
const formatHealthChannelLines = (summary, opts = {}) => {
	...
	for (const channelId of channelOrder) {
		...
		const selectedSummary = expectDefined(filteredSummaries && filteredSummaries.length > 0 ? filteredSummaries[0] : channelSummary, "channel health summary");
...
//#endregion
export { formatHealthChannelLines as n, formatHealthCheckFailure as r, formatGatewayClosedDiagnostic as t };
