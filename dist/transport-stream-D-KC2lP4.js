...
import { o as createProviderHttpError } from "./provider-http-errors-DrOMjuGn.js";
...
import { c as getEnvApiKey, r as calculateCost } from "./llm-23LMVVXI.js";
import { r as providerOperationRetryConfig } from "./operation-retry-Dmka1f2f.js";
import { S as resolveGoogleGemini3ThinkingLevel, T as stripInvalidGoogleThinkingBudget, h as isGoogleGemini3ProModel, m as isGoogleGemini3FlashModel, p as isGoogleGemini25ThinkingBudgetModel } from "./provider-stream-shared-BiURRLUJ.js";
import { _ as finalizeTransportStream, b as sanitizeTransportPayloadText, f as coerceTransportToolCallArguments, g as failTransportStream, m as createWritableTransportEventStream, p as createEmptyTransportUsage, v as mergeTransportHeaders, x as transformTransportMessages } from "./openai-transport-stream-810ZIbd4.js";
...
import { n as extractToolResultText, r as stripSystemPromptCacheBoundary, t as describeToolResultMediaPlaceholder } from "./provider-transport-runtime-BVpRn4AA.js";
import { r as stripGoogleProviderPrefix } from "./model-id-CAmKILzd.js";
import { d as normalizeGoogleApiBaseUrl } from "./provider-policy-DagFxEZx.js";
import "./thinking-api-Ci3xyPwM.js";
import { t as parseGeminiAuth } from "./gemini-auth-9ftktlLE.js";
import { n as resolveGoogleVertexAuthorizedUserHeaders, t as isGoogleVertexCredentialsMarker } from "./vertex-adc-CqpQnkCT.js";
...
function mapStopReasonString(reason) {
	switch (reason) {
		...
		case "MALFORMED_FUNCTION_CALL":
			console.warn("[google/transport-stream] Mapping MALFORMED_FUNCTION_CALL to stop for recovery");
			return "stop";
		default:
			console.warn("[google/transport-stream] Unmapped finishReason:", reason);
			return "error";
	}
}
...
function resolveGoogleThinkingConfig(model, options) {
	...
	if (!options?.reasoning || options.reasoning === "off") return getDisabledThinkingConfig(model.id);
...
function convertGoogleMessages(model, context) {
	...
	for (const msg of transformedMessages) {
		...
		if (msg.role === "toolResult") {
			...
			const imageContent = model.input.includes("image") ? msg.content.filter((item) => item.type === "image" && describeToolResultMediaPlaceholder([item]) !== void 0) : [];
			const mediaPlaceholder = describeToolResultMediaPlaceholder(msg.content);
			const responseValue = textResult ? sanitizeTransportPayloadText(textResult) : mediaPlaceholder ?? "";
...
function buildGoogleHeaders(model, apiKey, optionHeaders) {
	const authHeaders = apiKey ? parseGeminiAuth(apiKey).headers : void 0;
...
function isGoogleOauthApiKey(apiKey) {
	return Boolean(apiKey?.trimStart().startsWith("{") && parseGeminiAuth(apiKey).headers.Authorization);
}
function hasGoogleAuthHeader(headers) {
...
function collectGoogleTransportApiKeys(params) {
	if (params.kind !== "google-generative-ai" || !isOfficialGoogleGenerativeAiBaseUrl(params.model.baseUrl) || isGoogleOauthApiKey(params.primaryApiKey) || hasGoogleAuthHeader(params.model.headers) || hasGoogleAuthHeader(params.options?.headers)) return [];
	...
	return (rawModel, context, rawOptions) => {
		...
		(async () => {
			...
			try {
				...
				const openSse = async (apiKeyForRequest) => {
				...
				const sse = apiKeys.length > 0 ? await executeWithApiKeyRotation({
					provider: model.provider,
					apiKeys,
