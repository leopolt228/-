...
import { P as timestampMsToIsoString, b as parseStrictPositiveInteger, s as asFiniteNumber } from "./number-coercion-Crk_c9KW.js";
...
import { i as getMemoryCapabilityRegistration, s as listActiveMemoryPublicArtifacts } from "./memory-state-BkKwMbMM.js";
...
import { r as retryTransientMemoryRead } from "./hash-CLsGYYJA.js";
...
import { r as getActiveMemorySearchManager } from "./memory-host-search-THHuvRHZ.js";
import "./retry-runtime-DSaAoazp.js";
import "./param-readers-BngHHJgI.js";
import { t as fromMarkdown } from "./lib-vv6_0VBO.js";
import { A as loadMemoryWikiCompiledCache, E as createMemoryWikiCompiledCachePublicationId, M as resolveMemoryWikiCompiledCacheGeneration, P as writeMemoryWikiCompiledCache, S as writeMemoryWikiSourceSyncState, b as setImportedSourceEntry, c as readMemoryWikiImportRunRecord, g as pruneImportedSourceEntries, i as countMemoryWikiImportRunStateRows, j as reconcileMemoryWikiCompiledCacheOwner, k as invalidateMemoryWikiCompiledCache, l as resolveMemoryWikiImportRunsDir, p as assertMemoryWikiSourceSyncStateCapacity, t as MEMORY_WIKI_IMPORT_RUN_STATE_MAX_ENTRIES, u as writeMemoryWikiImportRunRecord, v as readMemoryWikiSourceSyncState, w as activateMemoryWikiCompiledCacheOwner, x as shouldSkipImportedSourceWrite } from "./import-runs-state-C0HM14oe.js";
import { i as resolveMemoryWikiAgentConfig, n as WIKI_SEARCH_CORPORA, t as WIKI_SEARCH_BACKENDS } from "./config-CRKKnYNv.js";
...
async function resolveMemoryWikiVaultSourceGeneration(vaultRoot) {
	const files = (await Promise.all(COMPILED_SOURCE_DIRECTORIES.map(async (relativeDir) => {
		...
		let entries;
		try {
			entries = await fs$1.readdir(dirPath, {
				withFileTypes: true,
				recursive: true
			});
		} catch (error) {
...
async function initializeMemoryWikiVault(config, options) {
	...
	activateMemoryWikiCompiledCacheOwner(config, await ensureMemoryWikiVaultGeneration(rootDir), (await loadMemoryWikiValidatedVaultIdentity(rootDir)).compiledCachePublicationId);
	await reconcileMemoryWikiCompiledCacheOwner(config, () => loadMemoryWikiValidatedVaultIdentity(rootDir));
	return {
		rootDir,
...
function buildCompiledCacheSnapshot(pagesInput) {
...
async function compileMemoryWikiVaultUnlocked(config) {
	...
	await writeMemoryWikiCompiledCache(config, compiledSnapshot, compiledCacheGeneration, compiledCachePublicationId, compiledInputIdentity.compiledCachePublicationId, async () => {
		...
		const verifiedGeneration = resolveMemoryWikiCompiledCacheGeneration(buildCompiledCacheSnapshot((await readPageSummaries(rootDir)).pages));
...
async function compileMemoryWikiVault(config) {
	return await withMemoryWikiVaultMutation(config.vault.path, () => compileMemoryWikiVaultUnlocked(config));
}
...
async function readQueryableWikiPagesByPaths(rootDir, files) {
	return await pMap(files, async (relativePath) => {
		const absolutePath = path.join(rootDir, relativePath);
...
async function searchWikiCorpus(params) {
	...
	const remainingPages = await readQueryableWikiPagesByPaths(rootDir, (await listWikiMarkdownFiles(rootDir)).filter((relativePath) => !seenPaths.has(relativePath)));
...
function isSourceWithinConfiguredPath(sourcePath, configuredPath) {
...
async function syncMemoryWikiUnsafeLocalSources(config) {
	...
	const { artifacts, unavailableConfiguredPaths } = await collectUnsafeLocalArtifacts(config.unsafeLocal.paths);
	const state = await readMemoryWikiSourceSyncState(config.vault.path);
	const activeKeys = /* @__PURE__ */ new Set();
	for (const [syncKey, entry] of Object.entries(state.entries)) if (entry.group === "unsafe-local" && unavailableConfiguredPaths.some((configuredPath) => isSourceWithinConfiguredPath(entry.sourcePath, configuredPath))) activeKeys.add(syncKey);
	assertMemoryWikiSourceSyncStateCapacity({
		state,
		group: "unsafe-local",
		incomingCount: (/* @__PURE__ */ new Set([...artifacts.map((artifact) => artifact.syncKey), ...activeKeys])).size
	});
	const results = await pMap(artifacts, async (artifact) => {
		const stats = await fs$1.stat(artifact.absolutePath);
		activeKeys.add(artifact.syncKey);
		...
		concurrency: UNSAFE_LOCAL_SYNC_CONCURRENCY,
		stopOnError: true
	});
...
async function syncMemoryWikiImportedSources(params) {
	...
	const matching = active.find((entry) => entry.requestKey === requestKey && entry.appConfig === params.appConfig);
	if (matching) return await matching.promise;
	const promise = withMemoryWikiVaultMutation(params.config.vault.path, () => syncMemoryWikiImportedSourcesOnce(params));
	const entry = {
		requestKey,
...
function registerWikiCli(program, registration) {
	...
	wiki.hook("preAction", () => {
		const requestedAgentId = wiki.opts().agent?.trim() || void 0;
...
//#endregion
export { loadMemoryWikiValidatedVaultIdentity as C, ensureMemoryWikiVaultGeneration as S, readQueryableWikiPages as _, syncMemoryWikiImportedSources as a, initializeMemoryWikiVault as b, runObsidianDaily as c, lintMemoryWikiVault as d, ingestMemoryWikiSource as f, getMemoryWikiPage as g, WIKI_SEARCH_MODES as h, resolveMemoryWikiStatus as i, runObsidianOpen as l, normalizeMemoryWikiMutationInput as m, buildMemoryWikiDoctorReport as n, probeObsidianCli as o, applyMemoryWikiMutation as p, renderMemoryWikiStatus as r, runObsidianCommand as s, registerWikiCli as t, runObsidianSearch as u, searchMemoryWiki as v, parseWikiMarkdown as x, compileMemoryWikiVault as y };
