import { a as normalizeLowercaseStringOrEmpty, c as normalizeOptionalString, d as normalizeStringifiedEntries } from "./string-coerce-DW4mBlAt.js";
import { n as sliceUtf16Safe, r as truncateUtf16Safe } from "./utf16-slice-lH-m0h6-.js";
import { r as createLazyRuntimeModule } from "./lazy-runtime-B-Fc-m0I.js";
import { C as resolveExpiresAtMsFromDurationMs, _ as parseStrictFiniteNumber, k as resolveOptionalIntegerOption, m as isFutureDateTimestampMs } from "./number-coercion-Crk_c9KW.js";
import { _ as uniqueStrings } from "./string-normalization-CRyoFBPt.js";
import { t as escapeRegExp } from "./regexp-BZyMFTlj.js";
import { r as logVerbose } from "./globals-DBBT7Ru5.js";
import { t as KeyedAsyncQueue } from "./keyed-async-queue-CTreGrmR.js";
import { o as getReplyPayloadTtsSupplement, r as buildTtsSupplementMediaPayload } from "./reply-payload-BtIUrr9c.js";
import { t as decodeHtmlEntities } from "./html-entities-CvDVeY8C.js";
import { D as formatLocationText, k as toLocationContext } from "./reply-payload-CPcXnHho.js";
import { N as resolveChannelStreamingBlockEnabled, S as resolveChannelProgressDraftMaxLines, a as createChannelProgressDraftGate, c as formatChannelProgressDraftText, d as isChannelProgressDraftWorkToolName, g as normalizeChannelProgressDraftLineIdentity, i as buildChannelProgressDraftLineForEntry, m as mergeChannelProgressDraftLine, o as formatChannelProgressDraftLine } from "./streaming-CeN4qI3u.js";
...
import { a as deliverWithFinalizableLivePreviewAdapter, n as createPreviewMessageReceipt, r as defineFinalizableLivePreviewAdapter } from "./live-wIeQu-vG.js";
...
import { h as toInboundMediaFacts } from "./kernel-BM-Mkfv5.js";
...
import { c as buildInboundHistoryFromEntries } from "./history-BCX82R6F.js";
...
import { a as resolveMatrixAccountConfig, i as resolveMatrixAccountAllowlistConfig } from "./account-config-S8LQ1GOC.js";
import { t as getMatrixRuntime } from "./runtime-Drg4hYqm.js";
import { n as resolveConfiguredMatrixBotUserIds } from "./accounts-z3wSTh4Y.js";
import { n as normalizeMatrixUserId, r as resolveMatrixAllowListMatch, t as normalizeMatrixAllowList } from "./allowlist-RzAK2K02.js";
import { t as isMatrixQualifiedUserId } from "./target-ids-B0f5Qh2N.js";
import { t as formatMatrixErrorMessage } from "./errors-DkzliO4p.js";
import { n as resolveMatrixRoomConfig } from "./rooms-DEDVaeK1.js";
import { t as resolveMatrixStoredSessionMeta } from "./session-store-metadata-DH5mDEz6.js";
import { _ as isPollStartType, a as sendMessageMatrix, g as isPollEventType, m as formatPollAsText, t as chunkMatrixText, y as parsePollStartContent } from "./send-BMVEUqQv.js";
import { o as MATRIX_OPENCLAW_FINALIZED_PREVIEW_KEY, r as promoteMatrixDirectRoomCandidate } from "./direct-management-D6z1RUj8.js";
...
import { i as createMatrixInboundEventDeduper } from "./inbound-dedupe-8I5aPRMp.js";
import { a as resolveMatrixMessageAttachment, c as fetchMatrixPollSnapshot, i as isLikelyBareFilename, n as formatMatrixMediaUnavailableText, o as resolveMatrixMessageBody, r as formatMatrixMessageText, t as formatMatrixMediaTooLargeText } from "./media-text-CMI-eApO.js";
import { n as setActiveMatrixClient } from "./active-client-CD1wI1Ud.js";
import { t as isBunRuntime } from "./runtime-BefyhPWv.js";
import { n as resolveMatrixAuth, r as resolveMatrixAuthContext, t as backfillMatrixAuthDeviceIdAfterStartup } from "./config-CrspAanc.js";
import { i as throwIfMatrixStartupAborted, r as isMatrixStartupAbortError } from "./startup-abort-D9iZK8RV.js";
import { i as resolveSharedMatrixClient, n as releaseSharedClientInstance } from "./shared-ByBMR70d.js";
import "./client-HJAK3fwW.js";
import { i as isMatrixMediaSizeLimitError, r as MatrixMediaSizeLimitError } from "./http-client-Bi36j_GQ.js";
import "./runtime-api-ZKiknwRf.js";
import { t as resolveMatrixTargets } from "./resolve-targets-lt_sEl20.js";
import { t as formatMatrixEncryptedEventDisabledWarning } from "./encryption-guidance-Cbmf4ZXP.js";
...
import { a as EventType, i as resolveMatrixThreadRouting, n as resolveMatrixReplyToEventId, o as RelationType, r as resolveMatrixThreadRootId, t as resolveMatrixInboundRoute } from "./route-C6-wvC6S.js";
...
function createDirectRoomTracker(client, opts = {}) {
	...
	const resolveDirectMemberFlag = async (roomId, userId) => {
		...
		rememberBounded(directMemberFlagCache, cacheKey, {
			isDirect,
			ts: now
	...
	const rememberLocallyPromotedDirectRoom = (roomId, remoteUserId) => {
		...
		rememberBounded(locallyPromotedDirectRooms, roomId, { remoteUserId: normalizedRemoteUserId });
	};
	return {
...
const loadMatrixDirectRoomDeps = createLazyRuntimeModule(() => Promise.all([import("./direct-management-DzIF7Hg_.js"), import("./direct-room--7Gc1VbC.js")]).then(([directManagementModule, directRoomModule]) => ({
...
function createMatrixVerificationEventRouter(params) {
	...
	async function routeVerificationSummary(summary) {
		...
		if (!trackBounded(routedVerificationSasFingerprints, sasFingerprint)) return;
	...
	function routeVerificationEvent(roomId, event) {
		...
		const routeTask = async () => {
			...
			if (stageNotice) {
				...
				if (trackBounded(routedVerificationStageNotices, stageKey)) notices.push(stageNotice);
			}
			if (summary && sasNotice) {
				const sasFingerprint = `${summary.id}:${JSON.stringify(summary.sas)}`;
				if (trackBounded(routedVerificationSasFingerprints, sasFingerprint)) notices.push(sasNotice);
			}
...
function registerMatrixMonitorEvents(params) {
	...
	client.on("room.event", (roomId, event) => {
		...
		if (eventType === EventType.RoomMember) {
			...
			logVerboseMessage(`matrix: member event room=${roomId} stateKey=${stateKey} membership=${membership ?? "unknown"}`);
		}
...
function decodeVisibleHtmlEntities(value) {
	return value.replace(MATRIX_HTML_ENTITY_RE, (entity) => {
		...
		return decoded === "\xA0" ? " " : decoded;
	});
}
function normalizeVisibleMentionText(value) {
	return normalizeLowercaseStringOrEmpty(decodeVisibleHtmlEntities(value.replace(/<[^>]+>/g, " ").replace(/[\u200b-\u200f\u202a-\u202e\u2060-\u206f]/g, "")).replace(/\s+/g, " "));
}
...
const loadMatrixPreflightAudioRuntime = createLazyRuntimeModule(() => import("./preflight-audio.runtime-BMCiLO77.js"));
...
const loadMatrixSendModule = createLazyRuntimeModule(() => import("./send-BQY4UseJ.js"));
const loadAcpBindingRuntime = createLazyRuntimeModule(() => import("./plugin-sdk/acp-binding-runtime.js"));
const loadSessionBindingRuntime = createLazyRuntimeModule(() => import("./plugin-sdk/session-binding-runtime.js"));
const loadMatrixReactionEvents = createLazyRuntimeModule(() => import("./reaction-events-CuAzWcw0.js"));
const loadMatrixDraftStream = createLazyRuntimeModule(() => import("./draft-stream-DDQebdqG.js"));
	...
	return async (roomId, event) => {
		...
		try {
			...
			const commitInboundEventIfClaimed = async () => {
				if (!inboundReplayClaim) return;
				await inboundReplayClaim.commit();
				inboundReplayClaim = void 0;
			};
			const readIngressPrefix = async () => {
				...
				if (eventId && inboundDeduper) {
					const claim = await inboundDeduper.claim({
						roomId,
						eventId
					});
					if (claim.kind === "claimed") inboundReplayClaim = claim.handle;
			...
			const storePath = resolveStorePathImpl(cfg.session?.store, { agentId: _route.agentId });
			const buildEnvelope = createChannelInboundEnvelopeBuilderImpl({
				cfg,
				route: _route
			});
			...
			const pushPlanProgress = async (steps, explanation) => {
				...
				if (!draftStream || previewToolProgressSuppressed) return;
				if (!progressDraftStreaming) {
					renderProgressDraft();
					return;
				}
			...
			const { deliver: deliverReply, onError: onReplyError, ...turnDispatcherOptions } = {
				...prefixOptions,
				...
				deliver: async (payload, info) => {
					if (draftStream && info.kind !== "tool" && !payload.isCompactionNotice) {
						...
						if (draftEventId && payload.text && !payload.isError && !hasMedia && !payloadReplyMismatch && !mustDeliverFinalNormally && !draftFinalTextNeedsNormalMentionDelivery) {
							...
							const mediaPayload = ttsSupplement && reusesDraftAsFinalText ? buildTtsSupplementMediaPayload(payload) : {
								...payload,
				...
				adapter: {
					...
					resolveTurn: () => ({
						...
						delivery: {
							...
							onError: (err, info) => onReplyError(err, info)
						},
						dispatcherOptions: {
						...
						replyOptions: {
							...
							onQueuedFollowupAdmitted: draftStream ? resetDraftDeliveryState : void 0,
...
const loadMatrixStartupMaintenanceDeps = createLazyRuntimeModule(() => Promise.all([
	import("./config-update-CjbkfXtW.js"),
	import("./device-health-CA7cIiLv.js"),
	import("./profile-hvH0xM3k.js"),
	import("./startup-verification-tziAqX18.js")
