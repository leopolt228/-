...
var MAX_PAYLOAD_BYTES = 256e3;
var MAX_PENDING_EVENTS = 256;
var MAX_TOOL_NAME_CHARS = 120;
...
function extractStandaloneCandidate(message, requireAssistantRole = false) {
  ...
  if (typeof record.content === "string") {
    return record.content.trim() ? { text: record.content, parts: [] } : void 0;
  }
  if (!Array.isArray(record.content)) {
    return void 0;
  }
  const candidate2 = { text: "", parts: [] };
  for (const [contentIndex, block] of record.content.entries()) {
    ...
    const start = candidate2.text.length;
    candidate2.text += value.text;
    candidate2.parts.push({ contentIndex, start, end: candidate2.text.length });
  }
  return candidate2.text.trim() ? candidate2 : void 0;
}
function scannedCall(scan) {
  ...
  if (scan.overCap && scan.payloadStart !== void 0) {
    return {
...
function scanHasNamedCandidate(scan) {
  const branches = [scan.json, scan.xmlish];
  ...
  while (index < text.length) {
    ...
    while (call && callStart < text.length) {
      if (call.incomplete && call.overCap) {
        ...
        while (structuralBoundaryIndex < structuralBoundaries.length && (structuralBoundaries[structuralBoundaryIndex] ?? 0) < (overCapOffset ?? Infinity)) {
...
function createCandidateScanView(candidate2) {
  const boundaries = candidate2.parts.slice(1).map((part) => part.start);
  return {
    boundaries,
...
function findCandidateCallSequences(candidate2, matcher) {
  const view = createCandidateScanView(candidate2);
...
function projectRangesOntoMessage(record, candidate2, ranges, preserveEmptyTextBlocks) {
  ...
  if (!Array.isArray(record.content)) {
    return { message: record, sourceToProjectedContentIndex: /* @__PURE__ */ new Map() };
  }
  const parts = new Map(candidate2.parts.map((part) => [part.contentIndex, part]));
  const content = [];
...
function projectScrubbedPlainTextToolCallMessage(params) {
  ...
  const candidate2 = extractStandaloneCandidate(
    params.message,
    params.requireAssistantRole === true
  );
  if (!record || !candidate2) {
    return void 0;
  }
  const sequences = findCandidateCallSequences(candidate2, params.matcher);
  const visibleOutsideCalls = Boolean(createRangeRemover(sequences)(candidate2.text).trim());
  const ranges = sequences.filter(
    (sequence) => params.forceKnownCandidates || sequence.overCap || visibleOutsideCalls || params.forceIncompleteCandidates && sequence.activeStart !== void 0
  );
  return ranges.length > 0 ? projectRangesOntoMessage(record, candidate2, ranges, params.preserveEmptyTextBlocks === true) : void 0;
}
function findPotentialCallStart(text, atLineStart, matcher) {
  for (let index = 0; index < text.length; ) {
    const lineStart = index === 0 && atLineStart || text[index - 1] === "\n" || text[index - 1] === "\r";
    if (!lineStart) {
      index += 1;
      continue;
    }
...
function cappedUtf8ByteLength(text) {
  return utf8ByteLengthWithinLimit(text, 0, text.length, MAX_PAYLOAD_BYTES) ?? MAX_PAYLOAD_BYTES + 1;
}
...
function queuePendingEvent(pending, record) {
  ...
  const canMerge = typeof previous?.delta === "string" && typeof event.delta === "string" && previous.type === event.type && eventContentIndex(previous) === eventContentIndex(event);
...
function projectEventIndex(event, projection) {
  ...
  return contentIndex === void 0 ? void 0 : { ...event, contentIndex };
}
function projectedTextForEvent(event, projection) {
  ...
  const block = Array.isArray(content) && projectedIndex !== void 0 ? asRecord(content[projectedIndex]) : void 0;
  return block?.type === "text" && typeof block.text === "string" ? block.text : void 0;
}
var XML_PARAMETER_CLOSE = "</parameter>";
var XML_FUNCTION_CLOSE = "</function>";
var XML_PARAMETER_OPEN = "<parameter=";
function createOverCapSuppressor(candidate2, matcher, force = false) {
  const view = createCandidateScanView(candidate2);
  ...
  const state = value?.json ?? (json.kind === "complete" ? { depth: 0, escaped: false, inString: false } : void 0);
  ...
  const jsonSuppressor = value ? {
    ...
    ...value.syntax === "named-bracket" ? { requiredClosing: `[/${name}]` } : { optionalClosings: [HARMONY_CALL_MARKER, END_TOOL_REQUEST, `[/${name}]`] }
  } : void 0;
  ...
  if (xmlish.kind === "prefix" && matches.xmlish && xmlish.candidate?.payload && (force || utf8ByteLengthWithinLimit(
    view.text,
    ...
    const carry = markerStart !== -1 && markers.some((marker) => isAsciiMarkerPrefixIgnoreCase(view.text, markerStart, marker)) ? view.text.slice(markerStart) : "";
    return {
      kind: "xml",
      carry,
      phase
  ...
  if (!value || !jsonSuppressor || !value.nameComplete && !value.payload || !matches.json || !state && !force || !value.payload && !force || !force && value.payload && utf8ByteLengthWithinLimit(
    view.text,
    value.payload.start,
    value.payload.end,
    MAX_PAYLOAD_BYTES
...
function classifyPending(pending, matcher, finalize = false) {
  const candidate2 = { text: pending.buffer, parts: pending.parts };
  const view = createCandidateScanView(candidate2);
  ...
  const sequences = findCandidateCallSequences(candidate2, matcher);
  ...
  if (leading?.activeStart !== void 0 && (pending.sequenceOverCap || overCapRanges.length > 0)) {
    const activeCandidate = {
      text: candidate2.text.slice(leading.activeStart),
      parts: candidate2.parts.filter((part) => part.end > (leading.activeStart ?? 0)).map((part) => ({
  ...
  if (overCapRanges.length > 0) {
    const text = createRangeRemover(overCapRanges)(candidate2.text);
    const suppressor = text ? void 0 : createOverCapSuppressor(candidate2, matcher);
    return suppressor ? { kind: "suppress", suppressor } : { kind: "stripped", text };
  }
  if (leading && leading.activeStart === void 0 && skipWhitespace(candidate2.text, leading.end) < candidate2.text.length) {
    return { kind: "stripped", text: createRangeRemover([leading])(candidate2.text) };
  }
  if (leading && leading.activeStart === void 0) {
    return pending.sequenceOverCap || pending.bufferBytes > MAX_PAYLOAD_BYTES ? { kind: "stripped", text: "" } : { kind: "complete" };
  }
  if (leading?.activeStart !== void 0) {
    return !hasNamedCandidate && finalize ? { kind: "false-positive" } : { kind: "incomplete" };
  }
  if (terminalScan.kind === "prefix" && !hasNamedCandidate && pending.bufferBytes > MAX_PAYLOAD_BYTES) {
    return { kind: "false-positive" };
  }
  if (terminalScan.kind === "prefix" && (!finalize || hasNamedCandidate)) {
    return { kind: "incomplete" };
  }
  return pending.sequenceOverCap ? { kind: "stripped", text: candidate2.text } : { kind: "false-positive" };
}
function consumeXmlSuppressor(suppressor, chunk) {
  ...
  while (true) {
    ...
    const markerPrefix = isAsciiMarkerPrefixIgnoreCase(text, markerStart, XML_FUNCTION_CLOSE) || isAsciiMarkerPrefixIgnoreCase(text, markerStart, XML_PARAMETER_OPEN);
    if (markerPrefix) {
    ...
    if (startsWithAsciiMarkerIgnoreCase(text, markerStart, XML_PARAMETER_OPEN)) {
      ...
      if (!name || name.length > MAX_TOOL_NAME_CHARS || Array.from(name).some((character) => !isXmlishNameChar(character))) {
        return { complete: true, suffix: text.slice(markerStart) };
      }
...
function consumeJsonSuppressor(suppressor, chunk) {
  ...
  if (suppressor.requiredClosing) {
    ...
    if (closing) {
      const end2 = consumeRemovedLineEnd(rest, closing.length);
      return { complete: true, suffix: rest.slice(end2) };
    }
  ...
  if (optionalClosing) {
    const end2 = consumeRemovedLineEnd(rest, optionalClosing.length);
    return { complete: true, suffix: rest.slice(end2) };
  }
...
function consumeOpeningSuppressor(suppressor, chunk) {
  if (suppressor.choice) {
    return suppressor.choice.kind === "xml" ? consumeXmlSuppressor(suppressor.choice, chunk) : consumeJsonSuppressor(suppressor.choice, chunk);
  }
  const text = suppressor.carry + chunk;
  suppressor.carry = "";
...
async function* normalizePlainTextToolCallStreamEvents(source, options) {
  ...
  const sanitizeEventPartial = (record, forceKnownCandidates = false) => {
    ...
    return projected ? { ...projected, partial: projection.message } : void 0;
  };
  const forceProjectPendingAux = (candidate2, projection, retainedTextContentIndex) => projectPendingAuxEvents(
    candidate2,
    projection,
  ...
  async function* normalizeEvents() {
    for await (const sourceEvent of source) {
      ...
      if (scrubFuturePartials && !pending && type !== "done" && type !== "error" && record.partial !== void 0) {
        ...
        record = projection ? { ...projectedEvent, partial: projection.message } : sanitizeEventPartial(projectedEvent, true) ?? projectedEvent;
      }
        ...
        while (true) {
          if (pending?.kind === "suppressing") {
            if (closesText) {
              ...
              const projectedText2 = projection && projectedTextForEvent(record, projection);
              const novelText2 = projectedText2?.slice(emittedTextUnits.get(key) ?? 0);
              if (novelText2 && projection) {
                yield createSyntheticTextDelta(record, novelText2, projection.message);
              }
              pending = void 0;
            ...
            const partialProjection2 = scrubSnapshot(record.partial, true, true);
            const partial2 = partialProjection2?.message;
            yield* forceProjectPendingAux(pending, partialProjection2);
          ...
          if (!pending) {
            ...
            if (callStart === null) {
              const held2 = heldTextStarts.get(key);
              if (held2) {
                yield held2;
                heldTextStarts.delete(key);
              }
              ...
              if (incoming) {
                ...
                const contentIndex2 = eventContentIndex(incomingRecord);
                preserveTerminalContentIndexes ||= (sequenceOverCap || continuesScrubbedSequence) && contentIndex2 > 0;
              }
              lineStarts.set(key, nextAtLineStart(atLineStart, incoming));
              break;
            }
            const visiblePrefix = incoming.slice(0, callStart);
            const emittedUnits2 = emittedTextUnits.get(key) ?? 0;
            const emittedPrefixUnits = authoritative ? emittedUnits2 : 0;
            ...
            const candidateRecord = typeof incomingRecord.delta === "string" ? { ...incomingRecord, delta: candidateText } : authoritative ? incomingRecord : { ...incomingRecord, content: candidateText };
            const held = heldTextStarts.get(key);
            heldTextStarts.delete(key);
            pending = createPendingState(
              ...
              authoritative ? callStart : emittedUnits2 + callStart
            );
            overCapSequenceOpen = false;
            ...
            if (authoritative) {
              ...
              const partIndex = pending.parts.findLastIndex(
                (part2) => part2.contentIndex === contentIndex2
              );
              ...
              if (part) {
                ...
                pending.parts = pending.parts.map(
                  (entry, index) => index < partIndex ? entry : index === partIndex ? { ...entry, end: entry.start + blockText.length } : {
                    ...entry,
          ...
          const shouldClassify = authoritative || pending.bufferBytes > MAX_PAYLOAD_BYTES || pending.buffer.length <= 256 || pending.buffer.length >= pending.nextScanChars;
          if (!shouldClassify) {
            break;
          }
          ...
          if (classification.kind === "trim") {
            ...
            const partialProjection2 = scrubSnapshot(record.partial, true, true);
            yield* forceProjectPendingAux(pending, partialProjection2);
            const candidate2 = classification.candidate;
            pending.buffer = candidate2.text;
            pending.bufferBytes = cappedUtf8ByteLength(candidate2.text);
            pending.entries = void 0;
            pending.entryBytes = 0;
            pending.nextScanChars = 256;
            pending.parts = candidate2.parts;
            pending.sequenceOverCap = true;
            pending.snapshotOffset = 0;
            pending.template = {
              ...pending.template,
              contentIndex: candidate2.parts[0]?.contentIndex ?? pending.template.contentIndex
            };
            break;
          }
          ...
          const authoritativeProjection = partialProjection ?? (authoritative ? scrubSnapshot({ role: "assistant", content: pending.buffer }, true, true) : void 0);
          ...
          const partial = outputProjection?.message ?? (contentIndex === 0 ? { role: "assistant", content: [{ type: "text", text: sanitizedText }] } : void 0);
          yield* forceProjectPendingAux(
            pending,
            outputProjection,
            sanitizedText ? contentIndex : void 0
          );
      ...
      if (type === "done") {
        ...
        const terminalHasIncompleteCandidate = terminalCandidate && findCandidateCallSequences(terminalCandidate, options.matcher).some(
          (sequence) => sequence.activeStart !== void 0
        );
        const terminalCandidateProjection = terminalHasIncompleteCandidate ? scrubSnapshot(record.message, preserveTerminalContentIndexes, true) : void 0;
        const normalized = forcedProjection ? { kind: "scrubbed", ...forcedProjection } : forceScrubTerminal ? void 0 : requestedNormalization ?? (terminalCandidateProjection ? { kind: "scrubbed", ...terminalCandidateProjection } : void 0);
...
function createPromotedPlainTextToolCallEvents(message) {
  ...
  return content.flatMap((block, contentIndex) => {
    const toolCall = asRecord2(block);
...
function projectStandalonePlainTextToolCallMessage(options) {
  const messageRecord = asRecord2(options.message);
