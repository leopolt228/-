...
function readLowerString(value) {
  return readString(value)?.toLowerCase();
}
function hasHardTimeoutMetadata(data, statusAlreadyTimeoutAttributed = false) {
  const timeoutPhase = readLowerString(data.timeoutPhase);
  return statusAlreadyTimeoutAttributed && data.providerStarted === true || timeoutPhase === "preflight" || timeoutPhase === "provider" || timeoutPhase === "post_turn";
}
function isLifecycleCancellation(data) {
  const status = readLowerString(data.status);
  const stopReason = readLowerString(data.stopReason);
  return status === "aborted" || status === "cancelled" || status === "canceled" || status === "killed" || stopReason === "aborted" || stopReason === "cancelled" || stopReason === "canceled" || stopReason === "killed" || stopReason === "auth-revoked" || stopReason === "restart" || stopReason === "rpc" || stopReason === "user" || data.aborted === true && stopReason === "stop";
}
function normalizeLifecycleEndEventType(data) {
  const status = readLowerString(data.status);
  const stopReason = readLowerString(data.stopReason);
  const statusAlreadyTimeoutAttributed = stopReason !== "restart" && (status === "timeout" || status === "timed_out" || data.aborted === true);
...
function normalizeAgentEventType(payload) {
  const stream = readString(payload.stream);
  const data = asRecord(payload.data);
  const phase = readString(data.phase);
  const status = readString(data.status);
  if (stream === "assistant") {
    return data.delta === true || typeof data.delta === "string" ? "assistant.delta" : "assistant.message";
  }
...
function normalizeNamedEventType(event) {
...
var RAW_EVENT_REPLAY_LIMIT = 1e3;
...
var GatewayClientTransport = class {
  constructor(options = {}) {
    this.eventsHub = new EventHub({
  ...
  connect() {
    ...
    this.connectPromise = new Promise((resolve, reject) => {
      ...
      const client = new GatewayClient({
        ...this.options,
        onEvent: (event) => {
          const normalized2 = toGatewayEvent(event);
          this.eventsHub.publish(normalized2);
          this.options.onEvent?.(normalized2);
        },
        onHelloOk: (_hello) => {
          try {
...
function isConnectableTransport(transport) {
  return typeof transport.connect === "function";
}

// packages/sdk/src/client.ts
var MAX_REPLAY_RUNS = 100;
var MAX_REPLAY_EVENTS_PER_RUN = 500;
var MAX_NORMALIZED_REPLAY_EVENTS = 2e3;
...
function runStatusFromWaitPayload(payload) {
  const record = typeof payload === "object" && payload !== null ? payload : {};
  const status = typeof record.status === "string" ? record.status.toLowerCase() : void 0;
  ...
  const timeoutPhase = typeof record.timeoutPhase === "string" ? record.timeoutPhase.toLowerCase() : void 0;
  const statusAlreadyTimeoutAttributed = status === "timeout" || status === "timed_out";
  const hardTimeout = !pendingError && (stopReason !== "restart" && record.providerStarted === true && statusAlreadyTimeoutAttributed || timeoutPhase === "preflight" || timeoutPhase === "provider" || timeoutPhase === "post_turn");
  const hasTerminalTimeoutMetadata = readOptionalTimestamp(record.endedAt) !== void 0 || !pendingError && readOptionalString(record.error) !== void 0 || stopReason.length > 0 || typeof record.livenessState === "string" || record.yielded === true;
  if (hardTimeout) {
    return "timed_out";
  }
  if (status === "aborted" || status === "cancelled" || status === "canceled" || status === "killed" || stopReason === "aborted" || stopReason === "cancelled" || stopReason === "canceled" || stopReason === "killed" || stopReason === "auth-revoked" || stopReason === "restart" || stopReason === "rpc" || stopReason === "user" || record.aborted === true && stopReason === "stop") {
    return "cancelled";
  }
  ...
  if (status === "timeout") {
    if (stopReason === "timeout" || stopReason === "timed_out" || record.aborted === true || hasTerminalTimeoutMetadata) {
      return "timed_out";
    }
    return "accepted";
  }
...
function timeoutSecondsFromMs(timeoutMs) {
  const normalized2 = normalizeTimeoutMs(timeoutMs);
  if (normalized2 === void 0) {
    return void 0;
  }
  return normalized2 === 0 ? 0 : Math.ceil(normalized2 / 1e3);
}
...
function assertNoUnsupportedRunOptions(params) {
  const unsupported = [
    params.workspace ? "workspace" : void 0,
    params.runtime ? "runtime" : void 0,
    params.environment ? "environment" : void 0,
    params.approvals ? "approvals" : void 0
  ].filter((value) => Boolean(value));
  if (unsupported.length === 0) {
    return;
  }
  throw new Error(
    `OpenClaw Gateway does not support per-run SDK option${unsupported.length === 1 ? "" : "s"} yet: ${unsupported.join(", ")}`
  );
}
function buildAgentParams(params) {
  ...
  return {
    ...
    ...timeoutSeconds !== void 0 ? { timeout: timeoutSeconds } : {},
    ...params.label ? { label: params.label } : {},
    idempotencyKey: params.idempotencyKey ?? randomUUID2()
  };
}
function unsupportedGatewayApi(api) {
  throw new Error(`${api} is not supported by the current OpenClaw Gateway yet`);
}
function asRecord2(value) {
  return typeof value === "object" && value !== null ? value : {};
}
function hasArtifactQueryScope(params) {
  const record = asRecord2(params);
  return [record.sessionKey, record.runId, record.taskId].some(
    (value) => typeof value === "string" && value.trim().length > 0
  );
}
function requireArtifactQueryScope(api, params) {
  if (!hasArtifactQueryScope(params)) {
    throw new Error(`${api} requires one of sessionKey, runId, or taskId`);
  }
  return params;
}
function hasToolsEffectiveSessionKey(params) {
  const record = asRecord2(params);
  return typeof record.sessionKey === "string" && record.sessionKey.trim().length > 0;
}
function requireToolsEffectiveSessionKey(params) {
  if (!hasToolsEffectiveSessionKey(params)) {
    throw new Error("oc.tools.effective requires sessionKey");
  }
  return params;
}
function readChatProjection(event) {
  const raw = event.raw;
  if (event.type !== "raw" || raw?.event !== "chat") {
    return void 0;
  }
  const payload = asRecord2(raw.payload);
  return payload.state === "delta" || payload.state === "final" ? { state: payload.state, payload } : void 0;
}
function readChatProjectionText(payload) {
  const message = asRecord2(payload.message);
...
function readChatProjectionDeltaText(payload) {
  return typeof payload.deltaText === "string" ? payload.deltaText : void 0;
}
function readChatProjectionReplace(payload) {
...
function isTerminalRunEvent(event) {
  return event.type === "run.completed" || event.type === "run.failed" || event.type === "run.cancelled" || event.type === "run.timed_out";
}
function normalizeChatProjectionEvent(event, projection, previousText) {
  const text = readChatProjectionText(projection.payload);
  const deltaText = readChatProjectionDeltaText(projection.payload);
  const hasPreviousText = previousText !== void 0;
  ...
  return {
    ...
    data: projection.state === "delta" ? text !== void 0 ? {
      text,
      delta: hasPreviousText ? deltaText ?? text : text,
      ...isReplacement ? { replace: true } : {}
    } : event.data : { phase: "end", ...text !== void 0 ? { outputText: text } : {} }
  };
}
var OpenClaw = class {
  constructor(options = {}) {
    this.normalizedEvents = new EventHub({
      replayLimit: MAX_NORMALIZED_REPLAY_EVENTS
    });
    this.replayByRunId = /* @__PURE__ */ new Map();
    this.connected = false;
    this.closed = false;
    this.eventPumpPromise = null;
    this.eventPumpReady = null;
    this.closePromise = null;
    this.transport = options.transport ?? new GatewayClientTransport({
  ...
  async *iterateRunEvents(runId, filter) {
    await this.connect();
    this.assertOpen();
    ...
    let previousChatProjectionText;
    const toRunStreamEvent = (event) => {
    ...
    const matches = (event) => event.runId === runId;
  ...
  startEventPump() {
    ...
    let ready = false;
    this.eventPumpReady = new Promise((resolve) => {
      markReady = () => {
        if (ready) {
          return;
        }
    ...
    this.eventPumpPromise = (async () => {
      ...
      try {
        ...
        while (true) {
          ...
          const normalized2 = normalizeGatewayEvent(result.value);
          this.recordReplayEvent(normalized2);
          this.normalizedEvents.publish(normalized2);
        }
      } catch (error) {
        pumpError = error;
        hasPumpError = true;
...
var Agent = class {
  ...
  async run(input) {
    const params = typeof input === "string" ? { input, agentId: this.id } : { ...input, agentId: this.id };
...
var Run = class {
  constructor(client, id, sessionKey) {
    this.client = client;
    this.id = id;
  ...
  async wait(options) {
    ...
    const error = readOptionalString(record.error) ? { message: readOptionalString(record.error) ?? "run failed" } : void 0;
    return {
      runId: this.id,
      status,
...
var Session = class {
  constructor(client, key, info) {
    this.client = client;
    this.key = key;
    this.info = info;
  }
  async send(input) {
    const params = typeof input === "string" ? { key: this.key, message: input } : { ...input, key: this.key };
    ...
    const raw = await this.client.request("sessions.send", params, {
      expectFinal: true,
      ...timeoutMs !== void 0 ? { timeoutMs: timeoutMs === 0 ? null : timeoutMs } : {}
    });
    const record = asRecord2(raw);
...
var AgentsNamespace = class {
  ...
  async list(params) {
    return await this.client.request("agents.list", params === void 0 ? {} : params);
  }
  async get(id) {
...
var SessionsNamespace = class {
  ...
  async list(params) {
    return await this.client.request("sessions.list", params === void 0 ? {} : params);
  }
  async create(params = {}) {
    const raw = await this.client.request("sessions.create", params);
    const record = asRecord2(raw);
    const key = readOptionalString(record.key) ?? readOptionalString(record.sessionKey) ?? params.key;
    if (!key) {
...
var RunsNamespace = class {
  ...
  async create(params) {
    const timeoutMs = normalizeTimeoutMs(params.timeoutMs);
    const normalizedParams = timeoutMs !== void 0 ? { ...params, timeoutMs } : params;
...
var RpcNamespace = class {
  constructor(client, prefix) {
    this.client = client;
    this.prefix = prefix;
  }
  async call(method, params, options) {
    return await this.client.request(`${this.prefix}.${method}`, params, options);
  }
};
var TasksNamespace = class extends RpcNamespace {
  constructor(client) {
    super(client, "tasks");
  }
  async list(params) {
    return await this.call("list", params === void 0 ? {} : params);
  }
  async get(taskId) {
...
var ModelsNamespace = class extends RpcNamespace {
  constructor(client) {
    super(client, "models");
  }
  async list(params) {
...
var ToolsNamespace = class extends RpcNamespace {
  constructor(client) {
    super(client, "tools");
  }
  async list(params) {
    return await this.call("catalog", params === void 0 ? {} : params);
  }
...
var ArtifactsNamespace = class extends RpcNamespace {
  constructor(client) {
...
var ApprovalsNamespace = class {
  constructor(client) {
    this.client = client;
  }
  async list(params) {
    return await this.client.request("exec.approval.list", params === void 0 ? {} : params);
  }
  async respond(approvalId, params) {
...
var EnvironmentsNamespace = class extends RpcNamespace {
  constructor(client) {
