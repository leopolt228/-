...
function maskSecretValue(token, options) {
  ...
  return `${options?.hinted ? maskToken2(maskable) : "***"}${suffix}`;
}
...
function redactPemBlock2(block) {
  const lines = block.split(/\r?\n/).filter(Boolean);
  if (lines.length < 2) {
    return "***";
  }
...
function redactMatch2(match, groups, pattern, context) {
  if (match.includes("PRIVATE KEY-----")) {
    return redactPemBlock2(match);
  }
  ...
  const masked = isShellReferencePattern ? maskToken2(token) : maskSecretValue(token, { hinted: true });
  if (token === match) {
    return masked;
  }
...
function redactText(text, patterns, options) {
  ...
  for (const pattern of patterns) {
    const replacer = (...args) => {
      ...
      return redactMatch2(match, groups, pattern, { input, offset });
    };
...
function redactAppSpecificPasswords(text) {
  return replacePatternBounded(
    ...
    (match, token) => looksLikeAppSpecificPassword(token) ? redactMatch2(match, [token], APP_SPECIFIC_PASSWORD_RE) : match
  );
}
...
function redactSensitiveText3(text, options) {
  if (!text) {
    return text;
  }
  const exactRedacted = redactRegisteredSecretValues(text, maskToken2);
...
function redactSensitiveFieldValueWithOptions(key, value, options, path6 = [key]) {
  const exactRedacted = redactRegisteredSecretValues(value, maskToken2);
  ...
  if (isSensitiveFieldKey(key)) {
    ...
    return maskToken2(exactRedacted);
  }
  return exactRedacted;
}
...
function redactSecrets(value) {
  ...
  if (typeof value === "string") {
    return redactSensitiveText3(value, options);
  }
  if (value === null || value === void 0) {
    return value;
  }
...
// src/infra/errors.ts
function formatErrorMessage3(err) {
  return formatErrorMessage(err, { redact: redactSensitiveText3 });
}
...
function requireNodeSqlite() {
  ...
  try {
    ...
    const message = formatErrorMessage3(err);
...
function sanitizeDiagnosticLogText(value, maxChars) {
  return clampDiagnosticLogText(
    redactSensitiveText3(clampDiagnosticLogText(value, maxChars)),
    maxChars
  );
}
...
function assignDiagnosticLogAttribute(attributes, state, key, value) {
  ...
  if (redactSensitiveText3(normalizedKey) !== normalizedKey) {
    return;
  }
...
function buildLogger(settings) {
  ...
  logger.attachTransport((logObj) => {
    try {
      ...
      const line = redactSensitiveText3(JSON.stringify(redactLogRecordForTransport(record)));
      const payload = `${line}
`;
...
function formatConsoleLine(opts) {
  ...
  const redactedMessage = redactSensitiveText3(opts.message);
...
function writeConsoleLine(level, line, opts = {}) {
  ...
  const redacted = opts.redacted ? sanitized : redactSensitiveText3(sanitized);
...
var MEMORY_INDEX_META_TABLE = "memory_index_meta";
var MEMORY_INDEX_SOURCES_TABLE = "memory_index_sources";
var MEMORY_INDEX_CHUNKS_TABLE = "memory_index_chunks";
var MEMORY_EMBEDDING_CACHE_TABLE = "memory_embedding_cache";
var MEMORY_INDEX_STATE_TABLE = "memory_index_state";
var MEMORY_INDEX_FTS_TABLE = "memory_index_chunks_fts";
var MEMORY_INDEX_PATHS_FTS_TABLE = "memory_index_paths_fts";
var MEMORY_INDEX_VECTOR_TABLE = "memory_index_chunks_vec";
var MEMORY_PATH_FTS_TRIGGER_DEFINITIONS = [
  {
...
var LEGACY_MEMORY_INDEX_TRIGGERS = [
...
var LEGACY_MEMORY_INDEX_SOURCE_COLUMNS = ["path", "source", "hash", "mtime", "size"];
var MEMORY_INDEX_SOURCE_COLUMNS = ["id", ...LEGACY_MEMORY_INDEX_SOURCE_COLUMNS];
var MEMORY_INDEX_SOURCE_COLUMN_TYPES = /* @__PURE__ */ new Map([
  ["id", "INTEGER"],
  ["path", "TEXT"],
  ["source", "TEXT"],
  ["hash", "TEXT"],
...
function tableColumnInfo(db, tableName, schema = "main") {
  const rows = db.prepare(`PRAGMA ${schema}.table_xinfo(${tableName})`).all();
  return rows.flatMap(
    (row) => typeof row.name === "string" && typeof row.type === "string" ? [
      {
        name: row.name,
...
function tableHasUniqueIndex(db, tableName, expectedColumns) {
  ...
  return indexes.some((index) => {
    ...
    ).all(index.name).filter((row) => Number(row.key ?? 0) === 1);
    return columns.length === expectedColumns.length && columns.every(
      (column, columnIndex) => Number(column.cid ?? -1) >= 0 && column.name === expectedColumns[columnIndex] && column.coll === "BINARY" && Number(column.sort_desc ?? 0) === 0
    );
  });
}
...
function tableHasCanonicalSourceColumnTypes(db) {
  ...
  return columns.every((column) => {
    ...
    if (column.type !== expectedType && !(column.name === "mtime" && column.type === "INTEGER") || column.defaultValue !== expectedDefault || column.hidden !== 0) {
      return false;
    }
    return true;
  });
...
function tableHasCanonicalSourceColumns(db) {
  return tableHasCanonicalSourceColumnTypes(db) && tableColumnInfo(db, MEMORY_INDEX_SOURCES_TABLE).every((column) => {
...
function tableHasIntegerRowIdPrimaryKey(db) {
  ...
  const primaryKeyIndex = db.prepare(`SELECT 1 AS found FROM pragma_index_list(?) WHERE origin = 'pk' LIMIT 1`).get(MEMORY_INDEX_SOURCES_TABLE);
  return primaryKeyIndex?.found !== 1;
}
function tableExists(db, tableName) {
  const row = db.prepare(`SELECT 1 AS found FROM sqlite_master WHERE type = 'table' AND name = ?`).get(tableName);
  return row?.found === 1;
}
...
function migrateMemoryIndexSourcesIdentity(db) {
  ...
  if (tableHasExactColumns(db, MEMORY_INDEX_SOURCES_TABLE, MEMORY_INDEX_SOURCE_COLUMNS)) {
    if (tableHasCanonicalSourceColumns(db) && tableHasIntegerRowIdPrimaryKey(db) && tableHasNoDeclaredCollations(db, MEMORY_INDEX_SOURCES_TABLE) && tableHasUniqueIndex(db, MEMORY_INDEX_SOURCES_TABLE, ["path", "source"])) {
      return;
    }
...
function hasLegacyMemoryIndexTables(db, schema = "main") {
  return tableHasExactColumns(db, "meta", ["key", "value"], schema) && tableHasExactColumns(db, "files", ["path", "source", "hash", "mtime", "size"], schema) && tableHasExactColumns(
    db,
    "chunks",
    [
      "id",
...
function ensureMemoryIndexSchema(params) {
  ...
  if (params.cacheEnabled) {
    const updatedAtIndex = embeddingCacheTable === MEMORY_EMBEDDING_CACHE_TABLE ? "idx_memory_embedding_cache_updated_at" : "idx_embedding_cache_updated_at";
    params.db.exec(`
