...
// packages/gateway-protocol/src/schema/environments.ts
var EnvironmentStatusSchema = Type3.String({
  enum: ["available", "unavailable", "starting", "stopping", "error"]
});
var WorkerEnvironmentStateSchema = Type3.Union([
  Type3.Literal("requested"),
  Type3.Literal("provisioning"),
  Type3.Literal("bootstrapping"),
  Type3.Literal("ready"),
  Type3.Literal("attached"),
  Type3.Literal("idle"),
  Type3.Literal("draining"),
  Type3.Literal("destroying"),
  Type3.Literal("destroyed"),
  Type3.Literal("failed"),
  Type3.Literal("orphaned")
]);
var WorkerTunnelStatusSchema = Type3.Union([
  Type3.Literal("stopped"),
  Type3.Literal("connecting"),
  Type3.Literal("connected"),
  Type3.Literal("reconnecting")
]);
var WorkerEnvironmentMetadataSchema = closedObject({
  providerId: NonEmptyString,
  leaseId: Type3.Optional(NonEmptyString),
  state: WorkerEnvironmentStateSchema,
  ageMs: Type3.Integer({ minimum: 0 }),
  idleMs: Type3.Optional(Type3.Integer({ minimum: 0 })),
  attachedSessionIds: Type3.Array(NonEmptyString),
...
function createEnvironmentSummarySchema() {
  return closedObject({
    ...
    capabilities: Type3.Optional(Type3.Array(NonEmptyString)),
    worker: Type3.Optional(WorkerEnvironmentMetadataSchema)
  });
}
...
var EnvironmentsListResultSchema = closedObject({
  environments: Type3.Array(EnvironmentSummarySchema),
  profiles: Type3.Optional(Type3.Array(WorkerEnvironmentProfileSummarySchema))
});
