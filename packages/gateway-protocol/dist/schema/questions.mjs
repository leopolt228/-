...
// packages/gateway-protocol/src/schema/questions.ts
var QuestionIdSchema = Type3.String({ pattern: "^[a-z][a-z0-9_]*$" });
var QuestionHeaderSchema = Type3.String({ maxLength: 12 });
...
var QuestionInputFields = {
  ...
  options: Type3.Array(QuestionOptionSchema, { maxItems: 4 }),
  multiSelect: Type3.Optional(Type3.Boolean()),
  isOther: Type3.Optional(Type3.Boolean()),
  isSecret: Type3.Optional(Type3.Boolean())
};
...
var QuestionAnswersSchema = closedObject({
  answers: Type3.Record(QuestionIdSchema, Type3.Array(Type3.String()))
});
var QuestionStatusSchema = Type3.Union([
  Type3.Literal("pending"),
  Type3.Literal("answered"),
  Type3.Literal("cancelled"),
  Type3.Literal("expired")
]);
var QuestionRecordSchema = closedObject({
  id: NonEmptyString,
  questions: Type3.Array(QuestionSchema, { minItems: 1, maxItems: 3 }),
  ...
  answers: Type3.Optional(QuestionAnswersSchema),
  resolvedBy: Type3.Optional(NonEmptyString)
});
var QuestionRequestParamsSchema = closedObject({
  id: Type3.Optional(NonEmptyString),
  questions: Type3.Array(QuestionRequestQuestionSchema, { minItems: 1, maxItems: 3 }),
  agentId: Type3.Optional(NonEmptyString),
  sessionKey: Type3.Optional(NonEmptyString),
...
var QuestionWaitAnswerResultSchema = Type3.Union([
  closedObject({ status: Type3.Literal("pending") }),
  closedObject({ status: Type3.Literal("answered"), answers: QuestionAnswersSchema }),
  closedObject({ status: Type3.Literal("cancelled") }),
  closedObject({ status: Type3.Literal("expired") })
]);
var QuestionResolveParamsSchema = Type3.Union([
  closedObject({
    id: NonEmptyString,
  ...
  closedObject({
    id: NonEmptyString,
    cancel: Type3.Literal(true),
    resolvedBy: Type3.Optional(NonEmptyString)
  })
]);
var QuestionResolveResultSchema = Type3.Union([
  closedObject({ status: Type3.Literal("answered"), answers: QuestionAnswersSchema }),
  closedObject({ status: Type3.Literal("cancelled") })
]);
...
var QuestionListResultSchema = closedObject({
  questions: Type3.Array(QuestionRecordSchema)
});
...
var QuestionResolvedEventSchema = withSince(
  ...
  Type3.Union([
    closedObject({
      id: NonEmptyString,
      status: Type3.Literal("answered"),
      answers: QuestionAnswersSchema
    }),
    closedObject({ id: NonEmptyString, status: Type3.Literal("cancelled") }),
    closedObject({ id: NonEmptyString, status: Type3.Literal("expired") })
  ])
);
export {
