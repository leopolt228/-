...
// packages/gateway-protocol/src/schema/worktrees.ts
var WorktreeNameSchema = Type3.String({ pattern: "^[a-z0-9][a-z0-9-]{0,63}$" });
var WorktreeRecordSchema = closedObject({
  id: NonEmptyString,
  name: WorktreeNameSchema,
  repoFingerprint: Type3.String({ pattern: "^[a-f0-9]{16}$" }),
  repoRoot: NonEmptyString,
  path: NonEmptyString,
  branch: NonEmptyString,
  baseRef: NonEmptyString,
  ownerKind: Type3.String({ enum: ["manual", "workboard", "session"] }),
  ownerId: Type3.Optional(NonEmptyString),
  snapshotRef: Type3.Optional(NonEmptyString),
  createdAt: Type3.Integer({ minimum: 0 }),
  lastActiveAt: Type3.Integer({ minimum: 0 }),
  removedAt: Type3.Optional(Type3.Integer({ minimum: 0 }))
});
var WorktreesListParamsSchema = closedObject({});
var WorktreesListResultSchema = closedObject({
  worktrees: Type3.Array(WorktreeRecordSchema)
});
var WorktreesCreateParamsSchema = closedObject({
  repoRoot: NonEmptyString,
  name: Type3.Optional(WorktreeNameSchema),
  baseRef: Type3.Optional(NonEmptyString)
});
...
var WorktreesGcResultSchema = closedObject({
  removed: Type3.Array(NonEmptyString),
  orphansDeleted: Type3.Integer({ minimum: 0 }),
  snapshotsPruned: Type3.Integer({ minimum: 0 })
});
export {
