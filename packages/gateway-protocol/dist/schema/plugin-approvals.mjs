...
var PluginApprovalRequestParamsSchema = closedObject({
  ...
  title: Type3.String({ minLength: 1, maxLength: PLUGIN_APPROVAL_TITLE_MAX_LENGTH }),
  description: Type3.String({ minLength: 1, maxLength: PLUGIN_APPROVAL_DESCRIPTION_MAX_LENGTH }),
  severity: Type3.Optional(Type3.String({ enum: ["info", "warning", "critical"] })),
  toolName: Type3.Optional(Type3.String()),
  toolCallId: Type3.Optional(Type3.String()),
  allowedDecisions: Type3.Optional(
    Type3.Array(Type3.String({ enum: ["allow-once", "allow-always", "deny"] }), {
      minItems: 1,
      maxItems: 3
    })
  ),
  ...
  timeoutMs: Type3.Optional(Type3.Integer({ minimum: 1, maximum: MAX_PLUGIN_APPROVAL_TIMEOUT_MS })),
