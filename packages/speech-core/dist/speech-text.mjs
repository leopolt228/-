...
function collectParsedAssistantTranscriptRoleInsertions(ir, options) {
  ...
  return (ir.annotations ?? []).filter((annotation) => annotation.type === "assistant_transcript_role").map((annotation) => ({ position: annotation.start, text: prefix }));
}
...
var CODE_HEAVY_SPOKEN_FALLBACK = "I've put the detailed response on screen.";
var CODE_HEAVY_FENCED_CHAR_RATIO = 0.5;
...
function isFenceCloser(line, opener) {
  const { content, container } = unwrapFenceContainer(line);
  if (container.blockquoteDepth !== opener.blockquoteDepth || container.listIndent !== opener.listIndent) {
    return false;
  }
  const match = /^(?: {0,3})(`+|~+)([ \t]*)$/u.exec(content);
  const fence = match?.[1];
