...
function coerceSecretRef(value) {
  ...
  if (typeof value === "string") {
    ...
    if (legacyPrefix) {
      const id2 = trimmed.slice(legacyPrefix.length);
      return ENV_SECRET_REF_ID_RE.test(id2) ? { source: "env", provider: DEFAULT_SECRET_PROVIDER_ALIAS, id: id2 } : null;
    }
  ...
  if (isRecord(value) && (value.source === "env" || value.source === "file" || value.source === "exec") && typeof value.id === "string" && value.id.trim().length > 0 && value.provider === void 0) {
    return {
      source: value.source,
...
function hasWebProviderEntryCredential(params) {
  ...
  const fromConfig = configuredRef ? "" : normalizeSecretInput(normalizeSecretInputString(rawValue));
  if (fromConfig) {
    return true;
  }
  if (params.provider.authProviderId && params.resolveProviderAuthValue?.(params.provider.authProviderId)) {
    return true;
  }
  ...
  const fallbackConfig = fallbackRef ? "" : normalizeSecretInput(normalizeSecretInputString(fallbackRawValue));
  if (fallbackConfig) {
    return true;
  }
  return Boolean(
    fallbackRef?.source === "env" ? params.resolveEnvValue({
      provider: params.provider,
...
function resolveWebProviderDefinition(params) {
  ...
  const provider = providers.find((entry) => entry.id === providerId) ?? providers.find(
    (entry) => entry.id === params.resolveFallbackProviderId?.({
      config: params.config,
      toolConfig: params.toolConfig,
      providers,
      providerId
